...
const WikiquoteApi = (() => {
  ...
  wqa.getSectionsForPage = async (pageId) => {
    try {
      ...
      const sectionArray = sections.filter(s => s.number.split('.')[0] === "1").map(s => s.index);
  ...
  wqa.getQuotesForSection = async (pageId, sectionIndex) => {
    try {
      const data = await makeRequest(
        `${API_URL}?format=json&action=parse&noimages=''&pageid=${pageId}&section=${sectionIndex}`);
  ...
  wqa.openSearch = async (titles) => {
    try {
      const response = await fetch(
        `${API_URL}?format=json&action=opensearch&namespace=0&suggest=&search=${titles}`);
      const data = await response.json();
      return data[1];
    } catch (error) {
      throw error;
