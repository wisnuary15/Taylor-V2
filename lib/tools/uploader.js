...
const referer = "https://krakenfiles.com",
  ...
  createFormData = async (content, fieldName) => {
    const { ext: ext, mime: mime } = await fileTypeFromBuffer(content) || {
      ext: "bin",
...
class Uploader {
  ...
  async uploadToKraken(content) {
    ...
    try {
      const { data: data } = await axios.get(referer), uploadUrl = data?.match(uloadUrlRegexStr)?.[1];
