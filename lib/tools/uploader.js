...
class Uploader {
  ...
  async top4top(content) {
    ...
    try {
      ...
      const result = informationSection.find('a').attr('href') || informationSection.find(
        '.inputbody p.btitle:contains("رابط الصورة المباشر") + input').val() || informationSection.find(
        '.inputbody p.btitle:contains("رابط الحذف") + input').val()
  ...
  async tmpfiles(content) {
    ...
    try {
      ...
      const shortenedURL = originalURL ? `https://tmpfiles.org/dl/${originalURL.split('/').slice(-2).join('/')}` :
        null;
      return shortenedURL;
    } catch (error) {
      handleErrorResponse(error, spinner);
  ...
  async Doodstream(content, key) {
    ...
    try {
      ...
      const response = await fetch((await (await fetch('https://doodapi.com/api/upload/server?key=' + (key ||
        '13527p8pcv54of4yjeryk'))).json()).result, {
        method: "POST",
        body: formData,
        headers: {
          "User-Agent": fakeUserAgent()
