...
const createFormData = async (content, fieldName) => {
    const {
        ext,
        mime
    } = await fileTypeFromBuffer(content) || { ext: 'bin', mime: 'application/octet-stream' };
    const blob = new Blob([content], {
        type: mime || 'application/octet-stream'
    });
    const formData = new FormData();
    formData.append(fieldName, blob, `${randomBytes}.${ext || 'bin'}`);
    return formData;
};
...
class Uploader {
    ...
    async ucarecdn(content) {
        ...
        try {
            ...
            return `https://ucarecdn.com/${file}/${randomBytes}.${ext || 'bin'}`;
        } catch (error) {
            handleErrorResponse(error, spinner);
        }
    }
    ...
    async top4top(content) {
        ...
        try {
            ...
            const form = await createFormData(content, "file_0_");
    ...
    async filebin(content) {
        ...
        try {
            const formData = await createFormData(content, "file");
            const binId = crypto.randomBytes(16).toString("base64").replace(/[/+=]/g, "");
            const uploadURL = `https://filebin.net/${binId}/${randomBytes}.${ext || 'bin'}`;
    ...
    async fexnet(content) {
        ...
        try {
            const formData = await createFormData(content, "file");
            formData.append("filename", `${randomBytes}.${ext || 'bin'}`);
