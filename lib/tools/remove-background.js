...
const randomBytes = crypto.randomBytes(8).toString("base64").replace(/[/+=]/g, ""),
  getUrlFromInput = async input => {
    if (Buffer.isBuffer(input)) {
      const { ext: ext, mime: mime } = await fileTypeFromBuffer(input) || {
        ext: "jpg",
        mime: "image/jpg"
    ...
    try {
      let media, mime;
      if (Buffer.isBuffer(input)) media = input, ({ mime: mime } = await fileTypeFromBuffer(input) || {
        mime: "image/jpg"
      });
      else {
        const response = await fetch(input);
        media = await response.arrayBuffer(), ({ mime: mime } = await fileTypeFromBuffer(media) || {
          mime: "image/jpg"
        });
      }
    ...
    try {
      const url = await getUrlFromInput(input),
        ...
        { mime: mime } = await fileTypeFromBuffer(media) || {
          mime: "image/jpg"
        },
...
export { RemoveBackground, RemoveBackgroundV2 };