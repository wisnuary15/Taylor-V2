...
class ShortLink {
  ...
  async onept(url) {
    try {
      const response = await fetch(
        `https://csclub.uwaterloo.ca/~phthakka/1pt/addURL.php?url=${encodeURIComponent(url)}`);
      const result = await response.json();
  ...
  async ssur(url) {
    try {
      const keys = ['nZ9ZzSa4LZ4o', 'Ed8nLSFpNVGB', 'YJimrVqxmExf', 'L9YRXGPugtet', 'HR7RDeKNVgTX', 'RKqh9qcjDoe4',
        'XoWtP22exnmy', 'GGFedvn7yhFZ', 'yJpFtTfXNZVi', 'MqQsBMbCvthf', 'MqQsBMbCvthf', 'vMd8zBusHzKk',
  ...
  async adfly(url) {
    try {
      const keys = ["9a283dafe534bddccc556153a37ed678", "d8a2283a6bbafbe31b442776fdc108ab",
        "9f7a757edc77cda2a8394aa6088c5c1a", "7efadeae65372322ee0205a4d9e2616a"
      ];
      const randomKey = keys[Math.floor(Math.random() * keys.length)];
      const shortenerUrl =
        `https://api.adf.ly/api.php?key=${randomKey}&uid=343503&advert_type=int&domain=adf.ly&url=${url}`;
      const response = await fetch(shortenerUrl);
