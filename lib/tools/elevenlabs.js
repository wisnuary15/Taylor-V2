...
const FORMATS = ['mp3_44100_64', 'mp3_44100_96', 'mp3_44100_128', 'mp3_44100_192', 'pcm_16000', 'pcm_22050',
  'pcm_24000', 'pcm_44100'
];
...
async function synthesize(ttsOptions) {
  try {
    ...
    const response = await apiCall("POST",
      `/v1/text-to-speech/${ttsOptions?.voice_id}/stream?output_format=${output_format}`, headers, requestBody);
    const contentType = response.headers.get('content-type');
    const responseData = contentType && contentType.includes('application/json') ? await response.json() :
