...
async function Lens(imgUrl) {
  try {
    ...
    form.append("encoded_image", new Blob([imageData], {
      type: "image/png"
    }), "image.png");
    const uploadResponse = await fetch(`https://lens.google.com/v3/upload?s=4&re=df&stcs=${imgUrl}`, {
      method: "POST",
      headers: {
    ...
    if (searchStart === -1) throw new Error("Search start not found in response");
    ...
    const searchUrl = "https://lens.google.com/search?ep=subb&re=df&s=4&p=" + partAfterSearch.substring(start, end);
    ...
    if (scriptContent) {
      ...
      return dataArray.flatMap(item => Array.isArray(item) ? item.flatMap(subItem => Array.isArray(subItem) && subItem[3] && subItem[7] && subItem[0]?.[0] && subItem[5] && subItem[11] && subItem[14] ? [{
        title: subItem[3] || "",
        domain: subItem[7] || "",
