...
class ImageProcessor {
  ...
  async processImage(input, endpoint, options) {
    try {
      const inputBuffer = Buffer.isBuffer(input) || (input instanceof ArrayBuffer || ArrayBuffer.isView(input)) ?
        input : this.urlRegex.test(input) ? await fetch(input).then((response) => response.arrayBuffer()) : Buffer
        .alloc(0);
