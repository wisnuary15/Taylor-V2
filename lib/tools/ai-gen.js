...
const agent = new https.Agent({
  ...
  Svgai = async prompt => {
    try {
      const { data: data } = await axios.post("https://api.svg.io:10003/api/createimg/ai", {
        prompt: prompt
      }, {
        headers: headers,
        httpsAgent: agent
      }), base64Data = data.images[0].replace(/^data:image\/svg\+xml;base64,/, ""), svgBuffer = Buffer.from(base64Data, "base64");
      return {
        svg: svgBuffer,
        png: await sharp(svgBuffer).png({
          quality: 100
        }).toBuffer()
      };
    } catch (error) {
    ...
    async scrape(url) {
      try {
        const { data: html } = await axios.get(url), $ = cheerio.load(html);
    ...
    async category(cat) {
      ...
      return categories.includes(cat) ? await this.scrape(`https://www.myinstants.com/en/categories/${cat}`) : (console.error("Invalid category."),
        categories);
    }
    async new() {
    ...
    try {
      ...
      const { batchTaskId: batchTaskId } = await createResponse.json(), urlGet = `https://web-backend-prod.zmo.ai/api/v1.0/microTask/makeUp/get?batchTaskId=${batchTaskId}`;
    ...
    try {
      ...
      if (!createResponse.ok) throw new Error(`Error creating task: ${createResponse.statusText}`);
      const { id: batchTaskId } = await createResponse.json(), urlGet = `https://arthub-gen-worker.repalash.workers.dev/api/v1/check/${batchTaskId}`;
      let taskDetails, attempts = 0;
      const maxAttempts = 5;
      do {
        await new Promise((resolve => setTimeout(resolve, 15e3)));
...
export { Svgai, MyInstant, Zmoai, Arthub, Limewire };