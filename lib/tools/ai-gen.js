...
const Svgai = async (prompt) => {
  try {
    ...
    const pngBuffer = await sharp(svgBuffer).png({
      quality: 100
    }).toBuffer();
    return {
      svg: svgBuffer,
...
const MyInstant = class {
  async scrape(url) {
    try {
      ...
      return $('.instant').map((_, el) => {
        ...
        const soundLink =
          `https://www.myinstants.com${$(el).find('button.small-button').attr('onclick').match(/play\('(.+?)'/)[1]}`;
  ...
  async category(cat) {
    const categories = ['games', 'anime manga', 'memes', 'movies', 'music', 'politics', 'pranks', 'reactions',
...
const Zmoai = async (prompt, identify = "f944236b0480a21d0344ad661b0bae9f", categoryId =
  "b8001af87354413387180815c5f250cf", styleCategoryIds = ["cdf3fddfee364bcfa31a38a9bb4d63fe"], scale = "1280x720",
  resolution = "1280x720", numOfImages = 1) => {
...
const Arthub = async (prompt, censor_nsfw = false, nsfw = true, cfg_scale = 7.5, height = 512, n = 1, sampler_name =
  "k_dpm_2", seed = "", steps = 25, width = 512, post_processing = ["RealESRGAN_x4plus"], r2 = true,
  trusted_workers = false, models = ["Deliberate"]) => {
...
const Limewire = async (prompt = 'A cute baby sea otter', negative_prompt = '', samples = 1, quality = 'LOW',
  guidance_scale = 50, aspect_ratio = '1:1', style = 'PHOTOREALISTIC') => {
