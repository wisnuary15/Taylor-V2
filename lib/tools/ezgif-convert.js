...
async function convert(fields) {
  ...
  let link = await axios({
    ...
    if (error.response) {
      throw new Error(JSON.stringify({
        ...
        data: error.response.data.length ? error.response
          .data : "Try again. If it continues, report to the creator.",
      }, null, 4));
    } else {
  ...
  let img_url =
    `https:${(image?.data?.toString()?.split(type.split.start)?.[1].split(type.split.end)?.[0])?.replace('https:', '')}`;
  if (img_url.includes('undefined')) throw new Error(
...
async function overlay(fields) {
  ...
  let img_url =
    `https:${(image?.data?.toString()?.split('<img src="')?.[1].split('" style="width:')?.[0])?.replace('https:', '')}`;
  if (img_url.includes('undefined')) throw new Error(
    `Something unknown happened here... please report to the creator`);
  return img_url;
}
...
async function render(fields) {
  ...
  html.data.toString().split('(drag and drop frames to change order)')[1].split(
    '<p class="options"><strong>Toggle a range of frames:</strong>')[0]?.split('<span class="frame-tools">').slice(
    0, -1).map(i => i.split('value="')[1].split('" name="files[]"')[0]).forEach(e => {
    fields['files[]'].push(e)
  });
  let image = await axios({
    method: 'post',
