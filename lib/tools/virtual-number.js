...
export class smstome {
  async Country() {
    try {
      const { data: data } = await axios.get("https://smstome.com"), $ = cheerio.load(data);
  ...
  async getNumber(country) {
    try {
      const { data: data } = await axios.get(`https://smstome.com/country/${country.toLowerCase()}`), $ = cheerio.load(data);
  ...
  async getMessage(url, page) {
    try {
      const { data: data } = await axios.get(page ? `${url}?page=${page}` : url), $ = cheerio.load(data);
...
export class sms24 {
  async Country() {
    try {
      const { data: data } = await axios.get("https://sms24.me/en/countries"), $ = cheerio.load(data);
  ...
  async getNumber(country) {
    try {
      const { data: data } = await axios.get(`https://sms24.me/en/countries/${country.toLowerCase()}`), $ = cheerio.load(data);
      return $(".callout").map(((_, callout) => ({
  ...
  async getMessage(number) {
    try {
      const { data: data } = await axios.get(`https://sms24.me/en/numbers/${parseInt(number)}`), $ = cheerio.load(data);
