...
//wiki
const speech = ['Noun', 'Pronoun', 'Adjectives', 'Numerals', 'Verb', 'Adverb', 'Article', 'Preposition', 'Conjunction',
...
const get_wiki_entry = async (word) => {
  try {
    ...
    $('.mw-parser-output').find('ol').each(function(i, elem) {
      if (i < cnt) {
        $(elem).find('ul').empty();
        let curspeech = $(elem).prev().prev().text();
        let onedefinition = {
          speech: curspeech,
          lines: []
        };
...
const wiktionaryId = async (word) => {
  const apiUrl =
    `https://id.wiktionary.org/w/api.php?action=query&format=json&srlimit=10&srsort=relevance&list=search&srsearch=${word}`;
  try {
