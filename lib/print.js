...
const formatSize = sizeFormatter({
  ...
  truncateText = text => {
    ...
    if (totalWords <= 40) return text;
    const startCount = Math.min(25, totalWords - 15);
    const endCount = Math.min(15, totalWords - startCount);
    return `${words.slice(0, startCount).join(" ")} ... ${words.slice(-endCount).join(" ")}`;
  };
class ClearLogger {
  logCount = 0;
...
export default async function(m, conn = {
  ...
  let database = db.data.database.listbot;
  if (!database) {
    db.data.database.listbot = {};
    database = db.data.database.listbot;
  }
  ...
  if ("string" == typeof m?.text && m?.text && m?.isCommand && m?.sender) {
    ...
    console.log(null != m.error ? `🚨 ${chalk.bold.red(truncateText(logMessage))}` : m.isCommand ? `⚙️ ${chalk.bold.yellow(truncateText(logMessage))}` : truncateText(logMessage)),
  ...
  if (m?.msg && m?.sender) {
    ...
    if (/document/i.test(attachmentType)) console.log(chalk.bold.cyan("╭──────────────────────────────────────···")),
      ...
      console.log(`📨 ${chalk.bold.redBright("Message Info")}: ${chalk.yellow("[")} ${chalk.green(m?.isGroup ? "Group" : "Private")} ${chalk.yellow("]")}`),
        ...
        console.log(`   ${chalk.bold.cyan("- Sender Name")}: ${m.name || m.pushName || conn.user?.name || "N/A"}`),
        console.log(`   ${chalk.bold.cyan("- Chat ID")}: ${m.chat?.split("@")[0] || m.key.remoteJid || "N/A"}`),
        console.log(`   ${chalk.bold.cyan("- Chat Name")}: ${chat || "N/A"}`), console.log(chalk.bold.redBright(`📹 Attached Video: ${truncateText(attachmentName)}`)),
        console.log(chalk.bold.cyan("╰──────────────────────────────────────···"));
      ...
      console.log(`📨 ${chalk.bold.redBright("Message Info")}: ${chalk.yellow("[")} ${chalk.green(m?.isGroup ? "Group" : "Private")} ${chalk.yellow("]")}`),
        ...
        console.log(`   ${chalk.bold.cyan("- Sender Name")}: ${m.name || m.pushName || conn.user?.name || "N/A"}`),
        console.log(`   ${chalk.bold.cyan("- Chat ID")}: ${m.chat?.split("@")[0] || m.key.remoteJid || "N/A"}`),
        console.log(`   ${chalk.bold.cyan("- Chat Name")}: ${chat || "N/A"}`), console.log(chalk.bold.redBright(`🎴 Attached Sticker: ${truncateText(attachmentName)}`)),
        console.log(chalk.bold.cyan("╰──────────────────────────────────────···"));
    }
  }
