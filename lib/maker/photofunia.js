...
async function getImageKey(url, image) {
  const u = new URL(url);
  try {
    const { coki: coki } = await getCookies(), form = new FormData;
...
async function photofunImg(url, image) {
  ...
  const { key: key, coki: coki } = await getImageKey(url, image);
  try {
...
async function photofunImg2(url, image, image2) {
  ...
  const { key: key, coki: coki } = await getImageKey(url, image), img2 = await getImageKey(url, image2);
  try {
    const form2 = new FormData;
...
export { photofunSearch, photofunEffect, photofunUse, photofunImg, photofunImg2, photofunText };