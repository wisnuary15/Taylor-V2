...
class Saweria {
  ...
  async getSaweria(url) {
    ...
    try {
      const html = await fetch(url).then(v => v.text()).then(v => cheerio.load(v)("script#__NEXT_DATA__").text().trim()).then(v => JSON.parse(v)).then(v => v.props.pageProps.data);
      return html;
    } catch (e) {
      throw e;
    }
  ...
  async createPayment(amount = 1e3, message = "hi", token = this.token) {
    ...
    if (amount < 1e3) throw new Error("Invalid Amount Of Money, Enter More Or Equal To 1000 RP");
    try {
  ...
  async status(id) {
    try {
      const data = await fetch(`${this.apiUrl}/donations/qris/${id}`).then(v => v.json());
      return {
        id: id,
  ...
  async getTransaction(page = 1) {
    if (!this.token) throw new Error("Login to your saweria account first");
    try {
      const res = await fetch(this.apiUrl + `/transactions?page=${page}&page_size=15`, {
        headers: {
          "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/127.0.0.0 Safari/537.36",
          Authorization: this.token,
          origin: this.baseUrl
