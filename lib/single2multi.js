...
export default async function single2multi(fileSingle, folderMulti, authState) {
  ...
  var writeData = (data, file) => {
    return fs.promises.writeFile(path.join(folderMulti, store.fixFileName(file)), JSON.stringify(data, store
      .JSONreplacer))
  }
  var getKeyByValue = (obj, value) => {
    return Object.keys(obj).find(key => obj[key] === value)
  }
  var keys = Object.fromEntries(Object.entries(authSingleKeys).map(([key, value]) => (value && [getKeyByValue(store
    .KEY_MAP, key), value])).filter(Boolean))
  await Promise.all([
