...
const agent = new https.Agent({
  ...
  agent: "http://proxy:clph123@103.123.63.106:3128"
});
...
class ImageEditor {
  ...
  async createImage(Name, Text, WL, title, textData) {
    try {
      const [bgImage, avatarImage] = await Promise.all([fetch(this.bgImageURL, {
        agent: agent
      }).then(res => res.arrayBuffer()).then(data => Jimp.read(Buffer.from(data))).catch(() => instance.get(this.bgImageURL, {
        responseType: "arraybuffer"
      }).then(res => Jimp.read(res.data))), fetch(this.avatarURL, {
        agent: agent
      }).then(res => res.arrayBuffer()).then(data => Jimp.read(Buffer.from(data))).catch(() => instance.get(this.avatarURL, {
        responseType: "arraybuffer"
