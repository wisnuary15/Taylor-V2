  ...
  async createImage(Name, Text, WL, title, textData) {
    try {
      ...
      textData.forEach(({
        ...
        bgImage.print(font16, centerX + 340 + labelWidth, textY + 2, v, 735 - (centerX + 340 + labelWidth), 490 -
          textY - 10);
      });
...
function generateUrl(type, params) {
  const baseUrls = {
    welcome: [`https://some-random-api.com/welcome/img/${(Math.floor(Math.random() * 7) + 1)}/stars`,
...
function OTP(Cods) {
  ...
  const themes = [`https://dummyimage.com/300.png/09f/fff&text=${Cods}`,
    `https://via.placeholder.com/300/fff?text=${Cods}`, `https://fakeimg.pl/300x300/ffffff/000000/?text=${Cods}`,
    `https://api.sylvain.pro/en/captcha?text=${Cods}`
  ];
  return pickRandom(themes);
}
export {
