...
const { flamming: flamming, dynamic: dynamic, estetik: estetik } = JSON.parse(readFileSync("./json/image/image.json"));
class ImageEditor {
  ...
  async createImage(Name, Text, WL, title, textData) {
    try {
      ...
      return textData.forEach((({ l: l, v: v }, i) => {
...
export { ImageEditor, Welcome, Leave, OTP, BannerBot };