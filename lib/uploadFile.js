...
export default async function(inp, option = 'all') {
  ...
  try {
    ...
    if (option !== 'image' && option !== 'all' && !validIndex) {
      ...
      throw new Error(
        `Invalid input. Please provide 'image', 'all', or a valid index.\nAvailable Indexes:\n${indexList}`);
    }
    const selectedProviders = option === 'image' ? providers.image : option === 'all' ? providers.all : validIndex ? [
      providerList[Number(option) - 1]
    ] : providers.all;
    for (const {
        name,
        func
