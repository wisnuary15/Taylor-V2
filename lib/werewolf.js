  ...
  for (let i = 0; i < room.player.length; i++) {
    if (room.player[i].isdead === false) {
      if (room.player[i].role === "werewolf" || room.player[i].role === "sorcerer") {
        ww += 1;
      } else if (room.player[i].role === "warga" || room.player[i].role === "guardian" || room.player[i].role ===
        "seer") {
        orang_baek += 1;
      }
    }
    ...
    for (let i = 0; i < x.player.length; i++) {
      if (x.player[i].role === "werewolf") {
        if (x.player[i].isdead != true) {
          textnya =
            `Silahkan pilih salah satu orang yang ingin kamu makan pada malam hari ini\n*LIST PLAYER*:\n${tok2}\n\nKetik *.wwpc kill nomor* untuk membunuh player`;
        ...
        if (x.player[i].isdead != true) {
          textnya =
            `Baiklah, siapa yang ingin kamu lihat peran nya kali ini.\n*LIST PLAYER*:${tok1}\n\nKetik *.wwpc dreamy nomor* untuk melihat role player`;
          await conn.sendMessage(x.player[i].id, {
            text: textnya,
            mentions: membernya,
          });
        ...
        if (x.player[i].isdead != true) {
          textnya =
            `Kamu adalah seorang*Guardian*, lindungi para warga, silahkan pilih salah 1 player yang ingin kamu lindungi\n*LIST PLAYER*:${tok1}\n\nKetik *.wwpc deff nomor* untuk melindungi player`;
          await conn.sendMessage(x.player[i].id, {
            text: textnya,
            mentions: membernya,
          });
        ...
        if (x.player[i].isdead != true) {
          textnya =
            `Baiklah, lihat apa yang bisa kamu buat, silakan pilih 1 orang yang ingin kamu buka identitasnya\n*LIST PLAYER*:${tok2}\n\nKetik *.wwpc sorcerer nomor* untuk melihat role player`;
          await conn.sendMessage(x.player[i].id, {
            text: textnya,
            mentions: membernya,
          });
