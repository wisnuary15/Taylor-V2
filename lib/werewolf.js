...
const findObject = (obj = {}, key, value) => {
  ...
  killWerewolf = (sender, id, data) => {
    ...
    if (!result) return !1;
    let { index: index, sesi: sesi, db: db } = result;
    data[sesi].player[index].number === id && (db.effect.includes("guardian") ? (data[sesi].guardian.push(parseInt(id)),
      data[sesi].dead.push(parseInt(id))) : db.effect.includes("guardian") || data[sesi].dead.push(parseInt(id)));
  },
  ...
  changeDay = (from, data) => {
    if (!(room = sesi(from, data))) return !1;
    "pagi" === room.time ? room.time = "voting" : "malem" === room.time ? (room.time = "pagi",
      room.day += 1) : "voting" === room.time && (room.time = "malem");
  },
  dayVoting = (from, data) => {
    if (!(room = sesi(from, data))) return !1;
  ...
  vote = (from, id, sender, data) => {
    ...
    if (-1 === idGet) return !1;
    const indexPlayer = room.player.findIndex((i => i.number === id));
    if (-1 === indexPlayer) return !1; -
    1 !== idGet && (room.player[idGet].isvote = !0), room.player[indexPlayer].vote += 1;
  },
  voteResult = (from, data) => !!(room = sesi(from, data)) && (room.player.sort(((a, b) => a.vote < b.vote ? 1 : -1)),
    0 === room.player[0]?.vote ? 0 : room.player[0]?.vote === room.player[1].vote ? 1 : room.player[0]),
  voteKill = (from, data) => !!(room = sesi(from, data)) && (room.player.sort(((a, b) => a.vote < b.vote ? 1 : -1)),
    0 === room.player[0]?.vote ? 0 : room.player[0]?.vote === room.player[1].vote ? 1 : void(room.player[0].isdead = !0)),
  resetVote = (from, data) => {
    if (!(room = sesi(from, data))) return !1;
...
async function malam(conn, x, data) {
  ...
  if (0 === hasil_vote) return textnya = "*⌂ W E R E W O L F - G A M E*\n\nTerlalu bimbang menentukan pilihan. Warga pun pulang ke rumah masing-masing, tidak ada yang dieksekusi hari ini. Bulan bersinar terang, malam yang mencekam telah datang. Semoga tidak ada yang mati malam ini. Pemain malam hari: kalian punya 90 detik untuk beraksi!",
    conn.sendMessage(x.room, {
      ...
      if (changeDay(x.room, data), voteDone(x.room, data), resetVote(x.room, data), clearAllVote(x.room, data),
        null != getWinner(x.room, data).status) return win(x, 1, conn, data);
    }));
