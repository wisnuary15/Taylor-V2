  ...
  constructor() {
    for (var i in this.tools = {
        flip: {
          ...
          encode: function(text) {
            for (var ch, ret = [], i = 0, len = text.length; i < len; i++) ch = text.charAt(i),
              i > 0 && ("̤" === ch || "̗" === ch || "̖" === ch || "̮" === ch) ? (ch = this.map[text.charAt(i - 1) + ch],
                ret.pop()) : void 0 === (ch = this.map[ch]) && (ch = text.charAt(i)), ret.push(ch);
        ...
        mirror: {
          ...
          encode: function(text) {
            for (var ch, ret = [], newLines = [], i = 0, len = text.length; i < len; i++) ch = text.charAt(i),
              i > 0 && ("̈" === ch || "̀" === ch || "́" === ch || "̂" === ch) ? (ch = this.map[text.charAt(i - 1) + ch],
                ret.pop()) : void 0 === (ch = this.map[ch]) && (ch = text.charAt(i)), "\n" === ch ? (newLines.push(ret.reverse().join("")),
                ret = []) : ret.push(ch);
        ...
        creepify: {
          ...
          encode: function(text) {
            var newChar, newText = "";
            for (var i in text) {
              if (newChar = text[i], this.options.middle && (newChar += this.diacriticsMiddle[Math.floor(Math.random() * this.diacriticsMiddle.length)]),
                this.options.top)
        ...
        bubbles: {
          ...
          encode: function(text) {
            var ch, ret = "",
              first = !0;
            for (var i in text) void 0 === (ch = this.map[text[i]]) && (text[i].charCodeAt(0) >= 33 ? (ch = text[i] + String.fromCharCode(8413),
                first || (ch = String.fromCharCode(8239) + String.fromCharCode(160) + String.fromCharCode(160) + String.fromCharCode(8239) + ch)) : ch = text[i]),
              ret += ch, first = "\n" === ch;
            return ret;
          },
          decode: function(text) {
        ...
        squares: {
          ...
          encode: function(text) {
            var ch, ret = "",
              first = !0;
            for (var i in text) text[i].charCodeAt(0) >= 33 ? (ch = text[i] + String.fromCharCode(8414),
                first || (ch = String.fromCharCode(8239) + String.fromCharCode(160) + String.fromCharCode(160) + String.fromCharCode(8239) + ch)) : ch = text[i],
              ret += ch, first = "\n" === ch;
            return ret;
          },
        ...
        roundsquares: {
          ...
          encode: function(text) {
            var ch, ret = "",
              first = !0;
            for (var i in text) text[i].charCodeAt(0) >= 33 ? (ch = text[i] + String.fromCharCode(8419),
                first || (ch = String.fromCharCode(160) + String.fromCharCode(160) + String.fromCharCode(160) + ch)) : ch = text[i],
              ret += ch, first = "\n" === ch;
            return ret;
          },
          decode: function(text) {
    ...
    this.getHTML = function(text) {
      for (var ch, html = "", lastSpaceWasNonBreaking = !0, highSurrogate = 0, codepoint = 0, i = 0, len = text.length; i < len; i++) 10 === (ch = text.charCodeAt(i)) || 13 === ch ? (html += "<br>\n",
        lastSpaceWasNonBreaking = !0) : 32 === ch ? lastSpaceWasNonBreaking ? (html += " ",
        lastSpaceWasNonBreaking = !1) : (html += "&nbsp;", lastSpaceWasNonBreaking = !0) : (ch >= 55296 && ch <= 56319 ? (highSurrogate = ch,
        codepoint = 0) : highSurrogate > 0 ? (ch >= 56320 && ch <= 57343 && (codepoint = 1024 * (highSurrogate - 55296) + (ch - 56320) + 65536),
        highSurrogate = 0) : codepoint = ch, 0 != codepoint && (html += "&#x" + codepoint.toString(16) + ";",
        lastSpaceWasNonBreaking = !0));
      return html;
    };
  }
}
