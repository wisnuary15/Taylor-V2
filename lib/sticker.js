...
import { ffmpeg, openwaSticker } from './converter.js'
...
async function sticker8(source, quality = 30) {
  try {
    if (!source) throw new Error('Source must be provided.');
    return await sharp(source).webp({ quality, lossless: true }).toBuffer();
  } catch (error) {
...
async function sticker9(inputBuffer, quality = 30) {
    try {
