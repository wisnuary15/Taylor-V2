...
import { ffmpeg as ffmpegConv, toSticker, openwaSticker, StickerTypes as _StickerTypes, imageToWebp, videoToWebp, mp42webp, png2webp } from "./converter.js";
import { video2Webp as _video2webp, image2Webp as _image2Webp } from "./webp2mp4.js";
...
const { Image: Image, WebPMux: WebPMux } = webpmux;
...
async function sticker2(media, url) {
  try {
    ...
    const ffmpegSpawn = spawn("ffmpeg", ["-y", "-i", inp, "-vf", "scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15,pad=320:320:-1:-1:color=#00000000,setsar=1", "-f", "webp", "-"]);
...
async function sticker3(media, url) {
  try {
    ...
    if (!/video/.test(type.mime)) throw new Error("Provided file is not a video");
...
async function sticker4(media, url) {
  try {
    ...
    const ffmpegArgs = ["-vf", "scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15,pad=320:320:-1:-1:color=#00000000,setsar=1"],
...
async function sticker6(media, url) {
  try {
    ...
    if ("bin" === type.ext) throw new Error("Unsupported file type");
...
async function sticker7(media, url) {
  try {
    ...
    if (!/image|video/.test(type.mime)) throw new Error("Provided file is neither an image nor a video");
...
async function video2webp(media) {
  ...
  fs.writeFileSync(tmpFileIn, media), await new Promise(((resolve, reject) => {
    Fluent_Ffmpeg(tmpFileIn).on("error", reject).on("end", (() => resolve(!0))).addOutputOptions(["-vcodec", "libwebp", "-vf", "scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15,pad=320:320:-1:-1:color=white@0.0,split[a][b];[a]palettegen=reserve_transparent=on:transparency_color=ffffff[p];[b][p]paletteuse", "-loop", "0", "-ss", "00:00:00", "-t", "00:00:05", "-preset", "default", "-an", "-vsync", "0"]).toFormat("webp").save(tmpFileOut);
  }));
...
async function sticker(media, url, ...args) {
  ...
  const functionsToTry = [global.support.ffmpeg && (global.support.convert || global.support.magick || global.support.gm) ? sticker2 : null, global.support.ffmpeg ? sticker6 : null, sticker10, sticker3, sticker11, sticker12, sticker13, global.support.ffmpeg && global.support.ffmpegWebp ? sticker4 : null, sticker1, sticker7, sticker5, sticker9, sticker8].filter(Boolean);
  for (let func of functionsToTry) try {
    if (stiker = await func(media, url, ...args), stiker.includes("html")) continue;
    if (stiker.includes("WEBP")) try {
      return await addExif(stiker, ...args);
    } catch (e) {
      return console.error(e), stiker;
    }
    throw new Error(stiker.toString());
  } catch (err) {
    lastError = err;
  }
  return console.error(lastError), lastError;
}
const support = {
  ffmpeg: !0,
  ffprobe: !0,
...
export { sticker, sticker1, sticker2, sticker3, sticker4, sticker5, sticker6, sticker7, sticker8, sticker9, sticker10, sticker11, sticker12, sticker13, video2webp, video2webp30, video2webp45, video2webp60, addExif, support };