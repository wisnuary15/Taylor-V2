...
export default class KomikCast {
  ...
  search = async (query, {
    ...
    try {
      ...
      const data = $('.list-update_item').filter((i, e) => type ? $(e).find('.type').text().trim() === type : true)
        .map((i, e) => ({
  ...
  info = async (url) => {
    try {
      ...
      const komikInfo = {
        ...
        totalChapters: $('.komik_info-content-info:contains("Total Chapter")').text().trim().replace(
          'Total Chapter:', ''),
