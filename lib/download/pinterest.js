...
class PinterestDownloader {
  ...
  async videoDown(link) {
    ...
    try {
      const response = await fetch('https://www.pinterest.com/resource/PinResource/get?data=' + encodeURIComponent(
        JSON.stringify({
          options: {
      ...
      const video = responseData.videos?.video_list?.V_720P ?? responseData.story_pin_data?.pages[0]?.blocks[0]?.video
        ?.video_list?.V_EXP7;
      if (!video) {
