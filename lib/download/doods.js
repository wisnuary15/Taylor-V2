...
Instance.interceptors.response.use((response => 200 !== response.status ? {
  ...
  error: !1,
  data: response.data
})), Instance.interceptors.request.use((config => (config.headers.Referer = config.url,
...
async function fetch_url(url, opts = {}) {
  const { data: data, error: error } = await Instance.get(url);
  ...
  if (!iframe_url) {
    const base_url = get_base_url(url).endsWith("/") ? get_base_url(url) : get_base_url(url) + "/",
      { path: path, token: token, expiry: expiry, char: char } = get_token(data),
...
async function pass_hash(url, opts = {}) {
  const { data: data, error: error } = await Instance.get(url, opts);
  if (error) throw new Error("Error fetching url");
  return data;
}
