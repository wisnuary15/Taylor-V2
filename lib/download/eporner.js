...
class Eporner {
  ...
  async download(url) {
    try {
      ...
      return { title, description, thumbnail, download };
    } catch (error) {
      console.error('Error fetching data:', error);
      return null;
    }
  ...
  async categoryList() {
    try {
      ...
      const links = $('a[href*="/cat/"]').map((index, element) => new URL($(element).attr('href'), baseURL).href).get().filter((link, index, self) => link && link.includes('/cat/') && !link.includes('/cat//') && self.indexOf(link) === index);
      return links;
    } catch (error) {
      console.error('Error fetching data:', error);
      return [];
