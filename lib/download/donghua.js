...
class Donghua {
  ...
  replacer(str) {
    return str.replace(new RegExp('diposting oleh', 'g'), 'author').replace(new RegExp('tipe', 'g'), 'type').replace(
      new RegExp('dirilis', 'g'), 'release').replace(new RegExp('total episode', 'g'), 'episode').replace(
      new RegExp('durasi', 'g'), 'duration').replace(new RegExp('diperbarui pada', 'g'), 'updated');
  }
  async fetch(url) {
    try {
  ...
  async stream(url) {
    try {
      ...
      return {
        ...
        data: {
          url: streamUrl,
          file: JSON.parse(streamData.trim().replace(/ /g, '').replace(/"/g, '').replace(/'/g, '"').replace(/src/g,
            '"src"').replace(/size/g, '"size"').replace(/type/g, '"type"'))
        }
      };
    } catch (error) {
      console.error(error);
