...
class MangaKakalot {
  ...
  async search(keyword) {
    try {
      ...
      $("div.story_item > div.story_item_right > span").each(((index, element) => {
        ...
        void 0 !== attribute && (attribute.startsWith("Author(s) :") && authors.push(attribute.substring(12).split(",")),
          attribute.startsWith("Updated :") && updatedAt.push(convertToDate(attribute.substring(10))),
          attribute.startsWith("View :") && views.push(attribute.substring(7)));
      }));
  ...
  async getMangasFromGenre(genre = "any", filters = {}) {
    const { page: page = 1, status: status = "any", type: type = "updated" } = filters, url = `https://mangakakalot.com/manga_list?type=${"updated" === type ? "latest" : "newest"}&category=${null != genre && "any" !== genre ? MangakakalotGenres[genre] : ""}&state=${"any" === status ? "all" : status}&page=${page}`;
    try {
      const response = await fetch(url),
        html = await response.text(),
        $ = cheerio.load(html),
        ...
        mangaList = titleURLs.map((({ title: title, url: url }, i) => ({
          title: title,
          url: url,
          views: views[i],
          coverImage: covers[i]
