...
class MangaKakalot {
  async chapterInfo(url) {
    try {
      ...
      return {
        ...
        serverOptions: Array.from($('.panel-option .pn-op-sv-img-btn').map((_, option) => $(option).attr('data-l'))
          .get()) || [],
  ...
  async chapterSearch(q) {
    ...
    try {
      ...
      return $('.daily-update .story_item').map((_, el) => ({
        ...
        author: $(el).find('span').filter((_, el) => $(el).text().includes('Author(s)')).text().replace(
          'Author(s) : ', '').trim() || '',
        updated: convertToDate($(el).find('span').filter((_, el) => $(el).text().includes('Updated')).text()
          .replace('Updated : ', '').trim()) || '',
        views: $(el).find('span').filter((_, el) => $(el).text().includes('View')).text().replace('View : ', '')
          .trim() || '',
      })).get();
    } catch (error) {
      console.error('Error:', error);
  ...
  async search(keyword) {
    try {
      ...
      const views = [];
      const updatedAt = [];
      const links = $('div.story_item > div.story_item_right > h3.story_name > a').map((index, element) => {
        const link = $(element).attr('href');
        if (typeof link !== 'undefined') return link;
      }).get();
      const titles = $('div.story_item > div.story_item_right > h3.story_name > a').map((index, element) => {
        const title = $(element).text();
        if (typeof title !== 'undefined') return title;
      }).get();
      const coverImage = $('div.story_item > a[rel="nofollow"] > img').map((index, element) => $(element).attr(
        'src') ?? '').get();
      ...
      const mangaList = new Array(titles.length).fill('').map((_, index) => ({
        title: titles[index],
        url: links[index],
  ...
  async getMangaMeta(url) {
    try {
      ...
      const altTitles = $('div.manga-info-top > ul.manga-info-text > li > h2.story-alternative').map((_, element) =>
        splitAltTitles($(element).text().substring(14))).get();
      ...
      const authors = $('div.manga-info-top > ul > li:contains("Author(s)") > a').map((_, element) => $(element)
      .text()).get();
      const genres = $('div.manga-info-top > ul > li:contains("Genres") > a').map((_, element) => $(element).text())
        .get();
      const ratingText = $(
      ...
      const chaptersNameURL = chapterDiv.find('span > a').map((_, element) => {
  ...
  async getMangasFromGenre(genre = 'any', filters = {}) {
    ...
    try {
      ...
      const html = await response.text();
      const $ = cheerio.load(html);
      const titleURLs = $('div.list-truyen-item-wrap > h3 > a').map((_, element) => {
      ...
      const views = $('div.list-truyen-item-wrap > div > span.aye_icon').map((_, element) => $(element).text()).get();
      const covers = $('div.list-truyen-item-wrap > a > img').map((_, element) => $(element).attr('src') ?? '').get();
      const mangaList = titleURLs.map(({
        title,
        url
      }, i) => ({
        title,
  ...
  async getPages(url) {
    try {
      ...
      const html = await response.text();
      const $ = cheerio.load(html);
      const pages = $('div.container-chapter-reader > img[src]').map((_, element) => $(element).attr('src')).get();
      return pages || [];
    } catch (error) {
      console.error('Error:', error);
    }
