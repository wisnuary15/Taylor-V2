  ...
  try {
    ...
    return element.find(".kglist321").each(function() {
      day = $(this).find("h2").text(), animeList = [], $(this).find("ul > li").each(function() {
        anime_name = $(this).find("a").text(), link = $(this).find("a").attr("href"),
          id = link.replace(url.baseUrl + "anime/", ""), animeList.push({
...
export const animeByGenre = async (pageNumber, id) => {
  ...
  try {
    ...
    return element.find(".col-md-4").each(function() {
      object = {}, object.anime_name = $(this).find(".col-anime-title").text(),
        ...
        object.release_date = $(this).find(".col-anime-date").text(), genreList = [],
        $(this).find(".col-anime-genre > a").each(function() {
          genre_name = $(this).text(), genre_link = $(this).attr("href"),
...
export const epsMirror = async (mirrorId, animeId) => {
  ...
  try {
    ...
    return obj.streamLink = streamLink, obj.link_stream = await get(streamLink),
      obj;
  } catch (error) {
