...
export const epsAnime = async id => {
  ...
  try {
    ...
    return $("#embed_holder > div.mirrorstream > ul.m360p > li").each(((idx, el) => {
      ...
      quality: "720p",
      mirrorList: mirror3
    }, "" === $("#venkonten > div.venser > div.venutama > div.download > ul > li:nth-child(1)").text() ? (console.log("ul is empty"),
      low_quality = _notFoundQualityHandler(response.data, 0), medium_quality = _notFoundQualityHandler(response.data, 1),
      high_quality = _notFoundQualityHandler(response.data, 2)) : (console.log("ul is not empty"),
      low_quality = _epsQualityFunction(0, response.data), medium_quality = _epsQualityFunction(1, response.data),
      high_quality = _epsQualityFunction(2, response.data)), obj.quality = {
      low_quality: low_quality,
      medium_quality: medium_quality,
      high_quality: high_quality
    }, obj;
