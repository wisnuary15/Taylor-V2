...
class InDown {
  async downloadMedia(instagramLink) {
    try {
      ...
      const inputValues = Object.fromEntries($('form#downloadForm input[name]').get().map((element) => [$(element)
      ...
      $$('div.container.mt-4#result div.row.justify-content-center div.col-md-4').map((_, el) => {
        ...
        const href = decodeURIComponent(mediaLink.find(type === 'image' ? 'div.mt-2.mb-2.text-center a' :
          'div video source').attr('href')) ?? '';
        const alternative = mediaLink.find(type === 'image' ? 'a.image-link' :
          'div.mt-3.text-center div.btn-group-vertical a').attr('href') ?? '';
        mediaSet.add({
          type,
          href,
          alternative
