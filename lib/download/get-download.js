...
class Download {
  ...
  instagram = url => new Promise((async (resolve, reject) => {
    let urls;
    /(https:\/\/)?instagram\.com\/p\/[^\/?#&]+/.test(url) && (urls = "download-photo-instagram"),
      /(https:\/\/)?instagram\.com\/stories\/highlight\/[^\/?#&]+/.test(url) && (urls = "download-highlights-instagram"),
      /(https:\/\/)?instagram\.com\/reel\/[^\/?#&]+/.test(url) && (urls = "download-reel-instagram"),
      /(https:\/\/)?instagram\.com\/stories\/[^\/?#&]+/.test(url) && (urls = "download-story-instagram");
  ...
  LikeDown = url => new Promise((async (resolve, reject) => {
    const { data: data } = await axios.request("https://likeedownloader.com/process", {
      method: "post",
  ...
  douyin = url => new Promise((async (resolve, reject) => {
    const { data: data } = await axios("https://www.tikdd.cc/g1.php", {
      headers: {
        "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
        "User-Agent": "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Mobile Safari/537.36"
      },
  ...
  krakenfiles = url => new Promise((async (resolve, reject) => {
    const { data: data } = await axios.get(url), $ = cheerio.load(data), fileHash = $("div.col-xl-4.col-lg-5.general-information").attr("data-file-hash"), tokens = $("input[name='token']").val(), result = {}, payload = new URLSearchParams(Object.entries({
      token: tokens
    })), { data: res } = await axios.post("https://s5.krakenfiles.com/download/" + fileHash, payload);
  ...
  cocofun = url => new Promise(((resolve, reject) => {
    axios({
      ...
      for (let i of res) {
        i.children && i.children[0] && i.children[0]?.data && (ress = i.children[0]?.data.split("window.APP_INITIAL_STATE=")[1],
  ...
  GDriveDl = async url => {
    ...
    let res = await fetch(`https://drive.google.com/uc?id=${id}&authuser=0&export=download`, {
      ...
      { fileName: fileName, sizeBytes: sizeBytes, downloadUrl: downloadUrl } = JSON.parse((await res.text()).slice(4));
  ...
  xbuddy = url => new Promise((async (resolve, reject) => {
    ...
    const { data: data } = await axios.post("https://ab.9xbud.com/token", {}, {
      headers: headers
    });
  ...
  terabox = urls => new Promise((async (resolve, reject) => {
    ...
    const { searchParams: requestUrl, href: href } = new URL(urls);
