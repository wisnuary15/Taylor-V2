...
class Download {
  tiktok = (url) => {
    return new Promise(async (resolve, reject) => {
      ...
      const res = {
        ...
        sound: $("div#button-download-ready .flex.flex-row.items-center.justify-center.mt-5.w-3/4 span", )
          .text().trim(),
  ...
  instagram = (url) => {
    return new Promise(async (resolve, reject) => {
      let urls;
      if (/(https:\/\/)?instagram\.com\/p\/[^\/?#&]+/.test(url)) urls = "download-photo-instagram";
      if (/(https:\/\/)?instagram\.com\/stories\/highlight\/[^\/?#&]+/.test(url)) urls =
        "download-highlights-instagram";
      if (/(https:\/\/)?instagram\.com\/reel\/[^\/?#&]+/.test(url)) urls = "download-reel-instagram";
      if (/(https:\/\/)?instagram\.com\/stories\/[^\/?#&]+/.test(url)) urls = "download-story-instagram";
    ...
    return new Promise((resolve, reject) => {
      axios({
        ...
        for (let i of res) {
          ...
          const result = {
            status: 200,
            topic: json.share.post.post.content ? json.share.post.post.content : json.share.post.post.topic
              .topic,
  ...
  jooxdl = (url) => {
    return new Promise(async (resolve, reject) => {
      await axios.get("http://api.joox.com/web-fcgi-bin/web_get_songinfo?songid=" + url +
...
function get(url, token) {
  return new Promise(async (resolve, reject) => {
    if (url.startsWith("https://vm.tiktok.com/") || url.startsWith("https://www.tiktok.com/") || url.startsWith(
        "https://m.tiktok.com/v/")) {
      axios.get('https://tikwm.com/api/?url=' + url).then((resp) => {
        const result = {
      ...
      try {
        ...
        `https://graph.facebook.com/v8.0/${vid}?fields=source&access_token=${token}`);
        ...
        if (data.source === undefined) {
          try {
            ...
            if (parsedData.data.video === null && url.indexOf('/permalink/') !== -1) {
              ...
              const videoUrl = parsedData.data.video.playable_url_quality_hd || parsedData.data.video
              .playable_url;
              const result = {
                facebook: videoUrl
              }
    ...
    } else if (url.startsWith("https://www.instagram.com/p/") || url.startsWith(
      "https://www.instagram.com/tv/") || url.startsWith("https://www.instagram.com/reel/") || url.startsWith(
    ...
    } else if (url.startsWith("https://www.instagram.com/stories/") || url.startsWith(
    ...
    } else if (url.startsWith("https://youtube.com/") || url.startsWith("https://www.youtube.com/") || url
      .startsWith("https://youtu.be/")) {
      const vid = url.match(
        /^(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/(?:shorts\/)?(?:watch\?v=)?|youtu\.be\/)([^?/]+)/)[1]
      axios.post("https://api.ytbvideoly.com/api/thirdvideo/parse",
        ...
        const formattedTime =
          `${Math.floor(response.data.data.duration / 60)}:${(response.data.data.duration % 60 < 10 ? "0" : "") + (response.data.data.duration % 60)}`;
        const result = {
          shorts: [{
