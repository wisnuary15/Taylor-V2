  ...
  async ytmp4(url) {
    try {
      ...
      try {
        ...
        return headRes.ok ? {
          url: link.url,
          buffer: Buffer.from(await fetch(link.url, {
            headers: {
              Referer: url
            }
...
async function ytmp4(url) {
  try {
    ...
    const data = await ytdl.getInfo(`https://www.youtube.com/watch?v=${id}`, kuki);
    const formats = data.formats;
    const video = formats.filter(format => format.container === "mp4" && format.hasVideo && format.hasAudio).map(format => format.url);
...
const Ytdl = {
    ...
    try {
      ...
      try {
        ...
        return headRes.ok ? {
          ...
          buffer: Buffer.from(await fetch(downloadUrl || downloadData.downloadUrlX, {
            headers: {
              Referer: url
            }
          }).then(res => res.arrayBuffer())),
  ...
  all: async (u, f) => {
    ...
    try {
      ...
      return headRes.ok ? {
        url: downloadUrl,
        buffer: Buffer.from(await fetch(downloadUrl, {
          headers: {
            Referer: url
          }
        }).then(res => res.arrayBuffer())),
