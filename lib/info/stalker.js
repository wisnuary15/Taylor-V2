...
class Stalker {
  ...
  instagram = async username => new Promise((async (resolve, reject) => {
    try {
      const { data: data } = await axios.get(this.instagramURL + "/search?user=" + username), $ = cheerio.load(data), search = $("div.row > div.col-sm-6.col-md-4.col-lg-3").map(((_, el) => ({
  ...
  twetter = async username => new Promise((async (resolve, reject) => {
    await axios.get(this.twetterURL + "/" + username).then((({ data: data }) => {
  ...
  npm = async pkg => new Promise((async (resolve, reject) => {
    await axios.get(this.npmURL + "/package/" + pkg).then((({ data: data }) => {
      let $ = cheerio.load(data),
        keywords = [],
        info = [],
        collaborator = [];
  ...
  youtube = user => new Promise((async (resolve, reject) => {
    ...
    await axios.post("https://ytlarge.com/youtube/channel-id-finder/channel-id-finder", data).then((({ data: data }) => {
      const $ = cheerio.load(data),
        ...
        channelDescription = ($('span:contains("Has This Channel Ever Advertised Before?:") + font').text().trim(),
          $('span:contains("Channel Description:") + span').text().trim()),
        profileImage = {},
        bannerImage = {};
