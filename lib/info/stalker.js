...
class Stalker {
    ...
    tiktok = async (username) => {
        return new Promise(async (resolve, reject) => {
            await axios
                .get(this.tiktokURL + "/user/" + username)
                .then((response) => {
                    const $ = cheerio.load(response.data);
                    const result = {
                        profile: $(
    ...
    github = async (username) => {
        return new Promise(async (resolve, reject) => {
            await axios
                .get(this.githubURL + "/users/" + username)
                .then((response) => {
                    ...
                    $("div.activity-posts").each(function(a, b) {
                        tweets.push({
                            author: {
                                ...
                                nickname: $(b).find("div.user-text3 > h4").text().split("@")[0] ||
                            ...
                            media: $(b).find("div.activity-descp > div > a").attr("href") ||
            ...
            await axios
                ...
                .then(({
                    ...
                    let hasil = {
                        ...
                        language: $("#top > div.w-100.ph0-l.ph3.ph4-m > h2 > div").text() ||
                            "Default",
                        publish: $(
                            "#top > div.w-100.ph0-l.ph3.ph4-m > span:nth-child(4)",
                        ).text(),
                        readme: $("#readme").text() || "",
                        explore: $("#package-tab-explore > span")
                            .text()
                            .replace(" Explore ", "") || "",
                        dependencies: $("#package-tab-dependencies > span")
                            .text()
                            .replace(" Dependencies", "") || "",
                        dependents: $("#package-tab-dependents > span")
                            .text()
                            .replace(" Dependents", "") || "",
                        version_count: $("#package-tab-versions > span")
                            .text()
                            .replace(" Versions", "") || "",
                        keywords: keywords || [],
                        install: $(
    ...
    freeFire = async (id) => {
        return new Promise(async (resolve, reject) => {
            ...
            await axios
                .post(this.freeFireURL + "/id/initPayment.action",
                    data
                )
                .then((response) => {
                    resolve({
    ...
    mobileLegends = async (id, zoneId) => {
        return new Promise(async (resolve, reject) => {
            ...
            await axios
                .post(this.mobileLegendsURL + "/api/transaction/v1/top-up/inquiry/store",
                    data
                )
                .then((response) => {
