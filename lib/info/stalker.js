...
class Stalker {
  ...
  instagram = async (username) => {
    return new Promise(async (resolve, reject) => {
      try {
        const {
          data
        } = await axios.get(this.instagramURL + "/search?user=" + username, );
        ...
        const result = {
          ...
          posts: $$("div.col-lg-5.separate-column > div.row > div.col > div.number-box > .count", ).eq(0)
            .text().trim(),
          followers: $$("div.col-lg-5.separate-column > div.row > div.col > div.number-box > .count", ).eq(
            1).text().trim(),
          following: $$("div.col-lg-5.separate-column > div.row > div.col > div.number-box > .count", ).eq(
            2).text().trim(),
        };
        resolve(result);
      } catch (e) {
  ...
  tiktok = async (username) => {
    return new Promise(async (resolve, reject) => {
      await axios.get(this.tiktokURL + "/user/" + username).then((response) => {
        const $ = cheerio.load(response.data);
        const result = {
          profile: $("div.row > div.col-lg-7.separate-column > div.user-info > figure > img", ).attr(
            "src"),
          username: $(
          ...
          likes: $("div.col-lg-5.separate-column > div.row > div.col > div.number-box > .count", ).eq(0)
            .text().trim(),
          followers: $("div.col-lg-5.separate-column > div.row > div.col > div.number-box > .count", )
            .eq(1).text().trim(),
          following: $("div.col-lg-5.separate-column > div.row > div.col > div.number-box > .count", )
            .eq(2).text().trim(),
        };
        resolve(result);
      }).catch((e) => {
      ...
      await axios.get(this.twetterURL + "/" + username).then(({
        ...
        $("div.activity-posts").each(function(a, b) {
          tweets.push({
            author: {
              ...
              nickname: $(b).find("div.user-text3 > h4").text().split("@")[0] || $(b).find(
                "div.user-text3 > h4").text().trim(),
            ...
            media: $(b).find("div.activity-descp > div > a").attr("href") || $(b).find(
              "div.activity-descp > p > video").attr("src") || $(b).find(
              "div.activity-descp > div > a > img").attr("src") || $(b).find(
              "div.activity-descp > div > a > video").attr("src") || "No Media Upload",
            retweet: $(b).find("div.like-comment-view > div > a:nth-child(1) > span").text()
        ...
        let hasil = {
          username: $(
            ...
            .text(),
          nickname: $("body > main > div.dash-dts > div > div > div:nth-child(1) > div > div > h3", )
            .text().split("@")[0] || $(
              "body > main > div.dash-dts > div > div > div:nth-child(1) > div > div > h3", ).text(),
          background: $(
  ...
  npm = async (pkg) => {
    return new Promise(async (resolve, reject) => {
      await axios.get(this.npmURL + "/package/" + pkg).then(({
        ...
        $("#top > div.fdbf4038.w-third-l.mt3.w-100.ph3.ph4-m.pv3.pv0-l > div.w-100 > ul > li", ).each(
          function(a, b) {
            collaborator.push({
        ...
        let hasil = {
          ...
          keywords: keywords || [],
          install: $("#top > div.fdbf4038.w-third-l.mt3.w-100.ph3.ph4-m.pv3.pv0-l > p > code > span", )
            .text(),
          info,
          collaborator,
        };
  ...
  youtube = (user) => {
    return new Promise(async (resolve, reject) => {
      ...
      await axios.post("https://ytlarge.com/youtube/channel-id-finder/channel-id-finder", data).then(({
        ...
        const channelUrl = $('a[href^="https://www.youtube.com/channel/"]', ).attr("href");
        const profileId = $('div[align="center"] b').text();
        const profileUrl = $("img").attr("src");
        const creationDate = $('span:contains("Channel Creation Date:") + span', ).text().trim();
        const videoCount = $('span:contains("Video Count:") + span').text().trim();
        const subscriberCount = $('span:contains("Subscriber Count:") + span').text().trim();
        const totalViews = $('span:contains("Total Views:") + span').text().trim();
        const country = $('span:contains("Country:") + span font').text().trim();
        const monetizationStatus = $('span:contains("Is This Channel Monetized?:") + font', ).text().split(
          ":")[1].trim();
        const hasAdvertisedBefore = $('span:contains("Has This Channel Ever Advertised Before?:") + font', )
          .text().trim();
        const channelDescription = $('span:contains("Channel Description:") + span', ).text().trim();
        const profileImage = {};
        const bannerImage = {};
        ...
        (bannerImage.image = $(".elementor-widget-text-editor").eq(1).find("a img").attr("src")),
  ...
  mobileLegends = async (id, zoneId) => {
    return new Promise(async (resolve, reject) => {
      ...
      await axios.post(this.mobileLegendsURL + "/api/transaction/v1/top-up/inquiry/store", data).then((
        response) => {
