...
class WABetaInfo {
  async home() {
    try {
      const data = await viewSource.view("https://wabetainfo.com", 2),
        $ = cheerio.load(data);
  ...
  async read(url) {
    try {
      const data = await viewSource.view(url, 2),
        $ = cheerio.load(data);
  ...
  async search(q) {
    try {
      const data = await viewSource.view(`https://wabetainfo.com/?s=${encodeURIComponent(q)}`, 2),
        $ = cheerio.load(data);
      return $('article[id^="post-"]').map((_, el) => ({
        title: $(".entry-title a", el).text().trim(),
