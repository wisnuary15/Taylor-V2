...
class WABetaInfo {
  async home() {
    try {
      const { data: data } = await axios.get("https://wabetainfo.com"), $ = cheerio.load(data);
  ...
  async search(q) {
    try {
      const { data: data } = await axios.get(`https://wabetainfo.com/?s=${encodeURIComponent(q)}`), $ = cheerio.load(data);
      return $('article[id^="post-"]').map(((_, el) => ({
        title: $(".entry-title a", el).text().trim(),
