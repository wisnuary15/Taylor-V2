...
class Melon {
  async songSearch(_section, _q, _page, _sort) {
    try {
      ...
      const songIds = Array.from($('tbody > tr > td:nth-of-type(1) input').map((i, el) => parseInt($(el).attr('value')
        .trim())));
      const songTitles = Array.from($('tbody > tr > td:nth-of-type(3) a.fc_gray').map((i, el) => $(el).attr('title')
        .trim()));
      const artists = Array.from($('tbody > tr > td:nth-of-type(4) span > a.fc_mgray').map((i, el) => $(el).attr(
        'title').split(' - 페이지 ')[0]?.trim()));
  ...
  async songDetail(_songId) {
    try {
      ...
      const songTitle = $song('#downloadfrm > div > div > div.entry > div.info > div.song_name').text().trim().split(
        '\t').splice(-1)[0];
      const albumTitle = $song('#downloadfrm > div > div > div.entry > div.meta > dl > dd:nth-child(2) > a').text()
        .trim();
      const artistName = $song('#downloadfrm > div > div > div.entry > div.info > div.artist > a > span:nth-child(1)')
        .text().trim();
      const artistThumbnail = $song(
        '#downloadfrm > div > div > div.entry > div.info > div.artist > a > span.thumb_atist > img').attr('src');
      const releasedAt = new Date($song('#downloadfrm > div > div > div.entry > div.meta > dl > dd:nth-child(4)')
        .text().trim()).toISOString();
      ...
      const thumbnailBaseURL = thumbnailRawURL.match(/\_500\.jpg/) ? thumbnailRawURL.split('_500.jpg')[0] :
        thumbnailRawURL.match(/\_1000\.jpg/) ? thumbnailRawURL.split('_1000.jpg')[0] : thumbnailRawURL.split('.jpg')[
          0];
      return {
        songId: _songId,
        songTitle,
  ...
  async albumDetail(_albumId) {
    try {
      ...
      const albumTitle = $album('#conts > div.section_info > div > div.entry > div.info > div.song_name').text()
      .trim().split('\t').splice(-1)[0];
      const artistName = $album('#conts > div.section_info > div > div.entry > div.info > div.artist > a').attr(
        'title').trim();
      const artistThumbnail = $album(
        '#conts > div.section_info > div > div.entry > div.info > div.artist > a > span.thumb_atist > img').attr(
        'src');
