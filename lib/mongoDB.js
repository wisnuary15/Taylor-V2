...
export class mongoDBV2 {
  constructor(url, options = defaultOptions) {
    this.url = url, this.options = options, this.models = [], this.data = {},
      this.lists, this.list, this.db = mongoose.createConnection(this.url, {
        ...this.options
      }), this.logDBState(), this.delete = {
        collection: async collectionName => {
          try {
  ...
  async write(data) {
    try {
      ...
      for (const key of Object.keys(data)) {
        ...
        await doc.deleteMany().catch(console.error), await doc.insertMany(docData),
          doc && key && listDoc.push({
            name: key
          });
      }
