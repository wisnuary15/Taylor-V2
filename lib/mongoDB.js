...
const { STATES: STATES } = mongoose, defaultOptions = {
  useNewUrlParser: !0,
...
export class mongoDB {
  constructor(url, options = defaultOptions) {
    this.url = url, this.options = options, this.data = this._data = {}, this._schema = {},
      this._model = {}, this.db = mongoose.createConnection(this.url, {
        ...this.options
      }), this.logDBState(), this.delete = {
        collection: async collectionName => {
          try {
            await this.db.dropCollection(collectionName), console.log(chalk.bgGreen.black(" MongoDB \n"), chalk.yellow(">>\n"), chalk.bgYellow.black(`Collection '${collectionName}' deleted successfully`));
          } catch (error) {
  ...
  async read() {
    try {
      ...
      if (this._model = this.db.model("data", schema), this._data = await this._model.findOne({}),
        this.data = this._data?.data ?? {}, !this._data) {
        const [_ignored, _data] = await Promise.all([this.write(this.data), this._model.findOne({})]);
        this._data = _data, this.data = this._data?.data ?? {};
      }
      return console.log(chalk.bgGreen.black(" MongoDB \n"), chalk.yellow(">>\n"), chalk.bgYellow.black("Read successful")),
        this.data;
    } catch (error) {
  ...
  async write(data) {
    try {
      ...
      const savedData = this._data ? await this._model.findOneAndUpdate({
        _id: this._data?._id
      }, {
        data: data
      }, {
        new: !0
      }) : await new this._model({
        data: data
      }).save();
      return this.data = savedData?.data ?? {}, console.log(chalk.bgGreen.black(" MongoDB \n"), chalk.yellow(">>\n"), chalk.bgYellow.black("Write successful")),
        savedData;
    } catch (error) {
      throw console.log(chalk.bgRed.black(" MongoDB \n"), chalk.yellow(">>\n"), chalk.bgRed.black(`Write error: ${error}`)),
        error;
    }
  }
  logDBState() {
...
export class mongoDBV2 {
  constructor(url, options = defaultOptions) {
    this.url = url, this.options = options, this.models = [], this.data = {}, this.lists,
      this.list, this.db = mongoose.createConnection(this.url, {
        ...this.options
      }), this.logDBState(), this.delete = {
        collection: async collectionName => {
          try {
  ...
  async read() {
    try {
      ...
      this.list = this.db.model("lists", schema), this.lists = await this.list.findOne({}),
        this.lists?.data || (await this.list.create({
          data: []
        }), this.lists = await this.list.findOne({}));
      const garbage = [];
      for (const { name: name } of this.lists?.data ?? []) {
        let collection;
        try {
          ...
          try {
            ...
            garbage.push(name), console.log(chalk.bgRed.black(" MongoDB \n"), chalk.yellow(">>\n"), e);
          }
        }
        if (collection) {
          this.models.push({
      ...
      if (!await this.list.findOneAndUpdate({
          _id: this.lists?._id
        }, {
    ...
    try {
      ...
      for (const key of Object.keys(data)) {
        const index = this.models.findIndex((v => v.name === key)),
          doc = -1 !== index ? this.models[index].model : this.db.model(key, new mongoose.Schema({
            data: Array
          }));
        this.models[-1 === index ? this.models.length : index] = {
          name: key,
          model: doc
        };
        ...
        await doc.deleteMany().catch(console.error), await doc.insertMany(docData), doc && key && listDoc.push({
          name: key
        });
      }
