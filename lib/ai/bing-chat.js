...
class BingChat {
  ...
  async createConversation() {
    try {
      ...
      this.json.arguments[0].conversationSignature = json.conversationSignature,
        this.json.arguments[0].participant.id = json.clientId, this.json.arguments[0].conversationId = json.conversationId;
    } catch (error) {
      throw console.log(chalk.red("Error in createConversation method:", error)),
        error;
    }
  }
  async sendMessage(msg) {
    try {
      ...
      throw console.log(chalk.red("Error in sendMessage method:", error)),
        error;
    }
  }
}
