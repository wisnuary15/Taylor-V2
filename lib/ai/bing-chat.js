    ...
    this.json = {
      arguments: [{
        ...
        optionsSets: ["nlu_direct_response_filter", "deepleo", "disable_emoji_spoken_text",
          "responsible_ai_policy_235", "enablemm", "galileo", "dlwebtrunc", "glpromptv3plus", "serploc",
          "jbf101", "dv3sugg",
        ],
        allowedMessageTypes: ["Chat", "InternalSearchQuery", "InternalSearchResult", "Disengaged",
          "InternalLoaderMessage", "RenderCardRequest", "AdsQuery", "SemanticSerp", "GenerateContentQuery",
          "SearchQuery",
        ],
        sliceIds: ["ssoverlap25", "sspltop5", "sswebtop2", "chk1cln", "fstldsydaoltt", "nofbkcf", "sugdivdis",
          "sydpayajax", "fixsacodecf", "185fdbk", "321sloc", "324hlthmons0", "403jbf101", "notigersccf",
          "udsdserlc", "udstrclm10cmp", "udstrclm10", "329v3pwebtrunc",
        ],
        verbosity: "verbose",
      ...
      return await new Promise((resolve, reject) => {
        ...
        chat.onmessage = (event) => {
          ...
          if (message === "{}") {
            ...
            this.json.invocationId = (parseInt(this.json.invocationId) + 1).toString();
            this.json.arguments[0].isStartOfSession = false;
          }
          if (json.type === 2) {
            const response = json.item.messages.filter(
              (e) => e.messageType === "Chat").reverse()[0] || json.item.messages.filter(
              (e) => e.author === "bot" && e.adaptiveCards[0]?.body[0]?.type === "TextBlock").reverse()[0];
            chat.close();
            resolve({
              ...
              card: (json.item.messages.filter(
                (e) => e.messageType === "InternalSearchResult").reverse()[0] || {}).groundingInfo,
              spokenText: (json.item.messages.filter(
                (e) => e.spokenText != undefined).reverse()[0] || {}).spokenText,
            });
          }
        };
      });
