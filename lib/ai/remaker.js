...
class RemakerAi {
  constructor(token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MTMwOTY4MjMsInByb2R1Y3RfY29kZSI6IjA2NzAwMyIsInRpbWUiOjE3MjU3OTc3ODl9.JCa-FpsS9dEpZj5g0kg-gcIHID1b_Vig71cThP0Xm4M") {
    this.headers = {
      Authorization: token,
      "Product-Code": "067003",
  ...
  async enhance(imageBuffer) {
    try {
      ...
      const response = await fetch("https://developer.remaker.ai/api/remaker/v1/ai-enhance/create-job", {
        method: "POST",
        body: formData,
  ...
  async pollForResult(jobId) {
    try {
      ...
      let attempts = 0;
      while (attempts < 60) {
        const output = await fetch(`https://developer.remaker.ai/api/remaker/v1/ai-enhance/${jobId}`, {
          method: "GET",
          headers: this.headers
        });
  ...
  async swapvideo(targetVideoUrl = "https://pai-data-5f568.oss-us-west-1.aliyuncs.com/test/mda-pi8ndtq6h3gkkic7.mp4", swapImageBuffer) {
    try {
      const formData = new FormData();
      ...
      const response = await fetch("https://developer.remaker.ai/api/remaker/v1/face-swap-video/create-job", {
        method: "POST",
        body: formData,
        headers: this.headers
      });
  ...
  async portrait(imageBuffers, styleName = "Photographic (Default)", prompt, negativePrompt, advanced = 1, styleStrengthRatio = 15, numOutputs = 2) {
    try {
      const formData = new FormData();
      ...
      for (const imageBuffer of imageBuffers) {
        ...
        formData.append("image_file", imageBlob, `image_${imageBuffers.indexOf(imageBuffer) + 1}.png`);
      }
      const response = await fetch("https://developer.remaker.ai/api/remaker/v1/ai-portrait/create-job", {
        method: "POST",
        body: formData,
        headers: this.headers
      });
  ...
  async baby(dadPhotoBuffer, momPhotoBuffer, gender = "boy", numOutputs = 4) {
    try {
      ...
      const response = await fetch("https://developer.remaker.ai/api/remaker/v1/ai-baby/create-job", {
        method: "POST",
        body: formData,
        headers: this.headers
      });
  ...
  async videoenhancer(targetVideo = "https://pai-data-5f568.oss-us-west-1.aliyuncs.com/test/mda-pi8ndtq6h3gkkic7.mp4") {
    try {
      const formData = new FormData();
      formData.append("target_video_url", targetVideo);
      const response = await fetch("https://developer.remaker.ai/api/remaker/v1/ai-video-enhancer/create-job", {
        method: "POST",
        body: formData,
        headers: this.headers
      });
