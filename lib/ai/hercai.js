...
class Hercai {
  constructor() {}
  async question({ model: model = "v2", content: content }) {
    if (["v2", "beta", "v3-beta"].some((ind => model === ind)) || (model = "v2"),
      !content || null == content) throw new Error("Please specify a question!");
    try {
  ...
  async drawImage({ model: model = "v1", prompt: prompt }) {
    if (["v1", "v2", "v2-beta", "v3", "lexica", "prodia"].some((ind => model === ind)) || (model = "prodia"),
      !prompt || null == prompt) throw new Error("Please specify a prompt!");
    try {
