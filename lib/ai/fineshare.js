...
class FineShare {
  ...
  async voiceUpload(uuid, host, audio, name, type) {
    try {
      const { ext: ext, mime: mime } = await fileTypeFromBuffer(audio) || {}, blob = new Blob([audio], {
        type: mime
  ...
  async getvoiceResult(uuid) {
    ...
    do {
      try {
        ...
        const response = await fetch(url, options),
          data = await response.json();
        data.url ? (this.spinner.succeed(chalk.green.bold("Success:") + chalk.white(" Get Voice Result request.")),
          dataUrl = data.url) : (this.spinner.text = "Waiting for URL...", this.spinner.render(),
