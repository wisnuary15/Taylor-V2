...
class RushChat {
  constructor(token) {
    ...
    this.token = token || "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJsb2dpblR5cGUiOiJsb2dpbiIsImxvZ2luSWQiOjE4MzUzMDI5MDcxNDQ2MzQzNzAsInJuU3RyIjoiWmxYbDZhdDJhMnB6d2dLQVBCamY1T0dSb0VDR0xWY0QifQ.1HzKHmTcdkvhMtuykuJ03_VNsePnXwQ_n-JBYA-T_tY";
  ...
  async sendRequest(content, conversationId = "1835302963874164737", replyMsgType) {
  ...
  async chat(content, conversationId = "1835302963874164737") {
    try {
      ...
      return text.split("\n\n").filter(Boolean).map(msg => msg.split("\n")).flat().filter(part => part.startsWith("data:")).map(part => JSON.parse(part.slice(5).trim())).map(parsed => parsed.data).join("").trim();
    } catch (error) {
  ...
  async image(content, conversationId = "1835302963874164737") {
    try {
      ...
      console.log("Raw response text:", text);
      const imgMsgId = text.split("\n\n").filter(Boolean).map(msg => msg.split("\n")).flat().filter(part => part.startsWith("data:")).map(part => JSON.parse(part.slice(5).trim())).find(parsed => parsed.msgId)?.msgId;
  ...
  async getImageResult(imgMsgId) {
    ...
    try {
      while (Date.now() - startTime < timeout) {
        const response = await fetch(`${this.apiUrl}/getImgGenResult?imgMsgId=${imgMsgId}`, {
          headers: {
      ...
      if (result === null) throw new Error("Timeout: Image result not available within 1 minute.");
      return result;
    } catch (error) {
