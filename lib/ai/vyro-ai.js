...
class VyroAI {
  constructor({ key: key = "vk-dk3DMbasbyjxOeOVu9k8lqnDAiKYs1NaqMEOBT6kwzHJS", link: link = !0, buffer: buffer = !0 }) {
  ...
  async catbox(content) {
    try {
      const { ext: ext, mime: mime } = await fileTypeFromBuffer(content) || {}, blob = new Blob([content], {
        type: mime
  ...
  async remixImage(prompt, styleId, imageBuffer) {
    const url = `${this.baseUrl}edits/remix`,
      { mime: mime } = await fileTypeFromBuffer(imageBuffer) || {},
