...
async function extractHtmlZip(zipBuffer) {
  try {
    ...
    const htmlFileEntries = zip.getEntries().filter(entry => entry.entryName.endsWith(".html"));
  ...
  async view(url, index = 0) {
    try {
      if (index === 0) {
        ...
        const response = await fetch(`https://wholly-api.skinnyrunner.com/get/website-data.php?get_html=${url}`, {
          method: "GET",
          headers: {
            "User-Agent": "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Mobile Safari/537.36",
            Accept: "*/*"
