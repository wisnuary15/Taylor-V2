...
const defaultImageOptions = ["-vcodec", "libwebp", "-vf", "scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15,pad=320:320:-1:-1:color=white@0.0,split[a][b];[a]palettegen=reserve_transparent=on:transparency_color=ffffff[p];[b][p]paletteuse", "-lossless", "0", "-compression_level", "6", "-qscale", "80", "-preset", "default"],
  defaultVideoOptions = ["-vcodec", "libwebp", "-vf", "scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15,pad=320:320:-1:-1:color=white@0.0,split[a][b];[a]palettegen=reserve_transparent=on:transparency_color=ffffff[p];[b][p]paletteuse", "-loop", "0", "-ss", "00:00:00.0", "-t", "00:00:05.0", "-lossless", "0", "-compression_level", "6", "-qscale", "80", "-preset", "default", "-an", "-vsync", "0"];
...
async function videoConvert(buffer, options = []) {
  return await (await spawn_ffmpeg(buffer, ["-c:v", "libx264", "-pix_fmt", "yuv420p", "-crf", "18", "-preset", "slow", "-tune", "film", "-vf", "scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15,pad=320:320:-1:-1", "-c:a", "aac", "-b:a", "192k", "-ar", "48000", ...options], "mp4", "mp4")).toBuffer();
}
...
async function convertToWebp(buffer, isAnimated, options) {
  return new Promise(async (resolve, reject) => {
    ...
    isAnimated ? (inputPath = `${tmpdir()}/${Math.random().toString(36)}.mp4`, optionsFfmpeg = ["-vcodec", "libwebp", "-filter:v", `fps=fps=${options.fps}`, "-lossless", "0", "-compression_level", "6", "-q:v", "10", "-qscale", "80", "-loop", "1", "-preset", "default", "-an", "-vsync", "0", "-s", "512:512"]) : (inputPath = `${tmpdir()}/${Math.random().toString(36)}.png`, buffer = await editImage(buffer, options.type), optionsFfmpeg = ["-vcodec", "libwebp", "-loop", "0", "-lossless", "0", "-compression_level", "6", "-q:v", "100", "-qscale", "80"]),
...
async function addExif(buffer, pack, author, metadata = {}, extra = {}) {
  ...
  delete cleanExtra.isId, delete cleanExtra.packname, delete cleanExtra.author,
    delete cleanExtra.isEmail, delete cleanExtra.isWeb, delete cleanExtra.categories,
    delete cleanExtra.androidLink, delete cleanExtra.isFirst, delete cleanExtra.osLink,
  ...
  return exif.writeUIntLE(jsonBuffer?.length, 14, 4), await img.load(buffer),
    img.exif = exif, await img.save(null);
}
