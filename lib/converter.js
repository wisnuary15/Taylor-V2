...
async function imageToWebp(buffer, stickerOptions = {author : 'Taylor-V2', pack : 'Wudysoft', fps : 30, type: 'default'}) {
try {
let options = {
            author: stickerOptions?.author || 'Taylor-V2',
            pack : stickerOptions?.pack || 'Wudysoft',
            fps: stickerOptions?.fps || 30,
...
let {ext} = await fileTypeFromBuffer(buffer)
  return await(await spawn_ffmpeg(buffer, [...defaultImageOptions, ...Object.entries(options).map(([key, value]) => `${key}=${value}`)], ext, 'webp')).toBuffer();
  } catch (e) {
  console.log(e)
  }
  }
...
async function videoToWebp(buffer, stickerOptions = {author : 'Taylor-V2', pack : 'Taylor-V2', fps : 30, type: 'default'}) {
try {
let options = {
            author: stickerOptions?.author || 'Taylor-V2',
            pack : stickerOptions?.pack || 'Wudysoft',
...
export const toSticker  = async (buffer, stickerOptions = {author : 'Taylor-V2', pack : 'Wudysoft', fps : 10, type: 'default'}) =>{
    try{
        let options = {
            author: stickerOptions?.author || 'Taylor-V2',
            pack : stickerOptions?.pack || 'Wudysoft',
