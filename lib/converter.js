...
async function imageToWebp(buffer, stickerOptions = {
  ...
  try {
    ...
    try {
      let { ext: ext } = await fileTypeFromBuffer(buffer);
...
export { imageToWebp, videoToWebp, toPTT, toAudio, toAudio8k, toVideo, toMp4, videoConvert, toGif, ffmpeg };
const webp2mp4File = source => new Promise(((resolve, reject) => {
  ...
  TelegraPh = Path => new Promise((async (resolve, reject) => {
    ...
    try {
      ...
      const { data: data } = await axios.post("https://telegra.ph/upload", form, {
        headers: {
          ...form.getHeaders()
        }
      });
...
const createSticker = async (buffer, options, metadata = {}) => {
  let { mime: mime } = await fileTypeFromBuffer(buffer), isAnimated = mime.startsWith("video") || mime.includes("gif");
...
async function convertToWebp(buffer, isAnimated, options) {
  return new Promise((async (resolve, reject) => {
    ...
    isAnimated ? (inputPath = `${tmpdir()}/${Math.random().toString(36)}.mp4`, optionsFfmpeg = ["-vcodec", "libwebp", "-filter:v", `fps=fps=${options.fps}`, "-lossless", "0", "-compression_level", "6", "-q:v", "10", "-qscale", "80", "-loop", "1", "-preset", "default", "-an", "-vsync", "0", "-s", "512:512"]) : (inputPath = `${tmpdir()}/${Math.random().toString(36)}.png`,
        buffer = await editImage(buffer, options.type), optionsFfmpeg = ["-vcodec", "libwebp", "-loop", "0", "-lossless", "0", "-compression_level", "6", "-q:v", "100", "-qscale", "80"]),
