...
async function spawn_ffmpeg(buffer, args = [], ext = '', ext2 = '') {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      else await fs.writeFile(tmp, buffer);
...
async function imageToWebp(buffer, stickerOptions = {
  ...
  try {
    ...
    try {
      ...
      return await (await spawn_ffmpeg(buffer, [...defaultImageOptions, ...Object.entries(opsi).map(([key,
...
export const toSticker = async (buffer, stickerOptions = {
  ...
  try {
    ...
    let bufferWebp = await createSticker(buffer, options, extra)
    return bufferWebp
  } catch (err) {
    throw err
  }
