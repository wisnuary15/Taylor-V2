...
function kusonimelatest() {
  return new Promise((resolve, reject) => {
    axios.get(`https://kusonime.com`).then(({
      ...
      const $ = cheerio.load(data)
      const hasil = [];
      $('#venkonten > div.vezone > div.venser > div > div.rseries > div.rapi > div.venz > ul > div').each(
        function(a, b) {
          result = {
...
function mangatoons(query) {
  return new Promise((resolve, reject) => {
    axios.get(`https://mangatoon.mobi/en/search?word=${query}`).then(({
      ...
      const $ = cheerio.load(data)
      const hasil = [];
      $('#page-content > div.search-page > div > div.comics-result > div.recommended-wrap > div > div ').each(
        function(a, b) {
          result = {
            status: 200,
            author: author,
  ...
  return new Promise(async (resolve, reject) => {
    axios.get('https://seegore.com/?s=' + query).then((dataa) => {
      ...
      axios.get(slink).then(({
        ...
        $('#post-items > li > article > div.content > header > div > div.bb-cat-links > a').each(function(
          e, f) {
          upl = $(f).text()
          uploader.push(upl)
        })
...
function doujindesu(url) {
  return new Promise((resolve, reject) => {
    ...
    axios.get(url).then((res) => {
      ...
      hasil.status = $('#archive > div > section > table > tbody > tr:nth-child(1) > td:nth-child(2) > a')
      .text()
      ...
      hasil.author = $('#archive > div > section > table > tbody > tr:nth-child(4) > td:nth-child(2) > a')
      .text()
      hasil.group = $('#archive > div > section > table > tbody > tr:nth-child(5) > td:nth-child(2) > a').text()
      hasil.rating = $('#archive > div > section > table > tbody > tr:nth-child(6) > td:nth-child(2) > div')
        .text()
      hasil.upload = $('#archive > div > section > table > tbody > tr.created.createdAt > td:nth-child(2)')
        .text()
      hasil.synopsis = $('#archive > div > section > div.pb-2 > p:nth-child(1)').text() ||
    ...
    axios.get(url).then(({
      ...
      $('#chapter_list > ul > li').each(function(a, b) {
        result = {
          ...
          dl_url: $(b).find('> div.chright > span > a').attr('href') ||
...
function kusonimeinfo(url) {
  return new Promise((resolve, reject) => {
    axios.get(url).then((res) => {
      ...
      hasil.synopsis = $('#venkonten > div.vezone > div.venser > div.venutama > div.lexot > p:nth-child(3)')
        .text()
      hasil.dl_url_360p = $(
...
function otakudesuinfo(url) {
  return new Promise((resolve, reject) => {
    ...
    axios.get(url).then((res) => {
      const $ = cheerio.load(res.data)
      hasil.judul = $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(1) > span')
        .text().split(': ')[1]
      hasil.japanese = $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(2) > span')
        .text().split(': ')[1]
      hasil.rating = $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(3) > span')
        .text().split(': ')[1]
      hasil.produser = $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(4) > span')
        .text().split(': ')[1]
      hasil.tipe = $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(5) > span')
        .text().split(': ')[1]
      hasil.anime_status = $(
        '#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(6) > span').text().split(
        ': ')[1]
      hasil.total_episode = $(
        '#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(7) > span').text().split(
        ': ')[1]
      hasil.durasi = $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(8) > span')
        .text().split(': ')[1]
      hasil.rilis = $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(9) > span')
        .text().split(': ')[1]
      hasil.studio = $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(10) > span')
        .text().split(': ')[1]
      hasil.genre = $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(11)').text()
        .split(': ')[1]
    ...
    axios({
      ...
      axios({
        ...
        axios({
          ...
          method: 'POST',
          data: new URLSearchParams(JSON.parse(gdata[1].replace(/\[/g, '').replace(/\]/g, '').replace(
            /text/g, 'text[]').replace(/text\[\]pro\.me/g, 'textpro.me'))),
          headers: {
            'cookie': '_ga=GA1.2.973149439.1655802326; __gads=ID=48c7bf36c499a70a-2250dc73a4d300d3:T=1655802326:RT=1655802326:S=ALNI_MZutfiaIw3EmxBoKDfSTxZMydnKrA; PHPSESSID=24eg6g44qnh7f34evgskg1ehf1; cookieconsent_status=dismiss; _gid=GA1.2.1310090408.1656776919; __gpi=UID=0000067655c41509:T=1655802326:RT=1656832156:S=ALNI_MYb5ILRvXyZT7o_Ts9jfr_e86-_8w; _gat_gtag_UA_114571019_5=1',
            'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/102.0.0.0 Safari/537.36'
          }
