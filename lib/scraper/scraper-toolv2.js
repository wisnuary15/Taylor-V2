...
async function tiktokTts(text, model) {
  try {
    const modelVoice = model || "en_us_001",
      { status: status, data: data } = await axios.post("https://tiktok-tts.weilnet.workers.dev/api/generation", {
        text: text,
        voice: modelVoice
      }, {
        headers: {
...
async function enhanceImg(url, scale) {
  const scaleNumber = scale || 2,
    { data: data } = await axios.post("https://toolsapi.spyne.ai/api/forward", {
      image_url: url,
      scale: scaleNumber,
      save_params: {
        extension: ".png",
...
async function cekResi(kurir, resi) {
  let { data: data } = await axios.post("https://pluginongkoskirim.com/front/resi", {
    kurir: kurir,
    resi: resi
  }, {
    headers: {
...
const baseCerpen = "http://cerpenmu.com/100-cerpen-kiriman-terbaru",
  ...
  fetchURL = async url => {
    ...
    try {
      const { data: data } = await axios.get(`${baseOtakudesu}?s=${query}&post_type=anime`), $ = cheerio.load(data), result = {
        status: !0,
        data: []
      };
    ...
    try {
      const { data: data } = await axios.get(`${baseFilmApik}?s=${query}`), $ = cheerio.load(data), result = {
        status: !0,
        data: []
      };
    ...
    try {
      const { data: data } = await axios.get(`${url}/play`), result = {
        status: !0,
        creator: "Wudysoft",
        Url: {}
      }, $ = cheerio.load(data);
...
async function findSongs(text) {
  try {
    const { data: data } = await axios.get("https://songsear.ch/q/" + encodeURIComponent(text)), $ = cheerio.load(data), result = {
    ...
    }, { data: lyricData } = await axios.get(`https://songsear.ch/api/song/${result.number}?text_only=true`), lyrics = lyricData.song.text_html.replace(/<br\/>/g, "\n").replace(/&#x27;/g, "'");
    return {
      status: !0,
      title: result.title,
      album: result.album,
...
async function igStalk(username) {
  try {
    const { data: data } = await axios.get(`${baseSSS}api/ig/userInfoByUsername/${username}`), pronouns = 0 === data.result.user.pronouns.length ? "" : data.result.user.pronouns.join("/"), res = data.result.user;
    return {
      status: !0,
      creator: "Wudysoft",
...
async function similarBand(query) {
  try {
    const { data: data } = await axios.get(`https://www.music-map.com/${query}`), result = [], $ = cheerio.load(data);
...
export { enhanceImg, cekResi, tiktokTts, ttsModel, anime, truthOrDare, getCerpen, getCerpenHorror, findSongs, igStalk, similarBand, otakuDesuSearch, filmApikS, filmApikDl };