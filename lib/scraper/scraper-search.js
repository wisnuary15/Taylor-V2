  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      $('#search_resultsRows > a').each((a, b) => {
        ...
        const judul = $(b).find(`div.responsive_search_name_combined > div.col.search_name.ellipsis > span`)
          .text()
        const harga = $(b).find(
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const hasil = $('#genresAndManufacturer').html().replace(/\n/g, '').replace(/<br>/g, '\n').replace(/\t/g,
        '').replace(/<b>/g, '').replace(/<\/div>/g, '\n').replace(/ /g, '').replace(/<\/b>/g, ' ').replace(
        /<[^>]*>/g, '')
...
function LinkWa(query) {
  return new Promise(async (resolve, reject) => {
    try {
      const {
        data
      } = await axios.get('http://ngarang.com/link-grup-wa/daftar-link-grup-wa.php?search=' + query +
        '&searchby=name')
      const $ = cheerio.load(data),
        _title = [],
        _link = [],
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      await axios.get(link).then(({
        ...
        const $$ = cheerio.load(data)
        hasil.thumb = 'https:' + $$(
          'div.col-sm-1.col-md-2.col-ml-3.col-lg-3.static-position > div > div > div').find('img').attr(
          'src')
        $$('div.col-sm-10.col-md-8.col-ml-6.col-lg-6 > div.mxm-lyrics').each(function(a, b) {
          hasil.lirik = $$(b).find('span > p > span').text() + '\n' + $$(b).find('span > div > p > span')
            .text()
        })
      })
      resolve(hasil)
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (status === 200) {
        ...
        $('aside.portable-infobox.pi-background.pi-border-color.pi-theme-wikia.pi-layout-default').each((i,
        u) => {
          rull.push($(u).html())
        })
...
const searchEmoji = async (q) => {
  try {
    ...
    const html = await response.text();
    const $ = cheerio.load(html);
    return $('#search__first .s__first__ul a').map((index, element) => 'https://emojigraph.org' + $(element)?.attr(
      'href')).get();
  } catch (error) {
...
const emojiPedia = async (emoji) => {
  try {
    ...
    const outSlug = new URL(getSlug.url).pathname.startsWith('/') ? new URL(getSlug.url).pathname.substring(1) :
...
const EmojiGG = async (query) => {
  ...
  try {
    ...
    const data = await response.json();
    return data.filter(s => s.title === q || s.title?.includes(q))?.length ? data.filter(s => s.title === q || s
      .title?.includes(q)) : null;
  } catch (error) {
