  ...
  async function shortener(t) {
    return (
      (html = (await axios("https://caliphdev.net", {
        method: "POST",
        data: new URLSearchParams(Object.entries({
          url: t
        })),
      })).data),
      ($ = cheerio.load(html)), $("#app-6 > input").attr("value"));
  }
  ...
  async function mediafires(t) {
    ...
    return (
      (mime = n.split(".")),
      (mime = mimetype.lookup(mime[1])), i.push({
        nama: n,
        mime: mime,
        size: r,
        link: o
    ...
    return new Promise(async (e) => {
      try {
        ...
        let i = await (await axios.post("https://downloader4twitter.com/", a)).data,
          o = cheerio.load(i),
          r = [];
        return (o("div.search-form-output").find("img").each((t, e) => /thumb/.test(o(e).attr("src")) ? "" : r
          .push({
            type: "jpg",
            url: o(e).attr("src")
