...
async function ephoto(url, texk) {
  ...
  let { data: data } = await axios.post("https://en.ephoto360.com/effect/create-image", new URLSearchParams(json), {
    headers: {
      "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/105.0.0.0 Safari/537.36",
      cookie: gT.headers["set-cookie"].join("; ")
    }
...
function otakudesu(judul) {
  return new Promise((async (resolve, reject) => {
    axios.get("https://otakudesu.watch/?s=" + judul + "&post_type=anime").then((({ data: data }) => {
      const $ = cheerio.load(data),
        result = {};
      let limk = $("#venkonten > div > div.venser > div > div > ul > li:nth-child(1) > h2 > a").attr("href");
      axios.get(limk).then((({ data: data }) => {
        const $$ = cheerio.load(data);
...
function covid() {
  return new Promise((async (resolve, reject) => {
    axios.get("https://covid19.go.id/").then((({ data: data }) => {
      const $ = cheerio.load(data),
        hasil = [];
...
function ongoing() {
  return new Promise(((reject, resolve) => {
    axios.get("https://otakudesu.link/ongoing//").then((({ data: data }) => {
      const $ = cheerio.load(data),
        result = [],
        img = [],
        epz = [],
...
function komiku(judul) {
  return new Promise((async (resolve, reject) => {
    axios.get("https://data.komiku.id/cari/?post_type=manga&s=" + encodeURIComponent(judul)).then((({ data: data }) => {
      const $ = cheerio.load(data),
        img = [],
        or = [],
        ind = [],
...
function tebakgambar() {
  return new Promise((async (resolve, reject) => {
    axios.get("https://jawabantebakgambar.net/all-answers/").then((({ data: data }) => {
      const $ = cheerio.load(data),
        result = [];
...
function surah(no) {
  return new Promise((async (resolve, reject) => {
    axios.get("https://kalam.sindonews.com/surah/" + no).then((({ data: data }) => {
      const $ = cheerio.load(data),
        result = [],
        ar = [],
        id = [],
...
function sholat(NO) {
  return new Promise((async (resolve, reject) => {
    axios.get("https://kalam.sindonews.com/jadwalsholat/" + NOMOR).then((({ data: data }) => {
      const $ = cheerio.load(data),
        result = {};
...
function lirik(judul) {
  return new Promise((async (resolve, reject) => {
    axios.get("https://www.musixmatch.com/search/" + judul).then((async ({ data: data }) => {
      const $ = cheerio.load(data),
        hasil = {};
...
function wattpad(judul) {
  return new Promise(((resolve, reject) => {
    axios.get("https://www.wattpad.com/search/" + judul).then((({ data: data }) => {
      const $ = cheerio.load(data),
        result = [],
        jdl = [],
        img = [],
...
function playstore(name) {
  return new Promise(((resolve, reject) => {
    axios.get("https://play.google.com/store/search?q=" + name + "&c=apps").then((({ data: data }) => {
      const $ = cheerio.load(data);
      let ln = [],
        nm = [],
        dv = [],
...
function tiktokdl(URL) {
  return new Promise((async (resolve, rejecet) => {
    let { data: data } = await axios.request({
...
export { cerpen, quotes, otakudesu, covid, ongoing, komiku, tebakgambar, surah, sholat, lirik, ffstalk, chara, instadown, gore, spotify, hentaivid, wattpad, playstore, linkwa, tiktokdl, joox, igstalk, twitter, ephoto, youtube, tiktok };