...
const snapsave = async url => {
  const { data: data } = await axios.post(baseUrl + "/action.php", "url=" + encodeURIComponent(url), {
    header: {
      referer: baseUrl
    }
...
async function tikmateApp(url) {
  const { data: data } = await axios.post("https://api.tikmate.app/api/lookup", "url=" + encodeURIComponent(url), {
    headers: {
