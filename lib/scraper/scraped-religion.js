...
const listsurah = async () => {
  try {
    const { data: data } = await axios.get("https://litequran.net/"), $ = cheerio.load(data);
    return {
  ...
  try {
    ...
    const { data: data } = await axios.get(`https://litequran.net/${surahName}`), $ = cheerio.load(data);
  ...
  try {
    surahName = surahName.toLowerCase().replace(/\s/g, "-");
    const { data: data } = await axios.get(`https://tafsirq.com/topik/${surahName}`), $ = cheerio.load(data);
    return {
...
export { listsurah, surah, tafsirsurah };