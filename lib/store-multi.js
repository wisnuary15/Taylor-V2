    ...
    function bind(ev, opts = {
        ...
        ev.on('group-participants.update', async function store(groupParticipantsUpdate) {
            ...
            switch (groupParticipantsUpdate.action) {
                case 'add':
                    metadataparticipants?.push(...groupParticipantsUpdateparticipants?.map(id => ({
                        id,
                        admin: null
                    })))
                    break
                ...
                case 'promote':
                    ...
                    break
                case 'remove':
                    metadata.participants = metadataparticipants?.filter(p => !groupParticipantsUpdateparticipants?.includes(p.id))
                    break
            }
