...
function makeInMemoryStore() {
  ...
  async function fetchImageUrl(jid, profilePictureUrl) {
    ...
    if (!chats[jid].imgUrl) {
      const url = await profilePictureUrl(jid, "image").catch(() => "https://telegra.ph/file/0b23910fbd9261c5aa06c.jpg");
      ...
      switch (type) {
        ...
        case "notify":
          for (const msg of newMessages) {
            ...
            if (type === "notify" && !(jid in chats)) ev.emit("chats.upsert", [{
              id: jid,
      ...
      switch (groupParticipantsUpdate.action) {
        ...
        case "promote":
          for (const participant of metadata.participants)
            if (groupParticipantsUpdate.participants.includes(participant.id)) participant.admin = groupParticipantsUpdate.action === "promote" ? "admin" : null;
          break;
        case "remove":
  ...
  function fromJSON(json) {
    ...
    for (const jid in json.messages) messages[jid] = json.messages[jid].map(m => m && proto.WebMessageInfo.fromObject(m)).filter(m => m && m.messageStubType != WAMessageStubType.CIPHERTEXT);
  }
...
const useMultiFileAuthState = baileysMultiFileAuthState || async function useMultiFileAuthState(folder) {
