...
function makeInMemoryStore() {
    ...
    function bind(ev, opts = {
        ...
        ev.on('group-participants.update', async function store(groupParticipantsUpdate) {
            ...
            case 'add':
                metadata.participants?.push(...groupParticipantsUpdateparticipants?.map(id => ({
                    id,
                    admin: null
                })))
                break
            ...
            case 'promote':
                for (var participant of metadata.participants)
                    if (groupParticipantsUpdateparticipants?.includes(participant.id))
                        participant.admin = groupParticipantsUpdate.action === 'promote' ? 'admin' : null
                break
            case 'remove':
                metadata.participants = metadata.participants?.filter(p => !groupParticipantsUpdateparticipants?.includes(p.id))
                break
            }
            Object.assign(chats[id], {
                metadata
