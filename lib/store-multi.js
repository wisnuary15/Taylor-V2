...
function makeInMemoryStore() {
  ...
  function bind(ev, opts = {
    ...
    ev.on('group-participants.update', async function store(groupParticipantsUpdate) {
      ...
      if (!metadata) return console.log(
        `Try to update group ${id} but metadata not found in 'group-participants.update'`)
      var groupParticipantsUpdates = metadata?.participants || [];
      switch (groupParticipantsUpdate.action) {
        case 'add':
          metadata.participants?.push(...groupParticipantsUpdates.map(id => ({
            id,
            admin: null
          })))
          break
        ...
        case 'promote':
          for (var participant of metadata.participants)
            if (groupParticipantsUpdates?.includes(participant.id)) participant.admin = groupParticipantsUpdate
              .action === 'promote' ? 'admin' : null
          break
        case 'remove':
          metadata.participants = metadata.participants.filter(p => !groupParticipantsUpdates?.includes(p.id))
          break
      }
      Object.assign(chats[id], {
        metadata
  ...
  function fromJSON(json) {
    ...
    for (var jid in json.messages) messages[jid] = json.messages[jid].map(m => m && proto.WebMessageInfo.fromObject(m))
