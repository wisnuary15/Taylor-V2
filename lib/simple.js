  ...
  let sock = Object.defineProperties(conn, {
    ...
    getFile: {
      ...
      async value(PATH, saveToFile = false) {
        ...
        const data = isBuffer ? PATH : isArrayBuffer ? Buffer.from(PATH) : Helper.isReadableStream(PATH) ?
          await stream2buffer(PATH) : /^data:.*?\/.*?;base64,/i.test(PATH) ? Buffer.from(PATH.split(",")[1],
            'base64') : (/^https?:\/\//.test(PATH) && await isTextContent(PATH)) ? await (res =
            await getDataBuffer(PATH)) : fs.existsSync(PATH) ? ((filename = PATH), fs.readFileSync(PATH)) : fs
          .readFileSync('./images/error.jpg') || Buffer.alloc(0);
        if (Buffer.byteLength(data) > 2000000000) throw new TypeError(
    ...
    sendFile: {
      ...
      async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
        try {
          ...
          // if ((res && res.status !== 200) || file.length <= 65536) throw new Error('File retrieval failed');
          ...
          if (mime.match(/^image\/webp/i)) {
            buffer = options.asImage ? await webp2png(file) : options.asVideo ? await webp2mp4(file) ||
              await webp2mp4File(file) : await writeExifWebp(file, options.packname && options.author ? {
                packname: options.packname,
                author: options.author
              } : options);
            file = buffer ? buffer : file;
            mtype = options.asImage ? 'image' : options.asVideo ? 'video' : 'sticker';
            mimetype = options.asImage ? 'image/png' : options.asVideo ? 'video/mp4' : mime;
          } else if (mime.match(/^image\//i)) {
            buffer = (options.asSticker && (await mediaToSticker(file) || await imageToWebp(file))) || (options
              .packname || options.author) && await writeExifImg((await mediaToSticker(file) ||
              await imageToWebp(file)), options);
            file = buffer ? buffer : file;
            mtype = buffer ? 'sticker' : 'image';
            mimetype = buffer ? 'image/webp' : mime;
          } else if (mime.match(/^video\//i)) {
            bufferAudio = options.as8k ? await toAudio8k(file, mime.ext) : options.asAudio ? await toAudio(file,
              mime.ext) : null;
            if (bufferAudio) {
              file = bufferAudio.data;
              pathFile = bufferAudio.filename;
              mtype = 'audio';
              mimetype = 'audio/mp4';
              ptt = false;
              seconds = Math.floor(await getAudioDuration(file)) || 45296000;
              waveform = (await getAudioWaveform(file)) || [100, 0, 100, 0, 100];
            } else {
              buffer = (options.asSticker && (await mediaToSticker(file) || await videoToWebp(file))) || (options
                .packname || options.author) && await writeExifVid((await mediaToSticker(file) ||
                await videoToWebp(file)), options);
              file = buffer ? buffer : file;
            ...
            ptt = false;
            mimetype = 'audio/mp4';
            seconds = Math.floor(await getAudioDuration(buffer.data)) || 45296000;
            waveform = (await getAudioWaveform(buffer.data)) || [100, 0, 100, 0, 100];
          } else {
            mtype = 'document';
            mimetype = mime;
          }
          mtype = options.asDocument ? 'document' : options.asLocation ? 'location' : options.asPtv ?
            'ptvMessage' : mtype;
          if (options.asPtv) {
            let {
              videoMessage
          ...
          const message = {
            ...
            caption: conn.temafont && !isNaN(parseInt(conn.temafont)) ? await generateFont(text, parseInt(conn
              .temafont)) : text,
            ptt,
            seconds,
            waveform,
          ...
          throw new Error('All methods to send message failed');
        }
      },
      enumerable: true
    },
...
`.trim()
        ...
        await conn.sendMessage(jid, {
          ...
          contacts: {
            ...options,
            displayName: (contacts.length >= 2 ? `${contacts.length} kontak` : contacts[0]?.displayName) ||
              null,
            contacts,
          }
        }, {
...
END:VCARD`.trim()
    ...
    sendListM: {
      async value(jid, button, rows, quoted, options = {}) {
        ...
        await conn.sendLists(jid, sections.title, listMessage.text, listMessage.footer, listMessage.buttonText,
          listMessage.sections, quoted, {
            ...options
          })
      }
    },
    ...
    generateProfilePicture: {
      async value(buffer) {
        ...
        const resz = jimp_1.getWidth() > jimp_1.getHeight() ? jimp_1.resize(550, Jimp.AUTO) : jimp_1.resize(Jimp
          .AUTO, 650)
    ...
    sendPayment: {
      async value(jid, amount, currency, text = '', from, image, options) {
        ...
        const requestPaymentMessage = {
          ...
          noteMessage: {
            extendedTextMessage: {
              text: (conn.temafont && !isNaN(parseInt(conn.temafont))) ? await generateFont(text, parseInt(conn
                .temafont)) : text || 'Example Payment Message'
            }
          },
    ...
    sendPoll: {
      async value(jid, text = '', optiPoll, options) {
        ...
        if (!options) options = {}
        const pollMessage = {
          name: (conn.temafont && !isNaN(parseInt(conn.temafont))) ? await generateFont(text, parseInt(conn
            .temafont)) : text,
    ...
    sendList: {
      async value(jid, title, text, footer, buttonText, buffer, listSections, quoted, options) {
        ...
        if (buffer && !Buffer.isBuffer(buffer) && (typeof buffer === 'string' || Array.isArray(buffer)))(options =
          quoted, quoted = listSections, listSections = buffer, buffer = null)
        if (!options) options = {}
    ...
    downloadMediaMessage: {
      async value(message, fileName) {
        ...
        if (fileName) {
          ...
          let trueFileName = fileName ? (fileName + "." + ftype.ext || mimetype.split("/")[1]) :
            `${Math.floor(Math.random() * 10000)}${(ftype.ext || mimetype.split("/")[1])}`
    ...
    */
    ...
    async value(jid, buffer, contentText, footerText, button1, id1, quoted, options) {
        ...
        const buttonMessage = {
            ...
            caption: contentText,
            footer: footerText,
            mentions: conn.parseMention(contentText),
            ...options,
            buttons: buttons,
            headerType: 4
        }
    ...
    */
    ...
    filter: {
      value(text = '') {
        let mati = ["q", "w", "r", "t", "y", "p", "s", "d", "f", "g", "h", "j", "k", "l", "z", "x", "c", "v", "b",
          "n", "m"
        ]
    ...
    reply: {
      ...
      async value(jid, text = '', quoted, options = {}) {
        ...
        const isBuffer = Buffer.isBuffer(PATH);
        const isArrayBuffer = PATH instanceof ArrayBuffer;
        const data = isBuffer ? PATH : isArrayBuffer ? Buffer.from(PATH) : /^data:.*?\/.*?;base64,/i.test(PATH) ?
          Buffer.from(PATH.split(",")[1], 'base64') : (/^https?:\/\//.test(PATH) && await isTextContent(PATH)) ? (
            await conn.getFile(PATH)).data : fs.existsSync(PATH) ? ((filename = PATH), fs.readFileSync(PATH)) :
          undefined;
        return data ? conn.sendFile(jid, data, 'file', '', quoted, false, {
          ...Object.assign({
            mentions: typeof text === 'string' ? conn.parseMention(text || '@0') : [],
            contextInfo: {
              mentionedJid: typeof text === 'string' ? conn.parseMention(text || '@0') : [],
        ...
        }) || text : await conn.sendMessage(jid, {
          ...Object.assign({
            mentions: typeof text === 'string' ? conn.parseMention(text || '@0') : [],
            contextInfo: {
              mentionedJid: typeof text === 'string' ? conn.parseMention(text || '@0') : [],
    ...
    */
    sendHydrated2: {
      ...
      async value(jid, text = '', footer = '', buffer, url, urlText, url2, urlText2, buttons, quoted, options) {
        ...
        if (buffer && !Buffer.isBuffer(buffer) && (typeof buffer === 'string' || Array.isArray(buffer)))(options =
          quoted, quoted = buttons, buttons = callText, callText = call, call = urlText, urlText = url, url =
          buffer, buffer = null)
        if (!options) options = {}
        let templateButtons = []
        if (url || urlText) {
    ...
    copyNForward: {
      ...
      async value(jid, message, forwardingScore = true, options = {}) {
        ...
        if (options.readViewOnce && message.message.viewOnceMessage?.message) {
          ...
          message.message = proto.Message.fromObject(JSON.parse(JSON.stringify(message.message.viewOnceMessage
            .message)))
        ...
        if (forwardingScore && typeof forwardingScore === 'number' && forwardingScore > 1) m[ctype].contextInfo
    ...
    parseMention: {
       ...
       */
      value(text = '') {
        const result = (typeof text === 'string' && !Buffer.isBuffer(text) && !(text instanceof Object)) ? [...text
          .matchAll(/(\d+)@?(@\d+)?/g)
        ].map(match => match[2] ? `${match[1]}@s.whatsapp.net` : `${match[1]}@s.whatsapp.net`).filter(entry =>
          /\d+@s\.whatsapp\.net/.test(entry)) : [];
        return result;
      },
      enumerable: true
    },
    ...
    loadMessage: {
      ...
      value(messageID) {
        ...
        { messages }]) => Object.entries(messages).find(([k, v]) => k === messageID || v?.key?.id ===
    ...
    sendGroupV4Invite: {
       ...
       * @param {*} options 
       */
      async value(jid, participant, inviteCode, inviteExpiration, groupName = 'unknown subject', caption =
        'Invitation to join my WhatsApp group', jpegThumbnail, options = {}) {
        let text = caption;
        const msg = proto.Message.fromObject({
          groupInviteMessage: proto.GroupInviteMessage.fromObject({
            ...
            caption: (conn.temafont && !isNaN(parseInt(conn.temafont))) ? await generateFont(text,
              parseInt(conn.temafont)) : text
          })
        })
    ...
    insertAllGroup: {
      async value() {
        ...
        const groups = Object.keys(conn.chats).filter(key => key.endsWith('@g.us')).map(key => conn.chats[
        key]) || {}
    ...
    sendStimg: {
      async value(jid, path, quoted, options = {}) {
        let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split(
            ',')[1], 'base64') : /^https?:\/\//.test(path) ? (await conn.getFile(path)).data : fs.existsSync(
          path) ? fs.readFileSync(path) : Buffer.alloc(0)
        let buffer
      ...
      async value(m) {
        ...
        for (const message of m) {
          try {
            ...
            if (!message) continue
            if (message.messageStubType && message.messageStubType != WAMessageStubType.CIPHERTEXT) conn
              .processMessageStubType(message).catch(console.error)
            const _mtype = Object.keys(message.message || {})
            const mtype = (!['senderKeyDistributionMessage', 'messageContextInfo']?.includes(_mtype[0]) && _mtype[
              0]) || (_mtype.length >= 3 && _mtype[1] !== 'messageContextInfo' && _mtype[1]) || _mtype[_mtype
              .length - 1]
            const chat = conn.decodeJid(message.key.remoteJid || message.message?.senderKeyDistributionMessage
              ?.groupId || '')
            if (message.message?.[mtype]?.contextInfo?.quotedMessage) {
              ...
              if ((remoteJid && remoteJid !== 'status@broadcast') && quoted) {
                ...
                quoted[qMtype].contextInfo.mentionedJid = context.mentionedJid || quoted[qMtype]?.contextInfo
                  .mentionedJid || []
                ...
                let qChatsMessages
                if ((qChatsMessages = Object.entries(qChats.messages)).length > 40) qChats.messages = Object
                  .fromEntries(qChatsMessages.slice(30, qChatsMessages.length)) // maybe avoid memory leak
              }
            }
            ...
            if (isGroup) {
              ...
              sender = conn.decodeJid(message.key?.fromMe && conn.user.id || message.participant || message.key
                ?.participant || chat || '')
              if (sender !== chat) {
            ...
            if (!['protocolMessage']?.includes(mtype) && !fromMe && message.messageStubType != WAMessageStubType
              .CIPHERTEXT && message.message) {
              delete message.message.messageContextInfo
              delete message.message.senderKeyDistributionMessage
              chats.messages[message.key.id] = JSON.parse(JSON.stringify(message, null, 2))
              let chatsMessages
              if ((chatsMessages = Object.entries(chats.messages)).length > 40) chats.messages = Object
                .fromEntries(chatsMessages.slice(30, chatsMessages.length))
            }
          } catch (e) {
            console.error(e)
          }
...
export function smsg(conn, m, hasParent) {
  ...
  if (m.message) {
    if (m.mtype === 'protocolMessage' && m.msg.key) {
      ...
      if (!protocolMessageKey.participant || protocolMessageKey.participant === 'status_me') protocolMessageKey
...
export function serialize() {
  ...
  return Object.defineProperties(proto.WebMessageInfo.prototype, {
    ...
    isBaileys: {
      get() {
        return ((this.id?.length === 16 || this.id?.length === 20) || (this.id?.startsWith('3EB0') && (this.id
          ?.length === 12 || this.id?.length === 16)) || (["BAE", "WA"]?.some(k => this.id?.startsWith(k) &&
          this.sender !== this.conn?.user.jid)) || false)
      }
    },
    chat: {
      get() {
        const senderKeyDistributionMessage = this.message?.senderKeyDistributionMessage?.groupId
        return (this.key?.remoteJid || (senderKeyDistributionMessage && senderKeyDistributionMessage !==
          'status@broadcast') || '').decodeJid()
      }
    },
    isGroup: {
      get() {
    ...
    sender: {
      get() {
        return this.conn?.decodeJid(this.key?.fromMe && this.conn?.user.id || this.participant || this.key
          .participant || this.chat || '')
      },
      enumerable: true
    },
    fromMe: {
    ...
    mtype: {
      get() {
        ...
        return (!['senderKeyDistributionMessage', 'messageContextInfo']?.includes(type[0]) && type[
          0]) || // Sometimes message in the front
          (type.length >= 3 && type[1] !== 'messageContextInfo' && type[
          1]) || // Sometimes message in midle if mtype length is greater than or equal to 3
    ...
    quoted: {
      get() {
        ...
        return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? {
          ...
          isBaileys: {
            get() {
              return ((this.id?.length === 16 || this.id?.length === 20) || (this.id?.startsWith('3EB0') &&
                (this.id?.length === 12 || this.id?.length === 16)) || (["BAE", "WA"]?.some(k => this.id
                ?.startsWith(k) && this.sender !== self.conn?.user.jid)) || false)
            },
            enumerable: true
          },
          sender: {
    ...
    return self.conn?.reply(
        chatId !== null && chatId !== undefined && typeof chatId !== 'object' ? chatId : this.chat, 
        text, 
        this.vM, 
        typeof chatId === 'object' ? chatId : options
          ...
          command: {
            get() {
              ...
              let match = (_prefix instanceof RegExp ? [
                [_prefix.exec(text || this.caption || this.contentText || this.selectedDisplayText ||
                  ''), _prefix]
              ] : Array.isArray(_prefix) ? _prefix.map(p => {
                let re = p instanceof RegExp ? p : new RegExp(str2Regex(p))
                return [re.exec(text || this.caption || this.contentText || this
                  .selectedDisplayText || ''), re]
              }) : typeof _prefix === "string" ? [
                [new RegExp(str2Regex(_prefix)).exec(text || this.caption || this.contentText || this
                  .selectedDisplayText || ''), new RegExp(str2Regex(_prefix))]
              ] : [
                [
                  [], new RegExp
                ]
              ]).find(p => p[1])
              let result = ((opts?.['multiprefix'] ?? true) && (match[0] || "")[0]) || ((opts?.[
                'noprefix'] ?? false) ? null : (match[0] || "")[0]);
              let noPrefix = !result ? (text || this.caption || this.contentText || this
                .selectedDisplayText || '') : (text || this.caption || this.contentText || this
                .selectedDisplayText || '').replace(result, "");
          ...
          device: {
            get() {
              return getDevice(this.vM.key?.id) ? (getDevice(this.vM.key?.id) === 'unknown' && os
                .platform() ? (os.platform() === 'android' ? 'android' : ['win32', 'darwin', 'linux']
                    ?.includes(os.platform()) ? 'desktop' : 'unknown') : getDevice(this.vM.key?.id)) :
                'unknown';
            },
            enumerable: true
          },
          isBot: {
            get() {
              return ["BAE", "WA"]?.some(k => (this.vM.key?.id).includes(k) && this.sender !== this.conn
                ?.user.jid);
            },
            enumerable: true
          }
    ...
    text: {
      get() {
        ...
        return typeof this._text === 'string' ? this._text : '' || (typeof text === 'string' ? text : (text
          ?.selectedDisplayText || text?.hydratedTemplate?.hydratedContentText || text)) || ''
      },
      set(str) {
    ...
    device: {
      get() {
        return getDevice(this.key?.id) ? (getDevice(this.key?.id) === 'unknown' && os.platform() ? (os
        .platform() === 'android' ? 'android' : ['win32', 'darwin', 'linux']?.includes(os.platform()) ?
          'desktop' : 'unknown') : getDevice(this.key?.id)) : 'unknown';
      },
      enumerable: true
    },
    isBot: {
      get() {
        return ["BAE", "WA"]?.some(k => (this.key?.id).includes(k) && this.sender !== this.conn?.user.jid);
      },
      enumerable: true
    }
  })
...
export function protoType() {
  ...
  // }
  Uint8Array.prototype.getFileType = ArrayBuffer.prototype.getFileType = Buffer.prototype.getFileType =
  ...
  Number.prototype.toTimeString = function toTimeString() {
    ...
    return (
      (days ? `${days} Days ☀ ️` : '') + (hours ? `${hours} Hours 🕐 ` : '') + (minutes ? `${minutes} Minute ⏰ ` :
        '') + (seconds ? `${seconds} Second ⏱️ ` : '')).trim()
  }
