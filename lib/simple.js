...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
        try {
            let text = caption;
            let { res, data: file, filename: pathFile, mime } = await conn.getFile(path, true);
            ...
            if (/^image\/webp$/i.test(mime)) {
                ...
                buffer = options.asSticker ? await mediaToSticker(file) || await imageToWebp(file) :
                    options.packname || options.author ? await writeExifImg(await mediaToSticker(file) || await imageToWebp(file), options) : null;
                file = buffer || file;
                ...
                buffer = options.asSticker ? await mediaToSticker(file) || await videoToWebp(file) :
                    options.packname || options.author ? await writeExifVid(await mediaToSticker(file) || await videoToWebp(file), options) : null;
                file = buffer || file;
            ...
            if (options.asPtv) {
                let { videoMessage } = await generateWAMessageContent({ video: file }, { upload: conn.waUploadToServer });
                file = videoMessage;
            }
            ...
            try {
                if (options.asPtv) {
                    m = await conn.relayMessage(jid, { [mtype]: file }, { ...opt, ...Object.assign({
                        mentions: typeof text === 'string' ? await conn.parseMention(text || '@0') : [],
                        contextInfo: {
                            mentionedJid: typeof text === 'string' ? await conn.parseMention(text || '@0') : [],
                        }
                    ...
                    m = await conn.sendMessage(jid, message, { ...opt, ...Object.assign({
                        mentions: typeof text === 'string' ? await conn.parseMention(text || '@0') : [],
                        contextInfo: {
                            mentionedJid: typeof text === 'string' ? await conn.parseMention(text || '@0') : [],
                        }
                ...
                if (!m) {
                    m = await conn.sendMessage(jid, { ...message, [mtype]: file }, { ...opt, ...Object.assign({
                        mentions: typeof text === 'string' ? await conn.parseMention(text || '@0') : [],
                        contextInfo: {
                            mentionedJid: typeof text === 'string' ? await conn.parseMention(text || '@0') : [],
                        }
