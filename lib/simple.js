...
END:VCARD`.trim()
    ...
    sendListM: {
      async value(jid, button, rows, quoted, options = {}) {
        ...
        await conn.sendLists(jid, sections.title, listMessage.text, listMessage.footer, null, listMessage.buttonText,
   ...
   * @param {String} text
   * @param {String} footer
   * @param {Buffer|String|null} buffer
   * @param {String} buttonText
   * @param {Array} listSections
   * @param {import('@whiskeysockets/baileys').proto.WebMessageInfo} quoted
   * @param {Object} options
   */
  async value(jid, title, text, footer, buffer, buttonText, listSections, quoted, options = {}) {
    // Construct sections based on the provided example
    const sectionses = listSections.map(section => ({
      title: section.title || '',
  ...
  async value(jid, text = '', quoted, options = {}) {
    ...
    const data = isBuffer ? PATH :
      ...
      (/^https?:\/\//.test(PATH) && await isTextContent(PATH)) ? (await conn.getFile(PATH)).data :
      fs.existsSync(PATH) ? ((filename = PATH), fs.readFileSync(PATH)) :
      null;

    if (data) {
      return await conn.sendFile(jid, data, 'file', '', quoted, false, options);
    } else {
      return await conn.sendMessage(jid, { text, ...Object.assign({
            mentions: typeof text === 'string' ? conn.parseMention(text || '@0') : [],
            contextInfo: {
              mentionedJid: typeof text === 'string' ? conn.parseMention(text || '@0') : [],
            }
    ...
    sendHydrated: {
      ...
      async value(jid, text = '', footer = '', b, c = '', d = '', e = '', f = '', buttons, quoted, options) {
        ...
        return !b ? await conn.sendMessage(jid, {
          ...
          quoted,
          ...options
        }) : await conn.sendFile(jid, b, footer, (conn.temafont && !isNaN(parseInt(conn.temafont))) ?
