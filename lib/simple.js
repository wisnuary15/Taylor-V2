            ...
            async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
                try {
                    ...
                    if (mime.match(/^image\/webp/i)) {
                        buffer = options.asImage ? await webp2png(file) :
                            options.asVideo ? await webp2mp4(file) || await webp2mp4File(file) :
                            await writeExifWebp(file, options.packname && options.author ? {
                        ...
                        buffer = (options.asSticker && (await mediaToSticker(file) || await imageToWebp(file))) ||
                            (options.packname || options.author) && await writeExifImg((await mediaToSticker(file) || await imageToWebp(file)), options);
                        file = buffer ? buffer : file;
                        mtype = buffer ? 'sticker' : 'image';
                        mimetype = buffer ? 'image/webp' : mime;
                    } else if (mime.match(/^video\//i)) {
                        bufferAudio = options.as8k ? await toAudio8k(file, mime.ext) :
                            options.asAudio ? await toAudio(file, mime.ext) :
                            null;
                        if (bufferAudio) {
                            ...
                            buffer = (options.asSticker && (await mediaToSticker(file) || await videoToWebp(file))) ||
                                (options.packname || options.author) && await writeExifVid((await mediaToSticker(file) || await videoToWebp(file)), options);
                            file = buffer ? buffer : file;
        ...
        sendKontak: {
            async value(jid, data, quoted, options) {
                ...
                for (let [number, nama, ponsel, email] of data) {
                    number = number.replace(/[^0-9]/g, '')
                    let njid = number + '@s.whatsapp.net'
                    let name = global.db.data.users[njid] ? global.db.data.users[njid].name : (await conn.getName(njid))
                    let biz = await conn.getBusinessProfile(njid).catch(_ => null) || {}
                    let vcard = `
BEGIN:VCARD
VERSION:3.0
    ...
    */
        ...
        reply: {
            ...
            async value(jid, text = '', quoted, options = {}) {
                ...
                const data = isBuffer ?
                    ...
                    undefined;
                return data ?
                    conn.sendFile(jid, data, 'file', '', quoted, false, {
                        ...Object.assign({
                            mentions: typeof text === 'string' ? conn.parseMention(text || '@0') : [],
                            contextInfo: {
                                mentionedJid: typeof text === 'string' ? conn.parseMention(text || '@0') : [],
        ...
        */
        ...
        getName: {
             ...
             */
            async value(jid = '0@s.whatsapp.net', withoutContact = false) {
                jid = conn.decodeJid(jid);
                withoutContact = conn.withoutContact || withoutContact;
                const v = jid.endsWith('@g.us') ?
                    await store.fetchGroupMetadata(jid, conn.groupMetadata) || await conn.groupMetadata(jid) || {} :
                ...
                const name = (withoutContact ? '' : v?.name) || v?.subject || v?.vname || v?.notify || v?.verifiedName;
                return name || PhoneNumber('+' + Number(parseInt(jid))).getNumber('international');
            },
            enumerable: true,
            writable: true,
        },
...
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        isBaileys: {
            get() {
                return ((this.id?.length === 16 || this.id?.length === 20) || (this.id?.startsWith('3EB0') && (this.id?.length === 12 || this.id?.length === 16)) || (["BAE", "WA"].some(k => this.id?.startsWith(k) && this.sender !== this.conn?.user.jid)) || false)
            }
        },
        chat: {
            get() {
        ...
        quoted: {
            get() {
                ...
                return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? {
                    ...
                    isBaileys: {
                        get() {
                            return ((this.id?.length === 16 || this.id?.length === 20) || (this.id?.startsWith('3EB0') && (this.id?.length === 12 || this.id?.length === 16)) || (["BAE", "WA"].some(k => this.id?.startsWith(k) && this.sender !== self.conn?.user.jid)) || false)
                        },
                        enumerable: true
                    },
                    sender: {
                    ...
                    device: {
                        get() {
                            return getDevice(this.vM.key?.id) ? (getDevice(this.vM.key?.id) === 'unknown' && os.platform() ? (os.platform() === 'android' ? 'Android Device' : ['win32', 'darwin', 'linux'].includes(os.platform()) ? 'Desktop' : 'Unknown Device') : getDevice(this.vM.key?.id)) : 'Unknown Device';
                        },
                        enumerable: true
                    },
                    isBot: {
                        get() {
                            return ["BAE", "WA"].some(k => (this.vM.key?.id).includes(k) && this.sender !== this.conn?.user.jid);
                        },
                        enumerable: true
                    }
                })
        ...
        device: {
            get() {
                return getDevice(this.key?.id) ? (getDevice(this.key?.id) === 'unknown' && os.platform() ? (os.platform() === 'android' ? 'Android Device' : ['win32', 'darwin', 'linux'].includes(os.platform()) ? 'Desktop' : 'Unknown Device') : getDevice(this.key?.id)) : 'Unknown Device';
            },
            enumerable: true
        },
        isBot: {
