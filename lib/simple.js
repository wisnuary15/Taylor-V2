...
export function serialize() {
  ...
  return Object.defineProperties(proto.WebMessageInfo.prototype, {
    ...
    isBaileys: {
      get() {
        return _.some(["BAE", "3EB0", "WA"], pfx => _.startsWith(this.id, pfx)) || _.includes([12, 16, 20, 40], this.id?.length);
      }
    },
    chat: {
      get() {
    ...
    quoted: {
      get() {
        ...
        return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === "string" ? {
          ...
          isBot: {
            get() {
              return _.some(["BAE", "3EB0", "WA"], pfx => _.startsWith(this.vM.key.id, pfx)) || _.includes([12, 16, 20, 40], this.vM.key.id?.length);
            },
            enumerable: true
          }
        });
    ...
    isBot: {
      get() {
        return _.some(["BAE", "3EB0", "WA"], pfx => _.startsWith(this.key.id, pfx)) || _.includes([12, 16, 20, 40], this.key.id?.length);
      },
      enumerable: true
    }
  });
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          const axiosResponse = await instance.get(url, axiosConfig);
