        ...
        sendFile: {
            ...
            async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
                ...
                if (options && (options.packname || options.author)) {
                    buffer = await writeExifWebp(file, options) || await writeExifImg(file, options)
                } else {
                    buffer = await imageToWebp(file)
                }
                mtype = 'sticker'
                file = buffer
                mimetype = 'image/webp'
                    ...
                    convert = await toAudio8k(file, type.ext) || await toAudio(file, type.ext)
                    file = convert.data
