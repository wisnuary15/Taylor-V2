...
END:VCARD`.trim();
    ...
    reply: {
      async value(jid, text = "", quoted, options = {}) {
        ...
        const action = data ? await conn.sendFile(jid, data, "file", "", quoted, false, options) : await conn.sendMessage(jid, {
          text: text,
          ...Object.assign({
            mentions: typeof text === "string" ? conn.parseMention(text || "@0") : [],
            contextInfo: {
    ...
    sendHydrated2: {
      async value(jid, text = "", footer = "", buffer, url, urlText, url2, urlText2, buttons, quoted, options) {
        ...
        if (buffer && !Buffer.isBuffer(buffer) && (typeof buffer === "string" || Array.isArray(buffer))) options = quoted,
          quoted = buttons, buttons = callText, callText = call, call = urlText,
          urlText = url, url = buffer, buffer = null;
        if (!options) options = {};
        let templateButtons = [];
        if (url || urlText) {
