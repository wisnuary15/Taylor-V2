...
END:VCARD`.trim();
    ...
    appenTextMessage: {
      async value(message, text, chatUpdate) {
        ...
        messages.key.fromMe = areJidsSameUser(message.sender.decodeJid(), (conn.user?.id || conn.user?.jid).decodeJid());
    ...
    cMod: {
      value(jid, message, text = "", sender = conn.user.jid, options = {}) {
        ...
        copy.key.fromMe = areJidsSameUser(sender.decodeJid(), (conn.user.id || conn.user.jid).decodeJid()) || false;
    ...
    fakeReply: {
      async value(jid, text = "", fakeJid = this.user.jid, fakeText = "", fakeGroupJid, options) {
        await this.reply(jid, text, {
          key: {
            fromMe: areJidsSameUser(fakeJid.decodeJid(), (conn.user.id || conn.user.jid).decodeJid()),
            participant: fakeJid,
            ...fakeGroupJid ? {
    ...
    getName: {
      value(jid = "", withoutContact = false) {
        ...
        v = jid === "0@s.whatsapp.net" ? {
          jid: jid,
          vname: "WhatsApp"
        } : areJidsSameUser(jid.decodeJid(), (conn.user.id || conn.user.jid).decodeJid()) ? conn.user : conn.chats[jid] || store?.chats[jid] || {};
        ...
        for (const message of m) {
          try {
            ...
            if (message.message?.[mtype]?.contextInfo?.quotedMessage) {
              ...
              if (remoteJid && remoteJid !== "status@broadcast" && quoted) {
                ...
                const qM = {
                  key: {
                    remoteJid: remoteJid,
                    fromMe: areJidsSameUser(remoteJid.decodeJid(), (conn.user.id || conn.user.jid).decodeJid()),
                    id: context.stanzaId,
            ...
            if (isGroup) {
              ...
              sender = conn.decodeJid(message.key?.fromMe && conn.user.jid || conn.user.id || message.participant || message.key?.participant || chat || "");
              if (sender !== chat) {
            ...
            const fromMe = message.key.fromMe || areJidsSameUser((sender || chat).decodeJid(), (conn.user.id || conn.user.jid).decodeJid());
...
export function smsg(conn, m, hasParent) {
  ...
  if (m.message) {
    if (m.mtype === "protocolMessage" && m.msg.key) {
      ...
      protocolMessageKey.fromMe = conn.decodeJid(protocolMessageKey.participant) === conn.decodeJid(conn.user.jid || conn.user.id);
      if (!protocolMessageKey.fromMe && protocolMessageKey.remoteJid === conn.decodeJid(conn.user.jid || conn.user.id)) protocolMessageKey.remoteJid = m.sender;
    }
    if (m.quoted)
...
export function serialize() {
  ...
  return Object.defineProperties(proto.WebMessageInfo.prototype, {
    ...
    sender: {
      get() {
        return this.conn?.decodeJid(this.key?.fromMe && this.conn?.user.jid || this.conn?.user.id || this.participant || this.key.participant || this.chat || "");
      },
      enumerable: true
    },
    fromMe: {
      get() {
        return !(this.key?.fromMe || areJidsSameUser(this.sender.decodeJid(), (this.conn?.user.id || this.conn?.user.jid).decodeJid()) || false);
      },
      enumerable: true
    },
    mtype: {
    ...
    quoted: {
      get() {
        ...
        return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === "string" ? {
          ...
          id: {
            get() {
              return contextInfo.stanzaId || this.vM.key.id || this.id;
            },
            enumerable: true
          },
          chat: {
          ...
          fromMe: {
            get() {
              return self.key.fromMe || areJidsSameUser(this.sender.decodeJid(), (self.conn?.user.id || self.conn?.user.jid).decodeJid()) || false;
            },
            enumerable: true
          },
          text: {
    ...
    getQuotedObj: {
      value() {
        ...
        const message = proto.WebMessageInfo.fromObject(this.conn?.loadMessage?.(this.chat, this.quoted.id) || this.conn?.loadMessage?.(null, this.quoted.id) || store?.loadMessage?.(this.chat, this.quoted.id) || this.quoted.vM || this.quoted.fakeObj);
