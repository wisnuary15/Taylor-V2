...
async function filesInit() {
  ...
  try {
    const results = await Promise.all(lodash.map(importPromises, promise => promise.then(value => ({
      status: "fulfilled",
      value: value
    }))?.catch(reason => ({
      status: "rejected",
    ...
    const successResults = lodash.map(lodash.filter(results, {
      status: "fulfilled"
    }), result => result.value.moduleName);
    const errorResults = lodash.map(lodash.filter(results, {
      status: "rejected"
    }), result => ({
