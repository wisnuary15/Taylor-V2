...
const [isCredsExist, isAuthSingleFileExist, authState] = await Promise.all([Helper.checkFilesExist(authFolder + "/creds.json"), Helper.checkFilesExist(authFile), storeSystem.useMultiFileAuthState(authFolder)]);
const logger = pino({
  level: "info"
}, stream);
process.on("unhandledRejection", (err, promise) => logger.error({
  ...
  reason: reason,
  promise: promise
}, "Unhandled Promise Rejection")).on("SIGTERM", () => logger.info("Received SIGTERM signal")).on("SIGINT", () => logger.info("Received SIGINT signal")).on("SIGUSR1", () => logger.info("Received SIGUSR1 signal")).on("SIGUSR2", () => logger.info("Received SIGUSR2 signal")).on("beforeExit", code => logger.warn({
  code: code
...
async function writeDatabase() {
  try {
    await Promise.allSettled([global.db.data ? global.db.write(global.db.data) : Promise.reject("global.db.data is null")]);
  } catch (error) {
    console.error(error);
  } finally {
    setTimeout(writeDatabase, 30 * 6e4);
  }
}
if (opts["server"]) {
...
async function filesInit() {
  ...
  try {
    const results = await Promise.all(importPromises.map(promise => promise.then(value => ({
      status: "fulfilled",
      value: value
    })).catch(reason => ({
      status: "rejected",
...
async function _quickTest() {
  let test = await Promise.allSettled([spawn("ffmpeg"), spawn("ffprobe"), spawn("ffmpeg", ["-hide_banner", "-loglevel", "error", "-filter_complex", "color", "-frames:v", "1", "-f", "webp", "-"]), spawn("convert"), spawn("magick"), spawn("gm"), spawn("find", ["--version"])].map(p => {
