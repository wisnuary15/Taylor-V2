...
async function createCertificate() {
    const sslFolderPath = path.join(process.cwd(), 'ssl');
    const certFolderPath = path.join(sslFolderPath, 'certs');
    const certFilePath = path.join(certFolderPath, 'lets-encrypt-x3-cross-signed.pem');

    try {
        existsSync(sslFolderPath) || mkdirSync(sslFolderPath);
        existsSync(certFolderPath) || mkdirSync(certFolderPath);
        existsSync(certFilePath) || writeFileSync(certFilePath, await (await fetch('https://letsencrypt.org/certs/lets-encrypt-x3-cross-signed.pem.txt')).text());
