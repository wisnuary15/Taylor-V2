...
if (isMainThread) {
  ...
  worker.on('exit', () => Promise.all(heapdumpPromises).then(heapdumpFiles => console.log('Heapdump files:',
    heapdumpFiles)).catch(error => console.error('Error generating heapdump:', error)));
...
const dbInstance = !dbUrl ? new JSONFile(dbName) : /^https?:\/\//.test(dbUrl) ? new cloudDBAdapter(dbUrl) :
  /^mongodb(\+srv)?:\/\//i.test(dbUrl) && opts.db ? new mongoDB(dbUrl) : /^mongodb(\+srv)?:\/\//i.test(dbUrl) && opts
  .dbv2 ? new mongoDBV2(dbUrl) : new JSONFile(dbName);
...
global.loadDatabase = async function loadDatabase() {
  ...
  return db.READ ? new Promise((resolve) => {
  ...
  }) : db.data === null ? (db.READ = true, await db.read().catch(console.error),
    (db.READ = null),
    (db.data = {
      users: {},
      chats: {},
    ...
    (db.chain = chain(db.data)), null) : null;
}
await loadDatabase();
const {
  version,
...
process.on('unhandledRejection', (err, promise) => logger.error({
  err,
  promise
}, 'Unhandled Rejection')).on('rejectionHandled', (promise) => logger.info({
  promise
  ...
  reason,
  promise
}, 'Unhandled Promise Rejection')).on('SIGTERM', () => logger.info('Received SIGTERM signal')).on('SIGINT', () =>
  logger.info('Received SIGINT signal')).on('SIGUSR1', () => logger.info('Received SIGUSR1 signal')).on('SIGUSR2',
() => logger.info('Received SIGUSR2 signal')).on('beforeExit', (code) => logger.warn({
  code
}, 'Process Before Exit')).on('exit', (code) => logger.info({
  code
}, 'Process Exit')).on('warning', (warning) => null).on('disconnect', () => logger.warn('Node Disconnected')).on(
  'message', (message) => logger.info({
    message
  }, 'Node Message')).on('multipleResolves', (type, promise) => null).on('childProcess', (childProcess) => logger
  .info({
    childProcess
  }, 'Child Process Event'));
...
if (Helper.opts['singleauth'] || Helper.opts['singleauthstate']) {
  if (!isCredsExist && isAuthSingleFileExist) {
    console.debug(chalk.bold.blue('- singleauth -'), chalk.bold.yellow('creds.json not found'), chalk.bold.green(
      'compiling singleauth to multiauth...'));
  ...
  } else if (!isAuthSingleFileExist) console.error(chalk.bold.blue('- singleauth -'), chalk.bold.red(
    'singleauth file not found'));
}
...
async function connectionUpdate(update) {
  ...
  if (connection === 'connecting') {
    ...
    if (timeout > 30) {
      console.log("Taylor-V2".main, ">>".yellow,
        `Session logout after 30 times reconnecting, This action will save your number from banned!`.warn);
  ...
  if (connection === 'open') {
    try {
      ...
      console.log("Taylor-V2".main, ">>".yellow,
        `Client connected on: ${conn?.user?.id.split(":")[0] || global.namebot}`.info);
      const infoMsg =
...
global.reloadHandler = async function reloadHandler(restatConn) {
  ...
  if (currentDateTime >= messageDateTime) {
    const chats = Object.entries(conn.chats).filter(([jid, chat]) => !jid.endsWith('@g.us') && chat.isChats).map((
      v) => v[0]);
  } else {
    const chats = Object.entries(conn.chats).filter(([jid, chat]) => !jid.endsWith('@g.us') && chat.isChats).map((
      v) => v[0]);
...
async function filesInit() {
  ...
  try {
    ...
    const errorMessagesText = errorResults.map((error, index) =>
...
async function libFiles() {
  ...
  const importPromises = CommandsFiles.map(async (file) => {
    const moduleName = path.join('/lib', path.relative(path.resolve(Helper.__dirname(import.meta.url), 'lib'),
      file));
    try {
      const fileContent = await readFileSync(file, 'utf-8');
      const err = syntaxerror(fileContent, file, {
...
const setNestedObject = (obj, path, value) => path.split('/').reduce((acc, key, index, keys) => (index === keys.length -
  1 ? (acc[key] = value) : (acc[key] = acc[key] || {})), obj);
...
async function _quickTest() {
  let test = await Promise.allSettled([
    spawn('ffmpeg'),
    spawn('ffprobe'),
    spawn('ffmpeg', ['-hide_banner', '-loglevel', 'error', '-filter_complex', 'color', '-frames:v', '1', '-f',
      'webp', '-'
    ]),
    spawn('convert'),
    spawn('magick'),
  ...
  let [ffmpeg, ffprobe, ffmpegWebp, convert, magick, gm, find] = test.map(result => result.status === 'fulfilled' ?
    result.value : false);
  console.log(test);
