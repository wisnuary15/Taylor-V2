...
const runSyntaxCheck = async () => {
  ...
  try {
    ...
    const testFile = async (file) => {
      try {
        ...
        if (syntaxErrorResult) {
          ...
          console.error(chalk.bold.red('An error occurred:'), syntaxErrorResult);
        }
      } catch (fileReadError) {
        const result = {
          'File Name': path.basename(file),
        ...
        console.error(chalk.bold.red('An error occurred:'), fileReadError);
        errorResults.push(result);
      }
    };
    ...
    for (const file of files) {
      ...
      spinnerInfo.spinner.text =
        `${chalk.bold.blue('🚀 Starting test...')} ${chalk.bold.gray('Checking files...')} ${chalk.bold.magenta(` ${Math.floor(progressPercentage)}% `)}`;
...
const formatResult = (result) => {
  ...
  return [
    ...
    Status === 'Success' ? chalk.bold.green(`Status: ${chalk.bold.blue(Status)}`) : chalk.bold.red(
      `Status: ${chalk.bold.blue(Status)}`),
