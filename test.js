...
const { url: url } = import.meta, { dirname: dirname } = path, __filename = url, __dirname = dirname(url), ECMA_VERSION = 2020, NOT_APPLICABLE = "N/A";
...
function stopSpinner(spinnerInfo) {
  return spinnerInfo ? (spinnerInfo.spinner.stop(), clearInterval(spinnerInfo.intervalId),
    console.log(chalk.bold.green("All files checked successfully.")), spinnerInfo.startTime) : null;
}
const runSyntaxCheck = async () => {
  ...
  try {
    ...
    stopSpinner(spinnerInfo), errorResults.length > 0 ? (console.log(chalk.bold.red(`\nTotal Errors: ${errorResults.length}`)),
      console.log("List Errors:"), errorResults.forEach(((result, index) => {
  ...
  const { "File Name": fileName, Line: Line, Status: Status, Size: Size, Path: Path } = result;
