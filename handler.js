  ...
  try {
    ...
    for (let name in global.plugins) {
      ...
      if (isAcc) {
        ...
        if (plugin.rowner && !(isROwner || isOwner) && (fail("owner", m, this), true) || plugin.rowner && !isROwner && (fail("rowner", m, this), true) || plugin.owner && !isOwner && (fail("owner", m, this), true) || plugin.mods && !isMods && (fail("mods", m, this), true) || plugin.premium && !isPrems && (fail("premium", m, this), true) || plugin.group && !m.isGroup && (fail("group", m, this), true) || plugin.botAdmin && !isBotAdmin && (fail("botAdmin", m, this), true) || plugin.admin && !isAdmin && (fail("admin", m, this), true) || plugin.private && m.isGroup && (fail("private", m, this), true) || plugin.register && !_user.registered && (fail("unreg", m, this), true) || plugin.nsfw && db.data?.chats[m.chat]?.nsfw && (fail("nsfw", m, this), true) || global.plugins[name]?.error && (fail("error", m, this), true) || opts["antirpg"] && db.data?.settings[this.user.jid]?.antirpg && plugin.tags && plugin.tags?.includes("rpg") && (fail("rpg", m, this), true) || opts["restrict"] && db.data?.settings[this.user.jid]?.restrict && plugin.tags && plugin.tags?.includes("admin") && (fail("restrict", m, this), true)) return false;
        m.isCommand = true;
...
export async function participantsUpdate({
  ...
  switch (action) {
    ...
    case "remove":
      if (chat.welcome) {
        ...
        for (let user of participants) {
          ...
          const welcomeText = isAddAction ? (chat.sWelcome || this.welcome || conn.welcome || `${emoji.welcome} Selamat datang, @user!`).replace("@subject", await this.getName(id)).replace("@desc", groupMetadata.desc?.toString() || "tidak diketahui") : chat.sBye || this.bye || conn.bye || `${emoji.bye} Sampai jumpa, @user!`;
    ...
    case "promote":
      ...
      const resultPromoteString = promoteString.split("_").map(word => word === "UNKNOWN" ? "Tidak Diketahui" : word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(" ");
      if (chat.detect) {
    ...
    case "demote":
      ...
      const resultDemoteString = demoteString.split("_").map(word => word === "UNKNOWN" ? "Tidak Diketahui" : word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(" ");
      if (chat.detect) {
...
export async function groupsUpdate(groupUpdates) {
  for (const groupUpdate of groupUpdates) {
    ...
    if (groupUpdate.desc) {
      ...
      text = (chats.sIcon || this.sIcon || conn.sIcon || `*${emoji.icon} Icon telah diubah menjadi*`).replace("@icon", groupUpdate.icon);
      stubType = 22;
    ...
    const resultString = inputString.split("_").map(word => word === "UNKNOWN" ? "Tidak Diketahui" : word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(" ");
    if (!text) continue;
