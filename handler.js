...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    const isROwner = [this.decodeJid(this.user.id), ...global.owner.map(([number]) => number)].map(v => v.replace(
      /[^0-9]/g, "") + "@s.whatsapp.net").includes(m.sender)
    ...
    let usedPrefix
    let _user = global.db.data && db.data?.users && db.data?.users[m.sender]
    ...
    for (let name in global.plugins) {
      ...
      if (m.isBaileys) return;
      if (opts && match && m) {
        let result = ((opts?.['multiprefix'] ?? true) && (match[0] || "")[0]) || ((opts?.['noprefix'] ?? false) ?
          null : (match[0] || "")[0]);
        usedPrefix = result;
        ...
        let isAccept = (prefixCommand instanceof RegExp && prefixCommand.test(command)) || (Array.isArray(
          prefixCommand) && prefixCommand.some(cmd => cmd instanceof RegExp ? cmd.test(command) : cmd ===
          command)) || (typeof prefixCommand === "string" && prefixCommand === command);
        m.prefix = !!result;
        m.command = {
          command,
          args,
        ...
        if (m.chat in db.data?.chats || m.sender in db.data?.users || this.user.jid in db.data?.settings) {
          ...
          if (name != "/plugins/Owner/owner-unbanchat.js" && name != "/plugins/Owner/owner-exec.js" && name !=
            "/plugins/Owner/owner-exec2.js" && name != "/plugins/Owner/tools-delete.js" && name !=
            "/plugins/Others/enable.js" && chat.isBanned) {
            continue
          }
          if (name != "/plugins/Owner/owner-unbanuser.js" && name != "/plugins/Owner/owner-exec.js" && name !=
            "/plugins/Owner/owner-exec2.js" && user.banned) {
            continue
          }
          if (name != "/plugins/Owner/owner-exec.js" && name != "/plugins/Owner/owner-exec2.js" && name !=
            "/plugins/Others/enable.js" && (_chat.self ?? _chat.pconly ?? _chat.gconly ?? _chat.swonly)) {
            continue
          }
        }
        if (
          (plugin.rowner && !(isROwner || isOwner) && (fail("owner", m, this), true)) || (plugin.rowner && !
            isROwner && (fail("rowner", m, this), true)) || (plugin.owner && !isOwner && (fail("owner", m, this),
            true)) || (plugin.mods && !isMods && (fail("mods", m, this), true)) || (plugin.premium && !isPrems && (
            fail("premium", m, this), true)) || (plugin.group && !m.isGroup && (fail("group", m, this), true)) || (
            plugin.botAdmin && !isBotAdmin && (fail("botAdmin", m, this), true)) || (plugin.admin && !isAdmin && (
            fail("admin", m, this), true)) || (plugin.private && m.isGroup && (fail("private", m, this), true)) || (
            plugin.register && !_user.registered && (fail("unreg", m, this), true)) || (plugin.nsfw && db.data?.chats[
            m.chat]?.nsfw && (fail("nsfw", m, this), true)) || (global.plugins[name]?.error && (fail("error", m,
            this), true)) || (opts['antirpg'] && db.data?.settings[this.user.jid]?.antirpg && plugin.tags && plugin
            .tags?.includes("rpg") && (fail("rpg", m, this), true)) || (opts['restrict'] && db.data?.settings[this
            .user.jid]?.restrict && plugin.tags && plugin.tags?.includes("admin") && (fail("restrict", m, this),
            true))) return false;
        m.isCommand = true
    ...
    if (m && m.plugin) {
      ...
      stat.success = !Number.isNaN(stat.success) ? (m.error != null ? stat.success : stat.success + 1) : (m.error !=
        null ? 0 : 1);
      stat.last = now;
      stat.lastSuccess = !Number.isNaN(stat.lastSuccess) ? (m.error != null ? stat.lastSuccess : now) : (m.error !=
        null ? 0 : now);
      stats[m.plugin] = stat;
    }
    try {
      if (!opts["noprint"] || db.data?.settings[this.user.jid]?.noprint) await (await import("./lib/print.js"))
        .default(m, this)
    } catch (e) {
...
export async function participantsUpdate({
  ...
  if (opts["self"] || db.data?.settings[this.user.jid]?.self || this.isInit) return;
  if (global.db.data === null) await loadDatabase();
  ...
  switch (action) {
    ...
    case "remove":
      if (chat.welcome) {
        ...
        user = user || this.user.jid;
          const isAddAction = action === "add";
          const welcomeText = isAddAction ? (chat.sWelcome || this.welcome || conn.welcome ||
            `${emoji.welcome} Selamat datang, @user!`).replace("@subject", await this.getName(id)).replace("@desc",
            groupMetadata.desc?.toString() || "tidak diketahui") : (chat.sBye || this.bye || conn.bye ||
            `${emoji.bye} Sampai jumpa, @user!`);
          const welran = Welcome(await this.profilePictureUrl(user, "image").catch(() => hwaifu.getRandom()), await this.getName(user), await this.getName(id), participants.length, katarandom.getRandom(), "Welcome") || thumb;
          const welbuff = await(await this.getFile(welran)).data || await(await fetch(welran)).arrayBuffer();
          const byeran = Leave(await this.profilePictureUrl(user, "image").catch(() => hwaifu.getRandom()), await this.getName(user), await this.getName(id), participants.length, katarandom.getRandom(), "Leave") || thumb;
          const byebuff = await(await this.getFile(byeran)).data || await(await fetch(byeran)).arrayBuffer();
          
          const editor = new ImageEditor(thumb, await this.profilePictureUrl(user, "image").catch(() => hwaifu.getRandom()));
const ieditwBuff = await editor.asyncWelcome(await this.getName(user), 'Welcome to our community!', katarandom.getRandom());
const ieditlBuff = await editor.asyncLeave(await this.getName(user), 'Leave in community!', katarandom.getRandom());
          const spaces = "                    "
          const lapor =
            `\n\n${emoji.mail} *Pesan:* Jika menemukan bug, error, atau kesulitan dalam penggunaan, silakan laporkan/tanyakan kepada ${emoji.owner}`;
          await this.reply(id, welcomeText.replace("@user", "@" + participants[0]?.split("@")[0]) + lapor, global.fakes || {
            key: {
            ...
            contextInfo: {
              ...
              externalAdReply: {
                ...
                thumbnail: (isAddAction ? (welbuff || ieditwBuff) : (ieditlBuff || byebuff)),
                sourceUrl: ""
              },
            },
          });
      ...
      break;
    case "promote":
      const promoteText = (chat.sPromote || this.spromote || conn.spromote ||
        `${emoji.promote} @user *telah diangkat menjadi Admin*`).replace("@user", "@" + participants[0]?.split("@")[
        0]);
      const promoteString = WAMessageStubType[29];
      const resultPromoteString = promoteString.split('_').map(word => word === 'UNKNOWN' ? 'Tidak Diketahui' : (word
        .charAt(0).toUpperCase() + word.slice(1).toLowerCase())).join(' ');
      if (chat.detect) {
        await this.reply(id, promoteText.trim(), global.fakes || {
          key: {
            participant: '0@s.whatsapp.net',
            remoteJid: 'status@broadcast'
          },
      ...
      break;
    case "demote":
      const demoteText = (chat.sDemote || this.sdemote || conn.sdemote ||
        `${emoji.demote} @user *tidak lagi menjadi Admin*`).replace("@user", "@" + participants[0]?.split("@")[0]);
      const demoteString = WAMessageStubType[30];
      const resultDemoteString = demoteString.split('_').map(word => word === 'UNKNOWN' ? 'Tidak Diketahui' : (word
        .charAt(0).toUpperCase() + word.slice(1).toLowerCase())).join(' ');
      if (chat.detect) {
        await this.reply(id, demoteText.trim(), null, {
          contextInfo: {
            mentionedJid: [participants[0]],
            externalAdReply: {
...
export async function groupsUpdate(groupUpdates) {
  for (const groupUpdate of groupUpdates) {
    ...
    if (opts["self"] || db.data?.settings[this.user.jid]?.self) continue;
    if (!id) continue
    ...
    if (groupUpdate.desc) {
      ...
      text = (chats.sSubject || this.sSubject || conn.sSubject ||
        `*${emoji.subject} Subjek telah diubah menjadi*\n@subject`).replace("@subject", groupUpdate.subject)
      stubType = 21
    } else if (groupUpdate.icon) {
      text = (chats.sIcon || this.sIcon || conn.sIcon || `*${emoji.icon} Icon telah diubah menjadi*`).replace("@icon",
        groupUpdate.icon)
      stubType = 22
    } else if (groupUpdate.revoke) {
      text = (chats.sRevoke || this.sRevoke || conn.sRevoke ||
        `*${emoji.revoke} Tautan grup telah diubah menjadi*\n@revoke`).replace("@revoke", groupUpdate.revoke)
      stubType = 1
    } else if (groupUpdate.announce === true) {
      text = (chats.sAnnounceOn || this.sAnnounceOn || conn.sAnnounceOn ||
        `*${emoji.announceOn} Grup telah ditutup!*`)
      stubType = 26
    } else if (groupUpdate.announce === false) {
      text = (chats.sAnnounceOff || this.sAnnounceOff || conn.sAnnounceOff ||
        `*${emoji.announceOff} Grup telah dibuka!*`)
      stubType = 26
    } else if (groupUpdate.restrict === true) {
      text = (chats.sRestrictOn || this.sRestrictOn || conn.sRestrictOn ||
        `*${emoji.restrictOn} Grup telah dibatasi hanya untuk peserta!*`)
      stubType = 25
    } else if (groupUpdate.restrict === false) {
      text = (chats.sRestrictOff || this.sRestrictOff || conn.sRestrictOff ||
        `*${emoji.restrictOff} Grup telah dibatasi hanya untuk admin!*`)
      stubType = 25
    }
    const inputString = WAMessageStubType[stubType];
    const resultString = inputString.split('_').map(word => word === 'UNKNOWN' ? 'Tidak Diketahui' : (word.charAt(0)
      .toUpperCase() + word.slice(1).toLowerCase())).join(' ');
    if (!text) continue
    await this.reply(id, text.trim(), global.fakes || {
      key: {
        participant: '0@s.whatsapp.net',
        remoteJid: 'status@broadcast'
      },
...
export async function deleteUpdate(message) {
  try {
    ...
    const caption =
      `❗ Terdeteksi @${(msg.key?.remoteJid?.split('@')[0] || participant?.split('@')[0] || msg.chat?.split('@')[0])} telah menghapus pesan.\nUntuk mematikan fitur ini, ketik\n*.off antidelete*\n\nUntuk menghapus pesan yang dikirim BOT, reply pesan dengan perintah\n*.delete*`;
    await this.reply(
      (msg.key?.remoteJid || participant || msg.chat), caption || 'Deleted Message', msg || null, {
        contextInfo: {
    ...
    await this.copyNForward((msg.key?.remoteJid || participant || msg.chat), msg || null, false).catch(e => console
      .log(e, msg));
  } catch (e) {
    console.error(e);
  }
  ...
  for (const {
    ...
    if (message.pollUpdates) {
      ...
      if (pollCreation) {
        ...
        this.appenTextMessage(message, message.pollUpdates[0]?.vote || pollMessage.filter((v) => v.voters.length !==
          0)[0]?.name, message.message);
      }
    }
  }
}
