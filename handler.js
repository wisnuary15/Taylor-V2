...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    const groupMetadata = (m.isGroup ? (this.chats[m.chat] || {}).metadata || await this.groupMetadata(m.chat).catch(console.error) : {}) || {};
...
export async function participantsUpdate({
  ...
  switch (action) {
    ...
    case "remove":
      if (chat.welcome) {
        const groupMetadata = await (conn || this).groupMetadata(id).catch(console.error) || ((conn || this).chats[id] || {}).metadata || {};
