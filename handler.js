...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        for (let name in global.plugins) {
            ...
            if (opts && match && m) {
                ...
                if (
                    ...
                    (plugin.nsfw && db.data?.chats[m.chat].nsfw && (fail("nsfw", m, this), true)) ||
                    (global.plugins[name].error && (fail("error", m, this), true)) ||
                    (opts['antirpg'] && db.data?.settings[this.user.jid].antirpg && plugin.tags && plugin.tags?.includes("rpg") && (fail("rpg", m, this), true)) ||
                    (opts['restrict'] && db.data?.settings[this.user.jid].restrict && plugin.tags && plugin.tags?.includes("admin") && (fail("restrict", m, this), true))
                ) return false;
                m.isCommand = true
        ...
        if (opts["autoread"] || db.data?.settings[this.user.jid].autoread)
    ...
    case "remove":
        if (chat.welcome) {
            ...
            for (let user of participants) {
                ...
                const welran = Welcome(await this.profilePictureUrl(user, "image").catch(() => hwaifu.getRandom()), await this.getName(user), await this.getName(id), participants.length, katarandom.getRandom(), "Welcome");
                const byeran = Leave(await this.profilePictureUrl(user, "image").catch(() => hwaifu.getRandom()), await this.getName(user), await this.getName(id), participants.length, katarandom.getRandom(), "Leave");
                const spaces = "                    "
