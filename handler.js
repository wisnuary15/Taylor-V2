...
export async function participantsUpdate({
  ...
  switch (action) {
    ...
    case "remove":
      if (chat.welcome) {
        ...
        for (let user of participants) {
          ...
          const ieditwBuff = await editor.asyncWelcome(await (conn || this).getName(user), "Welcome to our group!", welcomeText) || await (await fetch(thumb, {
            agent: agent
          })).arrayBuffer();
          const ieditlBuff = await editor.asyncLeave(await (conn || this).getName(user), "Leave in group!", welcomeText) || await (await fetch(thumb, {
            agent: agent
          })).arrayBuffer();
          const spaces = "                    ";
          ...
          await (conn || this).reply(id, welcomeText.replace("@user", "@" + participants[0]?.split("@")[0]) + lapor, global.fakes || {
            key: {
            ...
            contextInfo: {
              ...
              externalAdReply: {
                ...
                thumbnail: isAddAction ? ieditwBuff : ieditlBuff,
                sourceUrl: ""
              }
            }
          });
    ...
    case "promote":
      ...
      if (chat.detect) {
        await (conn || this).reply(id, promoteText.trim(), global.fakes || {
          key: {
            participant: "0@s.whatsapp.net",
            remoteJid: "status@broadcast"
          },
...
export async function groupsUpdate(groupUpdates) {
  for (const groupUpdate of groupUpdates) {
    ...
    if (!text) continue;
    await (conn || this).reply(id, text.trim(), global.fakes || {
      key: {
        participant: "0@s.whatsapp.net",
        remoteJid: "status@broadcast"
      },
