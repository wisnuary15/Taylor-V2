  ...
  try {
    ...
    for (let name in global.plugins) {
      ...
      if (opts && match && m) {
        ...
        if (xp > 200) this.sendMessage(m.chat, {
...
export async function participantsUpdate({
  ...
  switch (action) {
      ...
      if (chat.welcome) {
        ...
        for (let user of participants) {
          ...
          const welcomeText = isAddAction ? (chat.sWelcome || this.welcome || conn.welcome ||
            `${emoji.welcome} Selamat datang, @user!`).replace("@subject", this.getName(id)).replace("@desc",
            groupMetadata.desc?.toString() || "tidak diketahui") : (chat.sBye || this.bye || conn.bye ||
            `${emoji.bye} Sampai jumpa, @user!`);
          const welran = Welcome(await this.profilePictureUrl(user, "image").catch(() => hwaifu.getRandom()), this
            .getName(user), this.getName(id), participants.length, katarandom.getRandom(), "Welcome");
          const byeran = Leave(await this.profilePictureUrl(user, "image").catch(() => hwaifu.getRandom()), this
            .getName(user), this.getName(id), participants.length, katarandom.getRandom(), "Leave");
          const spaces = "                    "
          const lapor =
            `\n\n${emoji.mail} *Pesan:* Jika menemukan bug, error, atau kesulitan dalam penggunaan, silakan laporkan/tanyakan kepada ${emoji.owner}`;
          this.reply(id, welcomeText.replace("@user", "@" + participants[0]?.split("@")[0]) + lapor, global.fakes || {
            key: {
            ...
            contextInfo: {
              ...
              externalAdReply: {
                ...
                thumbnailUrl: isAddAction ? welran : byeran,
                sourceUrl: ""
              },
            },
          });
    ...
    case "promote":
      ...
      if (chat.detect) {
        this.reply(id, promoteText.trim(), global.fakes || {
          key: {
            participant: '0@s.whatsapp.net',
            remoteJid: 'status@broadcast'
          },
    ...
    case "demote":
      ...
      const resultDemoteString = demoteString.split('_').map(word => word === 'UNKNOWN' ? 'Tidak Diketahui' : (word
        .charAt(0).toUpperCase() + word.slice(1).toLowerCase())).join(' ');
      if (chat.detect) {
        this.reply(id, demoteText.trim(), null, {
          contextInfo: {
            mentionedJid: [participants[0]],
            externalAdReply: {
