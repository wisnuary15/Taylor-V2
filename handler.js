...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    const isROwner = [this.decodeJid?.(this.user.id), ..._.map(global.owner, ([number]) => number)].map(v => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net").includes(m.sender);
    const isOwner = isROwner || m.fromMe;
    const isMods = isOwner || _.map(global.mods, v => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net").includes(m.sender);
    const isPrems = isROwner || _.map(global.prems, v => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net").includes(m.sender);
