...
const { say: say } = cfonts, rl = createInterface(process.stdin, process.stdout), __filename = fileURLToPath(import.meta.url), __dirname = dirname(__filename), require = createRequire(import.meta.url), { name: name, author: author } = require(join(__dirname, "./package.json"));
say("Bot Whatsapp MD", {
  font: "chrome",
  align: "center",
  gradient: ["blue", "green"]
...
async function start(file) {
  ...
  if (app.get("/", ((req, res) => sendHtml(0, res, "home"))), app.get("/chat", ((req, res) => sendHtml(0, res, "chat"))),
    app.get("/game", ((req, res) => sendHtml(0, res, "game"))), app.get("/tools", ((req, res) => sendHtml(0, res, "tools"))),
    app.get("/music", ((req, res) => sendHtml(0, res, "music"))), server = app.listen(port, (() => {
      console.log(chalk.green(`🌐 Port ${port} is open`));
    })), server.on("error", (error => {
      "EADDRINUSE" === error.code ? (console.log(`Address localhost:${port} in use. Please retry when the port is available!`),
        server.close()) : console.error("Server error:", error);
    })), isRunning) return;
  isRunning = !0;
  ...
  const p = fork();
  p.on("message", (data => {
    console.log("[✅RECEIVED]", data), "reset" === data ? (p.process.kill(), isRunning = !1,
      start(file)) : "uptime" === data && p.send(process.uptime());
  })), p.on("exit", ((_, code) => {
  ...
  try {
    const { fetchLatestBaileysVersion: fetchLatestBaileysVersion } = await import("@whiskeysockets/baileys"), { version: version } = await fetchLatestBaileysVersion();
  ...
  const tableData = foldersInfo.map((({ folder: folder, files: files }) => ({
