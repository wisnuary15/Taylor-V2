const handler = async (m, {
    ...
    try {
        if (!m.quoted && !text) {
            conn.reply(m.chat, `Reply pesan report. Gunakan perintah ini:\n\nContoh:\n${usedPrefix + command} <reply/report>`, m)
            return;
        }
        let teks = `*BALASAN REPORT*\n\nDari: *@${m.sender.split('@')[0]}*\n\nPesan: ${text}\n`
        let response = conn.reply((conn.parseMention(m.quoted.text))[0], m.quoted ? teks + m.quoted.text : teks || m.text, null, {
            contextInfo: {
