...
const handler = async (m, { conn: conn, args: args, text: text, usedPrefix: usedPrefix, command: command }) => {
  ...
  try {
    let response = axios.get("https://api.weatherapi.com/v1/current.json?key=897dba35c1d94f4cbea134758220207&q=" + text),
      res = await response,
      { name: name, region: region, country: country, lat: lat, lon: lon, tz_id: tz_id, localtime_epoch: localtime_epoch, localtime: localtime } = res.data.location,
      { last_updated_epoch: last_updated_epoch, last_updated: last_updated, temp_c: temp_c, temp_f: temp_f, is_day: is_day, wind_mph: wind_mph, wind_kph: wind_kph, wind_degree: wind_degree, wind_dir: wind_dir, pressure_mb: pressure_mb, pressure_in: pressure_in, precip_mm: precip_mm, precip_in: precip_in, humidity: humidity, cloud: cloud, feelslike_c: feelslike_c, feelslike_f: feelslike_f, vis_km: vis_km, vis_miles: vis_miles, uv: uv, gust_mph: gust_mph, gust_kph: gust_kph } = res.data.current,
