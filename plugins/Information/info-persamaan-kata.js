...
const handler = async (m, { conn, usedPrefix, args, command }) => {
  ...
  try {
    const { synonyms, antonyms, imageLink } = await getSynonymsAndAntonyms(text);
...
async function getSynonymsAndAntonyms(searchTerm) {
  try {
    const formData = new URLSearchParams({ q: searchTerm });
    ...
    const synonyms = $('.thesaurus_group').eq(0).find('.word_thesaurus a')
      .map((_, el) => ({
        word: $(el).text(),
        link: $(el).attr('href')
      })).get();

    const antonyms = $('.thesaurus_group').eq(1).find('.word_thesaurus a')
      .map((_, el) => ({
        word: $(el).text(),
        link: $(el).attr('href')
      })).get();
    ...
    return { synonyms, antonyms, imageLink };
