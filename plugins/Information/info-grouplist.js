const handler = async (m, {
  ...
  if (args.length === 0) {
    ...
    if (index >= 0 && index < groups.length) {
      ...
      const adminList = group.participants.filter(p => p.admin === 'admin').map(a =>
        `- ${a.id.replace(/(\d+)@.+/, '@$1')}`).join('\n');
      const superAdminList = group.participants.filter(p => p.admin === 'superadmin').map(a =>
        `- ${a.id.replace(/(\d+)@.+/, '@$1')}`).join('\n');
