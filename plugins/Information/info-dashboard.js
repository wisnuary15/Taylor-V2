const handler = async (m, { conn: conn }) => {
  try {
    ...
    const formatNumber = (num) => {
      ...
      let suffixIndex = 0;
      while (num >= 1000 && suffixIndex < suffixes.length - 1) num /= 1000, suffixIndex++;
      return `${Math.round(num)}${suffixes[suffixIndex]}`;
    };

    const formatTime = (time) => {
      const timeDiff = Math.floor((Date.now() - time) / 1000);
      const intervals = { tahun: 31536000, bulan: 2592000, minggu: 604800, hari: 86400, jam: 3600, menit: 60, detik: 1 };

      for (const [unit, seconds] of Object.entries(intervals)) if (Math.floor(timeDiff / seconds) > 0) return `${Math.floor(timeDiff / seconds)} ${unit} yang lalu`;

      return 'Baru saja';
    };

    const output = sortedData.map(([name, { total, success, last, lastSuccess }], idx) =>
      `*${idx + 1}). ${name.split('/').pop().replace('.js', '')}*\n   - Total: \`${formatNumber(total)}\`\n   - Success: \`${formatNumber(success)}\`\n   - Last: \`${formatTime(last)}\`\n   - Last Success: \`${formatTime(lastSuccess)}\``).join('\n\n');
