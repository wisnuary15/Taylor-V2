...
const handler = async (m, {
  ...
  if (command === "creator") {
    ...
    conn.reply(m.chat,
      `Halo kak @${m.sender.split("@")[0]} itu nomor team developerku, jangan di apa-apain ya kak😖`, tag_own, {
        mentions: [m.sender]
      })
  }
  ...
  if (command === "owner") {
    try {
      const ownerPromises = owner.map(async (item, index) => [
        item[0],
        (conn.getName(item[0] + "@s.whatsapp.net")) || "Tidak diketahui", "👑 Owner", "Saya adalah Owner" ||
        "Tidak diketahui", "wudysoft@gmail.com", "🇮🇩 Indonesia", "🚀 https://aygemuy.github.io/",
