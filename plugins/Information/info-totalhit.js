const handler = async (m, {
  ...
  try {
    const stats = Object.entries(db.data.stats).map(([key, val]) => ({
      name: Array.isArray(plugins[key].help) ? plugins[key].help.join(' , ') : plugins[key].help || key.split(
        '/').pop().slice(0, -3),
      ...val
    })).filter(({
      name
...
const getTime = (ms) => {
  ...
  return `${now.days ? now.days + 'd' : ''} ${now.hours ? now.hours + 'h' : ''} ${now.minutes ? now.minutes + 'm' : ''} ${now.seconds ? now.seconds + 's' : ''} ${now.milliseconds ? now.milliseconds + 'ms' : ''}`
    .trim() || 'a few seconds';
};
const parseMs = (ms) => ({
