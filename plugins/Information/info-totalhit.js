const handler = async (m, { conn: conn }) => {
  try {
    const stats = Object.entries(db.data.stats).map((([key, val]) => ({
      ...
      ...val
    }))).filter((({ name: name }) => !/exec/.test(name))).sort(((a, b) => b.total - a.total)).slice(0, 50).map((({ name: name, total: total, last: last, success: success, lastSuccess: lastSuccess }, i) => `*${i + 1}.* ( *${name.split(" ")[0]}* )\n-   *Hits*: \`${total}\`\n-   *Success*: \`${success}\`\n-   *Last Used*: \`${getTime(last)} ago\`\n-   *Last Success*: \`${formatTime(lastSuccess)}\``)).join("\n\n");
