...
let handler = async (m, { conn, args, text }) => {
    ...
    try {
        ...
        else if (/image|video|viewOnce/g.test(b_)) link = await (b_.includes('video') ? uploadFile : uploadImage)(c_)
...
• sepia`
    ...
    let hoih
    switch (args[0]) {
        case 'blur': hoih = await blur(link, args[1]); break;
        case 'color': hoih = await color(link, args[1]); break;
        case 'flip': hoih = await flip(link); break;
        case 'flip2': hoih = await flip2(link); break;
        case 'gaussian': hoih = await gaussian(link, args[1]); break;
        case 'invert': hoih = await invert(link, args[1]); break;
        case 'mask': hoih = await mask(link, args[1]); break;
        case 'normalize': hoih = await normalize(link); break;
        case 'autocrop': hoih = await autocrop(link, args[1]); break;
        case 'background': hoih = await background(link, args[1]); break;
        case 'brightness': hoih = await brightness(link, args[1]); break;
        case 'contrast': hoih = await contrast(link, args[1]); break;
        case 'crop': hoih = await crop(link, args[1]); break;
        case 'dither565': hoih = await dither565(link); break;
        case 'fade': hoih = await fade(link, args[1]); break;
        case 'greyscale': hoih = await greyscale(link); break;
        case 'hasAlpha': hoih = await hasAlpha(link, args[1]); break;
        case 'mirror': hoih = await mirror(link); break;
        case 'mirror2': hoih = await mirror2(link); break;
        case 'opacity': hoih = await opacity(link, args[1]); break;
        case 'opaque': hoih = await opaque(link); break;
        case 'posterize': hoih = await posterize(link, args[1]); break;
        case 'sepia': hoih = await sepia(link); break;
        default: throw `Efek "${args[0]}" tidak dikenali`;
    }
