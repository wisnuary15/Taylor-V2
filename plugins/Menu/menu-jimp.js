...
const handler = async (m, { conn: conn, args: args, text: text }) => {
  ...
  try {
    /webp/g.test(b_) ? link = await webp2png(c_) : m.mentionedJid?.[0] ? link = await conn.profilePictureUrl(m.mentionedJid[0], "image") : /image|video|viewOnce/g.test(b_) && (link = await (b_.includes("video") ? uploadFile : uploadImage)(c_)),
      "string" != typeof link ? link = await uploadImage(c_) : /gif/g.test(b_) && (link = e_);
  } catch (e) {
    m.react(eror);
  }
