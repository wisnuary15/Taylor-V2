...
const handler = async (m, { conn: conn, groupMetadata: groupMetadata, usedPrefix: usedPrefix, text: text, args: args, isPrems: isPrems, isOwner: isOwner, command: command }) => {
  let who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? conn.user.jid : m.sender,
    btn = (await conn.profilePictureUrl(who).catch((_ => hwaifu.getRandom())),
      conn.getName(who), flaaa.getRandom(), [{
        urlButton: {
  ...
  try {
    if (command) switch (template) {
      ...
      case "filter":
        ...
        try {
          /webp/g.test(b_) ? d_ = await webp2png(c_) : /image/g.test(b_) ? d_ = await uploadImage(c_) : (/video/g.test(b_) || /viewOnce/g.test(b_)) && (d_ = await uploadFile(c_)),
            "string" != typeof d_ ? d_ = await uploadImage(c_) : /gif/g.test(b_) && (d_ = e_);
        } catch (e) {
          m.react(eror);
        }
      ...
      case "misc":
        ...
        try {
          /webp/g.test(b__) ? d__ = await webp2png(c__) : /image/g.test(b__) ? d__ = await uploadImage(c__) : (/video/g.test(b__) || /viewOnce/g.test(b__)) && (d__ = await uploadFile(c__)),
            "string" != typeof d__ ? d__ = await uploadImage(c__) : /gif/g.test(b__) && (d__ = e__);
        } catch (e) {
          m.react(eror);
        }
      ...
      case "overlay":
        ...
        try {
          /webp/g.test(b___) ? d___ = await webp2png(c___) : /image/g.test(b___) ? d___ = await uploadImage(c___) : (/video/g.test(b___) || /viewOnce/g.test(b___)) && (d___ = await uploadFile(c___)),
            "string" != typeof d___ ? d___ = await uploadImage(c___) : /gif/g.test(b___) && (d___ = e___);
        } catch (e) {
          m.react(eror);
        }
