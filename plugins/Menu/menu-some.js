...
`
    conn.sendFile(m.chat, logo, '', caption, m)
  }
  try {
    if (command) {
      switch (template) {
        ...
        ${kc.lyrics}
        `
          return conn.sendButtonImg(m.chat, kc.thumbnail.genius, kd, kc.link.genius, 'To Sticker', '.s', fakes,
            adReply)
          break
        case 'filter':
