...
const handler = async (m, {
  ...
  let soun = ["aku-ngakak", "anjay", "ara-ara2", "ara-ara-cowok", "ara-ara", "arigatou", "assalamualaikum", "asu",
    "ayank", "bacot", "bahagia-aku", "baka", "bansos", "beat-box2", "beat-box", "biasalah", "bidadari", "bot",
    "buka-pintu", "canda-anjing", "cepetan", "china", "cuekin-terus", "daisuki-dayo", "daisuki", "dengan-mu",
    "Donasiku", "gaboleh-gitu", "gak-lucu", "gamau", "gay", "gelay", "gitar", "gomenasai", "hai-bot", "hampa",
    "hayo", "hp-iphone", "ih-wibu", "i-like-you", "india", "karna-lo-wibu", "kiss", "kontol", "ku-coba",
    "maju-wibu", "makasih", "mastah", "menuasli", "menuku", "menu", "MenuYuki", "nande-nande", "nani",
    "ngadi-ngadi", "nikah", "nuina", "onichan", "ownerku", "owner-sange", "pak-sapardi", "pale", "pantek",
    "pasi-pasi", "punten", "sayang", "siapa-sih", "sudah-biasa", "summertime", "tanya-bapak-lu", "to-the-bone",
    "wajib", "waku", "woi", "yamete", "yowaimo", "yoyowaimo"
  ].getRandom()
  ...
  let arrayMenu = ["all", "absen", "admin", "advanced", "anonymous", "audio", "ai", "gpt", "Baileys", "database",
    "downloader", "edukasi", "fun", "game", "genshin", "group", "host", "info", "internet", "jadian", "jadibot",
    "kerang", "main", "maker", "misc", "music", "nocategory", "nsfw", "nulis", "owner", "premium", "primbon",
    "quotes", "quran", "random", "rpg", "sticker", "tools", "vote", "xp", "store", "virus", "thnks"
  ]
...
${v.rowId}`.trim()
      ...
      if (conn.temamenu.id === 1) {
        ...
        return conn.sendFile(m.chat, Buffer.alloc(0), "D A S H B O A R D", caption, fakes, false, {
    ...
    let after = conn.menu.after || (conn.user.jid === conn.user.jid ? "" :
      `Powered by https://wa.me/${conn.user.jid.split('@')[0]}`) + defaultMenu.after
    let _text = [
      before, ...Object.keys(tags).map(tag => {
        return header.replace(/%category/g, (tags[tag]).toUpperCase()) + "\n" + [...help.filter(menu => menu
            .tags && menu.tags.includes(tag) && menu.help).map(menu => {
            return menu.help.map(help => {
              return body.replace(/%cmd/g, menu.prefix ? help : "%_p" + help).replace(/%isGroup/g, menu
                .group ? "`Ⓖ` " : "").replace(/%isPrivate/g, menu.private ? "`Ⓟ` " : "").replace(
                /%isOwner/g, menu.owner ? "`Ⓞ` " : "").replace(/%isPremium/g, menu.premium ? "`🅟` " :
                "").replace(/%isLimit/g, menu.limit ? "`Ⓛ` " : "").replace(/%isError/g, menu.error ?
                "`❌` " : "").replace(/<([^>]*)>/g, "[$1]").trim()
            }).join("\n")
          }),
          footer
    ...
    if (conn.temamenu.id === 1) {
      ...
      conn.sendFile(m.chat, Buffer.alloc(0), "D A S H B O A R D", caption, fakes, false, {
        mimetype: [dpptx, ddocx, dxlsx, dpdf, drtf].getRandom(),
        fileLength: fsizedoc,
        pageCount: fpagedoc,
        jpegThumbnail: await conn.resize([thumbdoc, thumb].getRandom(), 300, 150),
