...
`;
    ...
    if (teks === "404") {
      ...
      switch (id) {
        ...
        case 9:
        case 10: {
          const btn = conn.ctaButton.setBody(caption).setFooter(author).setDocument(Buffer.alloc(0), {
            mimetype: [dpptx, ddocx, dxlsx, dpdf, drtf].getRandom(),
            fileName: ucapan + m.name,
            fileLength: fsizedoc,
            pageCount: fpagedoc,
            jpegThumbnail: id === 9 ? await conn.resize(thumbdoc, 300, 150) : undefined
          }).addReply("Menu List", _p + "menulist").addUrl("Official Group", sgc).addSelection(listMessage.buttonText).makeSections(listMessage.text, "").contextInfo([adReply.contextInfo, adReplyS.contextInfo].getRandom());
        ...
        break;
        case 11: {
          const btn = conn.ctaButton.setBody(caption).setFooter(author).addReply("Menu List", _p + "menulist").addUrl("Official Group", sgc).contextInfo([adReply.contextInfo, adReplyS.contextInfo].getRandom());
          return await btn.run(m.chat, conn, m);
        }
        break;
        case 12: {
          const btn = conn.ctaButton.setBody(caption).setFooter(author).addReply("Menu List", _p + "menulist").addUrl("Official Group", sgc).contextInfo([fakeig.contextInfo, fakeyt.contextInfo].getRandom());
          return await btn.run(m.chat, conn, m);
        }
        break;
        default:
