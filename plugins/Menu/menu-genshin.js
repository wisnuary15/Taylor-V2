  ...
  try {
    ...
    let _text = [
      before, ...Object.keys(tags).map(tag => {
        return header.replace(/%category/g, tags[tag]) + '\n' + [...help.filter(menu => menu.tags && menu.tags
            .includes(tag) && menu.help).map(menu => {
    ...
    /*conn.sendHydrated(m.chat, text.replace(`si <character>`, `si <character>${readMore}`).trim(), packname + ' - ' + author, nais, 'https://bit.ly/3eggcxd', 'Minimalist ツ Sweet', null, null, [
    	['Premium', '/premium'],
    	['Speed', '/ping'],
    	['Owner', '/owner']
    ], m)*/
    conn.sendFile(m.chat, pp ? pp : nais, '', text.replace(`si <character>`, `si <character>${readMore}`).trim(), m)
  } catch (e) {
