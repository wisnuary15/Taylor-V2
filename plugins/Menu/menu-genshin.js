...
const defaultMenu = {
  ...
  handler = async (m, { conn: conn, usedPrefix: _p, __dirname: __dirname }) => {
    try {
      padLead(ranNumb(39), 3);
      let nais = fs.readFileSync("./thumbnail.jpg"),
        help = (JSON.parse(await promises.readFile(join(__dirname, "../package.json")).catch((_ => ({})))),
          Object.values(plugins).filter((plugin => !plugin.disabled)).map((plugin => ({
