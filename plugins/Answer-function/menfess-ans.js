...
export async function before(m) {
  ...
  if ((m.text === 'BALAS PESAN' || m.text === '') && m.quoted?.mtype === 'buttonsMessage') {
    ...
    const txt =
      `Hᴀɪ ᴋᴀᴋ @${mf.dari.split('@')[0]}, ᴋᴀᴍᴜ ᴍᴇɴᴇʀɪᴛᴇ ʙᴀʟᴀsᴀɴ ɴɪʜ. Pesan yang kamu kirim sebelumnya:\n${mf.pesan}\n\nPesan balasannya:\n${m.text}\n`
      .trim();
    const sblm = `Pᴇsᴀɴ Aɴᴅᴀ Sᴇʙᴇʟᴜᴍɴʏᴀ ➛ ${mf.pesan}\nPᴇsᴀɴ Bᴀʟᴀsᴀɴɴʏᴀ ➨ ${m.text}`;
    this.reply(m.chat, txt + '\n' + sblm, null).then(() => {
      this.reply(m.chat, 'Berhasil Mengirim balasan.', null).then(() => {
        delay(1500);
