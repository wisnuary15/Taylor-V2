...
handler.all = async function(m) {
  ...
  if (mf && (m.text === 'balas' || m.text === 'Balas' || m.text === '') && m.quoted?.mtype ===
    'extendedTextMessage') return m.reply("Silahkan kirim pesan balasan kamu.")
  let txt =
    `Hai kak @${mf.dari.split('@')[0]}, kamu menerima balasan nih.\n\nPesan yang kamu kirim sebelumnya:\n${mf.pesan}\n\nPesan balasannya:\n${m.text}\n`
    .trim();
  this.reply(mf.dari, txt, null).then(() => {
