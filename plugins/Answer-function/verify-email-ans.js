...
export async function before(m) {
  ...
  if (m.quoted?.id === this.regmail[id][0]?.id) {
    ...
    m.text.toLowerCase() === json.jawaban.toLowerCase().trim() ? (db.data.users[m.sender].exp += this.regmail[id][2],
      await this.reply(m.chat, `*✅ Nomor Anda telah berhasil diverifikasi.*\n+${this.regmail[id][2]} XP`, m),
      clearTimeout(this.regmail[id][3]), delete this.regmail[id]) : similarity(m.text.toLowerCase(), json.jawaban.toLowerCase().trim()) >= .72 ? m.reply("*❗ Dikit Lagi!*") : await this.reply(m.chat, "*❌ Kode verifikasi Anda salah.*", m);
  }
  return !0;
}
export const exp = 0;
