...
export async function before(m) {
  let id = m.chat
  if (!m.quoted || !m.quoted?.fromMe || !m.quoted?.isBaileys || !m.text || !/Ketik.*hotp/i.test(m.quoted?.text) ||
    /.*hotp/i.test(m.text)) return !0
  this.regmail = this.regmail ? this.regmail : {}
  if (!(id in this.regmail)) return this.reply(m.chat, '*❗ Kode verifikasi Anda telah kedaluwarsa.*', m)
  if (m.quoted?.id === this.regmail[id][0]?.id) {
    ...
    if (m.text.toLowerCase() === json.jawaban.toLowerCase().trim()) {
      db.data.users[m.sender].exp += this.regmail[id][2]
      ...
      `*❗ Dikit Lagi!*`)
    else this.reply(m.chat, `*❌ Kode verifikasi Anda salah.*`, m)
  }
  return !0
}
export const exp = 0
