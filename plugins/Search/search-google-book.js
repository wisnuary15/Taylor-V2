...
const handler = async (m, {
  ...
  if (lister.includes(feature)) {
    ...
    if (feature === "sort") {
      ...
      conn.reply(m.chat, `*${htki} 📺 Books Search 🔎 ${htka}*\n${capt}`, m)
    }
    if (feature === "all") {
      let data = await getBookInfo(feature, querys)
      let capt = await formatData([data])
    ...
    if (feature === "file") {
      conn.sendFile(m.chat, querys, 'Boom nya kak!', '', m, false, {
        asDocument: true
      })
    }
  }
...
async function getBookInfo(type, query) {
  const response = await fetch(
    `https://www.googleapis.com/books/v1/volumes?q=${query}&download=epub&key=AIzaSyD4bymyi_wD_OIO9kEP26jir5rR3ftnkRg`
    )
