...
${wait}`
        conn.sendFile(m.chat, resl.ogImage, "", cap, m)
        conn.sendFile(m.chat, resl.downloadLink, resl.ogTitle, '', m, false, {
          asDocument: true
        })
      } catch (e) {
        m.react(eror)
...
`
        conn.sendFile(m.chat, resl.author.photo, "", cap, m)
      } catch (e) {
        m.react(eror)
      }
    }
...
async function getQuotes(page) {
  try {
    let url =
    'https://foulabook.com/ar/quotes'; // Ganti URL_HALAMAN_WEB dengan URL halaman web yang berisi kode HTML yang diberikan
    if (page !== '') {
...
async function getCategory() {
  try {
    const url =
    'https://foulabook.com/ar/categories'; // Ganti URL_HALAMAN_WEB dengan URL halaman web yang berisi kode HTML yang diberikan
    const response = await fetch(url);
    const body = await response.text();
    const $ = cheerio.load(body);
    const results = [];
