...
const handler = async (m, {
  ...
  try {
    ...
    if (!output || !output.posts || !output.posts.post || output.posts.post.length === 0) {
      return m.reply("Tidak ada hasil ditemukan.");
    }
    const posts = output.posts.post.slice(0, 25);
    const buttons = conn.ctaButton.setBody("Hasil pencarian:").setFooter("Klik pada setiap tombol untuk melihat gambar.").addSelection("Klik di sini").makeSections("Anime Wall", "Pilih konten");
    posts.forEach((post, idx) => {
      buttons.makeRow(``, `⭐ *Score:* ${post.score}`, `🔗 *Tags:* ${post.tags}`, `${usedPrefix}get ${post.file_url}`);
    });
...
handler.tags = ["search"];
handler.help = ["e621 <query>", "gelbooru <query>", "rule34 <query>", "danbooru <query>", "konachan <query>", "konachan18 <query>", "hypnohub <query>", "xbooru <query>", "realbooru <query>", "furrybooru <query>"];
