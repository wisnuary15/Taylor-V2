...
const handler = async (m, { conn: conn, usedPrefix: usedPrefix, args: args, text: text, command: command }) => {
  let urut = text.split("|"),
    one = urut[0],
    two = urut[1],
    three = urut[2];
  ...
  if ("imagesget" === command) try {
    let caption = "*[ Result ]*\n*Width:* " + two + "\n*Height:* " + three;
    "picsum.photos" === one && await conn.sendFile(m.chat, API("https://picsum.photos/" + two + "/" + three), "", caption, m),
      "unsplash.it" === one && await conn.sendFile(m.chat, API("https://unsplash.it/" + two + "/" + three + "?random"), "", caption, m),
      "random.imagecdn.app" === one && await conn.sendFile(m.chat, API("https://random.imagecdn.app/" + two + "/" + three), "", caption, m);
  } catch (e) {
    m.react(eror);
  }
};
