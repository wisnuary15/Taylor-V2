...
`.trim()
        ...
        conn.reply(m.chat, teks, m, adReply)
      } catch (e) {
        m.react(eror)
      }
    }
...
async function Search(QUERY) {
  const SEARCH_URL = "https://api.musixmatch.com/ws/1.1/track.search?q_track=" + QUERY +
...
async function Top() {
  const TOP_TEN_SONGS_URL =
    "https://api.musixmatch.com/ws/1.1/chart.tracks.get?chart_name=top&page=1&page_size=10&country=ind&f_has_lyrics=1&apikey=" +
    API_KEY;
...
async function Lyrics(TRACK_ID) {
  const FETCH_LYRICS_URL = "https://api.musixmatch.com/ws/1.1/track.lyrics.get?commontrack_id=" + TRACK_ID +
    "&apikey=" + API_KEY;
...
async function Track(TRACK_ID) {
  const FETCH_TRACK_URL = "https://api.musixmatch.com/ws/1.1/track.get?commontrack_id=" + TRACK_ID + "&apikey=" +
    API_KEY;
