...
const handler = async (m, {
  ...
  try {
    ...
    if (!index && !chapter) {
      ...
      const buttons = conn.ctaButton.setBody(`Pilih konten dari hasil pencarian untuk "${query}".`).setFooter("Informasi lebih lanjut dapat ditemukan di AnimeBatch.").addSelection("Klik di sini").makeSections("AnimeBatch", "Pilih konten");
      searchResults.forEach((result, idx) => {
        buttons.makeRow(``, `📰 *Judul:* ${result.title}`, `⭐ *Score:* ${result.score}`, `${usedPrefix}${command} ${query}|${idx + 1}`);
      });
    ...
    if (index > searchResults.length) {
      return m.reply(`ℹ️ Indeks tidak valid. Mohon berikan nomor antara 1 dan ${searchResults.length}.`);
    }
    ...
    if (!chapter) {
      ...
      const buttons = conn.ctaButton.setBody(`Detail untuk konten "${selectedResult.title}".`).setFooter("Informasi lebih lanjut dapat ditemukan di AnimeBatch.").addSelection("Klik di sini").makeSections("AnimeBatch", "Pilih episode");
...
- Episode:`;
  ...
  - Links: ${download.links.map(link => `${Object.keys(link)[0]} (${Object.values(link)[0]})`).join(", ")}`;
          buttons.makeRow(``, `📚 *Episode ${idx + 1} (${download.title})*`, `⭐ *Quality:* ${download.quality}`, `${usedPrefix}${command} ${query}|${index}|${idx + 1}`);
        });
      } else {
        replyMessage += `
