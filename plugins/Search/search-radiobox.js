...
async function searchRadiobox(q) {
  try {
    ...
    const stations = $('#stations .stations__station').map((index, element) => {
      const tags = $(element).find('.stations__station__tags a').map((index, tagElement) => $(tagElement).text())
        .get()
      return {
...
async function playlistRadiobox(url) {
  const response = await fetch(url.endsWith('/playlist') ? url : url +
    '/playlist') // Ganti URL dengan URL sumber data Anda
  ...
  $('section.playlist table.tablelist-schedule tbody tr').each((index, element) => {
    ...
    const filteredResultObj = Object.fromEntries(Object.entries(resultObj).filter(([_, value]) => value !==
