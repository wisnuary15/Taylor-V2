  ...
  if (lister.includes(feature)) {
    if ("gc" === feature) {
      ...
      let msge = Object.entries(await conn.chats).filter(([nama]) => !nama.endsWith("s.whatsapp.net")).map(([nama, isi]) => ({
        ...
        messages
      }) => Object.values(messages)).filter(obj => obj.message.extendedTextMessage && obj.message.extendedTextMessage.text.includes(inputs) || obj.message.conversation && obj.message.conversation.includes(inputs));
