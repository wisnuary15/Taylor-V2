...
*Date* : ${v.date}
  ...
  if (command === 'anikyojin') {
    ...
    if (json.isError === true) m.react(eror)
...
handler.help = ['anibatch', 'animeindo', 'drivenime', 'anikyoji', 'animeplanet', 'anisearch', 'anoboy', 'fanfox',
  'gogoanime', 'kiryu', 'kissmanga', 'klikmanga', 'komiku', 'mangadex', 'manganato', 'myanimelist', 'nimegami',
  'samehadaku', 'amino', 'googleit', 'groupwhatsapp', 'layarkaca', 'mangatoon', 'musicfinder'
]
handler.command = ['anibatch', 'animeindo', 'drivenime', 'anikyojin', 'animeplanet', 'anisearch', 'anoboy', 'fanfox',
  'gogoanime', 'kiryu', 'kissmanga', 'klikmanga', 'komiku', 'mangadex', 'manganato', 'myanimelist', 'nimegami',
  'samehadaku', 'amino', 'googleit', 'groupwhatsapp', 'layarkaca', 'mangatoon', 'musicfinder'
]
...
async function searchAnoboy(query) {
  return await fetch('http://anoboy.web.id/search/' + query).then(response => response.text()).then(body => {
...
async function searchKlikmanga(query) {
  ...
  try {
    ...
    $('div.c-tabs-item').each((index, element) => {
      const data = {
        imageUrl: $(element).find(
          'div.row.c-tabs-item__content > div.col-4.col-12.col-md-2 > div.tab-thumb > a > img').attr(
          'data-src') || 'kosong',
        title: $(element).find('div.tab-summary > div.post-title > h3 > a').text() || 'kosong',
        alternateTitles: $(element).find('div.post-content_item.mg_alternative > div.summary-content').text() ||
          'kosong',
        authors: $(element).find('div.post-content_item.mg_author > div.summary-content > a').toArray().map(
          author => $(author).text()) || ['kosong'],
        illustrator: $(element).find('div.post-content_item.mg_artists > div.summary-content > a').text() ||
          'kosong',
        genres: $(element).find('div.post-content_item.mg_genres > div.summary-content > a').toArray().map(
          genre => $(genre).text()) || ['kosong'],
        status: $(element).find('div.post-content_item.mg_status > div.summary-content').text() || 'kosong',
        releaseYear: $(element).find('div.post-content_item.mg_release > div.summary-content.release-year > a')
          .text() || 'kosong',
        latestChapter: $(element).find('div.tab-meta > div.meta-item.latest-chap > span.chapter > a').text() ||
          'kosong',
        url: $(element).find('div.tab-thumb > a').attr('href') ||
        'kosong', // Get the image URL from the href attribute
        releaseUrl: $(element).find('div.meta-item.post-on > span > a').attr('href') ||
          'kosong' // Get the release URL from the href attribute
      };
...
async function searchMangadex(query) {
  ...
  if (res.status !== 200) throw new Error(`Failed to search for manga. Status ${res.status}`);
  const {
    data
  } = await res.json();
  return data.map(({
