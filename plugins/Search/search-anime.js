...
const handler = async (m, { conn: conn, usedPrefix: usedPrefix, text: text, args: args, command: command }) => {
  let who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? conn.user.jid : m.sender,
    name = (await conn.profilePictureUrl(who).catch((_ => hwaifu.getRandom())),
      conn.getName(who));
  ...
  if ("anoboy" === command) {
    let teks = (await searchAnoboy(text)).map(((v, index) => `*[ ${index + 1} ]*\n*Title* : ${v.title}\n*Link* : ${v.url}\n*Thumb* : ${v.thumbnail}\n${v.postedBy}\n${v.episode}\n${v.status}\n   `.trim())).filter((v => v)).join("\n\n________________________\n\n");
    m.reply(teks);
  }
  if ("fanfox" === command) {
  ...
  return await fetch("http://anoboy.web.id/search/" + query).then((response => response.text())).then((body => {
    ...
    return $("div.bsx").each(((index, element) => {
      const $element = $(element),
        title = $element.find('h2[itemprop="headline"] a').text(),
        url = $element.find('h2[itemprop="headline"] a').attr("href"),
        thumbnail = $element.find('img[itemprop="image"]').attr("src"),
        postedBy = $element.find(".inf span:nth-child(2)").text().trim(),
        episode = $element.find(".inf span:nth-child(3)").text().trim(),
        status = $element.find(".inf span:nth-child(4)").text().trim();
      results.push({
        title: title,
        url: url,
        thumbnail: thumbnail,
  ...
  try {
    ...
    return $("div.daftar > div.bge").each(((index, element) => {
      ...
      $(element).find(".kan .new1").each(((idx, el) => {
        ...
        chapters: chapters.length > 0 ? chapters.map((({ title: title, number: number }, index) => `\n${index + 1}.\n${title}\n${number}\n\n`)).join("") : "Tidak diketahui"
      });
    })), results;
  } catch (error) {
    return console.error(error), [];
...
async function searchMangadex(query) {
  ...
  return data.map((({ attributes: attributes }) => attributes));
}
