...
`
        conn.sendFile(m.chat, resl.image, "", cap + chap, m)
      } catch (e) {
        m.react(eror)
      }
    }
    if (feature === "pdf") {
      ...
      try {
        ...
        conn.sendFile(m.chat, buff, 'Nih kak!', '', m, false, {
          asDocument: true
        })
      } catch (e) {
        m.react(eror)
  ...
  try {
    ...
    const shikigamiArray = $('.c-tabs-item__content').map((index, element) => {
      ...
      return {
        ...
        authors: $element.find('.mg_author a').map((index, authorElement) => $(authorElement).text().trim())
        .get(),
        artists: $element.find('.mg_artists a').map((index, artistElement) => $(artistElement).text().trim())
        .get(),
...
async function getDetails(url) {
  try {
    ...
    manga.info = {
      ...
      authors: $('.post-content_item:contains("Author(s)") .summary-content a').map((index, authorElement) => $(
        authorElement).text().trim()).get(),
      artists: $('.post-content_item:contains("Artist(s)") .summary-content a').map((index, artistElement) => $(
        artistElement).text().trim()).get(),
      genres: $('.post-content_item:contains("Genre(s)") .summary-content a').map((index, genreElement) => $(
        genreElement).text().trim()).get(),
