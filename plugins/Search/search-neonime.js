...
const handler = async (m, {
  ...
  try {
    ...
    if (!index && !chapter) {
      ...
      const buttons = conn.ctaButton.setBody(`Pilih anime dari hasil pencarian untuk "${query}".`).setFooter("Informasi lebih lanjut dapat ditemukan di NeoNime.").addSelection("Klik di sini").makeSections("NeoNime", "Pilih anime");
      searchResults.forEach((result, idx) => {
        buttons.makeRow(``, `📺 *[ RESULT ${idx + 1} ]*`, `📰 *Judul:* ${result.title}`, `${usedPrefix}${command} ${query}|${idx + 1}`);
      });
    ...
    if (!chapter) {
      ...
      const buttons = conn.ctaButton.setBody(`Informasi untuk anime "${selectedResult.title}".`).setFooter("Informasi lebih lanjut dapat ditemukan di NeoNime.").addSelection("Klik di sini").makeSections("NeoNime", "Pilih episode");
...
- Link: ${selectedResult.link}`;
      if (infoResult.download.length > 0) {
        infoResult.download.forEach((download, idx) => {
          buttons.makeRow(``, `📚 *Episode ${idx + 1}*`, `⭐ *Quality:* ${download.quality}`, `${usedPrefix}${command} ${query}|${index}|${idx + 1}`);
        });
      } else {
