...
const getProductList = async (provider, country) => {
    try {
...
const handler = async (m, {
    ...
    try {
        ...
        if (!countryList.includes(country)) {
            return m.reply(`🚫 Country *${country.toUpperCase()}* is not available. Available countries are:\n\n${countryList.map(c => `- ${c.toUpperCase()}`).join('\n')}`);
        }
        const productList = await getProductList(provider, country);
        const filteredProducts = Object.keys(productList).filter(key => productList[key].Price <= price);
        if (filteredProducts.length === 0) {
            return m.reply(`🚫 No products found for *${provider.toUpperCase()}* in *${country.toUpperCase()}* under *${price}*.`);
        }
        const reply = `*💡 Products for ${provider.toUpperCase()} in ${country.toUpperCase()} under ${price}:*\n\n${
            filteredProducts.map(key => 
                `- *${key.toUpperCase()}:* ${productList[key].Price}\n- *Category:* ${productList[key].Category}\n- *Qty:* ${productList[key].Qty}`).join('\n\n')
        }`;
        m.reply(reply);
    } catch (error) {
