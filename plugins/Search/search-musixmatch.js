...
const handler = async (m, {
  ...
  const captiond =
    `*乂 Judul 乂*\n${v.judul}\n*乂 Penyanyi 乂*\n${v.penyanyi}\n\n*乂 Lirik 乂*\n${v.lirik}\n\n*乂 Thumb 乂*\n${v.thumb}`
  conn.sendFile(m.chat, v.thumb, "result", captiond, m)
}
...
async function MusixMatch(query) {
  try {
    ...
    const url = ch('#search-all-results').find(
    ...
    $('script').remove();
    const title = $('#site > div > div > div > main > div > div > div.mxm-track-banner.top > div > div > div').find(
      'div.col-sm-10.col-md-8.col-ml-9.col-lg-9.static-position > div.track-title-header > div.mxm-track-title > h1'
      ).text().trim();
    const artist = $(
        '#site > div > div > div > main > div > div > div > div > div > div > div> div > div > h2 > span').text()
      .trim();
    const thumb = $('#site > div > div > div > main > div > div > div.mxm-track-banner.top > div > div > div').find(
      'div.col-sm-1.col-md-2.col-ml-3.col-lg-3.static-position > div > div > div > img').attr('src');
    let lyrics = "";
    $('#site > div > div > div > main > div > div > div.mxm-track-lyrics-container').find(
      'div.container > div > div > div > div.col-sm-10.col-md-8.col-ml-6.col-lg-6 > div.mxm-lyrics').each(function(
      a, b) {
