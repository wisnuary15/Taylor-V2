...
*Jawaban:* ${['Iya keknya', 'Y saja', 'Gtau', 'Gk', 'No', 'Ynkts', '...', 'Tanya yg lain', 'Bisajadi'].getRandom()}
  ...
  if (command === 'xpinterest') {
    ...
    conn.sendFile(m.chat, rant, '', rant, m)
  }
...
*like:* ${tek.like}
`
    conn.sendFile(m.chat, tek.media, '', cap, m)
  }
  ...
  if (command === 'xhentai') {
    ...
    return conn.sendList(m.chat, htki + ' 📺 Hentai Search 🔎 ' + htka,
      `⚡ Silakan pilih Hentai Search di tombol di bawah...\n*Teks yang anda kirim:* ${text}\n\nKetik ulang *${usedPrefix + command}* teks anda untuk mengubah teks lagi`,
      author, `☂️ Hentai Search Disini ☂️`, listSections, m)
  }
}
handler.command = ['xhentai', 'xstyletext', 'xringtone', 'xumma', 'xquotesnime', 'xwikimedia', 'xwallpaper',
  'xpinterest', 'xaiovideodl', 'apakah2', 'turu', 'turnbackhoax'
]
    ...
    axios.get(`https://www.besthdwallpaper.com/search?CurrentPage=${page}&q=${title}`).then(({
      ...
      $('div.grid-item').each(function(a, b) {
        hasil.push({
          ...
          image: [$(b).find('picture > img').attr('data-src') || $(b).find('picture > img').attr(
            'src'), $(b).find('picture > source:nth-child(1)').attr('srcset'), $(b).find(
            'picture > source:nth-child(2)').attr('srcset')]
        })
      })
      resolve(hasil)
    })
  ...
  return new Promise((resolve, reject) => {
    axios.get(url).then((res) => {
      ...
      let hasil = {
        ...
        media: $('#article-content > div > iframe').attr('src') ? [$('#article-content > div > iframe')
          .attr('src')
        ] : image,
