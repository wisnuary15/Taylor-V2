...
const handler = async (m, { conn: conn, text: text, args: args, usedPrefix: usedPrefix, command: command }) => {
  ...
  if ("turu" === command) {
    ...
    "on" === args[0] ? (keban = !0, m.reply("Bot Turu Dulu bang :>!")) : "off" === args[0] && (keban = !1,
...
async function xwallpaper(title, page = "1") {
  return new Promise(((resolve, reject) => {
    axios.get(`https://www.besthdwallpaper.com/search?CurrentPage=${page}&q=${title}`).then((({ data: data }) => {
...
async function xstyletext(teks) {
  return new Promise(((resolve, reject) => {
    axios.get("http://qaz.wtf/u/convert.cgi?text=" + teks).then((({ data: data }) => {
      let $ = cheerio.load(data),
        hasil = [];
