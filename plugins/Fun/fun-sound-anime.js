...
const handler = async (m, { conn: conn, usedPrefix: usedPrefix, text: text, command: command }) => {
...
async function voiceV1() {
  try {
    ...
    }))).get().filter((({ name: name, link: link }) => "" !== name && void 0 !== link));
  } catch (error) {
...
async function ListVoice(url) {
  try {
    ...
    }))).get().filter((({ name: name, link: link, quality: quality }) => "" !== name && void 0 !== link && "" !== quality));
  } catch (error) {
    return console.error("Error fetching and parsing data:", error), [];
  }
}