...
const handler = async (m, {
  ...
  if (!args[0]) {
    ...
    const message = voices.length === 0 ? '❓ Daftar suara kosong.' :
  ...
  if (args.length === 1 && /^\d+$/.test(args[0])) {
    ...
    if (linkIndex >= 0 && linkIndex < voices.length) {
      ...
      const message = listVoices.length === 0 ? '❓ Daftar suara kosong.' :
  ...
  if (args.length === 2 && /^\d+$/.test(args[0]) && /^\d+$/.test(args[1])) {
    ...
    if (linkIndex >= 0 && linkIndex < voices.length) {
      ...
      if (voiceIndex >= 0 && voiceIndex < listVoices.length) {
        return conn.sendFile(m.chat, listVoices[voiceIndex].link, '', m, null, adReply);
      } else {
        return m.reply(
  ...
  if (args.length === 2 && args[0]?.toLowerCase() === 'v2' && /^\d+$/.test(args[1])) {
    ...
    if (voiceIndex >= 0 && voiceIndex < voices.length) {
      return conn.sendFile(m.chat, voices[voiceIndex].link, '', m, null, adReply);
    } else {
      return m.reply(
  ...
  if (args[0]?.toLowerCase() === 'v3' && args[1]) {
    ...
    if (soundLink) {
      return conn.sendFile(m.chat, soundLink, args[1] + '.mp3', '', fakes, null, adReply);
    } else {
      return m.reply('❌ Suara v3 tidak valid. Harap pilih suara yang valid.\nContoh penggunaan: *voice v3 ara*');
    }
  }
  return m.reply(
