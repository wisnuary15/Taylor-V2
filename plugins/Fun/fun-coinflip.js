...
const handler = async (m, { conn: conn, args: args, text: text }) => {
  ...
  try {
    ...
    })), "atas" === terbang ? (player.money += coins, player.exp += exp, db.data.users[m.sender].tiketcoin += 1) : "bawah" === terbang && (player.money -= coins,
      player.exp -= exp, db.data.users[m.sender].tiketcoin -= 1);
  } catch (error) {
