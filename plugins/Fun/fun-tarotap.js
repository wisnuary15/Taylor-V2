  ...
  try {
    ...
    switch (command) {
      case "tarotapdaily": {
        ...
        const dailyResult = await tarotap.daily(name?.trim() || "Anonymous", "en", "", date?.trim() || new Date().toISOString().split("T")[0]);
      ...
      case "tarotapyesno": {
        const yesnoResult = await tarotap.yesno("en", "one-card", inputText.trim());
...
handler.before = async (m, {
  ...
  if (!db.data.dbai.tarotap || m.isBaileys || !(m.sender in db.data.dbai.tarotap)) return;
  const {
    key: {
      id: keyId
    },
  ...
  if (m.quoted?.id === keyId && cmd && m.text.trim()) {
    ...
    try {
      ...
      switch (cmd) {
        ...
        case "tarotapyesno": {
          const yesnoResult = await tarotap.yesno("en", "one-card", m.text.trim());
          answer = `Jawaban: ${yesnoResult.answer}\nPenjelasan: ${yesnoResult.explanation}\nKartu: ${yesnoResult.cards[0].name}\nGambar: https://tarotap.com${yesnoResult.cards[0].imageUrl}\nTerbalik: ${yesnoResult.cards[0].isReversed ? "Ya" : "Tidak"}`;
          break;
        }
      }
