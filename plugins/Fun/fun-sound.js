...
const toM = a => "@" + a.split("@")[0],
  ...
  isNumber = x => !isNaN(x),
  handler = async (m, { conn: conn, usedPrefix: usedPrefix, text: text, command: command }) => {
    ...
    if (!text) {
      ...
      throw "mangkane" === command && (exampleUsage = `🎵 Contoh Penggunaan:\n${usedPrefix + command} 1`),
        "ringtone" === command && (exampleUsage = `🎵 Contoh Penggunaan:\n${usedPrefix + command} black cover`),
        exampleUsage;
    }
    try {
      let vn;
        ...
        const response = await fetch(API("btchx", "/api/search/ringtone", {
          ...
          formattedData = {
            ...
            rows: [{
              ...
              rows: Object.values(result.result).map(((v, index) => ({
                title: v.title,
