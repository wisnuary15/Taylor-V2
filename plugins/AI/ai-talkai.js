  ...
  try {
    ...
    if (!urutan || isNaN(urutan) || urutan > data.length) {
      return m.reply("Input query!\n*Example:*\n.talkai [nomor]|[query]\n\n*Pilih angka yang ada*\n" + data.map((
        item, index) => `*${index + 1}.* ${item.title}`).join("\n"));
    }
    ...
    if (openAIResponse) {
      if (out === "image") {
        ...
        const result = openAIResponse;
        const anu = (result.split('\n').filter(line => line.trim().startsWith('data: ')).map(line => line.replace(
          /data: |\n/g, '')).join('').replace(/\\n/g, '\n')) || '';
        await conn.sendMessage(m.chat, {
          text: anu
        }, {
      ...
      m.reply("Tidak ada respons dari OpenAI atau terjadi kesalahan.");
    }
  } catch (e) {
    console.error('Terjadi kesalahan:', e);
    m.reply("Terjadi kesalahan dalam melakukan permintaan.");
  }
};
...
function userAgent() {
  const androidVersions = ['4.0.3', '4.1.1', '4.2.2', '4.3', '4.4', '5.0.2', '5.1', '6.0', '7.0', '8.0', '9.0', '10.0',
    '11.0'
  ];
