...
const handler = async (m, {
  ...
  if ("gizaicreate" === command) {
    ...
    if (text) {
      ...
      return m.reply(`Karakter diatur menjadi: *${db.data.dbai.gizai[m.chat].name}*`);
    }
  }
  if ("gizaiset" === command) {
  ...
  if ("gizai" === command) {
    ...
    m.react(wait);
    try {
      const summary = await chatAI(db.data.dbai.gizai[m.chat].profile.join(" "), "Perpendek kalimat yang ada dan perbaiki kata katanya agar lebih simpel dan pendek."),
        output = await chatAI(text, summary);
      output ? m.reply(`*${db.data.dbai.gizai[m.chat].name}*\n\n${output}`) : m.reply("Tidak ada output yang dihasilkan.");
    } catch (error) {
