...
const handler = async (m, {
  ...
  try {
    const database = db.data.database.sdxlimg2img;
    const q = m.quoted || m;
...
async function transformSDXL(params) {
  try {
...
async function generateImage(params) {
  try {
    const generate = await tryModule(prodiaClient.transformSDXL, transformSDXL)(params);
