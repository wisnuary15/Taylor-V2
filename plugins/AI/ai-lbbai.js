...
const handler = async (m, {
  ...
  db.data.dbai.lbbai = db.data.dbai.lbbai || {};
  ...
  if ("lbbaicreate" === command) {
    ...
    if (text) return db.data.dbai.lbbai[m.chat] = {
      ...db.data.dbai.lbbai[m.chat],
      name: m.name,
      profile: [...db.data.dbai.lbbai[m.chat].profile || [], text]
    }, m.reply(`Karakter diatur menjadi: *${db.data.dbai.lbbai[m.chat].name}*`);
  }
  if ("lbbaiset" === command) {
    ...
    if (selectedCharacter) return db.data.dbai.lbbai[m.chat] = {
      ...db.data.dbai.lbbai[m.chat],
  ...
  if (!db.data.dbai.lbbai[m.chat].name && !db.data.dbai.lbbai[m.chat].profile) return m.reply(`Atur karakter sebelum menggunakan.\nGunakan command *${usedPrefix}lbbaiset* untuk mengatur karakter.\nKarakter yang tersedia:\n${categoryNames.map((v, i) => `*${i + 1}.* ${v}`).join("\n")}`);
  if ("lbbai" === command) {
    const text = args.length >= 1 ? args.slice(0).join(" ") : m.quoted && m.quoted?.text || m.quoted?.caption || m.quoted?.description || null;
    if (!text) return m.reply(`Masukkan teks atau reply pesan dengan teks yang ingin diolah.\nContoh penggunaan:\n*${usedPrefix}${command} Hai, apa kabar?*`);
    m.react(wait);
    try {
      const summary = await chatAI(db.data.dbai.lbbai[m.chat].profile.join(" "), "Perpendek kalimat yang ada dan perbaiki kata katanya agar lebih simpel dan pendek."),
        output = await chatAI(text, summary);
      output ? m.reply(`*${db.data.dbai.lbbai[m.chat].name}*\n\n${output}`) : m.reply("Tidak ada output yang dihasilkan.");
    } catch (error) {
