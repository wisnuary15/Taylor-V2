...
const handler = async (m, {
  ...
  if ("amigochatcreate" === command) {
    ...
    if (text) {
      ...
      return m.reply(`Karakter custom oleh: *${db.data.dbai.amigochat[m.sender].name}*`);
    }
  }
  ...
  if ("amigochatset" === command) {
    ...
    if (selectedCharacter) {
      ...
      return m.reply(`Karakter diatur menjadi: *${db.data.dbai.amigochat[m.sender].name}*`);
    }
  ...
  if ("amigochat" === command) {
    const text = args.length >= 1 ? args.slice(0).join(" ") : m.quoted && m.quoted?.text || m.quoted?.caption || m.quoted?.description || null;
    if (!text) return m.reply(`Masukkan teks atau reply pesan dengan teks yang ingin diolah.\nContoh penggunaan:\n*${usedPrefix}${command} Hai, apa kabar?*`);
    if (!db.data.dbai.amigochat[m.sender]?.name && !db.data.dbai.amigochat[m.sender]?.profile) {
    ...
    try {
      ...
      if (!summary) return m.reply(`Atur karakter sebelum menggunakan.\nGunakan command *${usedPrefix}amigochatset* untuk mengatur karakter.\nKarakter yang tersedia:\n${categoryNames.map((v, i) => `*${i + 1}.* ${v}`).join("\n")}`);
