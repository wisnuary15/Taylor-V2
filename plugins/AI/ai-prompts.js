...
const handler = async (m, {
  ...
  try {
    if (command === "promptgen") {
      ...
      const concept = text || 'best';
      const data = await getArtList(concept);
      if (!data || !data.candidates || !data.candidates[0] || !data.candidates[0]?.content || !data.candidates[0]
        ?.content.parts || !data.candidates[0]?.content.parts[0]) {
        throw new Error("Invalid response format");
      }
      const artListText = JSON.parse(data.candidates[0]?.content.parts[0]?.text);
      const sortedArtListMessage = ["*Art List*", ...artListText.sort((a, b) => a.prompt.localeCompare(b.prompt))
        .map((artObject, index) => ({
          index: index + 1,
      ...
      conn.reply(m.chat, sortedArtListMessage, m);
    }
  } catch (error) {
...
handler.help = ["prompts", "promptgen"];
handler.tags = ["ai"];
handler.command = /^(prompts|promptgen)$/i;
