...
const handler = async (m, {
  ...
  if (command === "tuneaiset") {
    ...
    if (selectedCharacter) {
      ...
      return m.reply(`Karakter diatur menjadi: *${db.data.dbai.tuneai[m.sender].name}*`);
    } else {
  ...
  if (command === "tuneaimodel") {
    ...
    if (!selectedModel) {
      const modelList = allowedModels.map((v, i) => `*${i + 1}.* ${v}`).join("\n");
      return m.reply(`Nomor model tidak valid. Pilih nomor antara 1 dan ${allowedModels.length}.\nModel yang tersedia:\n${modelList}`);
    }
    if (!db.data.dbai.tuneai[m.sender]) {
      return m.reply("Setel karakter terlebih dahulu dengan command *tuneaiset*.");
    }
    db.data.dbai.tuneai[m.sender].model = selectedModel;
    return m.reply(`Model diatur menjadi: *${selectedModel}*`);
  }
  if (!db.data.dbai.tuneai[m.sender]?.name || !db.data.dbai.tuneai[m.sender]?.profile) {
  ...
  if (command === "tuneai") {
    ...
    try {
      ...
      const output = await TuneAi(text, db.data.dbai.tuneai[m.sender].profile, model);
      if (output) {
        const response = await conn.reply(m.chat, `*${db.data.dbai.tuneai[m.sender].name}*\n\n${output}`, m);
...
handler.before = async (m, {
  ...
  if (!db.data.dbai.tuneai || m.isBaileys || !(m.sender in db.data.dbai.tuneai)) return;
  const {
    name,
    profile,
    lastMessageId,
  ...
  if (lastMessageId && m.quoted?.id === lastMessageId && m.text.trim()) {
    m.react("wait");
    try {
      const output = await TuneAi(m.text.trim(), profile, model || "tune-gpt-4o-mini");
      if (output) {
