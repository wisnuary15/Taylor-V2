...
const handler = async (m, {
  ...
  m.react("wait");
  try {
    const tokenData = await prefindAI(`${baseURL}/auth/device-token/create`, "POST");
...
handler.before = async (m, {
  ...
  if (!db.data.dbai.prefindchat || m.isBaileys || !(m.sender in db.data.dbai.prefindchat)) return;
  const {
    key: {
      id: keyId
    }
