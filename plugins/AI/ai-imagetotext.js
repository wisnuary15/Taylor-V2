...
async function ImageToText(link, prompt = "") {
  const url = "https://image-to-text.ai/?ref=taaft&utm_source=taaft&utm_medium=referral";
  const headers = {
...
const handler = async (m, {
  ...
  const inputText = args.length ? args.join(" ") : q?.text || q?.caption || q?.description || null;
...
handler.help = ["imagetotext *[Reply image]*"], handler.tags = ["ai"], handler.command = /^(imagetotext)$/i;
