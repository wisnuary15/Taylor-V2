...
const handler = async (m, {
  ...
  try {
    const database = db.data.database.img2img;
    const q = m.quoted || m;
...
async function transform(params) {
  try {
    const url = "https://api.prodia.com/v1/sd/transform";
    const options = {
      method: "POST",
      headers: {
...
async function generateImage(params) {
  try {
    const generate = await tryModule(prodiaClient.transform, transform)(params);
