...
async function swapImage(targetBase64, targetUrl) {
  ...
  if (!targetBase64 || !sourceBase64) throw new Error("Failed to convert images");
...
const handler = async (m, {
  ...
  try {
    ...
    const inputUrl = args.length ? args.join(" ") : m.quoted?.text || m.quoted?.caption || m.quoted?.description || null;
    if (!inputUrl) return m.reply(`Masukkan teks atau reply pesan dengan teks yang ingin diolah.`);
    m.react(wait);
