  ...
  switch (command) {
    case "roleplaisearch":
      ...
      try {
        ...
        const buttons = conn.ctaButton.setBody(`Pilih bot dari hasil pencarian untuk "${inputText}".`).setFooter("Informasi lebih lanjut dapat ditemukan di Roleplai.").addSelection("Klik di sini").makeSections("Roleplai", "Pilih bot");
        searchResults.forEach((result, idx) => {
          buttons.makeRow(``, `${result.name}`, `${result.description}`, `${usedPrefix}roleplaiset ${result.bid}`);
        });
...
handler.before = async (m, {
  ...
  db.data.dbai.roleplai = db.data.dbai.roleplai || {};
  if (!db.data.dbai.roleplai || m.isBaileys || !(m.sender in db.data.dbai.roleplai)) return;
  const senderId = m.sender;
  db.data.dbai.roleplai[senderId] = db.data.dbai.roleplai[senderId] || {
    bid: null,
    key: {
