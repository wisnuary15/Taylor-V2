...
const handler = async (m, {
  ...
  try {
    const database = db.data.database.facerestore;
    const q = m.quoted || m;
    ...
    if (!database[m.chat].inputData) {
      database[m.chat].inputData = await tryModule(getModels);
    }
    const {
      model = [],
        style = [],
...
async function getModels() {
  try {
    ...
    return {
      model: [...data[0]?.["enum"], ...await prodiaClient.getModels() || []],
      style: data[10]?.["enum"],
      sampler: [...data[8]?.["enum"], ...await prodiaClient.getSamplers() || []]
    };
  } catch (error) {
