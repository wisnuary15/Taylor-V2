...
const handler = async (m, {
  ...
  try {
    ...
    } = command === "regemflux" ? await conn.sendMessage(m.chat, {
      image: {
        url: answer || thumb
      },
...
handler.before = async (m, {
  ...
  if (!db.data.dbai.regem || m.isBaileys || !(m.sender in db.data.dbai.regem)) return;
  const {
    key: {
      id: keyId
    },
  ...
  if (m.quoted?.id === keyId && cmd && m.text.trim()) {
    ...
    try {
      ...
      } = cmd === "regemflux" ? await conn.sendMessage(m.chat, {
        image: {
          url: answer || thumb
        },
        caption: `✨ *\`AI Image Generated\`* ✨\n\n📝 *Prompt:* ${m.text.trim()}`,
