...
const handler = async (m, {
  ...
  try {
    const database = db.data.database.sdxl;
    const q = m.quoted || m;
...
async function generateImageSDXL(params) {
  try {
    const url = "https://api.prodia.com/v1/sdxl/generate";
    const options = {
      method: "POST",
      headers: {
...
async function generateImage(params) {
  try {
    const generate = await tryModule(prodiaClient.generateImageSDXL, generateImageSDXL)(params);
