...
const handler = async (m, {
  ...
  try {
    const database = db.data.database.sdxlinpainting;
    const q = m.quoted || m;
...
async function inpaintingSDXL(params) {
  try {
    const url = "https://api.prodia.com/v1/sdxl/inpainting";
    const options = {
      method: "POST",
      headers: {
...
async function generateImage(params) {
  try {
    const generate = await tryModule(prodiaClient.inpaintingSDXL, inpaintingSDXL)(params);
