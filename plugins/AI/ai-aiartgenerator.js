...
const handler = async (m, {
  ...
  const model = inputText.match(/--model=(\w+)/) ? inputText.match(/--model=(\w+)/)[1] : "sdxl-lightning";
  const style = inputText.match(/--style=(\w+)/) ? inputText.match(/--style=(\w+)/)[1] : "3D Model";
  const prompt = inputText.replace(/--model=\w+/, "").replace(/--style=\w+/, "").trim();
  m.react(wait);
  try {
...
handler.before = async (m, {
  ...
  if (!db.data.dbai.aiartgenerator || m.isBaileys || !(m.sender in db.data.dbai.aiartgenerator)) return;
  const {
    key: {
      id: keyId
    }
  ...
  if (m.quoted?.id === keyId && m.text.trim()) {
    m.react(wait);
    try {
      const model = m.text.match(/--model=(\w+)/) ? m.text.match(/--model=(\w+)/)[1] : "sdxl-lightning";
      const style = m.text.match(/--style=(\w+)/) ? m.text.match(/--style=(\w+)/)[1] : "3D Model";
      const prompt = m.text.replace(/--model=\w+/, "").replace(/--style=\w+/, "").trim();
      const images = await AiArtGenerator(prompt, style, model);
      const tag = `@${m.sender.split("@")[0]}`;
      for (const imageBase64 of images) {
        const base64Data = imageBase64.split(",")[1];
