    ...
    switch (command) {
      case "nekomori":
        ...
        if (assistantId) {
          ...
          m.reply("No persona ID set. Use .nekomoriset or .nekomorisearch command to set persona ID. ❗");
        }
        break;
      case "nekomorisearch":
        const searchResults = await nekomori.search(text);
        if (searchResults.length) {
          const buttons = conn.ctaButton.setBody("Select Character Below.").setFooter("More details can be found at nekomori.").addSelection("Click Here", `${usedPrefix}nekomorisearch ${text}`).makeSections("nekomori", "Select Character");
          searchResults.forEach((result, index) => {
            buttons.makeRow(``, `${result.title}`, `${result.description}`, `${usedPrefix}nekomoriset ${result.link}`);
          });
      ...
      case "nekomoriset":
        if (text) {
          ...
          m.reply(`Please provide a persona ID to set. Example: ${usedPrefix}nekomoriset your_persona_id`);
        }
    }
  } catch (error) {
...
handler.before = async (m, {
  ...
  if (!db.data.dbai.nekomori || m.isBaileys || !(m.chat in db.data.dbai.nekomori)) return;
  const {
    keyId,
    assistantId,
    cmd
