...
const RandomAi = async (cmd, inputText) => {
  const params = new URLSearchParams(cmd === "farid" ? {
...
const handler = async (m, {
  ...
  if (!conn.randomai) conn.randomai = {};
...
handler.before = async (m, {
  ...
  if (!conn.randomai || m.isBaileys || !(m.sender in conn.randomai)) return;
  const {
    key: {
      id: keyId
    },
  ...
  if (m.quoted?.id === keyId && cmd && m.text.trim()) {
    ...
    try {
      ...
      conn.randomai[m.sender].key.id = newKeyId;
      m.react(sukses);
    } catch (error) {
