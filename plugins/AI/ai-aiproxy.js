...
const handler = async (m, {
  ...
  if (command === "aiproxyset") {
    ...
    if (selectedCharacter) {
      ...
      return m.reply(`Karakter diatur menjadi: *${db.data.dbai.aiproxy[m.sender].name}*`);
    } else {
  ...
  if (!db.data.dbai.aiproxy[m.sender]?.name || !db.data.dbai.aiproxy[m.sender]?.profile) {
  ...
  if (command === "aiproxy") {
    ...
    try {
      ...
      if (output) {
        const response = await conn.reply(m.chat, `*${db.data.dbai.aiproxy[m.sender].name}*\n\n${output}`, m);
...
handler.before = async (m, {
  ...
  if (!db.data.dbai.aiproxy || m.isBaileys || !(m.sender in db.data.dbai.aiproxy)) return;
  const {
    name,
    profile,
    lastMessageId
