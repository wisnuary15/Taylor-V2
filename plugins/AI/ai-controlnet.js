...
const handler = async (m, { command: command, usedPrefix: usedPrefix, conn: conn, text: text, args: args }) => {
  const samplr = await getModels(),
    input_data = samplr[6].enum,
    input_dataa = samplr[7].enum;
...
handler.help = ["controlnet *[nomor]|[query]*"], handler.tags = ["ai"], handler.command = /^(controlnet)$/i;
