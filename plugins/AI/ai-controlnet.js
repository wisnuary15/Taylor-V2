...
const handler = async (m, {
  ...
  try {
    const database = db.data.database.controlnet;
    const q = m.quoted || m;
    ...
    const moduleIndex = parseInt(trimmedText.match(/--module\s+(\d+)/)?.[1] ?? 0, 10) - 1;
    ...
    if (moduleIndex >= 0 && module[moduleIndex]) {
      database[m.chat].module = module[moduleIndex];
      caption += `✅ *Module berhasil diatur*\n- Module: ${module[moduleIndex].replace(/[_-]/g, " ").replace(/\..*/, "")}\n`;
    } else if (moduleIndex >= 0 && module.length >= 0) {
      caption += `❌ *Module tidak valid!*\nBerikut daftar module yang tersedia:\n${module.map((item, index) => `*${index + 1}.* ${item}`).join("\n")}`;
    }
    ...
    const moduleToUse = database[m.chat].module || _.sample(module);
    ...
    const generateImageParams = {
      ...
      model: modelToUse,
      controlnet_model: modelToUse,
      controlnet_module: moduleToUse,
      prompt: encodeURIComponent(trimmedText),
    ...
    if (!prompt || !modelToUse || !moduleToUse || !styleToUse || !samplerToUse) {
      return m.reply(infoMsg);
    }
    m.react(wait);
...
async function controlNet(params) {
  try {
    const url = "https://api.prodia.com/v1/sd/controlnet";
    const options = {
      method: "POST",
      headers: {
...
async function generateImage(params) {
  try {
    const generate = await tryModule(prodiaClient.controlNet, controlNet)(params);
