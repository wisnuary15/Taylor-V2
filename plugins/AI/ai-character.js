...
*• Example search Chara:* ${usedPrefix + command} search *[characters name]*`);
  ...
  if (keyword === "search") {
    ...
    let karakter = search.result.characters.map(
      (a, index) => `*[ ${index + 1}. ${a.participant__name} ]*
*• Greeting:* \`${a.greeting}\`
*• Visibility:* ${a.visibility}
*• Creator:* ${a.user__username}`, ).join("\n\n");
    const reply = conn.reply(m.chat, karakter, m, {
      contextInfo: {
        mentionedJid: [],
        groupMentions: [],
        externalAdReply: {
          ...
          thumbnailUrl: "https://characterai.io/i/200/static/avatars/" + search.result.characters[0]
            ?.avatar_file_name,
          sourceUrl: "",
          mediaType: 1,
    ...
    conn.reply(m.chat, `*[ KETIK ANGKA 1 SAMPAI ${search.result.characters.length} ]*
...
*• Description:* ${info.result.character.description}`;
    const q = conn.reply(m.chat, caption, m, {
      contextInfo: {
        mentionedJid: [],
        groupMentions: [],
        externalAdReply: {
...
handler.before = async (m, {
  ...
  const chat = await fetchData(
    `https://onesytex.my.id/api/beta-character-ai?text=${m.text}&external_id=${conn.characterai[m.sender].id}`);
  conn.reply(m.chat, chat.result.reply, m, {
    contextInfo: {
      mentionedJid: [],
      groupMentions: [],
      externalAdReply: {
