...
const fetchData = async url => {
...
}, handler = async ({ conn: conn, args: args, usedPrefix: usedPrefix, command: command }) => {
  ...
  if ("search" === keyword) {
    ...
    let karakter = search.result.characters.map(((a, index) => `*[ ${index + 1}. ${a.participant__name} ]*\n*• Greeting:* \`${a.greeting}\`\n*• Visibility:* ${a.visibility}\n*• Creator:* ${a.user__username}`)).join("\n\n");
    ...
    if (!conn.characterai[m.sender]) throw "*[ KAMU BELUM MENCARI CHARACTER ]*\n> _ketik *.characterai search* untuk mencari characters favorit mu_";
    conn.characterai[m.sender].isChats = !0, m.reply("_*[ ✓ ] Room chat berhasil dibuat*_");
  } else if ("off" === keyword) {
    if (!conn.characterai[m.sender]) throw "*[ KAMU BELUM MENCARI CHARACTER ]*\n> _ketik *.characterai search* untuk mencari characters favorit mu_";
    conn.characterai[m.sender].isChats = !1, m.reply("_*[ ✓ ] Berhasil keluar dari Room chat*_");
  }
};
handler.before = async (m, { conn: conn, usedPrefix: usedPrefix }) => {
  if (conn.characterai = conn.characterai ? conn.characterai : {}, !m.text) return;
