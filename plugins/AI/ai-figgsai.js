  ...
  try {
    switch (command) {
      ...
      case "figgsaiset": {
        ...
        if (isNaN(index)) {
          m.reply(`⚠️ *Please provide an index to set the character ID.*\nExample: ${usedPrefix}figgsaiset 1`);
          return;
        }
        const combinedFeed = [...existingFeed, ...feedData.filter(bot => !existingFeed.some(existingBot => existingBot.id === bot.id))];
        ...
        if (index >= 1 && index <= combinedFeed.length) {
          ...
          const buttons = conn.ctaButton.setBody("🌟 *Select Character Below:*").setFooter("More details can be found at figgsai.").addSelection("Click Here", `${usedPrefix}figgsaiset`).makeSections("figgsai", "Select Character");
          combinedFeed.forEach((bot, index) => {
            buttons.makeRow(``, `${bot.name}`, `Character ID: ${bot.id}`, `${usedPrefix}figgsaiset ${index + 1}`);
          });
...
handler.before = async (m, {
  ...
  db.data.dbai.figgsai = db.data.dbai.figgsai || {};
  if (!db.data.dbai.figgsai || m.isBaileys || !(m.sender in db.data.dbai.figgsai)) return;
  const userData = db.data.dbai.figgsai[m.sender];
  const {
    botId,
    chatId,
  ...
  if (keyId && m.quoted?.id === keyId) {
    if (botId && chatId && firstMessage && name && description && m.text.trim()) {
      m.react(wait);
      try {
