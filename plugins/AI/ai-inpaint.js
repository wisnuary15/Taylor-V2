...
const handler = async (m, {
  ...
  try {
    const database = db.data.database.inpainting;
    const q = m.quoted || m;
...
async function inpainting(params) {
  try {
    const url = "https://api.prodia.com/v1/sd/inpainting";
    const options = {
      method: "POST",
      headers: {
...
async function generateImage(params) {
  try {
    const generate = await tryModule(prodiaClient.inpainting, inpainting)(params);
