...
const handler = async (m, {
  ...
  if (db.data.dbai.burnhair = db.data.dbai.burnhair || {
      model: "gpt-3.5-turbo"
    }, !text) return m.reply(`Masukkan teks atau balas pesan dengan teks yang ingin diolah.\nContoh penggunaan:\n*${usedPrefix}${command} Hai, apa kabar?*`);
  try {
    switch (command) {
      case "aiburnhair":
        const aiResponse = await AIBurnHair(db.data.dbai.burnhair.model, [{
          role: "system",
      ...
      case "modelburnhair":
        ...
        return db.data.dbai.burnhair.model = modelData.data[index].id, m.reply(`Sukses set model: ${modelData.data[index].id}`);
      case "ttsburnhair":
