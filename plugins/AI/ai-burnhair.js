  ...
  try {
    switch (command) {
      ...
      case "modelburnhair":
        ...
        const modelData = await ModelBurnHair();
        if (index < 0 || index >= modelData.data.length) {
          const categoryList = modelData.data.map((v, i) => `*${i + 1}.* ${v.id}`).join('\n');
          return m.reply(
            `Nomor kategori tidak valid. Pilih nomor antara 1 dan ${modelData.data.length}.\nKategori yang tersedia:\n${categoryList}`
            );
        }
        conn.burnhair.model = modelData.data[index].id;
        return m.reply(`Sukses set model: ${modelData.data[index].id}`);
      case "ttsburnhair":
        const ttsData = await TtsBurnHair(text);
        return conn.sendFile(m.chat, ttsData, '', '', m, true);
      case "dalleburnhair":
        const dalleData = await DalleBurnHair(text);
        return conn.sendFile(m.chat, dalleData.data[0]?.url, '', dalleData.data[0]?.revised_prompt, m);
    }
  } catch (e) {
...
handler.tags = ["ai"];
handler.command = /^(aiburnhair|modelburnhair|ttsburnhair|dalleburnhair)$/i;
