...
const handler = async (m, {
  ...
  db.data.dbai.hcrIds = db.data.dbai.hcrIds || {};
  ...
  try {
    let message = "";
    if (command === "hcr") {
      const externalId = db.data.dbai.hcrIds[m.chat];
      if (externalId) {
      ...
      message = index >= 0 && index < hcrList.length ? (db.data.dbai.hcrIds[m.chat] = hcrList[index], `External ID set successfully to ${hcrList[index]}! ✅`) : `Invalid index. Please select a valid index from the list:\n${hcrList.map((hcr, idx) => `${idx + 1}. ${hcr}`).join("\n")}`;
    } else {
