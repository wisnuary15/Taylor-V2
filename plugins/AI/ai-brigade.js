...
const handler = async (m, {
  ...
  db.data.dbai.brigadeIds = db.data.dbai.brigadeIds || {};
  ...
  try {
    let message = "";
    if ("brigade" === command) message = db.data.dbai.brigadeIds[m.chat] ? (await apiClient.ReqChat(db.data.dbai.brigadeIds[m.chat], text)).response ?? "No reply from AI." : "No external ID set. Use .brigadeset command to set external ID. ❗";
      ...
      if (text) {
        ...
        index >= 0 && index < brigadeList.length ? (db.data.dbai.brigadeIds[m.chat] = brigadeList[index], message = "External ID set successfully! ✅") : message = `Invalid index. Please select a valid index from the list:\n${brigadeOptions}`;
