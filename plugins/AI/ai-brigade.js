...
const handler = async (m, {
  ...
  try {
    let message = '';
    if (command === 'brigade') {
      message = conn.brigadeIds[m.chat] ? ((await apiClient.ReqChat(conn.brigadeIds[m.chat], text)).response ??
        'No reply from AI.') : 'No external ID set. Use .brigadeset command to set external ID. ❗';
