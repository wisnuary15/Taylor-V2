  ...
  try {
    ...
    if ("brigade" === command) message = conn.brigadeIds[m.chat] ? (await apiClient.ReqChat(conn.brigadeIds[m.chat], text)).response ?? "No reply from AI." : "No external ID set. Use .brigadeset command to set external ID. ❗";
      ...
      if (text) {
        ...
        index >= 0 && index < brigadeList.length ? (conn.brigadeIds[m.chat] = brigadeList[index],
          message = "External ID set successfully! ✅") : message = `Invalid index. Please select a valid index from the list:\n${brigadeOptions}`;
