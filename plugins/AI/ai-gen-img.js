  ...
  try {
    ...
    switch (command) {
      ...
      case "txt2img4":
        ...
        return;
        case "txt2img5":
        await conn.sendMessage(m.chat, { image: { url: (await stabilityai(text))[0]?.url }, caption: `Image for ${text}` }, {
          quoted: m
        });
        return;
      default:
    ...
    if (data) {
      const imageUrl = command === "svgai" ? (data.data[0]?.generated_png_s3_url || data.data[0]?.png_s3_url) :
                       command === "arthub" ? data.generations[0]?.img :
                       command === "limewire" ? data.data[0]?.asset_url :
                       null;
...
async function stabilityai(prompt) {
    ...
    try {
        ...
        if (joinData.event_id) {
            ...
            return processStartsData?.success ? processStartsData.output.data : 
                   processStartsData2?.success ? processStartsData2.output.data : 
                   null;
        }
    } catch (e) {
