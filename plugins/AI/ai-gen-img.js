  ...
  try {
    ...
    switch (command) {
      ...
      case `genimg${versionCommand}`:
        await conn.sendFile(m.chat, `https://api-xovvip.vercel.app/text2img?text=${encodeURIComponent(text)}`, "", `Image for ${text}`, m, false, {
          mentions: [m.sender]
        });
        return;
      case "txt2avatar":
...
handler.tags = ["ai"];
handler.command = /^(svgai|arthub|limewire|genimg[1-9]|txt2avatar|halfillust)$/i;
...
async function stabilityai(prompt) {
  ...
  try {
    const {
      event_id
    } = await (await fetch(`${BASE_URL}queue/join?__theme=light`, {
      method: "POST",
      headers: {
    ...
    if (event_id) {
      const inputString = await (await fetch(`${BASE_URL}queue/data?session_hash=${session_hash}`)).text();
      const urls = inputString.split("\n").filter(line => line.startsWith("data:")).map(line => JSON.parse(line.slice(5).trim())).slice(-2).map(entry => entry?.output?.data?.[0]?.url);
      return urls[0];
    }
    return null;
  } catch (e) {
