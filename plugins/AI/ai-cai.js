...
const handler = async (m, {
  ...
  try {
    ...
    if (command === 'caiinfo') {
      ...
      message = conn.externalIds[m.chat] ? ((await apiClient.sendMessage(conn.externalIds[m.chat], text)).result
          .replies[0]?.text ?? 'No reply from AI.') :
        'No external ID set. Use .caiset command to set external ID. ❗';
...
function formatSearchResults(characters) {
  return characters.slice(0, 10).map((char, index) =>
