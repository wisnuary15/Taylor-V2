  ...
  try {
    ...
    switch (command) {
      ...
      case "caistats":
        ...
        break;
      case "cai":
        const externalId = db.data.dbai.externalIds[m.chat]?.externalId;
        if (externalId) {
          const response = await apiClient.sendMessage(externalId, text);
          message = response.result.replies[0]?.text || "No reply from AI.";
        } else {
          message = "No external ID set. Use .caiset or .caisearch command to set external ID. ❗";
        }
        break;
      case "caisearch":
        const searchResults = (await apiClient.searchCharacters(text)).result.characters;
        if (searchResults.length) {
          message = "Pilih nomor untuk set external ID atau lakukan tindakan lainnya.";
          buttons = conn.ctaButton.setBody("Pilih Karakter di bawah ini.").setFooter("Keterangan lebih lanjut dapat ditemukan di Character Ai.").addSelection("Klik di sini", `${usedPrefix}caisearch ${text}`).makeSections("Character Ai", "Pilih Karakter");
          searchResults.forEach((result, index) => {
            buttons.makeRow(``, `${result.participant__name}`, `${result.greeting}`, `${usedPrefix}caiset ${result.external_id}`);
          });
        } else {
      ...
      case "caiset":
        if (text) {
          db.data.dbai.externalIds[m.chat] = {
    ...
    if (command === "cai") {
      ...
      db.data.dbai.externalIds[m.chat] = {
        externalId: db.data.dbai.externalIds[m.chat]?.externalId,
        keyId: keyId
      };
      m.react(sukses);
    } else {
...
handler.before = async (m, {
  ...
  if (!db.data.dbai.externalIds || m.isBaileys || !(m.chat in db.data.dbai.externalIds)) return;
  const {
    keyId,
    externalId
  } = db.data.dbai.externalIds[m.chat];
  if (m.quoted?.id === keyId && externalId && m.text.trim()) {
    ...
    try {
      ...
      db.data.dbai.externalIds[m.chat].keyId = newKeyId;
      m.react(sukses);
    } catch (error) {
