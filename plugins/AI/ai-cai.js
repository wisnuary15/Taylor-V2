  ...
  try {
    ...
    switch (command) {
      ...
      case "caistats":
        ...
        break;
      case "cai":
        const externalId = conn.externalIds[m.chat]?.externalId;
        if (externalId) {
    ...
    if (command === "cai") {
      ...
      conn.externalIds[m.chat] = {
        externalId: conn.externalIds[m.chat]?.externalId,
        keyId: keyId
      };
      m.react(sukses);
    } else {
...
handler.before = async (m, {
  ...
  if (!conn.externalIds || m.isBaileys || !(m.chat in conn.externalIds)) return;
  const {
    keyId,
    externalId
  } = conn.externalIds[m.chat];
  if (m.quoted?.id === keyId && externalId && m.text.trim()) {
    ...
    try {
      ...
      conn.externalIds[m.chat].keyId = newKeyId;
      m.react(sukses);
    } catch (error) {
