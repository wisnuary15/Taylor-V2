...
const handler = async (m, {
  ...
  m.react(wait);
  const [prompt, style, model] = text.split("|").filter(Boolean).map(s => s.trim());
  const endpoint = (() => {
    switch (command) {
      case "jadighibli":
...
handler.help = ["jadighibli (Balas foto)", "jadivaporwave (Balas foto)", "jadidreamscape (Balas foto)", "jadivintagecomic (Balas foto)", "jadicyberpunk (Balas foto)", "jadigta (Balas foto)", "aitransform (Balas foto)"].map(v => v),
  handler.tags = ["tools"], handler.command = /^(jadighibli|jadivaporwave|jadidreamscape|jadivintagecomic|jadicyberpunk|jadigta|aitransform)$/i,
  handler.limit = !0;
