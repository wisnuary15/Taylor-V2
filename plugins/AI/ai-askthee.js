...
const handler = async (m, {
  ...
  const inputText = args.length ? args.join(" ").trim() : m.quoted?.text || m.quoted?.caption || m.quoted?.description || null;
  const textToProcess = modelIndex !== null ? inputText.replace(/--model=\d+$/, "").trim() : inputText;
  if (!textToProcess) {
...
handler.before = async (m, {
  ...
  if (!db.data.dbai.askthee || m.isBaileys || !(m.sender in db.data.dbai.askthee)) return;
  const {
    key: {
      id: keyId
    }
  ...
  const modelIndex = modelIndexMatch ? parseInt(modelIndexMatch[1], 10) : null;
  const textToProcess = modelIndex !== null ? m.text.replace(/--model=\d+$/, "").trim() : m.text;
