...
length: 4
  ...
  try {
    const {
      data
    } = await axios.post("https://app.illuminarty.ai/api/analysis/text", formData, {
      headers: {
...
const handler = async (m, {
  ...
  try {
    ...
    if (command === "ishuman") {
      ...
      res = isHuman === 100 ? `🧑‍💻 *Teks ini terdeteksi dibuat oleh Manusia*\n\n*Umpan Balik:* ${feedback}\n*Kalimat Asli:* ${originalParagraph}\n*Bahasa Terdeteksi:* ${detected_language}\n*Jumlah Kata:* ${textWords}\n*Jumlah Kata AI:* ${aiWords}\n*Persentase Palsu:* ${Math.round(fakePercentage * 100) / 100}%` : isHuman === 0 ? `🤖 *Teks ini terdeteksi dibuat oleh Bot*\n\n*Kalimat Asli:* ${originalParagraph}\n*Bahasa Terdeteksi:* ${detected_language}\n*Jumlah Kata:* ${textWords}\n*Jumlah Kata AI:* ${aiWords}\n*Persentase Palsu:* ${Math.round(fakePercentage * 100) / 100}%` : `🔍 *Persentase manusia:* ${Math.round(isHuman * 100) / 100}%\n*Umpan Balik:* ${additional_feedback}\n*Kalimat Asli:* ${originalParagraph}\n*Bahasa Terdeteksi:* ${detected_language}\n*Jumlah Kata:* ${textWords}\n*Jumlah Kata AI:* ${aiWords}\n*Persentase Palsu:* ${Math.round(fakePercentage * 100) / 100}%`;
    } else if (command === "illuminarty") {
      const result = await analyzeText(text);
      const probability = result.data.probability;
      res = probability >= .5 ? `🤖 *Teks ini terdeteksi dibuat oleh AI (${Math.round(probability * 1e4) / 100}%)*` : `🧑‍💻 *Teks ini terdeteksi dibuat oleh Manusia (${Math.round(probability * 1e4) / 100}%)*`;
    }
    m.reply(res);
  } catch {
    m.react(eror);
