...
const handler = async (m, {
  ...
  if (!imageRegex.test(mime)) return m.reply("Hanya format PNG, JPG, atau JPEG yang diterima.");
  m.react(wait);
  try {
    const media = await q.download();
    const link = await uploadFile(media, 5);
    const prompt = await getImagePrompt(link);
    return m.reply(prompt || "Tidak ada respons dari OpenAI atau terjadi kesalahan.");
    m.react(sukses);
  } catch (error) {
