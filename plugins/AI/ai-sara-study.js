...
const handler = async (m, {
  ...
  const inputText = args.join(" ") || m.quoted?.text || m.quoted?.caption || m.quoted?.description;
  if (!inputText) return m.reply(`Masukkan teks atau reply dengan teks.\nContoh:\n*${usedPrefix}${command} Hai, apa kabar?*`);
  const q = m.quoted || m;
