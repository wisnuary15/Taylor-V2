...
const handler = async (m, {
  ...
  try {
    ...
    if (!username || !provider) {
      const buttons = conn.ctaButton.setBody(`⚙️ *Pilih layanan roasting:*\n- Instagram\n- TikTok\n- Threads\n\nGunakan format berikut untuk memilih layanan:\n\n*roasting [instagram/tiktok/threads] | [username]*`).addSelection("🔍 Pilih layanan").makeSections("📜 Daftar Layanan", "Pilih layanan");
      const services = [{
        name: "Instagram",
        key: "instagram"
      }, {
      ...
      services.forEach(service => {
        buttons.makeRow("", service.name, "Pilih: " + service.name, `${usedPrefix}${command} ${username || "username"}|${service.key}`);
      });
    ...
    switch (provider) {
      ...
      case "threads":
        ...
        break;
      default:
        return m.reply("⚠️ Provider tidak dikenal! Gunakan: *instagram*, *tiktok*, atau *threads*.");
    }
    if (!result) {
