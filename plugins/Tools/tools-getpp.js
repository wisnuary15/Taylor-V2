const handler = async (m, { text, conn }) => {
    try {
        ...
        whoArr = whoArr.filter((jid, index, self) => self.indexOf(jid) === index);
        ...
        for (const who of validNumbers.slice(0, 5)) {
            ...
            for (const item of (data.filter(item => item.exists)).filter((item, index, self) => index === self.findIndex((t) => t.jid === item.jid))) {
                const profileJid = item.jid;
                try {
                    const response = await conn.profilePictureUrl(profileJid, 'image');
                    await conn.sendFile(m.chat, response, 'profile.jpg', `Profile: @${profileJid.split('@')[0]}`, m, null, { mentions: [profileJid] });
                } catch (error) {
