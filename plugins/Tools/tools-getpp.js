const handler = async (m, {
  ...
  try {
    let whoArr = [...(m.quoted ? [m.quoted?.sender || m.quoted?.key?.remoteJid] : []), ...(text ? conn.parseMention(
      text) : []), ...(m.mentionedJid?.length ? m.mentionedJid : []), ...(!m.quoted && !text && !m.mentionedJid
      ?.length && m.sender ? [m.sender] : [])];
    whoArr = [...new Set(whoArr)];
    const validNumbers = whoArr.map(jid => (jid.match(/\d+/g) || []).join('')).filter(phone => phone.length >= 10 &&
      phone.length <= 15 && parseInt(phone, 10) >= 1000000000 && parseInt(phone, 10) <= 999999999999999);
    const totalNumbers = validNumbers.length;
    if (totalNumbers > 0) {
      conn.reply(m.chat,
        `📊 *Total Number*: *${totalNumbers}*\n\n${validNumbers.map((number, index) => `*${index + 1}.* @${number}`).join('\n')}`,
        m, {
          mentions: validNumbers.map(v => v + '@s.whatsapp.net')
        });
    } else {
      conn.reply(m.chat, 'Tidak ada nomor yang valid ditemukan.', m);
      return;
    }
    for (const who of validNumbers.slice(0, 5)) {
      ...
      if (profileData) {
        ...
        try {
          ...
          conn.sendFile(m.chat, response, 'profile.jpg', `Profile: @${profileJid.split('@')[0]}`, m, null, {
            mentions: [profileJid]
          });
        } catch (error) {
