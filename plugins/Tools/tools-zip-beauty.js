...
const { js_beautify: js_beautify, css_beautify: css_beautify, html_beautify: html_beautify } = jsbeautify, handler = async (m, { args: args, command: command }) => {
  try {
    ...
    for (const zipEntry of zip.getEntries()) zipEntry.entryName.endsWith(".js") && beautifyPromises.push((async zipEntry => {
      ...
      try {
        let result;
        "beautyjszip" === command && (result = js_beautify(jsCode)), "beautycsszip" === command && (result = css_beautify(jsCode)),
          "beautyhtmlzip" === command && (result = html_beautify(jsCode)), zip.updateFile(zipEntry.entryName, Buffer.from(result, "utf8")),
