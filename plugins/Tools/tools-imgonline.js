...
Contoh Penggunaan: .imgonline 1|order1|order2 atau .imgonline 1`);
  ...
  try {
    ...
    if (!order) throw new Error(
      "ℹ️ Input tidak valid. Gunakan format: .imgonline <urutan>|[order1]|[order2] atau .imgonline <urutan>\n\nℹ️ Contoh Penggunaan: .imgonline 1|order1|order2 atau .imgonline 1"
      );
    const numericOrder = parseInt(order);
    if (isNaN(numericOrder) || numericOrder <= 0 || numericOrder > 18) throw new Error(
      "ℹ️ Urutan fungsi tidak valid. Gunakan nomor fungsi antara 1 dan 18.");
    m.reply("*ᴜᴘʟᴏᴀᴅɪɴɢ...*");
    let output;
    ...
    output && output.url && output.url.length > 0 ? conn.sendFile(m.chat, output.url[0], '', '', m) : output &&
      output.text ? conn.reply(m.chat, output.text, m) : null;
  } catch (error) {
