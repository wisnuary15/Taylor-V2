...
const handler = async (m, { conn: conn, isOwner: isOwner, usedPrefix: usedPrefix, command: command, args: args }) => {
  ...
  try {
    ...
    const { anilist: { id: id = "Tidak diketahui", idMal: idMal = "Tidak diketahui", title: title = {}, synonyms: synonyms, isAdult: isAdult }, filename: filename, episode: episode = "Tidak diketahui", from: from, to: to, similarity: similarity, video: video, image: image } = result[0], nativeTitle = title.native || "Tidak diketahui", romajiTitle = title.romaji || "Tidak diketahui";
...
async function mp4ToWebp(file, stickerMetadata) {
  stickerMetadata ? (stickerMetadata.pack || (stickerMetadata.pack = "‎"), stickerMetadata.author || (stickerMetadata.author = "‎"),
    stickerMetadata.crop || (stickerMetadata.crop = !1)) : stickerMetadata || (stickerMetadata = {
    pack: "‎",
    author: "‎",
    crop: !1
  });
