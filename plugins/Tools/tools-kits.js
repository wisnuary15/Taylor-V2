...
const handler = async (m, {
  ...
  try {
    ...
    if (!/audio/.test(mime)) return m.reply(
      `reply audio/mp3 you want to convert to kits-ai with caption *${usedPrefix + command}* ( index model )`)
    ...
    if (output.audioUrl) {
      conn.sendFile(m.chat, output.audioUrl, '', output.type, m, null, {
        ptt: true,
