  ...
  try {
    ...
    if (res) {
      ...
      if (inviteUrl) {
        ...
        if (Object.keys(newsletterInfo).map((key => {
            let value = newsletterInfo[key];
            "subscribers" === key ? value = value ? value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") : "Tidak ada" : "creation_time" === key || "nameTime" === key || "descriptionTime" === key ? value = value ? new Date(1e3 * value).toLocaleString() : "Tidak ada" : "preview" === key && (value = value ? getUrlFromDirectPath(value) : "Tidak ada"),
