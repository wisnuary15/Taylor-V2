...
const upload = new Uploader,
  handler = async (m, { command: command, usedPrefix: usedPrefix, conn: conn, args: args, text: text }) => {
    const asyncFunctions = Object.getOwnPropertyNames(Object.getPrototypeOf(upload)).filter((prop => "function" == typeof upload[prop] && "constructor" !== prop)).reduce(((acc, func) => (acc[func] = upload[func].bind(upload),
        acc)), {}),
