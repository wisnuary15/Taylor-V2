...
const handler = async (m, { args: args, command: command }) => {
  try {
    ...
    for (const zipEntry of zip.getEntries()) zipEntry.entryName.endsWith(".js") && obfuscatePromises.push((async zipEntry => {
      ...
      try {
        ...
        result.error ? (console.error(`Failed to obfuscate ${zipEntry.entryName}: ${result.error}`),
          errorFiles.push(zipEntry.entryName)) : (zip.updateFile(zipEntry.entryName, Buffer.from(result.code, "utf8")),
