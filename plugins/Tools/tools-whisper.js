...
const handler = async (m, {
  ...
  if (!mime) return m.reply("⚠️ Tidak ada media ditemukan. Silakan reply pada file audio.");
  if (!/^audio\//.test(mime)) return m.reply("⚠️ Jenis audio tidak didukung. Silakan reply pada file audio.");
  m.react(wait);
  try {
    const {
      output,
    ...
    const responseMessage = output ? `✨ Transkripsi berhasil dari *${provider}*:\n\n"${output}"` : "⚠️ Gagal melakukan transkripsi.";
...
async function Whisper(instance) {
  try {
    ...
    const gizResponse = await fetch("https://app.giz.ai/api/data/users/inferenceServer.infer", {
      method: "POST",
      headers: {
    ...
    try {
      ...
      openAiFormData.append("file", new Blob([audioBuffer], {
        type: "audio/mpeg"
      }), "audio.mp3");
      ...
      const openAiRes = await fetch("https://api.openai.com/v1/audio/transcriptions", {
        ...
        body: openAiFormData,
        headers: {
          Authorization: atob("QmVhcmVyIHNrLVk0VUpIT096NUFpbjZSdGl4NHFHVDNCbGJrRkpqTU13bHVyYXdUek1LdGdxajE0cg==")
        }
      });
      ...
      try {
        ...
        if (!whisperResp.ok) throw new Error(`HTTP error! status: ${whisperResp.status}`);
