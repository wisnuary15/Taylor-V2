...
const handler = async (m, {
  ...
  if (!/^application\/zip$/.test(mime)) return m.reply("Unsupported file type. Please reply to a ZIP file.");
  m.react(wait);
  try {
    const output = await ZipPass(media);
    const responseMessage = output.processing && output.password ? `*Processing:*\n- \`${output.processing}\`\n*Password:*\n- \`${output.password}\`` : "Failed to extract the password.";
...
async function ZipPass(content) {
  try {
    const response = await fetch("https://passwordrecovery.io/zip-file-password-removal/");
    ...
    const [, processing = "Not found", password = "Not found"] = (await postResponse.text()).match(/Processing: (.+)<br>Result: (.+)/) || [];
    return {
