...
const handler = async (m, { conn: conn, usedPrefix: usedPrefix, command: command }) => {
  try {
    ...
    if (!/video|audio/.test(mime)) throw `Reply audio/video with command ${usedPrefix + command}`;
    {
      ...
      m.react(wait);
      let { status: status, metadata: metadata } = await acr.identify(buffer);
      if (0 !== status.code) throw status.msg;
      let { title: title, artists: artists, album: album, genres: genres, release_date: release_date } = metadata.music[0], txt = `*• Title:* ${title}${artists ? `\n*• Artists:* ${artists.map((v => v.name)).join(", ")}` : ""}`;
