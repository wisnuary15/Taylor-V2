...
async function combineVideosToBuffer(videoData) {
  ...
  try {
    for (const {
      ...
      of videoData) {
      const tempFilePath = path.join(tempDir, `${Date.now()}_${path.basename(url)}`);
...
async function getDataSibi(inputValue) {
  ...
  const url = isSingleWord ? "https://pmpk.kemdikbud.go.id/sibi/pencarian/pencariankata" : "https://pmpk.kemdikbud.go.id/sibi/pencarian/pencarianadvance";
  try {
    ...
    return $("div.card-body .col-sm-4").map((_, el) => ({
