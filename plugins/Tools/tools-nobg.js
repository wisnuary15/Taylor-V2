...
const handler = async (m, {
  ...
  if (/image/g.test(mime) && !/webp/g.test(mime)) {
    const methods = [async () => {
      ...
      formData.append("upfile", new Blob([imageBuffer], {
        type: "image/jpeg"
      }));
      ...
      if (data.byteLength < 1024) throw new Error("Ukuran gambar kurang dari 1KB");
      await conn.sendFile(m.chat, Buffer.from(data), "removebg.png", `*✨ Latar belakang berhasil dihapus!*\n*Dibuat oleh:* Photiu`, m);
      ...
      if (buffer.byteLength < 1024) throw new Error("Ukuran gambar kurang dari 1KB");
      await conn.sendFile(m.chat, Buffer.from(buffer), "", `*✨ Latar belakang berhasil dihapus!*\n*Dibuat oleh:* Remove.bg`, m);
      console.log("Remove.bg method succeeded.");
    }, async () => {
      console.log("Trying Lolhuman method...");
      const img = await webp2png(await q.download());
      const url = API("lolhuman", "/api/removebg", {
        img: img
      }, "apikey");
      const response = await fetch(url);
      const buffer = await response.arrayBuffer();
      if (buffer.byteLength < 1024) throw new Error("Ukuran gambar kurang dari 1KB");
      await conn.sendFile(m.chat, Buffer.from(buffer), "removebg.png", `*✨ Latar belakang berhasil dihapus!*\n*Dibuat oleh:* Lolhuman`, m);
    ...
    for (const method of methods) {
      try {
        ...
        if (e.message === "Ukuran gambar kurang dari 1KB") {
          m.react(eror);
          m.reply("Gambar yang dihasilkan kurang dari 1KB, tidak dapat menghapus latar belakang.");
          return;
        }
        continue;
      }
