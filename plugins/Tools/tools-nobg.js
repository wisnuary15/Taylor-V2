...
const handler = async (m, {
  ...
  if (/image/g.test(mime) && !/webp/g.test(mime)) {
    try {
      ...
      conn.sendFile(m.chat, Buffer.from(buffer), '', '*Made by:* api.remove.bg', m)
    } catch (e) {
      try {
        let img = await q?.download()
        let out = await uploadImage(img)
        let url = "https://removebg.api.akuari.my.id/other/removebgg?gambar=" + out
        conn.sendFile(m.chat, url, 'removebg.png', '*Made by:* akuari', m)
      } catch (e) {
        let img = await webp2png(await q?.download()),
          url = API('lolhuman', '/api/removebg', {
            img
          }, 'apikey')
        conn.sendFile(m.chat, url, 'removebg.png', '*Made by:* lolhuman', m)
      }
    }
  } else {
