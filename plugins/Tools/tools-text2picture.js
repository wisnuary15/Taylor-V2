...
const handler = async (m, {
  ...
  if (!text) return m.reply(`Masukkan teks atau reply pesan dengan teks.\nContoh: *${usedPrefix}${command} Hai, apa kabar?*`);
  try {
    const buffer = command === "txt2gif" ? await createAnimatedImage(text.trim()) : await createStaticImage(text.trim());
...
const createStaticImage = async text => {
  try {
    const fontPath = path.join("./src/font", fs.readdirSync("./src/font").find(file => file.endsWith(".ttf")));
    const maxWidth = 1024;
    ...
    const finalFontSize = textHeight > maxHeight ? Math.floor(fontSize * (maxHeight / textHeight)) : fontSize;
...
const createAnimatedImage = async text => {
  try {
    ...
    const command = colors.map((color, index) => `convert -background transparent -fill ${color} -font "${fontPath}" -pointsize ${finalFontSize} \
            -gravity center -stroke black -strokewidth 2 -size ${maxWidth}x${maxHeight} label:"${text}" \
            -resize ${maxWidth}x${maxHeight}+0+0 -gravity center -extent ${maxWidth}x${maxHeight} webp:frame_${index}.webp`).join(" && ");
