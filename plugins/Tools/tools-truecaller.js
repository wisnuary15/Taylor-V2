...
const handler = async (m, {
  ...
  try {
    ...
    if (command === 'truecaller') {
      ...
      if (method && number && ((method === 'send' && !otp) || (method === 'verify' && key && otp) || (method ===
          'search' && key))) {
        switch (method) {
          case 'send':
        ...
        message =
          'Format pesan tidak sesuai. Pastikan Anda telah memasukkan method, nomor, key, dan otp dengan benar. ❗';
      }
    } else {
