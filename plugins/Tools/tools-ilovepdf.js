import {
  IlovePDF
} from "../../lib/tools/ilovepdf.js";
const pdfTools = ["compress", "extract", "htmlpdf", "imagepdf", "merge", "officepdf", "pagenumber", "pdfa", "pdfjpg", "pdfocr", "protect", "repair", "rotate", "split", "unlock", "validatepdfa", "watermark"];
const imageTools = ["compressimage", "cropimage", "convertimage", "removebackgroundimage", "repairimage", "resizeimage"];
...
const handler = async (m, {
  ...
  } else if (toolIndex >= pdfTools.length && toolIndex < pdfTools.length + imageTools.length) {
  ...
  try {
    ...
    if (result?.fileBuffer) {
      await conn.sendFile(m.chat, result.fileBuffer, "", `File berhasil dihasilkan dengan tool *${tool.toUpperCase()}*${extraPayloadStr ? `\n📋 Payload tambahan: ${extraPayloadStr}` : ""}.`, m);
      m.react(sukses);
    } else {
