...
const handler = async (m, {
  ...
  try {
    const isDocument = text === "doc" && m.quoted && m.quoted?.mtype.includes("document");
    const docBuffer = isDocument && m.quoted?.download ? await m.quoted.download() : null;
    const content = docBuffer ? docBuffer.toString("utf-8") : text;
    const message = command === "packed" ? await obfuscator.obfuscateCode(ENC_OPTIONS.NORMAL, "on", "on", content) : command === "unpacked" ? obfuscator.unPacked(content) : null;
