  ...
  try {
    ...
    switch (command) {
      ...
      case "decrypt":
        ...
        result = command === "compile" ? (await compiles(text))?.output : (await decrypts(text))?.result;
        break;
      case "recognition":
        let file = text && isUrl(text) ? text : m.quoted ? await uploadFile(await m.quoted.download()) : null;
        if (!file) {
          return m.reply(`Masukkan teks atau reply pesan dengan teks yang ingin diolah.\nContoh penggunaan:\n*${usedPrefix}${command} Hai, apa kabar?*`);
        }
...
async function recognition(url) {
  ...
  try {
    const response = await fetch("https://imphnen-ai.vercel.app/api/asr/music_recognition", {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
