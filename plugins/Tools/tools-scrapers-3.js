...
const handler = async (m, {
  ...
  let lister = ["android1", "anime", "anoboydl", "anoboys", "apkmirror", "apkmody", "artinama", "asupanfilm",
    "asupanfilminfo", "bacaresep", "carigc", "cariresep", "chara", "corona", "devianart", "dewabatch", "drakor",
    "film", "gempa", "ghfollower", "ghfollowing", "ghuser", "goredl", "happymod", "happymoddl", "hentaivid",
    "igdll", "igstalk", "job", "kiryu", "konachan", "manga", "mangatoon", "mediafire", "palingmurah", "pinterest",
    "pornvid", "randomgore", "randomtt", "searchgore", "sfiledown", "sfilesearch", "soundcloud", "stickersearch",
    "textmakervid", "trendtwit", "wallpapercave", "wallpapercraft", "wallpaperhd", "wattpad", "webtoons",
    "wikisearch", "zerochan"
  ]
...
handler.help = ["scrap3 type query"]
handler.tags = ["internet"]
handler.command = /^(scrap3)$/i
...
async function pornvid() {
  return new Promise((resolve, reject) => {
    axios.get('https://tikporntok.com/?random=1').then((res) => {
      ...
      hasil.thumb = $('article > div.video-wrapper.vxplayer > div.vx_el').attr('data-poster') ||
      ...
      hasil.video = $('article > div.video-wrapper.vxplayer > div.vx_el').attr('src') || $(
          'article > div.video-wrapper.vxplayer > div.vx_el').attr('data-src') ||
        'https://4.bp.blogspot.com/-hyMqjmQQq4o/W6al-Rk4IpI/AAAAAAAADJ4/m-lVBA_GC9Q5d4BIQg8ZO3fYmQQC3LqSACLcBGAs/s1600/404_not_found.png'
      resolve({
        status: res.status,
        creator: 'Wudysoft',
  ...
  return new Promise((resolve) => {
    axios.get(link).then(({
      ...
      const info = {
        ...
        alurcerita_imdb: $('body > div > div:nth-child(5) > div.card-body.p-2 > ul > li:nth-child(2)')
        .text().split(' Alur Cerita IMDb: ')[1],
        alurcerita_tmdb: $('body > div > div:nth-child(5) > div.card-body.p-2 > ul > li:nth-child(3)')
        .text().split(' Alur Cerita TMDb: ')[1],
        direksi: $('body > div > div:nth-child(5) > div.card-body.p-2 > ul > li:nth-child(4)').text().split(
          ' Direksi: ')[1],
        pemeran: $('body > div > div:nth-child(5) > div.card-body.p-2 > ul > li:nth-child(5)').text().split(
          ' Pemeran: ')[1],
        kategori: $('body > div > div:nth-child(5) > div.card-body.p-2 > ul > li:nth-child(6)').text()
          .split(' Kategori: ')[1],
        negara: $('body > div > div:nth-child(5) > div.card-body.p-2 > ul > li:nth-child(7)').text().split(
          ' Negara: ')[1],
        tahun_rilis: $('body > div > div:nth-child(5) > div.card-body.p-2 > ul > li:nth-child(8)').text()
          .split(' Tahun Rilis: ')[1],
        durasi: $('body > div > div:nth-child(5) > div.card-body.p-2 > ul > li:nth-child(9)').text().split(
          ' Durasi: ')[1],
        skor: $('body > div > div:nth-child(5) > div.card-body.p-2 > ul > li:nth-child(10)').text().split(
          ' Skor: ')[1],
        kualitas: $('body > div > div:nth-child(5) > div.card-body.p-2 > ul > li:nth-child(11)').text()
          .split(' Kualitas: ')[1],
        jenis: $('body > div > div:nth-child(5) > div.card-body.p-2 > ul > li:nth-child(12)').text().split(
          ' Jenis: ')[1]
      }
      resolve(info)
    })
  ...
  return new Promise((resolve, reject) => {
    axios.get('https://brainans.com/search?query=' + query).then(({
      ...
      axios.get('https://brainans.com/' + luser).then(({
        ...
        const $$ = cheerio.load(data)
        let vlink = [];
        $$('#videos_container > div > div.content__list.grid.infinite_scroll.cards > div > div > a').each(
          function(a, b) {
...
async function zerochan(query) {
  return new Promise((resolve, reject) => {
    axios.get('https://www.zerochan.net/search?q=' + query).then(({
      ...
      for (let i = 0; i < judul.length; i++) {
        result.push('https://s1.zerochan.net/' + judul[i].replace(/\ /g, '.') + '.600.' + id[i].split('/')[
          1] + '.jpg')
      }
      resolve({
        creator: 'Wudysoft',
      ...
      axios.get('https://www.anime-planet.com' + linkp).then((data) => {
        ...
        resolve({
          ...
          gender: $$('#siteContainer > section.pure-g.entryBar > div:nth-child(1)').text().split(
            '\nGender: ')[1],
          warna_rambut: $$('#siteContainer > section.pure-g.entryBar > div:nth-child(2)').text()
            .split('\nHair Color: ')[1],
          warna_mata: $$(
...
async function anoboys(query) {
  return new Promise((resolve, reject) => {
    axios.get('https://anoboy.media/?s=' + query).then(({
      ...
      const thumb = [];
      const uptime = [];
      $('body > div.wrap > div.container > div.column-content > a > div > div.amvj > h3').each(function(a,
      b) {
        let jud = $(b).text();
        judul.push(jud)
      })
...
async function soundcloud(link) {
  return new Promise((resolve, reject) => {
    ...
    request(options, async function(error, response, body) {
      ...
      resolve({
        judul: $('#header > div > div > div.col-lg-8 > div > table > tbody > tr > td:nth-child(2)')
          .text(),
        download_count: $(
          '#header > div > div > div.col-lg-8 > div > table > tbody > tr > td:nth-child(3)').text(),
        thumb: $(
...
async function bacaresep(query) {
  return new Promise(async (resolve, reject) => {
    axios.get(query).then(({
      ...
      const judul = $(
        'body > div.all-wrapper.with-animations > div.single-panel.os-container > div.single-panel-main > div.single-title > h1'
        ).text();
      const waktu = $(
...
async function searchgore(query) {
  return new Promise(async (resolve, reject) => {
    axios.get('https://seegore.com/?s=' + query).then(dataa => {
      ...
      let rand = Math.floor(Math.random() * pagina) + 1
      if (rand === 1) {
        var slink = 'https://seegore.com/?s=' + query
      } else {
        var slink = `https://seegore.com/page/${rand}/?s=${query}`
      }
...
async function randomgore() {
  return new Promise(async (resolve, reject) => {
    axios.get('https://seegore.com/gore/').then(dataa => {
      const $$$ = cheerio.load(dataa)
      let pagina = $$$(
        '#main > div.container.main-container.bb-stretched-full > div > div > div > div > div > div > nav > ul > li:nth-child(4) > a'
        ).text();
      let rand = Math.floor(Math.random() * pagina) + 1
      let randvid = Math.floor(Math.random() * 16) + 1
      if (rand === 1) {
        var slink = 'https://seegore.com/gore/'
      } else {
        var slink = `https://seegore.com/gore/page/${rand}/`
      }
      axios.get(slink).then(({
        data
      }) => {
        ...
        const result = [];
        const username = [];
        const linkp = $(`#post-items > li:nth-child(${randvid}) > article > div.post-thumbnail > a`)
          .attr('href')
        const thumbb = $(
...
async function apkmirror(query) {
  return new Promise((resolve, reject) => {
    axios.get('https://www.apkmirror.com/?post_type=app_release&searchtype=apk&s=' + query).then(({
      ...
      $('#content > div > div > div.appRow > div > div > div > div.downloadIconPositioning > a').each(
        function(e, f) {
...
async function sfiledown(link) {
  return new Promise((resolve, reject) => {
    axios.get(link).then(({
      ...
      const desc = $('body > div.w3-row-padding.w3-container.w3-white > div > div:nth-child(7) > center > h1')
        .text();
      const type = $(
        'body > div.w3-row-padding.w3-container.w3-white > div > div:nth-child(4) > a:nth-child(3)').text();
      const upload = $('body > div.w3-row-padding.w3-container.w3-white > div > div:nth-child(5)').text();
      const uploader = $(
        'body > div.w3-row-padding.w3-container.w3-white > div > div:nth-child(4) > a:nth-child(2)').text();
...
async function android1(query) {
  return new Promise((resolve, reject) => {
    axios.get('https://an1.com/tags/MOD/?story=' + query + '&do=search&subaction=search').then(({
      ...
      $('body > div.page > div > div > div.app_list > div > div > div.cont > div.data > div.name > a').each(
        function(i, j) {
...
async function apkmody(query) {
  return new Promise((resolve, reject) => {
    axios.get('https://apkmody.io/?s=' + query).then(({
      ...
      const thumb = [];
      const format = [];
      $('#primary > section:nth-child(3) > div > div > div > article > a > div > div > div > h2').each(
        function(a, b) {
          let nem = $(b).text();
          nama.push(nem)
        })
      $('#primary > section:nth-child(3) > div > div > div > article > a > div > div > p').each(function(c,
      d) {
        let modd = $(d).text();
        mod.push(modd.split('\n')[1])
      })
...
async function drakor(query) {
  return new Promise((resolve, reject) => {
    ...
    axios.get('https://drakorasia.net/?s=' + queryy + '&post_type=post').then(({
      ...
      $('#post > div > div.title.text-center.absolute.bottom-0.w-full.py-2.pb-4.px-3 > a > h2').each(function(
        c, d) {
        let titel = $(d).text();
        judul.push(titel)
      })
  ...
  return new Promise((resolve, reject) => {
    ...
    axios.get('https://konachan.net/post?tags=' + text + '+').then(({
      ...
      axios.get('https://konachan.net/post?page=' + mat + '&tags=' + text + '+').then(({
        ...
        const $ = cheerio.load(data)
        const result = [];
        $('#post-list > div.content > div:nth-child(4) > ul > li > a.directlink.largeimg').each(function(
          a, b) {
          result.push($(b).attr('href'))
        })
        resolve(result)
