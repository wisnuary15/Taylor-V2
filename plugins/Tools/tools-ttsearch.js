...
const randomString = (length, charset = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789') =>
    Array.from(new Array(length), () => charset.charAt(randomInt(charset.length))).join('');
...
async function tiktokVideoInfo(videoId) {
    ...
    try {
        ...
        if (response.status < 300) {
            ...
            const videoUrl = videoInfo?.video?.play_addr?.url_list[0] || videoInfo?.video?.play_addr_265?.url_list[0] || videoInfo?.video?.play_addr_264?.url_list[0];
            return videoUrl;
        } else {
            return null;
        }
