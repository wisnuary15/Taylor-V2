...
handler.help = ["ttsearch <text>"], handler.tags = ["tools"], handler.command = /^(ttsearch)$/i;
...
async function tiktokSearchVideo(searchKeywords, offset = 0) {
  ...
  try {
    ...
    if (response.status < 300) {
      const { has_more: has_more, data: data } = response.data;
      return {
...
async function tiktokVideoInfo(videoId) {
  ...
  try {
    ...
    if (response.status < 300) {
      const { aweme_list: aweme_list } = response.data, videoInfo = aweme_list[0];
