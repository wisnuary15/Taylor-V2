...
const handler = async (m, {
  ...
  const output = await tiktokSearchVideo(tema, 0);
  const data = output.results;
  m.react(wait)
  try {
    const mesg = "Input query!\n*Example:*\n" + usedPrefix + command +
      " [query]|[nomor]\n\n*Pilih angka yg ada*\n" + data.map((item, index) => `*${index + 1}.* ${item.title}`)
      .join("\n")
    ...
    const videoSource = await tiktokVideoInfo(searchResult.video_id);
    if (videoSource) {
      const caption =
      ...
      conn.sendFile(m.chat, videoSource || searchResult.video_pic, "", caption, m)
    }
  } catch (e) {
    m.react(eror)
  }
};
handler.help = ['ttsearch <text>']
handler.tags = ['tools']
handler.command = /^(ttsearch)$/i
export default handler;
const randomString = (length, charset = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789') => Array.from(
  new Array(length), () => charset.charAt(randomInt(charset.length))).join('');
const formatNumber = (num) => {
  const suffixes = ['', 'k', 'M', 'B', 'T'];
  const numString = Math.abs(num).toString();
  const numDigits = numString.length;
  return numDigits <= 3 ? numString :
    `${(num / 1000 ** Math.floor((numDigits - 1) / 3)).toFixed(1).replace(/\.0$/, '')}${suffixes[Math.floor((numDigits - 1) / 3)]}`;
};
...
async function tiktokVideoInfo(videoId) {
  ...
  try {
    ...
    if (response.status < 300) {
      ...
      const videoInfo = aweme_list[0];
      const videoUrl = videoInfo?.video?.play_addr?.url_list[0] || videoInfo?.video?.play_addr_265?.url_list[0] ||
        videoInfo?.video?.play_addr_264?.url_list[0];
      return videoUrl;
    } else {
      return null;
    }
