...
const handler = async (m, {
  ...
  try {
    ...
    if (!/image/g.test(mime) || /webp/g.test(mime)) return m.reply("*Balas gambar yang valid*");
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          return m.reply("*Gagal memproses gambar menggunakan Widipe. Coba lagi nanti.*");
        }
      }
    }
    if (response) {
...
const kenlieRemini = async query => {
  try {
    const res = await fetch(`https://api.kenliejugarap.com/reminibymarjhun/?url=${encodeURIComponent(query)}`);
    const {
      image_data: url
    } = await res.json();
    return url;
