...
async function Barbie(input) {
    try {
        const data = Buffer.isBuffer(input)
            ? input
            : Buffer.from((await axios.get(input, { responseType: 'arraybuffer' })).data);

        const { ext, mime } = await fileTypeFromBuffer(data) || { ext: 'jpg', mime: 'image/jpg' };
        const hash = createHash('sha256').update(data).digest('hex');
        const filename = `${hash}.${ext}`;

        const formData = new FormData();
        formData.append('myfile', new Blob([data], { type: mime }), filename);
