...
const handler = async (m, {
  ...
  let lister = ["aiovideo", "apkdl2", "apkdl2inf", "apkdl2s", "apkdl2ss", "apkkkk", "apkkkks", "attp", "drakor",
    "film", "imdb", "jadwaltv", "joox", "jooxdl", "memeku", "mod1", "mod2", "musicaldown", "playstoredl",
    "playstoredld", "playstoredlll", "ringtone", "searchIlust", "sfiledl", "tiktoktren", "ttp", "turnbackhoax",
    "w9apps", "wallpaperggpa", "otakudesuSearch", "otakudesuDetail", "otakudesuDownload", "anime", "manga",
    "wibu", "fandom", "character", "topAnime", "topManga"
  ]
...
async function w9apps(query) {
  return new Promise((resolve, reject) => {
    ...
    axios(config).then(({
      ...
      let tokenpo = $$1(
        'html > body.notranslate > div.body-content.body-content-in > div > div.pc-wrap.downloading > div > div > div.downloading-info.flex-1 > h1 > a.pc-btn-download.j-app-download'
        ).attr('href')
      resolve(token)
    }).catch(reject)
  })
...
async function mod1(query) {
  return new Promise((resolve, reject) => {
    ...
    let token = $$('#blocks-left > div.post-content > div.post-outer.clearfix > div.download_button > a').attr(
      'href')
    const era = axios.get(token)
    const $g = cheerio.load(era.data)
    let tokensd = $g('#dlbox > ul.dl > a:nth-child(1)').attr('href')
    resolve(tokensd).catch(reject)
  })
}
...
async function apkdl2s(query) {
  return new Promise((resolve, reject) => {
    axios.get(`https://apkpure.com/search?q=${query}`).then(({
      ...
      const $$ = cheerio.load(data)
      let token = $$(
        'div.main.search-app > div.left > div.box > div.first.brand.is-brand > div.first-info.brand-info > div.info a'
        ).attr('href')
      let tokenv = $$(
        'div.main.search-app > div.left > div.box > div.first.brand.is-brand > div.first-info.brand-info > div.info a'
        ).attr('href')
...
async function apkdl2inf(query) {
  return new Promise((resolve, reject) => {
    axios.get(`${query}`).then(({
      ...
      const $$ = cheerio.load(data)
      let token = $$(
        'div.main.search-app > div.left > div.box > div.first.brand.is-brand > div.first-info.brand-info > a.da'
        ).attr('href')
      resolve(token)
    }).catch(reject)
  })
    ...
    axios.get("https://brainans.com/").then(async (data) => {
      ...
      async function getmetadata(link, views) {
        ...
        result["result"].push({
          ...
          upload_time: $$("#card-page").find("div.main__user-desc.align-self-center.ml-2").text()
            .split($$("#card-page").find("div.main__user-desc.align-self-center.ml-2 > a").text())[1]
            .trim(),
  ...
  return new Promise((resolve, reject) => {
    axios.get(`https://173.212.240.190///?s=${query}&post_type=post`).then(({
      ...
      $('#post > div ').each(function(a, b) {
        result = {
          status: 200,
          judul: $(b).find('> div.title.text-center.absolute.bottom-0.w-full.py-2.pb-4.px-3 > h2 > a')
            .text().trim(),
          years: $(b).find(
          ...
          url: $(b).find('> div.title.text-center.absolute.bottom-0.w-full.py-2.pb-4.px-3 > h2 > a')
            .attr('href')
        }
        hasil.push(result)
      })
...
async function musicaldown(URL) {
  return new Promise((resolve, rejecet) => {
    axios.get('https://musicaldown.com/id', {
      ...
      const token_name = $("#submit-form > div").find("div:nth-child(1) > input[type=hidden]:nth-child(2)")
        .attr("name")
      const token_ = $("#submit-form > div").find("div:nth-child(1) > input[type=hidden]:nth-child(2)").attr(
        "value")
      const verify = $("#submit-form > div").find("div:nth-child(1) > input[type=hidden]:nth-child(3)").attr(
        "value")
      let data = {
        [`${url_name}`]: URL,
        [`${token_name}`]: token_,
      ...
      axios.request({
        ...
        axios.request({
          ...
          const result = {
            video: ch(
              'body > div.welcome.section > div > div:nth-child(2) > div.col.s12.l8 > a.btn.waves-effect.waves-light.orange:nth-child(3)'
              ).attr('href'),
            audio: hc(
              'body > div.welcome.section > div > div:nth-child(2) > div.col.s12.l8 > a:nth-child(5)'
              ).attr('href'),
            video_original: ch(
              'body > div.welcome.section > div > div:nth-child(2) > div.col.s12.l8 > a.btn.waves-effect.waves-light.orange:nth-child(4)'
              ).attr('href'),
            audio_original: hc(
              'body > div.welcome.section > div > div:nth-child(2) > div.col.s12.l8 > a:nth-child(9)'
              ).attr('href'),
            preview: ch(
              'body > div.welcome.section > div > div:nth-child(2) > div.col.s12.l4 > img').attr(
              'src')
          }
          resolve(result)
        })
...
async function joox(query) {
  return new Promise((resolve, reject) => {
    ...
    axios.get('http://api.joox.com/web-fcgi-bin//web_search?lang=id&country=id&type=0&search_input=' + query +
      '&pn=1&sin=0&ein=29&_=' + time).then(({
      data
    }) => {
      let result = []
      let hasil = []
      ...
      for (let i = 0; i < data.itemlist.length; i++) {
        const get =
          `http://api.joox.com/web-fcgi-bin/web_get_songinfo?songid=${ids[i]}&lang=id&country=id&from_type=null&channel_id=null&_=`
...
async function jooxdl(url) {
  return new Promise((resolve, reject) => {
    axios.get('http://api.joox.com/web-fcgi-bin/web_get_songinfo?songid=' + url +
      '&lang=id&country=id&from_type=null&channel_id=null&_=' + (new Date).getTime(), {
        headers: {
          'Content-Type': 'application/json',
          'Cookie': 'wmid=142420656; user_type=1; country=id; session_key=2a5d97d05dc8fe238150184eaf3519ad;',
          'useragent': 'Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/48.0.2564.109 Safari/537.36'
...
async function sfiledl(URL) {
  return new Promise((resolve, reject) => {
    axios.request({
      ...
      const $ = cheerio.load(res.data)
      const token = $(
        '#content > div > div.middle > div.right > article:nth-child(1) > section:nth-child(3) > div.buttons > form > input[type=hidden]:nth-child(1)'
        ).attr('value')
      const title = $(
          '#content > div > div.middle > div.right > article:nth-child(1) > section.box-content.meta > h1')
        .text().trim()
      const _size = $(
          '#content > div > div.middle > div.right > article:nth-child(1) > section.box-content.meta > p')
        .text().trim()
      ...
      axios({
        ...
        const ch = cheerio.load(respon.data)
        const url = ch('#content > div > div.middle > div.right > article:nth-child(1) > section > p > a')
          .attr('href')
        const result = {
          title: title,
          size: size,
  ...
  return new Promise((resolve, reject) => {
    axios({
      ...
      let hasil = {
        title: {
          indonesia: $('#venkonten > div.venser > div.jdlrx > h1').text(),
          anonym: $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(1) > span')
            .text().replace('Judul: ', ''),
          japanese: $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(2) > span')
            .text().replace('Japanese: ', '')
        },
        score: $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(3) > span')
          .text().replace('Skor: ', ''),
        producer: $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(4) > span')
          .text().replace('Produser: ', ''),
        type: $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(5) > span')
        .text().replace('Tipe: ', ''),
        status: $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(6) > span')
          .text().replace('Status: ', ''),
        total_eps: $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(7) > span')
          .text().replace('Total Episode: ', ''),
        duration: $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(8) > span')
          .text().replace('Durasi: ', ''),
        release: $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(9) > span')
          .text().replace('Tanggal Rilis: ', ''),
        studio: $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(10) > span')
          .text().replace('Studio: ', ''),
        genre: $('#venkonten > div.venser > div.fotoanime > div.infozin > div > p:nth-child(11) > span')
          .text().replace('Genre: ', ''),
        synopsis: $('#venkonten > div.venser > div.fotoanime > div.sinopc > p').text(),
        link_eps: link_eps
      }
  ...
  return new Promise((resolve, reject) => {
    axios({
      ...
      let hasil = {
        ...
        post: $('#venkonten > div.venser > div.venutama > div.kategoz > span:nth-child(2)').text().replace(
          'Posted by ', ''),
        release: $('#venkonten > div.venser > div.venutama > div.kategoz > span:nth-child(4)').text()
          .replace('Release on ', ''),
        credit: $('#venkonten > div.venser > div.cukder > div.infozin > div > p:nth-child(1)').text()
          .replace('Credit: ', ''),
        encoder: $('#venkonten > div.venser > div.cukder > div.infozin > div > p:nth-child(2)').text()
          .replace('Encoder: ', ''),
        genres: $('#venkonten > div.venser > div.cukder > div.infozin > div > p:nth-child(3)').text()
          .replace('Genres: ', ''),
        duration: $('#venkonten > div.venser > div.cukder > div.infozin > div > p:nth-child(4)').text()
          .replace('Duration: ', ''),
        type: $('#venkonten > div.venser > div.cukder > div.infozin > div > p:nth-child(5)').text().replace(
          'Tipe: ', ''),
  ...
  return new Promise((resolve, reject) => {
    axios.get('https://myanimelist.net/anime.php?cat=anime&q=' + q).then((get) => {
      ...
      axios.get(random).then((res) => {
        ...
        let related = []
        $$(
            '#content > table > tbody > tr > td:nth-child(2) > div.js-scrollfix-bottom-rel > table > tbody > tr:nth-child(3) > td > table > tbody > tr')
          .each(function(a, b) {
            related.push({
        ...
        let character = []
        $$(
            '#content > table > tbody > tr > td:nth-child(2) > div > table > tbody > tr:nth-child(3) > td > div.detail-characters-list.clearfix')
          .eq(0).find('table').each(function(a, b) {
            character.push({
              character: {
                ...
                image: $$(b).find('tbody > tr > td.ac.borderClass > div > a > img').attr(
                  'data-src') || $$(b).find('tbody > tr > td.ac.borderClass > div > a > img').attr(
                    'src')
              },
              voice_actor: {
                name: $$(b).find(
                  ...
                  .text(),
                origin: $$(b).find(
                  'tbody > tr > td:nth-child(3) > table > tbody > tr > td.va-t.ar.pl4.pr4 > small'
                  ).text(),
                detail: $$(b).find(
                    'tbody > tr > td:nth-child(3) > table > tbody > tr > td.va-t.ar.pl4.pr4 > a')
                  .attr('href'),
                image: $$(b).find('table > tbody > tr > td:nth-child(2) > div > a > img').attr(
                  'data-src') || $$(b).find(
                  'table > tbody > tr > td:nth-child(2) > div > a > img').attr('src')
              }
            })
          })
        let staff = []
        $$(
            '#content > table > tbody > tr > td:nth-child(2) > div > table > tbody > tr:nth-child(3) > td > div.detail-characters-list.clearfix')
          .eq(1).find('table').each(function(a, b) {
            staff.push({
        ...
        let hasil = {
          ...
          info: info,
          image: $$('#content > table > tbody > tr > td.borderClass > div > div:nth-child(1) > a > img')
            .attr('data-src') || $$(
              '#content > table > tbody > tr > td.borderClass > div > div:nth-child(1) > a > img').attr(
              'src'),
          trailer: $$('div.anime-detail-header-video.di-tc.va-t.pl16 > div.video-promotion > a').attr(
            'href'),
          synopsis: $$(
            '#content > table > tbody > tr > td:nth-child(2) > div > table > tbody > tr:nth-child(1) > td > p'
            ).text(),
          related,
          character,
          staff
  ...
  return new Promise((resolve, reject) => {
    axios.get('https://myanimelist.net/manga.php?cat=manga&q=' + q).then((get) => {
      ...
      axios.get(random).then((res) => {
        ...
        let character = []
        $$(
            '#content > table > tbody > tr > td:nth-child(2) > div.js-scrollfix-bottom-rel > table > tbody > tr:nth-child(3) > td > div.detail-characters-list.clearfix > div.left-column.fl-l.divider > table')
          .each(function(a, b) {
            character.push({
              character: {
                name: $$(b).find('tbody > tr > td:nth-child(2) > a').text(),
      ...
      axios.get(random + '/pics').then((res) => {
        ...
        $$('#content > table > tbody > tr > td:nth-child(2) > table > tbody > tr').each(function(a, b) {
          hasil.push({
            ...
            image: $$(b).find('a').attr('href') || $(b).find('img').attr('src') || $$(b).find(
              'img').attr('data-src')
          })
        })
        resolve(hasil)
...
async function fandom(q) {
  return new Promise((resolve, reject) => {
    axios.get(`https://jump.fandom.com/wiki/Special:Search?query=${q}&scope=internal&navigationSearch=true`).then(
      (get) => {
        let $ = cheerio.load(get.data)
        let getres = []
        ...
        axios.get(random).then((res) => {
          ...
          let hasil = {
            title: $$(
              'body > div.main-container > div.resizable-container > div.page.has-right-rail > main > div.page-header > div.page-header__bottom > div.page-header__title-wrapper > h1'
              ).text().trim(),
            info: $$('#mw-content-text > div > p:nth-child(2)').text(),
            plot: $$('#mw-content-text > div > p:nth-child(6)').text(),
            image: $$('#mw-content-text > div > table > tbody > tr:nth-child(1) > td > a').attr('href') ||
              $$('#mw-content-text > div > table > tbody > tr:nth-child(1) > td > a > img').attr('src'),
            part,
            info,
            lisensi: $$(
      ...
      axios.get(random).then((res) => {
        ...
        $$('#content > table > tbody > tr > td:nth-child(2) > table').each(function(a, b) {
          voice.push({
            ...
            image: $$(b).find('td:nth-child(1) > div > a > img').attr('data-src') || $$(b).find(
              'td:nth-child(1) > div > a > img').attr('src')
          })
        })
        let animeography = []
        $$('#content > table > tbody > tr > td.borderClass > table:nth-child(6) > tbody > tr').each(
          function(a, b) {
            animeography.push({
              name: $$(b).find('td:nth-child(2) > a').text(),
        ...
        let mangaography = []
        $$('#content > table > tbody > tr > td.borderClass > table:nth-child(9) > tbody > tr').each(
          function(a, b) {
            mangaography.push({
              name: $$(b).find('td:nth-child(2) > a').text(),
              status: $$(b).find('td:nth-child(2) > div > small').text(),
        ...
        let hasil = {
          ...
          image: $$('#content > table > tbody > tr > td.borderClass > div:nth-child(1) > a > img').attr(
              'data-src') || $$(
              '#content > table > tbody > tr > td.borderClass > div:nth-child(1) > a > img').attr(
            'src'),
          detail: $$('#content > table > tbody > tr > td:nth-child(2)').text().split('Characters')[1]
            .split('Voice Actors')[0]?.trim(),
          voice_actor: voice,
          animeography,
          mangaography
        }
    ...
    axios.get('https://myanimelist.net/topanime.php?type=' + type).then(({
      ...
      $('tr.ranking-list').each(function(a, b) {
        hasil.push({
          ...
          info: $(b).find('td.title.al.va-t.word-break > div > div.information.di-ib.mt4').text()
            .trim(),
          rating: $(b).find('td.score.ac.fs14 > div').text(),
          detail: $(b).find('td.title.al.va-t.word-break > div > div.di-ib.clearfix > h3 > a').attr(
            'href'),
          image: $(b).find('td.title.al.va-t.word-break > a > img').attr('data-src') || $(b).find(
            'td.title.al.va-t.word-break > a > img').attr('src')
        })
      })
      resolve(hasil)
    })
    ...
    axios.get('https://myanimelist.net/topmanga.php?type=' + type).then(({
      ...
      $('tr.ranking-list').each(function(a, b) {
        hasil.push({
          ...
          info: $(b).find('td.title.al.va-t.clearfix.word-break > div > div.information.di-ib.mt4')
            .text().trim(),
          rating: $(b).find('td.score.ac.fs14 > div').text(),
          detail: $(b).find('td.title.al.va-t.clearfix.word-break > div > h3 > a').attr('href'),
          image: $(b).find('td.title.al.va-t.clearfix.word-break > a > img').attr('data-src') || $(b)
            .find('td.title.al.va-t.clearfix.word-break > a > img').attr('src')
        })
      })
      resolve(hasil)
    })
