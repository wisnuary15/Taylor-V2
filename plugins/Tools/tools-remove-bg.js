...
const handler = async (m, {
  ...
  try {
    let buffer, media = await q?.download(),
      sauce = await RemoveBackground(media) || await RemoveBackgroundV2(media) || await rembg(media) || await removebg(media);
...
handler.help = ["rmvbg"], handler.tags = ["tools"], handler.command = ["rmvbg"];
