...
const handler = async (m, { args: args }) => {
  ...
  try {
    const response = await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${args[0]}&units=metric&appid=060a6bcfa19809c2cd4d97a212b19273`),
      { name: name, sys: { country: country }, weather: [{ description: description }], main: { temp: temp, temp_min: temp_min, temp_max: temp_max, humidity: humidity }, wind: { speed: speed } } = response.data,
