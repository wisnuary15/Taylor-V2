...
const handler = async (m, {
  ...
  try {
    ...
    if (query.length >= 1) {
      ...
      if (isNaN(indexRepo) || indexRepo < 1 || indexRepo > repoList.length) {
        ...
        const repoListMessage = "List Repo (gunakan angka untuk memilih):\n" + repoList.map((repo, index) =>
          `${index + 1}. ${repo}`).join('\n');
        m.reply(errorMessage + repoListMessage);
        return;
      }
      ...
      if (isNaN(indexGetApk) || indexGetApk < 1 || indexGetApk > apkList.length) {
        const errorMessage =
          `Index APK tidak valid. Gunakan angka dari 1 hingga ${apkList.length} untuk memilih APK dari list.`;
        m.reply(errorMessage);
        return;
      }
    ...
    console.error(error);
    m.reply("Terjadi kesalahan dalam menjalankan perintah. Silakan coba lagi nanti.");
  }
}
...
async function getApk(repo, page) {
  try {
    ...
    return $('.approw').map((index, element) => {
      ...
      const downloadLinks = $(element).find('.paddedlink[href^="https://"]').map((index, linkElement) => $(
        linkElement).attr('href')).get();
      return {
        title,
        version,
        date,
