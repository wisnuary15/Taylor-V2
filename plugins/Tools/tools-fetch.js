...
const handler = async (m, {
    ...
    try {
        ...
        for (const url of totalLinks) {
            for (const fetcher of fetchers) {
                try {
                    ...
                    const txt = fetcher.lib === got ? response.body : fetcher.lib === axios ? response.data : await response.text();
                    if (!txt || !txt.trim()) {
        ...
        for (const [id, {
            ...
            const finalContentDisposition = contentDisposition && contentDisposition.match(/filename[^;=\n]*=(["']?)(.*?)\1/)?.[2] || '';
            if (/^(text\/(plain|html|xml)|application\/(json|.*\+xml)|.*\/(javascript|xml|x-www-form-urlencoded))/.test(finalContentType)) {
                if (finalContentLength > 65536) {
                    ...
                    conn.sendFile(m.chat, Buffer.from(txt), finalContentDisposition || finalContentType || 'Tidak diketahui', caption, m);
                } else {
                    replyCount++;
                    await delay(1024);
                    let parsedTxt;
        ...
        m.reply(`Fetching completed in ${formatDuration(elapsedTime * 1000)}. Successfully fetched ${successCount} out of ${totalLinks.length} links.\n📝 Replies Sent: ${replyCount}\n📈 Media Sent: ${mediaCount}`);
    } catch (e) {
        throw e;
    }
};
