...
const parseOptions = (inputText) => {
  ...
  inputText.split('--').slice(1).forEach(opt => {
    const [key, ...values] = opt.trim().split(' ');
    const value = values.join(' ').replace(/(^"|"$)/g, ''); // remove surrounding quotes
    switch (key) {
      case 'method':
      case 'm':
      ...
      for (const fetcher of fetchers) {
        try {
          const response = await fetcher.lib(url, {
            ...
            body: options.form || (options.method === 'POST' ? JSON.stringify(options.data) : null),
            redirect: options.redirect
          });
          const contentType = response.headers.get('content-type');
          const contentDisposition = response.headers.get('content-disposition');
          const contentLength = response.headers.get('content-length');
          const txt = fetcher.lib === got ? response.body : fetcher.lib === axios ? response.data : await response
            .text();
          if (!txt || !txt.trim()) {
          ...
          successCount++;
          break; // If a fetcher succeeds, break out of the loop
        } catch (err) {
          console.error(`Error fetching link: ${url}`, err);
          continue; // Try next fetcher if current one fails
        }
      }
    }
    for (const [id, {
      ...
      const finalContentType = contentType && contentType.split(';')[0]?.trim() || '';
      const finalContentDisposition = contentDisposition && contentDisposition.match(
        /filename[^;=\n]*=(["']?)(.*?)\1/)?.[2] || '';
      if (/^(text\/(plain|html|xml)|application\/(json|.*\+xml)|.*\/(javascript|xml|x-www-form-urlencoded))/.test(
          finalContentType)) {
        if (finalContentLength > 65536) {
          ...
          conn.sendFile(m.chat, Buffer.from(txt), finalContentDisposition || finalContentType || 'Tidak diketahui',
            caption, m);
        } else {
          replyCount++;
          await delay(1024);
        ...
        const caption =
          `🔗 *Link:* ${url}\n📄 *Type:* ${finalContentType}\n📊 *Size:* ${formatSize(finalContentLength)}`;
        conn.sendFile(m.chat, url, finalContentDisposition || finalContentType || 'Tidak diketahui', caption, m);
      }
    }
    const elapsedTime = (Date.now() - Date.now()) / 1000;
    m.reply(
      `Fetching completed in ${formatDuration(elapsedTime * 1000)}. Successfully fetched ${successCount} out of ${totalLinks.length} links.\n📝 Replies Sent: ${replyCount}\n📈 Media Sent: ${mediaCount}`
      );
  } catch (e) {
    throw e;
  }
