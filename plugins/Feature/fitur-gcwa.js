...
async function scrapeData(q) {
  try {
    const response = await fetch("http://ngarang.com/link-grup-wa/daftar-link-grup-wa.php?search=" + q +
      "&searchby=name") // Ganti URL dengan URL yang sesuai
    const html = await response.text()
    const $ = cheerio.load(html)
    return $(".wa-chat").map((index, element) => ({
      currentTitle: $(element).find(".wa-chat-body .wa-chat-title-container .wa-chat-title .wa-chat-title-text")
        .text().trim(),
