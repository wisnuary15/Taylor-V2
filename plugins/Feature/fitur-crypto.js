...
const handler = async (m, {
  ...
  if (!text) {
    return m.reply("🔍 *Cari Cryptocurrency*\n\nContoh penggunaan:\n*.crypto bitcoin*");
  }
  m.react(wait);
  try {
    const results = await coingecko(text);
    if (!results || results.length === 0) {
      m.reply("⚠️ *Tidak ditemukan*: Cryptocurrency yang kamu cari tidak tersedia.");
      return;
    }
    const message = results.map(result => `💰 *${result.cryptoName}*\n📈 Perubahan Harga: ${result.priceChange}\n`).join("\n");
...
async function coingecko(search) {
  ...
  try {
    ...
    const result = data.filter(crypto => crypto.name.toLowerCase().includes(search.toLowerCase())).map(crypto => ({
