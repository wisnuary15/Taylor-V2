...
const handler = async (m, { conn: conn, groupMetadata: groupMetadata, usedPrefix: usedPrefix, text: text, args: args, command: command }) => {
  let who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? conn.user.jid : m.sender,
    name = (await conn.profilePictureUrl(who).catch((_ => hoppai.getRandom())),
      conn.getName(who));
  flaaa.getRandom();
  if ("createqr" === command) {
  ...
  if ("nekos" === command) {
    ...
    let f = await fetch(`https://nekos.best/api/v2/${args[0]}`),
      x = await f.json();
    "neko" === args[0] && await conn.sendFile(m.chat, x.results[0]?.url, "", x.results[0]?.anime_name, m),
      "waifu" === args[0] && await conn.sendFile(m.chat, x.results[0]?.url, "", x.results[0]?.anime_name, m),
      "kitsune" === args[0] && await conn.sendFile(m.chat, x.results[0]?.url, "", x.results[0]?.anime_name, m);
    try {
