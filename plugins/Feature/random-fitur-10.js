...
const handler = async (m, {
  ...
  if (command === 'createqr') {
    ...
    conn.sendFile(m.chat, res, 'image.png', "Your Text: \n" + text, m)
  }
  if (command === 'catboys') {
    if (args[0] === 'img') {
      let f = await fetch(`https://api.catboys.com/img`)
      let x = await f.json()
      conn.sendFile(m.chat, x.url, 'image.png', wm, m)
    }
    if (args[0] === '8ball') {
...
`
    ...
    let x = await f.json()
    if (args[0] === 'neko') {
      conn.sendFile(m.chat, x.results[0]?.url, '', x.results[0]?.anime_name, m)
    }
    if (args[0] === 'waifu') {
      conn.sendFile(m.chat, x.results[0]?.url, '', x.results[0]?.anime_name, m)
    }
    ...
    try {
      conn.sendFile(m.chat, x.results[0]?.url, '', x.results[0]?.anime_name, false, {
        mimetype: 'image/gif',
  ...
  if (command === 'avatar') {
    ...
    let av = ["https://crafatar.com/avatars/", "https://crafatar.com/renders/head/",
      "https://crafatar.com/renders/body/", "https://crafatar.com/skins/", "https://crafatar.com/capes/"
    ]
