...
const handler = async (m, { conn: conn, usedPrefix: usedPrefix, text: text, args: args, command: command }) => {
  if ("sfx" === command) {
    ...
    try {
      ...
      if (!(choice >= 0 && choice < dapet.length)) return m.reply(`⚠️ Pilihan tidak valid. Silakan pilih nomor urutan yang benar dari daftar berikut:\n\n${list}\nKetik ulang *${usedPrefix + command}* teks anda untuk mengubah teks lagi`);
      {
        let v = dapet[choice],
          teks = `✨ *Name :* ${v.name}\n🌈 *Color :* ${v.color}\n🔗 *Slug :* ${v.slug}\n📜 *Description :* ${v.description}`;
        m.reply(teks), await conn.sendFile(m.chat, v.sound, "", "", m);
      }
    } catch (e) {
  ...
  if ("sfx2" === command) {
    ...
    if (!(choice >= 0 && choice < dapet.length)) return m.reply(`⚠️ Pilihan tidak valid. Silakan pilih nomor urutan yang benar dari daftar berikut:\n\n${list}\nKetik ulang *${usedPrefix + command}* teks anda untuk mengubah teks lagi`);
    {
      let v = dapet[choice],
        teks = `${v.name}\n🆔 *ID:* ${v.id}\n📜 *LICENSE:* ${v.license}\n👤 *USERNAME:* ${v.username}\n🏷️ *TAGS:* ${Array.from(v.tags)}\n`;
      m.reply(teks), await conn.sendFile(m.chat, `http://freesound.org/apiv2/sounds/${v.id}/?token=I4LLx1YDPjNbkBCuK0zYbQAV9njbRLJ9ZhctDhGP`, "", "", m);
    }
  }
  if ("sfx3" === command) {
