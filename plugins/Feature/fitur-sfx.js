...
const handler = async (m, {
    ...
    if (command === "sfx2") {
        ...
        if (choice >= 0 && choice < dapet.length) {
            ...
            let teks = `${v.name}\n🆔 *ID:* ${v.id}\n📜 *LICENSE:* ${v.license}\n👤 *USERNAME:* ${v.username}\n🏷️ *TAGS:* ${Array.from(v.tags)}\n`
            m.reply(teks)
            conn.sendFile(m.chat, `http://freesound.org/apiv2/sounds/${v.id}/?token=I4LLx1YDPjNbkBCuK0zYbQAV9njbRLJ9ZhctDhGP`, '', '', m)
        } else {
            return m.reply(`⚠️ Pilihan tidak valid. Silakan pilih nomor urutan yang benar dari daftar berikut:\n\n${list}\nKetik ulang *${usedPrefix + command}* teks anda untuk mengubah teks lagi`)
        }
    }
    if (command === "sfx3") {
        ...
        if (choice >= 0 && choice < hasil.length) {
            ...
            conn.sendFile(m.chat, `https://raw.githubusercontent.com/AyGemuy/HAORI-API/main/audio/${sound}.mp3`, sound + '.mp3', teks, m)
        } else {
            return m.reply(`⚠️ Pilihan tidak valid. Silakan pilih nomor urutan yang benar dari daftar berikut:\n\n${list}\nKetik ulang *${usedPrefix + command}* teks anda untuk mengubah teks lagi`)
        }
    }
    if (command === "sfx4") {
        ...
        if (choice >= 0 && choice < hasil.length) {
            ...
            conn.sendFile(m.chat, `https://raw.githubusercontent.com/AyGemuy/HAORI-API/main/sound/sound${sound}.mp3`, 'sound' + sound + '.mp3', teks, m)
        } else {
            return m.reply(`⚠️ Pilihan tidak valid. Silakan pilih nomor urutan yang benar dari daftar berikut:\n\n${list}\nKetik ulang *${usedPrefix + command}* teks anda untuk mengubah teks lagi`)
        }
    }
    if (command === "sfx5") {
        ...
        if (choice >= 0 && choice < hasil.length) {
            let sound = hasil[choice]
            let teks = `✨ *Sound :* Sound ke- ${sound}\n✍️ *By :* ${author}`
            conn.sendFile(m.chat, `https://raw.githubusercontent.com/WH-MODS-BOT/Sounds/main/sound${sound}.mp3`, 'sound' + sound + '.mp3', teks, m)
        } else {
            return m.reply(`⚠️ Pilihan tidak valid. Silakan pilih nomor urutan yang benar dari daftar berikut:\n\n${list}\nKetik ulang *${usedPrefix + command}* teks anda untuk mengubah teks lagi`)
        }
    }
    if (command === "smap") {
        ...
        conn.sendFile(m.chat, `https://static-maps.yandex.ru/1.x/?lang=id-ID&ll=${xx[1][0]?.longitude},${xx[1][0]?.latitude}&z=12&l=map&size=600,300`, '', teks, m)
    }
}
