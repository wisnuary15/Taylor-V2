  ...
  try {
    ...
    switch (command) {
      ...
      case "youporndl":
        ...
        buffer = (await conn.getFile(item.videoUrl)).data || (await conn.getFile("https://cors.caliph.my.id/" + item.videoUrl)).data;
        await conn.sendMessage(m.chat, {
          video: buffer,
          caption: teksdl + " 📥"
        }, {
...
async function youpornSearch(q) {
  try {
    const response = await fetch(`https://cors.caliph.my.id/https://www.youporn.com?query=${q}`);
  ...
  try {
    ...
    if (jsonDataMatch) {
      try {
        ...
        const responseD = await fetch(`https://cors.caliph.my.id/${data}`);
