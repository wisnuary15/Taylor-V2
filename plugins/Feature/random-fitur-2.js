...
const handler = async (m, {
  ...
  if (command === 'ceritahoror') {
    let res = await fetch(`https://api.lolhuman.xyz/api/ceritahoror?apikey=${lolkey}`)
  ...
  *Story:* ${has.story}`, author, has.thumbnail, [
  ...
  if (command === 'growiki') {
    ...
    let res = await fetch(`https://api.lolhuman.xyz/api/growiki?apikey=${lolkey}&query=${text}`)
    let sul = await res.json()
    let has = sul.result
...
`, author, has.img, [
  ...
  if (command === 'growstocks') {
    if (!text) throw `Contoh penggunaan ${usedPrefix}${command} magplant`
    let res = await fetch(`https://api.lolhuman.xyz/api/growstocks?apikey=${lolkey}&query=${text}`)
    let sul = await res.json()
    let has = sul.result
    await conn.sendButton(m.chat, `*Name:* ${has.name}
  *Desc:* ${has.desc}
...
`, author, has.img, [
  ...
  if (command === 'gsearch') {
    if (!text) throw `Contoh penggunaan ${usedPrefix}${command} magplant`
    let res = await fetch(`https://api.lolhuman.xyz/api/gsearch?apikey=${lolkey}&query=${text}`)
    let sul = await res.json()
    let has = sul.result
...
`, author, null, [
  ...
  if (command === 'gsmarena') {
    ...
    let res = await fetch(`https://api.lolhuman.xyz/api/gsmarena?apikey=${lolkey}&query=${text}`)
    let sul = await res.json()
    let has = sul.result
...
`, author, has.phone_image, [
  ...
  if (command === 'jadwalbola') {
    let f = await fetch(`https://api.lolhuman.xyz/api/jadwalbola?apikey=${lolkey}`)
    let xx = await f.json()
    let v = xx.result
    let teks = v.map(v => {
      return `
*Hari:* ${v.hari}
  ...
  *Tv:* ${v.tv}
  ...
  if (command === 'jadwaltv') {
    let json = await fetch(`https://api.lolhuman.xyz/api/jadwaltv/now?apikey=${lolkey}`)
  ...
  *tvri:* ${has.result.tvri}
  ...
  if (command === 'jalantikus') {
    let f = await fetch(`https://api.lolhuman.xyz/api/jalantikus?apikey=${lolkey}`)
    let xx = await f.json()
    let v = xx.result
    let teks = v.map(v => {
      return `
*Name:* ${v.title}
  ...
  *category:* ${v.category}
  ...
  if (command === 'jaraktempuh') {
    ...
    let f = await fetch(
      `https://api.lolhuman.xyz/api/jaraktempuh?apikey=${lolkey}&kota1=${args[0]}&kota2=${args[1]}`)
    let xx = await f.json()
    let v = xx.result
    let teks = v.map(v => {
      return `
...
handler.command = handler.help = ['ceritahoror', 'growiki', 'growstocks', 'gsearch', 'gsmarena', 'jadwalbola',
  'jadwaltv', 'jalantikus', 'jaraktempuh', 'random', 'random2'
]
handler.tags = ['random']
