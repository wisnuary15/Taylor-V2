...
const handler = async (m, {
  ...
  if (command === 'nhentai') {
    ...
    try {
      let gas = await fetch(`https://api.lolhuman.xyz/api/nhentai/${args[0]}?apikey=${lolkey}`)
  ...
  if (command === 'wikisearch') {
    let res = await fetch('https://id.wikipedia.org/w/api.php?action=query&list=search&srsearch=' + text +
      '&format=json')
    let json = await res.json()
    let results = Object.values(json.query.search).map((v) =>
      `*Title:*\n${v.title}\n*Snippet:*\n${v.snippet}\n*Time:*\n${v.timestamp}`)
