...
const handler = async (m, {
  ...
  try {
    ...
    switch (command) {
      ...
      case "emoji":
      case "emojis":
        const emojiUrlList = await searchEmoji(args[0]);
      ...
      case "emojigg":
        const emojiGgResults = await EmojiGG(args[0]);
        if (!emojiGgResults.length) {
          return m.reply("❌ Emoji tidak ditemukan atau input tidak valid. Silakan coba lagi.");
        }
        if (!args[1]) {
          const buttons = conn.ctaButton.setBody(`Daftar vendor untuk *${args[0]}*:\n\n${emojiGgResults.map((vendor, index) => `*${index + 1}.* ${vendor.title}`).join("\n")}\n\nContoh: *${usedPrefix + command}* [emoji] [vendor]`).setFooter("Klik tombol di bawah untuk memilih vendor.").addSelection("Pilih Vendor").makeSections("Daftar Vendor", "Pilih Vendor");
          emojiGgResults.forEach((vendor, index) => buttons.makeRow("", vendor.title, `Pilih ${vendor.title}`, `${usedPrefix + command} ${args[0]} ${index + 1}`));
          return buttons.run(m.chat, conn, m);
        }
        ...
        m.react(wait);
        await m.reply(`Informasi emoji untuk *${args[0]}* (${selectedGgVendor.title}):\n\n*Description:* ${selectedGgVendor.description}\n*Gambar:* ${selectedGgVendor.image}`);
        sticker = await createSticker(null, selectedGgVendor.image, packname, m.name, 60);
        return m.reply(sticker);
      case "emojipedia":
        const emojiPediaResults = await emojiPedia(args[0]);
        if (!emojiPediaResults.length) {
          return m.reply("❌ Emoji tidak ditemukan atau input tidak valid. Silakan coba lagi.");
        }
        if (!args[1]) {
          const buttons = conn.ctaButton.setBody(`Daftar vendor untuk *${args[0]}*:\n\n${emojiPediaResults.map((vendor, index) => `*${index + 1}.* ${vendor.name}`).join("\n")}\n\nContoh: *${usedPrefix + command}* [emoji] [vendor]`).setFooter("Klik tombol di bawah untuk memilih vendor.").addSelection("Pilih Vendor").makeSections("Daftar Vendor", "Pilih Vendor");
          emojiPediaResults.forEach((vendor, index) => buttons.makeRow("", vendor.name, `Pilih ${vendor.name}`, `${usedPrefix + command} ${args[0]} ${index + 1}`));
          return buttons.run(m.chat, conn, m);
        }
      ...
      case "emojiall":
        ...
        if (!args[1]) {
          ...
          allVendors.forEach((vendor, index) => buttons.makeRow("", vendor.name, `Pilih ${vendor.name}`, `${usedPrefix + command} ${args[0]} ${index + 1}`));
          return buttons.run(m.chat, conn, m);
        }
