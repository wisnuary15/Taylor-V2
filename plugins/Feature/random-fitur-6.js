...
*Result:* ${x.conversion_result}`
    conn.sendFile(m.chat, logo, "", "*[ Result ]*\n" + caption, m)
  }
...
*Ip:* ${x.ip}`
  ...
  if (command === "emojimix3") {
    ...
    let anu = await fetchJson(
      `https://tenor.googleapis.com/v2/featured?key=AIzaSyAyimkuYQYF_FXVALexPuGQctUWRURdCYQ&contentfilter=high&media_filter=png_transparent&component=proactive&collection=emoji_kitchen_v5&q=${encodeURIComponent(args[0])}`
      )
  ...
  if (command === "emojimix4") {
    ...
    let stiker = await sticker(false, anu.result, packname, author)
    conn.sendFile(m.chat, stiker, "sticker.webp", "", m)
  }
  if (command === "calc2") {
  ...
  if (command === "gqr") {
    ...
    let caption = `*Hasil:*`
    conn.sendFile(m.chat, img, "", "*[ Result ]*\n" + caption, m)
  }
  if (command === "jamdunia") {
    if (!text) throw `Contoh penggunaan ${usedPrefix}${command} query`
    m.react(wait)
    ...
    let caption = xx.result.map((v, index) => {
      return `
*time:* ${v.time}
*name:* ${v.name}
*timeZone:* ${v.timeZone}
...
*Awards:* ${x.Awards}
`
    conn.sendFile(m.chat, x.Poster, "", "*[ Result ]*\n" + caption, m)
  }
  if (command === "mvsearch2") {
    ...
    let f = await fetch(`https://www.omdbapi.com/?apikey=742b2d09&t=${text}&plot=full`)
    let x = await f.json()
    let caption = x.results.map((v, index) => {
      return `
...
`
    ...
    conn.sendFile(m.chat, "https://image.tmdb.org/t/p/w500/" + x.poster_path, "", "*[ Result ]*\n" + caption, m)
  }
  if (command === "lmaker") {
...
1 - 6 : kgf`)
    ...
    let caption = `*⎔┉━「 ${command} 」━┉⎔*
🤠 *Query* : ${thm}`
    conn.sendFile(m.chat, images, "", "*[ Result ]*\n" + caption, m)
  }
  if (command === "quotes") {
    let x = await Quotes()
...
🤠 ${x.author}`
    conn.sendFile(m.chat, logo, "", "*[ Result ]*\n" + caption, m)
    if (args[0] === "islami") {
      let ai = await (await fetch(API("lolhuman", "/quotes/islami", {}, "apikey"))).json()
...
🤠 ${author}`
  ...
  if (command === "wallhaven") {
    ...
    if (input) {
      ...
      } else return m.reply("*Example:*\n.wallhaven |hot\n\n*Pilih type yg ada*\n" + arr.map((v, index) => "  ○ " +
        v).join("\n"))
      let f = await fetch(ling)
      let x = await f.json()
      let caption = `Copyright: ${x.text.copyright}`
      conn.sendFile(m.chat, x.data.pic, "", "*[ Result ]*\n" + caption, m)
    } else {
      var ure
      try {
      ...
      let x = await f.json()
      let caption = x.data.map((v, index) => {
        return `
ID: ${v.id}
Views: ${v.views}
Category: ${v.category}
...
`
      ...
      conn.sendFile(m.chat, x.data[0]?.path, "", "*[ Result ]*\n" + caption, m)
    }
  }
}
handler.command = handler.help = ["exchange", "ipcountry", "emojimix3", "emojimix4", "calc2", "mvsearch", "mvsearch2",
  "lmaker", "quotes", "jtmeme", "wallhaven", "jamdunia", "gqr"
]
handler.tags = ["tools"]
