...
`
        listSections.push(["Num. " + ++index, [
          [x.result.recitations[0]?.name + "\n\n", usedPrefix + "get " + x.result.recitations[0]
            ?.audio_url, cp
          ],
          [x.result.recitations[1].name + "\n\n", usedPrefix + "get " + x.result.recitations[1].audio_url,
            cp
          ],
          [x.result.recitations[2].name + "\n\n", usedPrefix + "get " + x.result.recitations[2].audio_url,
            cp
          ],
        ]])
      })
      return conn.sendList(m.chat, htki + " 🗒️ List Ayat " + htka, "⚡ Silakan pilih ayat dari " + x.result.name +
        " ( " + x.result.name_translations.ar + " )", author, "[ Ayat ]", listSections, m)
    }
    if (args[0] === 'likeedl') {
      if (!one) return m.reply(
        `Example: ${usedPrefix + command} ${args[0]} |https://likee.video/@vicky_marpaung/video/7006676628722311449`
        )
...
`
      conn.sendFile(m.chat, v.medias[0]?.url, '', caption, m)
    }
    if (args[0] === 'storysad') {
...
*respon:* ${v.respon}
`
      conn.sendFile(m.chat, v.respon, '', caption, m)
    }
    if (args[0] === 'textpro2') {
...
*weapon:* ${x.weapon}
`
    conn.sendFile(m.chat, x.image, '', caption, m)
  }
  if (command === 'ytreels') {
...
*size:* ${x.result.size}
`
    conn.sendFile(m.chat, x.result.link, '', caption, m)
  }
}
