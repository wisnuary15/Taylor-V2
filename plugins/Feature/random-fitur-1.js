...
const handler = async (m, { conn: conn, groupMetadata: groupMetadata, usedPrefix: usedPrefix, text: text, args: args, command: command }) => {
  ...
  if ("asmaulhusna" === command) {
    let xf = await asmaulhusna(),
      { index: index, latin: latin, arabic: arabic, translation_id: translation_id, translation_en: translation_en } = xf,
      teks = `\n  ${index}\n  ${latin}\n  ${arabic}\n  ${translation_id}\n  ${translation_en}\n`;
  ...
  if ("memeindo" === command) {
    let caption = `Nihh banh ${command} nya`,
      url = `https://api.lolhuman.xyz/api/meme/memeindo?apikey=${lolkey}`;
    await conn.sendFile(m.chat, url, "", caption, m);
  }
  if ("randommeme" === command) {
    let caption = `Nihh banh ${command} nya`,
      url = `https://api.lolhuman.xyz/api/random/meme?apikey=${lolkey}`;
    await conn.sendFile(m.chat, url, "", caption, m);
  }
  if ("memedarkjoke" === command) {
    let caption = `Nihh banh ${command} nya`,
      url = `https://api.lolhuman.xyz/api/meme/darkjoke?apikey=${lolkey}`;
    await conn.sendFile(m.chat, url, "", caption, m);
  }
  if ("beasiswa" === command) {
  ...
  if ("stimker" === command) {
    let apilol = "https://api.lolhuman.xyz/api/sticker/",
      lis = [apilol + "anjing?apikey=" + lolkey, apilol + "patrick?apikey=" + lolkey, apilol + "amongus?apikey=" + lolkey, apilol + "gawrgura?apikey=" + lolkey, apilol + "bucinstick?apikey=" + lolkey],
