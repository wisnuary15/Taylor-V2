...
async function asmaulhusna(index) {
  if (!asmaulhusnajson) {
    const response = await fetch("https://raw.githubusercontent.com/BochilTeam/database/master/religi/asmaulhusna.json");
    if (!response.ok) {
      throw new Error(`Failed to fetch asmaulhusna data: ${response.statusText}`);
    }
    asmaulhusnajson = await response.json();
  }
  if (index === undefined) {
