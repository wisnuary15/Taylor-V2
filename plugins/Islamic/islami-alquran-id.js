    ...
    switch (commandArgs) {
      case 'all':
        ...
        allData.forEach((surah) => {
          replyMsg +=
            `Surah ${surah.surahNumber}: ${surah.name} (${surah.translation}) - ${surah.numberOfAyahs} ayahs\n`;
        });
        break;
      case 'specific':
        if (!args[1]) {
          replyMsg = 'Masukkan nomor surah untuk perintah specific.';
        } else {
          const surahNumber = parseInt(args[1]);
          const specificSurahData = await quran.Specific(surahNumber);
          ...
          replyMsg +=
            `Bismillah: ${specificSurahData.bismillah.arab} (${specificSurahData.bismillah.translation})\n`;
          replyMsg += `Bismillah Audio: ${specificSurahData.bismillah.audio.alafasy}\n`;
        }
        break;
      case 'ayahs':
        if (!args[1]) {
          replyMsg = 'Masukkan nomor surah untuk perintah ayahs.';
        } else {
          const surahNumber = parseInt(args[1]);
          const ayahsData = await quran.Ayahs(surahNumber);
          replyMsg = `Ayahs:\n`;
      ...
      case 'specificayah':
        if (!args[1] || !args[2]) {
          replyMsg = 'Masukkan nomor surah dan nomor ayah untuk perintah specificayah.';
        } else {
          const surahNumber = parseInt(args[1]);
          const ayahNumber = parseInt(args[2]);
          const specificAyahData = await quran.SpecificAyah(surahNumber, ayahNumber);
          replyMsg =
            `Ayah ${specificAyahData.number.inSurah} of Surah ${specificAyahData.name} (${specificAyahData.translation}):\n`;
      ...
      default:
        replyMsg =
          'Perintah tidak valid. Gunakan `quranid all`, `quranid specific [nomor_surah]`, `quranid ayahs [nomor_surah]`, `quranid specificayah [nomor_surah] [nomor_ayah]`, atau `quranid random`.';
        break;
    }
  } catch (error) {
    console.error('Error:', error);
    replyMsg = 'Terjadi kesalahan saat memproses perintah. Silakan coba lagi.';
  }
  m.reply(replyMsg);
};
handler.help = ["quranid"];
handler.tags = ["quran"];
handler.command = /^(quranid)$/i;
