...
const handler = async (m, {
  ...
  try {
    const [kitabIndex, page = 0, index] = text.split("|").map(str => str.trim());
    if (!kitabIndex) {
      const kitabList = await getKitabList();
      const buttons = conn.ctaButton.setBody("🌐 *Daftar Kitab:*").setFooter("⚡ Pilih kitab berdasarkan nomor indeks:").addSelection("Klik di sini").makeSections("Pilih Kitab", "kitabList");
      kitabList.forEach((kitab, idx) => {
        buttons.makeRow("", `📚 ${kitab.nama_kitab}`, `Pilih Kitab ${idx + 1}`, `${usedPrefix + command} ${idx + 1}`);
      });
      return buttons.run(m.chat, conn, m);
    }
    const kitabList = await getKitabList();
    const kitabIndexInt = parseInt(kitabIndex);
    if (isNaN(kitabIndexInt) || kitabIndexInt < 1 || kitabIndexInt > kitabList.length) {
    ...
    if (isNaN(selectedIndex) || selectedIndex < 1 || selectedIndex > result?.data.length) {
      const buttons = conn.ctaButton.setBody("🌐 *Daftar Hadits:*").setFooter("⚡ Pilih hadits berikut:").addSelection("Klik di sini").makeSections("Pilih Hadits", "haditsList");
      result?.data.forEach((h, idx) => {
        buttons.makeRow("", `📜 ${h.kitab}`, `Lihat Hadits ${idx + 1}`, `${usedPrefix + command} ${kitabIndex}|${page}|${idx + 1}`);
      });
      if (result.current_page < result.last_page) {
        buttons.makeRow("", "➡️ Halaman Berikut", `Lihat Halaman ${result.current_page + 1}`, `${usedPrefix + command} ${kitabIndex}|${result.current_page + 1}`);
      }
      return buttons.run(m.chat, conn, m);
    }
