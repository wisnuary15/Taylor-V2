...
const handler = async (m, {
  ...
  if (!text) return conn.reply(m.chat, `Masukkan teks atau reply pesan.\nContoh:\n${usedPrefix}${command} jakarta\natau\n${usedPrefix}${command} 1\natau\n${usedPrefix}${command} 2 3`, m);
  m.react(wait);
  try {
    ...
    if (surahInput) {
      ...
      const ayatNumber = ayatInput && ayatInput.length > 1 ? ayatInput[1] : null;
      const alquran = await umrotix.alquran(surahNumber, ayatNumber);
      const pesanAyat = ayatNumber ? `\`${alquran.teks}\`\n- ${alquran.terjemahan}` : alquran.map(el => `\`${el.teks}\`\n- ${el.terjemahan}`).join("\n\n");
      conn.reply(m.chat, `📖 *Al-Quran Surah ${surahNumber}*:\n\n${pesanAyat}`, m);
    } else {
      const jadwal = await umrotix.jadwalSholat(text);
      const pesanJadwal = jadwal.map(el => `⏰ *${el.nama}:* ${el.waktu}`).join("\n");
