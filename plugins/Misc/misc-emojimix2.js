...
const handler = async (m, {
  ...
  let examples = "Input wsf or wbs\n*example:*\n" + usedPrefix + command + " " + emo + usedPrefix + emo +
    usedPrefix + arrs.getRandom()
  let example = "Input emoji\n*example:*\n" + usedPrefix + command + " " + emo + usedPrefix + emo
  if (a && b) {
    try {
      if (!c) {
        m.react(wait)
        let anu = await (await fetch("https://api.andeer.top/API/emojimix.php?emoji1=" + encodeURIComponent(a) +
          "&emoji2=" + encodeURIComponent(b))).json()
        if (anu.code !== 1) return m.reply("*Can't mix these 2 emojis!*");
