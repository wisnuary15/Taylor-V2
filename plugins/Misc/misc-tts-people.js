...
const handler = async (m, {
  ...
  let text = args.length >= 1 ? args.slice(0).join(" ") : m.quoted?.text || m.quoted?.caption || m.quoted?.description || null;
  if (!text) {
  ...
  try {
    ...
    switch (command) {
      case "mrbeast":
        res = await fetchAudioBuffer("https://sandipbaruwal.onrender.com/beast?text=", text, m);
        break;
      case "mrbeastv2":
        res = await fetchAudioJSON("https://www.api.vyturex.com/beast?query=", text, m);
        break;
      case "snoop":
        res = await fetchAudioBuffer("https://sandipbaruwal.onrender.com/snoop?text=", text, m);
        break;
      case "jane":
        res = await fetchAudioBuffer("https://sandipbaruwal.onrender.com/jane?text=", text, m);
        break;
      default:
    ...
    m.reply("Terjadi kesalahan saat memproses permintaan."), console.error("An error occurred:", e);
  }
};
handler.help = ["mrbeast <teks>", "mrbeastv2 <teks>", "snoop <teks>", "jane <teks>"];
handler.tags = ["tools"];
