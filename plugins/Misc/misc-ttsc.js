...
const fakeYouToken = "187b56b2217ac09dbe6ae610f19b35dfbc53cdd5857f818f03b45d048287b4bc",
  handler = async (m, { conn: conn, isOwner: isOwner, usedPrefix: usedPrefix, command: command, args: args }) => {
    ...
    const { modelToken: modelToken, title: title } = await getModelByIndex(ListVoice, atas);
    m.reply(wait + "\n" + title);
    try {
...
async function getModelByIndex(arrayObject, index) {
  ...
  if (model) {
    const { model_token: model_token, title: title } = model;
    return {
...
async function requestSpeech(voice, message) {
  return new Promise((async (resolve, reject) => {
    ...
    return json ? json.success ? void await poll(json.inference_job_token).then(resolve).catch(reject) : (reject(`Failed voice request! ${json.error_reason}`),
      void console.error(json)) : void 0;
  }));
}