...
const handler = async (m, {
  ...
  const text = args.filter(arg => !arg.startsWith("--")).join(" ") || (m.quoted?.text || null);
  if (!text) {
    return m.reply(`Masukkan input. Contoh penggunaan: ${usedPrefix}${command} <teks> [--option=value]`);
  }
  m.react(wait);
  ...
  let allVoices = [];
  languageKeys.forEach(language => allVoices = allVoices.concat(voices[language].ShortName));
  ...
  const language = languageKeys.find(lang => voices[lang].ShortName.includes(shortName));
