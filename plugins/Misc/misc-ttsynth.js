...
class Ttsynth {
  ...
  async create(contentText, languageId = "id-id", voiceId = 729) {
    try {
      ...
      return result?.code === 200 ? result.data?.id : Promise.reject(result?.msg || "Error");
    } catch (err) {
  ...
  async poll(recordId, timeout = 6e4, interval = 2e3) {
    try {
      ...
      while (Date.now() < end) {
        const res = await fetch(`https://ttsynthcom.erweima.ai/api/v1/generate/getGenerateRecordDetail?recordId=${recordId}`, {
          headers: this.headers
        });
        const result = await res.json();
...
const handler = async (m, {
  ...
  if (!query) return m.reply("Input query!\n*Example:*\n" + usedPrefix + command + " [query] --lang=id-id --id=769");
  m.react(wait);
  try {
    ...
    await conn.sendFile(m.chat, audioUrl, "audio.mp3", "", m, true, {
      ...
      contextInfo: {
        ...
        externalAdReply: {
          ...
          thumbnailUrl: "https://www.respeecher.com/hs-fs/hubfs/What-is-Text-to-Speech-TTS%29-Initial-Speech-Synthesis-Explained-Respeecher-voice-cloning-software.jpeg?width=1191&height=744",
          sourceUrl: ""
        }
      }
    });
