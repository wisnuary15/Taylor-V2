...
let handler = async (m, {
    ...
    try {
        ...
        let dls = "Download audio success (V1)";
        let ytthumb = (await conn.getFile(Ytdl.meta.image)?.data) || '';
        let doc = {
            audio: Ytdl.buffer,
            mimetype: "audio/mp4",
        ...
        try {
            ...
            let dls = "Download audio success (V2)";
            let ytthumb = (await conn.getFile(yt.thumbnail)?.data) || '';
            let doc = {
                audio: {
                    url: link
                },
