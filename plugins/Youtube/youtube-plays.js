...
const handler = async (m, { conn: conn, command: command, args: args, usedPrefix: usedPrefix }) => {
  ...
  const result = await searchAndDownloadMusic(text),
    fullText = `${`\n    *Title:* ${result.title}\n    *Description:* ${result.description}\n    *Duration:* ${result.duration}\n    *Author:* ${result.author}\n    *Video URL:* ${result.videoUrl}\n  `}\n\n${result.allLinks.map(((link, index) => {
        const sectionNumber = index + 1, {quality: quality, type: type, size: size} = link;
        return ` * $ { sectionNumber }.*$ { type }* $ { quality }*-$ { size }
  `;
    })).join("\n")}`, { key: key } = await conn.reply(m.chat, fullText, m);
...
handler.before = async (m, { conn: conn }) => {
    ...
    const { result: result, key: key, timeout: timeout } = conn.youtubePlay[m.sender];
...
async function searchAndDownloadMusic(query) {
  try {
    const { videos: videos } = await yts(query);
