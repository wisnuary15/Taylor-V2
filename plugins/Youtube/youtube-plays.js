...
const handler = async (m, {
  ...
  db.data.dbbot.youtubePlay = db.data.dbbot.youtubePlay ? db.data.dbbot.youtubePlay : {},
    m.react(wait);
...
return ` * $ {
  ...
  db.data.dbbot.youtubePlay[m.sender] = {
    result: result,
    key: key,
    timeout: setTimeout(() => {
      conn.sendMessage(m.chat, {
        delete: key
      }), delete db.data.dbbot.youtubePlay[m.sender];
    }, 6e4)
  };
};
handler.before = async (m, {
    ...
    if (db.data.dbbot.youtubePlay = db.data.dbbot.youtubePlay ? db.data.dbbot.youtubePlay : {}, m.isBaileys || !(m.sender in db.data.dbbot.youtubePlay)) return;
    const {
      result,
      key,
      timeout
    ...
    if (inputNumber >= 1 && inputNumber <= result.allLinks.length) {
      ...
      m.reply(buffer), m.reply(`Anda memilih pilihan nomor *${inputNumber}*\n*Stream:* ${await shortUrl(selectedUrl)}`),
        conn.sendMessage(m.chat, {
          delete: key
        }), clearTimeout(timeout), delete db.data.dbbot.youtubePlay[m.sender];
