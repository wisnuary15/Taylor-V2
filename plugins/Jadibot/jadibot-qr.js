...
const handler = async (m, {
  ...
  exec(comb.toString("utf-8"), async (err, stdout, stderr) => {
    ...
    async function jddt() {
      ...
      args[0] ? fs.writeFileSync("./JadibotSession/" + uniqid + "/creds.json", JSON.stringify(JSON.parse(Buffer.from(args[0], "base64").toString("utf-8")), null, "\t")) : "";
      const {
        state,
        saveState,
        saveCreds
      } = await useMultiFileAuthState("./JadibotSession/" + uniqid);
      ...
      async function connectionUpdate(update) {
        ...
        if (connection === "open") {
          ...
          if (!args[0]) parentw.sendMessage(m.chat, {
            text: usedPrefix + command + " " + Buffer.from(fs.readFileSync("./JadibotSession/" + uniqid + "/creds.json"), "utf-8").toString("base64")
          }, {
            quoted: m
          });
        }
