...
let handler = async (m, {
  ...
  if (!(args[0] && args[0] == "plz" || conn.user.jid == _conn.user.jid) && !m.fromMe) {
    return _conn.sendMessage(m.chat, {
      text: message1
    }, {
      quoted: m
    });
  ...
  async function serbot() {
    ...
    const {
      ...
      saveState,
      saveCreds
    } = await useMultiFileAuthState(`./${folderBot}/${authFolderB}`);
    ...
    if (!fs.existsSync(`./${folderBot}/` + authFolderB + "/creds.json")) {
      ...
      setTimeout(async () => {
        ...
        codeA = await parent.sendMessage(m.chat, {
          text: txt.trim(),
          mentions: [m.sender]
        }, {
          quoted: m
        });
        codeB = await parent.sendMessage(m.chat, {
          text: codeBot
        }, {
          quoted: m
        });
    ...
    async function connectionUpdate(update) {
      ...
      if (connection == "open") {
        ...
        await parent.sendMessage(conn.user.jid, {
          text: usedPrefix + command + " " + Buffer.from(fs.readFileSync(`./${folderBot}/` + authFolderB + "/creds.json"), "utf-8").toString("base64")
        }, {
          quoted: m
        });
      }
