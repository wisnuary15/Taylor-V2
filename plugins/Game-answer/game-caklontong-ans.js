...
export async function before(m) {
  ...
  if (this.caklontong = this.caklontong ? this.caklontong : {}, !(id in this.caklontong)) return await this.reply(m.chat, "Soal caklontong itu telah berakhir", m);
  if (m.quoted?.id === this.caklontong[id][0]?.id) {
    ...
    }), clearTimeout(this.caklontong[id][3]), delete this.caklontong[id]) : similarity(m.text.toLowerCase(), json.jawaban.toLowerCase().trim()) >= .72 ? await this.reply(m.chat, "❗ *Sedikit Lagi!*", m, {
      contextInfo: {
        mentionedJid: [m.sender],
        externalAdReply: {
