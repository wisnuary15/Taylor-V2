...
export async function before(m) {
  ...
  if (m.quoted?.id === this.tebaklogo[id][0]?.id) {
    ...
    m.text.toLowerCase() === json.hasil.data.jawaban.toLowerCase().trim() ? (db.data.users[m.sender].exp += this.tebaklogo[id][2],
      await this.reply(m.chat, `✅ *Benar!*\n+${this.tebaklogo[id][2]} XP`, m), clearTimeout(this.tebaklogo[id][3]),
      delete this.tebaklogo[id]) : similarity(m.text.toLowerCase(), json.hasil.data.jawaban.toLowerCase().trim()) >= .72 ? m.reply("❗ *Dikit Lagi!*") : await this.reply(m.chat, "❌ *Salah!*", m);
  }
  return !0;
}
export const exp = 0;
