...
export async function before(m) {
  let id = m.chat
  if (!m.quoted || !m.quoted?.fromMe || !m.quoted?.isBaileys || !m.text || !/Ketik.*hlog/i.test(m.quoted?.text) ||
    /.*hlog/i.test(m.text)) return !0
  this.tebaklogo = this.tebaklogo ? this.tebaklogo : {}
  if (!(id in this.tebaklogo)) return this.reply(m.chat, 'Soal itu telah berakhir', m)
  if (m.quoted?.id === this.tebaklogo[id][0]?.id) {
    ...
    if (m.text.toLowerCase() === json.hasil.data.jawaban.toLowerCase().trim()) {
      ...
      clearTimeout(this.tebaklogo[id][3])
      delete this.tebaklogo[id]
    } else if (similarity(m.text.toLowerCase(), json.hasil.data.jawaban.toLowerCase().trim()) >= threshold) m.reply(
      `❗ *Dikit Lagi!*`)
