...
export async function before(m) {
  ...
  if (this.tebakchara = this.tebakchara ? this.tebakchara : {}, !(id in this.tebakchara)) return await this.reply(m.chat, "Soal tebakchara itu telah berakhir", m);
  if (m.quoted?.id === this.tebakchara[id][0]?.id) {
    ...
    }), clearTimeout(this.tebakchara[id][3]), delete this.tebakchara[id]) : similarity(m.text.toLowerCase(), json.name.toLowerCase().trim()) >= .72 ? await this.reply(m.chat, "❗ *Sedikit Lagi!*", m, {
      contextInfo: {
        mentionedJid: [m.sender],
        externalAdReply: {
