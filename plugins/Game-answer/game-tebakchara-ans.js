...
export async function before(m) {
  ...
  if (m.quoted?.id === this.tebakchara[id][0]?.id) {
    ...
    m.text.toLowerCase() === json.name.toLowerCase().trim() ? (db.data.users[m.sender].exp += this.tebakchara[id][2],
      await this.reply(m.chat, `✅ *Benar!*\n+${this.tebakchara[id][2]} XP`, m), clearTimeout(this.tebakchara[id][3]),
      delete this.tebakchara[id]) : similarity(m.text.toLowerCase(), json.name.toLowerCase().trim()) >= .72 ? m.reply("❗ *Dikit Lagi!*") : await this.reply(m.chat, "❌ *Salah!*", m);
  }
  return !0;
}
export const exp = 0;
