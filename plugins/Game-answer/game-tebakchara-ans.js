...
export async function before(m) {
  let id = m.chat
  if (!m.quoted || !m.quoted?.fromMe || !m.quoted?.isBaileys || !m.text || !/Ketik.*hcha/i.test(m.quoted?.text) ||
    /.*hcha/i.test(m.text)) return !0
  this.tebakchara = this.tebakchara ? this.tebakchara : {}
  if (!(id in this.tebakchara)) return this.reply(m.chat, 'Soal itu telah berakhir', m)
