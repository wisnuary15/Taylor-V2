...
export async function before(m) {
  ...
  if (this.tebakgame = this.tebakgame ? this.tebakgame : {}, !(id in this.tebakgame)) return await this.reply(m.chat, "Soal tebakgame itu telah berakhir", m);
  if (m.quoted?.id === this.tebakgame[id][0]?.id) {
    ...
    }), clearTimeout(this.tebakgame[id][3]), delete this.tebakgame[id]) : similarity(m.text.toLowerCase(), json.jawaban.toLowerCase().trim()) >= .72 ? await this.reply(m.chat, "❗ *Sedikit Lagi!*", m, {
      contextInfo: {
        mentionedJid: [m.sender],
        externalAdReply: {
