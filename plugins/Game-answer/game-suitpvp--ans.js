export async function before(m) {
  db.data.game.suitpvp = db.data.game.suitpvp ? db.data.game.suitpvp : {}, db.data.users[m.sender].suit < 0 && (db.data.users[m.sender].suit = 0);
  let room = Object.values(db.data.game.suitpvp).find(room => room.id && room.status && [room.p, room.p2].includes(m.sender));
  if (room) {
    let win = "",
      tie = !1;
    if (m.sender === room.p2 && /^(acc(ept)?|terima|gas|oke?|tolak|gamau|nanti|ga(k.)?bisa)/i.test(m.text) && m.isGroup && "wait" === room.status) {
      if (/^(tolak|gamau|nanti|ga(k.)?bisa)/i.test(m.text)) return await this.reply(m.chat, `@${room.p2.split("@")[0]} menolak suit, suit dibatalkan`, m),
        delete db.data.game.suitpvp[room.id], !0;
    ...
    }), tie || (db.data.users[win === room.p ? room.p : room.p2].exp += room.poin, db.data.users[win === room.p ? room.p2 : room.p].exp += room.poin_lose), delete db.data.game.suitpvp[room.id]);
  }
  return !0;
}
