export async function before(m) {
  ...
  if (room) {
    let win = ''
    let tie = false
    if (m.sender === room.p2 && /^(acc(ept)?|terima|gas|oke?|tolak|gamau|nanti|ga(k.)?bisa)/i.test(m.text) && m
      .isGroup && room.status === 'wait') {
...
klik wa.me/${this.user.jid.split('@')[0]}`, m.chat, {
      ...
      if (!room.pilih) this.reply(room.p,
        `Silahkan pilih\nMenang +${room.poin}XP\nKalah -${room.poin_lose}XP\n\nBatu🗿 ( Batu )\nKertas📄 ( Kertas )\nGunting✂️ ( Gunting )`,
        m);
      if (!room.pilih2) this.reply(room.p2,
        `Silahkan pilih\nMenang +${room.poin}XP\nKalah -${room.poin_lose}XP\n\nBatu🗿 ( Batu )\nKertas📄 ( Kertas )\nGunting✂️ ( Gunting )`,
        m);
      room.waktu_milih = setTimeout(() => {
        if (!room.pilih && !room.pilih2) this.reply(m.chat, `Kedua pemain tidak niat main,\nSuit dibatalkan`)
          ...
          this.reply(m.chat,
            `@${(room.pilih ? room.p2 : room.p).split('@')[0]} tidak memilih suit, game berakhir`, m)
