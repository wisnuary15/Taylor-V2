...
export async function before(m) {
  ...
  if (!isSurrender) {
    ...
    if (index < 0) {
      if (Math.max(...room.jawaban.filter((_, index) => !room.terjawab[index]).map(jawaban => similarity(jawaban,
          text))) >= threshold) m.reply('❗ *Dikit Lagi!*')
      return !0
    }
...
${isSurrender ? '' : `+${room.winScore} XP tiap jawaban benar`}
    `.trim()
  const msg = this.reply(m.chat, caption, null, {
