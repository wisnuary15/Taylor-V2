...
export async function before(m) {
  db.data.game.familygame = db.data.game.familygame ? db.data.game.familygame : {};
  ...
  if (!isSurrender) {
    ...
    if (index < 0) return Math.max(...room.jawaban.filter((_, index) => !room.terjawab[index]).map(jawaban => similarity(jawaban, text))) >= .72 && await this.reply(m.chat, "❗ *Sedikit Lagi!*", m, {
      contextInfo: {
  ...
  return room.msg = msg, (isWin || isSurrender) && delete db.data.game.familygame[id], !0;
}