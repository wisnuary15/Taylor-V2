...
export async function before(m) {
  let id = m.chat
  if (!m.quoted || !m.quoted?.fromMe || !m.quoted?.isBaileys || !m.text || !/Ketik.*hani/i.test(m.quoted?.text) ||
    /.*hani/i.test(m.text)) return !0
  this.tebakanjime = this.tebakanjime ? this.tebakanjime : {}
  if (!(id in this.tebakanjime)) return this.reply(m.chat, 'Soal itu telah berakhir', m)
