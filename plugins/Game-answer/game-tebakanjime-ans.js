...
export async function before(m) {
  ...
  if (this.tebakanjime = this.tebakanjime ? this.tebakanjime : {}, !(id in this.tebakanjime)) return await this.reply(m.chat, "Soal tebakanjime itu telah berakhir", m);
  if (m.quoted?.id === this.tebakanjime[id][0]?.id) {
    ...
    }), clearTimeout(this.tebakanjime[id][3]), delete this.tebakanjime[id]) : similarity(m.text.toLowerCase(), json.name.toLowerCase().trim()) >= .72 ? await this.reply(m.chat, "❗ *Sedikit Lagi!*", m, {
      contextInfo: {
        mentionedJid: [m.sender],
        externalAdReply: {
