...
export async function before(m) {
  let id = m.chat
  if (!m.quoted || !m.quoted?.fromMe || !m.quoted?.isBaileys || !m.text || !/Ketik.*hkim/i.test(m.quoted?.text) ||
    /.*hkim/i.test(m.text)) return !0
  this.tebakkimia = this.tebakkimia ? this.tebakkimia : {}
  if (!(id in this.tebakkimia)) return this.reply(m.chat, 'Soal itu telah berakhir', m)
  if (m.quoted?.id === this.tebakkimia[id][0]?.id) {
    ...
    if (m.text.toLowerCase() === json.unsur.toLowerCase().trim()) {
      ...
      clearTimeout(this.tebakkimia[id][3])
      delete this.tebakkimia[id]
    } else if (similarity(m.text.toLowerCase(), json.unsur.toLowerCase().trim()) >= threshold) m.reply(
      `❗ *Dikit Lagi!*`)
