...
export async function before(m) {
  ...
  if (m.quoted?.id === this.tebakkimia[id][0]?.id) {
    ...
    m.text.toLowerCase() === json.unsur.toLowerCase().trim() ? (db.data.users[m.sender].exp += this.tebakkimia[id][2],
      await this.reply(m.chat, `✅ *Benar!*\n+${this.tebakkimia[id][2]} XP`, m), clearTimeout(this.tebakkimia[id][3]),
      delete this.tebakkimia[id]) : similarity(m.text.toLowerCase(), json.unsur.toLowerCase().trim()) >= .72 ? m.reply("❗ *Dikit Lagi!*") : await this.reply(m.chat, "❌ *Salah!*", m);
  }
  return !0;
}
export const exp = 0;
