...
export async function before(m) {
  let id = m.chat
  if (!m.quoted || !m.quoted?.fromMe || !m.quoted?.isBaileys || !m.text || !/Ketik.*hjkt/i.test(m.quoted?.text) ||
    /.*hjkt/i.test(m.text)) return !0
  this.tebakjkt48 = this.tebakjkt48 ? this.tebakjkt48 : {}
  if (!(id in this.tebakjkt48)) return this.reply(m.chat, 'Soal itu telah berakhir', m)
