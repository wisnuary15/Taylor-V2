...
export async function before(m) {
  ...
  if (m.quoted?.id === this.tebakjkt48[id][0]?.id) {
    ...
    m.text.toLowerCase() === json.name.toLowerCase().trim() ? (db.data.users[m.sender].exp += this.tebakjkt48[id][2],
      await this.reply(m.chat, `✅ *Benar!*\n+${this.tebakjkt48[id][2]} XP`, m), clearTimeout(this.tebakjkt48[id][3]),
      delete this.tebakjkt48[id]) : similarity(m.text.toLowerCase(), json.name.toLowerCase().trim()) >= .72 ? m.reply("❗ *Dikit Lagi!*") : await this.reply(m.chat, "❌ *Salah!*", m);
  }
  return !0;
}
export const exp = 0;
