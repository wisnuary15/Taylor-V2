...
export async function before(m) {
  let id = m.chat
  if (!m.quoted || !m.quoted?.fromMe || !m.quoted?.isBaileys || !m.text || !/Ketik.*hsur/i.test(m.quoted?.text) ||
    /.*hsur/i.test(m.text)) return !0
  this.tebaksurah = this.tebaksurah ? this.tebaksurah : {}
  if (!(id in this.tebaksurah)) return this.reply(m.chat, 'Soal itu telah berakhir', m)
  if (m.quoted?.id === this.tebaksurah[id][0]?.id) {
    ...
    if (m.text.toLowerCase() === json.surah.englishName.toLowerCase().trim()) {
      ...
      clearTimeout(this.tebaksurah[id][3])
      delete this.tebaksurah[id]
    } else if (similarity(m.text.toLowerCase(), json.surah.englishName.toLowerCase().trim()) >= threshold) m.reply(
      `❗ *Dikit Lagi!*`)
