...
export async function before(m) {
  ...
  if (m.quoted?.id === this.tebaksurah[id][0]?.id) {
    ...
    m.text.toLowerCase() === json.surah.englishName.toLowerCase().trim() ? (db.data.users[m.sender].exp += this.tebaksurah[id][2],
      await this.reply(m.chat, `✅ *Benar!*\n+${this.tebaksurah[id][2]} XP`, m), clearTimeout(this.tebaksurah[id][3]),
      delete this.tebaksurah[id]) : similarity(m.text.toLowerCase(), json.surah.englishName.toLowerCase().trim()) >= .72 ? m.reply("❗ *Dikit Lagi!*") : await this.reply(m.chat, "❌ *Salah!*", m);
  }
  return !0;
}
export const exp = 0;
