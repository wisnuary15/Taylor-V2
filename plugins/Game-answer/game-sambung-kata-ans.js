...
export async function before(m) {
    ...
    if (m.quoted.id === this.skata[id][0]?.id) {
        ...
        let res = await fetch('https://api.lolhuman.xyz/api/sambungkata?apikey=' + lolkey + '&text=' + m.text.toLowerCase())
        ...
        this.skata[id] = [
            this.sendFile(m.chat, imgr + 'Lanjut', '', '*Lanjut mulai dari kata: * ' + answerF.toUpperCase() + '\n\n*Awalan:* ' + (answerF.slice(-1)).toUpperCase() + '... ?\n\n*balas pesan ini untuk menjawab!*', m),
            answerF,
            this.skata[id][2]
        ]
        return !0
