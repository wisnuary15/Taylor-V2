...
export async function before(m) {
  ...
  else /^cancel?$/i.test(txt) && (delete this.chessgame[m.chat],
    await this.reply(m.chat, "Berhasil keluar dari sesi Chess.", m));
  if (this.chessgame[m.chat] && /^\S+(\s|[\W])\S+$/.test(txt) && this.chessgame[m.chat].turn === this.chessgame[m.chat].player2) {
    ...
    if (m.sender !== this.chessgame[m.chat].player2) await this.reply(m.chat, "❌ Bukan giliran Anda.", m);
      ...
      const encodedFen = encodeURIComponent(chess.fen()),
        ...
        boardUrl = (this.chessgame[m.chat].turn,
          this.chessgame[m.chat].player1, `https://www.chess.com/dynboard?fen=${encodedFen}&board=graffiti&piece=graffiti&size=3&coordinates=inside${flipParam}`);
      let soal;
      try {
        soal = await this.sendMessage(m.chat, {
          image: {
