...
export async function before(m) {
  ...
  if (m.quoted?.id === this.tebaktebakan[id][0]?.id) {
    ...
    m.text.toLowerCase() === json.jawaban.toLowerCase().trim() ? (db.data.users[m.sender].exp += this.tebaktebakan[id][2],
      await this.reply(m.chat, `✅ *Benar!*\n+${this.tebaktebakan[id][2]} XP`, m), clearTimeout(this.tebaktebakan[id][3]),
      delete this.tebaktebakan[id]) : similarity(m.text.toLowerCase(), json.jawaban.toLowerCase().trim()) >= .72 ? m.reply("❗ *Dikit Lagi!*") : await this.reply(m.chat, "❌ *Salah!*", m);
  }
  return !0;
}
export const exp = 0;
