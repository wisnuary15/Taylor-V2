...
export async function before(m) {
  ...
  if (this.tebaktebakan = this.tebaktebakan ? this.tebaktebakan : {}, !(id in this.tebaktebakan)) return await this.reply(m.chat, "Soal tebaktebakan itu telah berakhir", m);
  if (m.quoted?.id === this.tebaktebakan[id][0]?.id) {
    ...
    }), clearTimeout(this.tebaktebakan[id][3]), delete this.tebaktebakan[id]) : similarity(m.text.toLowerCase(), json.jawaban.toLowerCase().trim()) >= .72 ? await this.reply(m.chat, "❗ *Sedikit Lagi!*", m, {
      contextInfo: {
        mentionedJid: [m.sender],
        externalAdReply: {
