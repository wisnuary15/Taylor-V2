export async function before(m) {
  try {
    ...
    if (this.bomb = this.bomb ? this.bomb : {}, /^((me)?nyerah|surr?ender)$/i.test(m.text) && this.bomb && id in this.bomb && (await this.reply(m.chat, "🚩 Menyerah", m),
        clearTimeout(this.bomb[id][2]), delete this.bomb[id]), this.bomb[id] && m.quoted && m.quoted?.id === this.bomb[id][3].id && !isNaN(body)) {
