...
handler.before = async function(m) {
  ...
  if (m.quoted?.id === this.math[id][0]?.id) {
    ...
    m.text === math.result ? (db.data.users[m.sender].exp += math.bonus, clearTimeout(this.math[id][3]),
      delete this.math[id], await this.reply(m.chat, `✅ *Benar!*\n+${math.bonus} XP`, m)) : 0 == --this.math[id][2] ? (clearTimeout(this.math[id][3]),
      delete this.math[id], await this.reply(m.chat, `❗ *Kesempatan habis!*\nJawaban: *${math.result}*`, m)) : m.reply(`❌ *Jawaban Salah!*\nMasih ada ${this.math[id][2]} kesempatan`);
  }
  return !0;
};
