...
export async function before(m) {
  ...
  try {
    if (res = "5" === m.text ? await fetch(`https://api.lolhuman.xyz/api/akinator/back?apikey=${lolkey}&server=${server}&session=${session}&signature=${signature}&step=${step}`) : await fetch(`https://api.lolhuman.xyz/api/akinator/answer?apikey=${lolkey}&server=${server}&frontaddr=${frontaddr}&session=${session}&signature=${signature}&step=${step}&answer=${m.text}`), anu = await res.json(), "200" != anu.status) return aki.sesi = !1, aki.soal = null,
