...
export async function before(m) {
  ...
  if (this.tebakingambar = this.tebakingambar ? this.tebakingambar : {}, !(id in this.tebakingambar)) return await this.reply(m.chat, "Soal tebakingambar itu telah berakhir", m);
  if (m.quoted?.id === this.tebakingambar[id][0]?.id) {
    ...
    }), clearTimeout(this.tebakingambar[id][3]), delete this.tebakingambar[id]) : similarity(m.text.toLowerCase(), json.jawaban.toLowerCase().trim()) >= .72 ? await this.reply(m.chat, "❗ *Sedikit Lagi!*", m, {
      contextInfo: {
        mentionedJid: [m.sender],
        externalAdReply: {
