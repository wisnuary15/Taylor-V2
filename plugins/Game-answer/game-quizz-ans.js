...
export async function before(m) {
  ...
  if (m.quoted?.id === this.quizz[id][0]?.id) {
    ...
    m.text.toLowerCase() === json[0]?.jawaban.toLowerCase().trim() ? (db.data.users[m.sender].exp += this.quizz[id][2],
      await this.reply(m.chat, `✅ *Benar!*\n+${this.quizz[id][2]} XP`, m), clearTimeout(this.quizz[id][3]),
      delete this.quizz[id]) : similarity(m.text.toLowerCase(), json[0]?.jawaban.toLowerCase().trim()) >= .72 ? m.reply("❗ *Dikit Lagi!*") : await this.reply(m.chat, "❌ *Salah!*", m);
  }
  return !0;
}
export const exp = 0;
