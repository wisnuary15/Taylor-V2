...
export async function before(m) {
  ...
  if (this.quizz = this.quizz ? this.quizz : {}, !(id in this.quizz)) return await this.reply(m.chat, "Soal quizz itu telah berakhir", m);
  if (m.quoted?.id === this.quizz[id][0]?.id) {
    ...
    }), clearTimeout(this.quizz[id][3]), delete this.quizz[id]) : similarity(m.text.toLowerCase(), json[0]?.jawaban.toLowerCase().trim()) >= .72 ? await this.reply(m.chat, "❗ *Sedikit Lagi!*", m, {
      contextInfo: {
        mentionedJid: [m.sender],
        externalAdReply: {
