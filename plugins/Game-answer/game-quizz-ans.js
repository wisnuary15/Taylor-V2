...
export async function before(m) {
  let id = m.chat
  if (!m.quoted || !m.quoted?.fromMe || !m.quoted?.isBaileys || !m.text || !/Ketik.*quizzh/i.test(m.quoted?.text) ||
    /.*quizzh/i.test(m.text)) return !0
  this.quizz = this.quizz ? this.quizz : {}
  if (!(id in this.quizz)) return this.reply(m.chat, 'Soal itu telah berakhir', m)
  if (m.quoted?.id === this.quizz[id][0]?.id) {
    ...
    if (m.text.toLowerCase() === json[0]?.jawaban.toLowerCase().trim()) {
      db.data.users[m.sender].exp += this.quizz[id][2]
      this.reply(m.chat, `✅ *Benar!*\n+${this.quizz[id][2]} XP`, m)
      clearTimeout(this.quizz[id][3])
      delete this.quizz[id]
    } else if (similarity(m.text.toLowerCase(), json[0]?.jawaban.toLowerCase().trim()) >= threshold) m.reply(
      `❗ *Dikit Lagi!*`)
