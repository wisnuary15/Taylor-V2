...
export async function before(m) {
  let id = m.chat
  if (!m.quoted || !m.quoted?.fromMe || !m.quoted?.isBaileys || !m.text || !/Ketik.*quest/i.test(m.quoted?.text) ||
    /.*quest/i.test(m.text)) return !0
  this.question = this.question ? this.question : {}
  if (!(id in this.question)) return this.reply(m.chat, 'Soal itu telah berakhir', m)
  if (m.quoted?.id === this.question[id][0]?.id) {
    ...
    if (m.text.toLowerCase() === json.results[0]?.correct_answer.toLowerCase().trim()) {
      db.data.users[m.sender].exp += this.question[id][2]
      this.reply(m.chat, `✅ *Benar!*\n+${this.question[id][2]} XP`, m)
      clearTimeout(this.question[id][3])
      delete this.question[id]
    } else if (similarity(m.text.toLowerCase(), json.results[0]?.correct_answer.toLowerCase().trim()) >= threshold) m
      .reply(`❗ *Dikit Lagi!*`)
