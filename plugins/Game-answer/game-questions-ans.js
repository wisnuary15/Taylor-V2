...
export async function before(m) {
  ...
  if (m.quoted?.id === this.question[id][0]?.id) {
    ...
    m.text.toLowerCase() === json.results[0]?.correct_answer.toLowerCase().trim() ? (db.data.users[m.sender].exp += this.question[id][2],
      await this.reply(m.chat, `✅ *Benar!*\n+${this.question[id][2]} XP`, m), clearTimeout(this.question[id][3]),
      delete this.question[id]) : similarity(m.text.toLowerCase(), json.results[0]?.correct_answer.toLowerCase().trim()) >= .72 ? m.reply("❗ *Dikit Lagi!*") : await this.reply(m.chat, "❌ *Salah!*", m);
  }
  return !0;
}
export const exp = 0;
