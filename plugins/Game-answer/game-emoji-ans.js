...
export async function before(m) {
  let id = m.chat
  if (!m.quoted || !m.quoted?.fromMe || !m.quoted?.isBaileys || !m.text || !/Ketik.*hemo/i.test(m.quoted?.text) ||
    /.*hemo/i.test(m.text)) return !0
  this.tebakemoji = this.tebakemoji ? this.tebakemoji : {}
  if (!(id in this.tebakemoji)) return this.reply(m.chat, 'Soal itu telah berakhir', m)
  if (m.quoted?.id === this.tebakemoji[id][0]?.id) {
    ...
    if (m.text.toLowerCase() === (json.unicodeName).toLowerCase().trim()) {
      db.data.users[m.sender].exp += this.tebakemoji[id][2]
      this.reply(m.chat, `✅ *Benar!*\n+${this.tebakemoji[id][2]} XP`, m)
      clearTimeout(this.tebakemoji[id][3])
      delete this.tebakemoji[id]
    } else if (similarity(m.text.toLowerCase(), (json.unicodeName).toLowerCase().trim()) >= threshold) m.reply(
      `❗ *Dikit Lagi!*`)
