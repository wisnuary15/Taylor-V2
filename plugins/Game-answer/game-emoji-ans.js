...
export async function before(m) {
  ...
  if (m.quoted?.id === this.tebakemoji[id][0]?.id) {
    ...
    m.text.toLowerCase() === json.unicodeName.toLowerCase().trim() ? (db.data.users[m.sender].exp += this.tebakemoji[id][2],
      await this.reply(m.chat, `✅ *Benar!*\n+${this.tebakemoji[id][2]} XP`, m), clearTimeout(this.tebakemoji[id][3]),
      delete this.tebakemoji[id]) : similarity(m.text.toLowerCase(), json.unicodeName.toLowerCase().trim()) >= .72 ? m.reply("❗ *Dikit Lagi!*") : await this.reply(m.chat, "❌ *Salah!*", m);
  }
  return !0;
}
export const exp = 0;
