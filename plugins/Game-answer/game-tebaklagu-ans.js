...
export async function before(m) {
  let id = m.chat
  if (!m.quoted || !m.quoted?.fromMe || !m.quoted?.isBaileys || !m.text || !/Ketik.*hlag/i.test(m.quoted?.text) ||
    /.*hlag/i.test(m.text)) return !0
  this.tebaklagu = this.tebaklagu ? this.tebaklagu : {}
  if (!(id in this.tebaklagu)) return this.reply(m.chat, 'Soal itu telah berakhir', m)
  if (m.quoted?.id === this.tebaklagu[id][0]?.id) {
    ...
    if (m.text.toLowerCase() === json.judul.toLowerCase().trim()) {
      ...
      clearTimeout(this.tebaklagu[id][3])
      delete this.tebaklagu[id]
    } else if (similarity(m.text.toLowerCase(), json.judul.toLowerCase().trim()) >= threshold) m.reply(
      `❗ *Dikit Lagi!*`)
