...
export async function before(m) {
  ...
  if (m.quoted?.id === this.tebaklagu[id][0]?.id) {
    ...
    m.text.toLowerCase() === json.judul.toLowerCase().trim() ? (db.data.users[m.sender].exp += this.tebaklagu[id][2],
      await this.reply(m.chat, `✅ *Benar!*\n+${this.tebaklagu[id][2]} XP`, m), clearTimeout(this.tebaklagu[id][3]),
      delete this.tebaklagu[id]) : similarity(m.text.toLowerCase(), json.judul.toLowerCase().trim()) >= .72 ? m.reply("❗ *Dikit Lagi!*") : await this.reply(m.chat, "❌ *Salah!*", m);
  }
  return !0;
}
export const exp = 0;
