...
export async function before(m) {
  let id = m.chat
  if (!m.quoted || !m.quoted?.fromMe || !m.quoted?.isBaileys || !m.text || !/Ketik.*htri/i.test(m.quoted?.text) ||
    /.*htri/i.test(m.text)) return !0
  this.trivias = this.trivias ? this.trivias : {}
  if (!(id in this.trivias)) return this.reply(m.chat, 'Soal itu telah berakhir', m)
  if (m.quoted?.id === this.trivias[id][0]?.id) {
    ...
    if (m.text.toLowerCase() === jawaban.toLowerCase().trim()) {
      db.data.users[m.sender].exp += this.trivias[id][2]
      this.reply(m.chat, `✅ *Benar!*\n+${this.trivias[id][2]} XP`, m)
      clearTimeout(this.trivias[id][3])
      delete this.trivias[id]
    } else if (similarity(m.text.toLowerCase(), jawaban.toLowerCase().trim()) >= threshold) m.reply(`❗ *Dikit Lagi!*`)
