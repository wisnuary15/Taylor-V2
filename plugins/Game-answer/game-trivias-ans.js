...
export async function before(m) {
  ...
  if (m.quoted?.id === this.trivias[id][0]?.id) {
    ...
    m.text.toLowerCase() === jawaban.toLowerCase().trim() ? (db.data.users[m.sender].exp += this.trivias[id][2],
      await this.reply(m.chat, `✅ *Benar!*\n+${this.trivias[id][2]} XP`, m), clearTimeout(this.trivias[id][3]),
      delete this.trivias[id]) : similarity(m.text.toLowerCase(), jawaban.toLowerCase().trim()) >= .72 ? m.reply("❗ *Dikit Lagi!*") : await this.reply(m.chat, "❌ *Salah!*", m);
  }
  return !0;
}
export const exp = 0;
