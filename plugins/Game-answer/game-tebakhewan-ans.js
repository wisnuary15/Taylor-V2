...
export async function before(m) {
  ...
  if (m.quoted?.id === this.tebakhewan[id][0]?.id) {
    ...
    m.text.toLowerCase() === json.title.toLowerCase().trim() ? (db.data.users[m.sender].exp += this.tebakhewan[id][2],
      await this.reply(m.chat, `✅ *Benar!*\n+${this.tebakhewan[id][2]} XP`, m), clearTimeout(this.tebakhewan[id][3]),
      delete this.tebakhewan[id]) : similarity(m.text.toLowerCase(), json.title.toLowerCase().trim()) >= .72 ? m.reply("❗ *Dikit Lagi!*") : await this.reply(m.chat, "❌ *Salah!*", m);
  }
  return !0;
}
export const exp = 0;
