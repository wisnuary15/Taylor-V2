...
async function freemake(ytUrl) {
  try {
    const response = await fetch(`https://downloader.freemake.com/api/videoinfo/${getId(ytUrl)}`);
...
const handler = async (m, {
  ...
  try {
    ...
    if (qualityLabel) {
      const quality = qualities.find(q => q.qualityInfo.qualityLabel === qualityLabel);
      if (quality) {
        selectedQualityUrl = quality.url;
      } else {
        return m.reply(`Quality label tidak valid. Available quality labels: ${qualities.map(q => q.qualityInfo.qualityLabel).join(", ")}`);
      }
    } else {
