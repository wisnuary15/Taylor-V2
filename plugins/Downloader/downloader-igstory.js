...
const handler = async (m, { conn: conn, args: args, usedPrefix: usedPrefix, command: command }) => {
  ...
  try {
    ...
    for (let { url: url, preview: preview } of anuku) conn.sendMedia(m.chat, url, null, {
      mentions: [m.sender],
    ...
    try {
      ...
      for (const { downloadUrl: downloadUrl, url: url, preview: preview, type: type, fileType: fileType } of res2) conn.sendMedia(m.chat, url, null, {
        mentions: [m.sender],
        jpegThumbnail: await (await fetch(preview)).arrayBuffer(),
        caption: `🚀 *Link:* ${await (await axios.get(`https://tinyurl.com/api-create.php?url=${url}`)).data}`
      });
