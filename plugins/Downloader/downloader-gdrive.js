...
const handler = async (m, {
  ...
  try {
    ...
    if (!someincludes(['kB', 'KB'], res.fileSize.slice(-2)) && parseInt(res.fileSize) > 500) return m.reply(
    ...
    m.reply(txt)
    if (!res.downloadUrl) m.react(eror)
    conn.sendFile(m.chat, res.downloadUrl, res.fileName + res.mimetype, res.fileName + res.mimetype, m)
  } catch (e) {
    console.log(e)
