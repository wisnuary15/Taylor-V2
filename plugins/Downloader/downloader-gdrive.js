...
const handler = async (m, {
  ...
  try {
    ...
    try {
      let res = await drive(args[0]);
      let txt = "*[ Downloading file ]*\n\n";
      txt += `*Name :* ${res.name}\n`, res.download || m.react(eror), await conn.sendFile(m.chat, res.download, res.name, res.name, m);
    } catch (e) {
...
async function drive(url) {
  try {
    ...
    const name = $("head").find('meta[property="og:title"]').attr("content") || "";
