...
async function krakenFiles(link) {
  try {
    const { data: data } = await axios.get(link), $ = load(data), result = {
      ...
      hash: hash
    }, payload = `------WebKitFormBoundary7MA4YWxkTrZu0gW\r\nContent-Disposition: form-data; name="token"\r\n\r\n${token}\r\n------WebKitFormBoundary7MA4YWxkTrZu0gW--`, { data: json } = await axios.post(`https://krakenfiles.com/download/${hash}`, payload, {
      headers: headers
    });
