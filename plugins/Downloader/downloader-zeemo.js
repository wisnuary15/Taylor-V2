...
const handler = async (m, {
  ...
  try {
    ...
    if (!ytRegex.test(text)) return m.reply(`URL YouTube tidak valid.\nContoh penggunaan:\n*${usedPrefix}${command} https://youtube.com/watch?v=YQHsXMglC9A*`);
    const isMP3 = /^zeemomp3$/i.test(command);
    m.react(wait);
    const data = await new ZeemoDl().download(text);
    if (!data?.data) throw new Error("Video tidak ditemukan. Silakan coba URL lain.");
    const {
...
class ZeemoDl {
  constructor() {
    ...
    this.urlv2 = "https://api.zeemo.ai/hy-caption-front/api/v1/video-download/youtube-video-info";
    this.headers = {
