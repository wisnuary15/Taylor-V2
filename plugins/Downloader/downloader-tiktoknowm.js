...
const handler = async (m, {
  ...
  try {
    ...
    if (args[0]) return conn.sendList(m.chat, htki + ' 📺 Tiktok Search 🔎 ' + htka,
      `⚡ Silakan pilih Tiktok Search di tombol di bawah...\n*Teks yang anda kirim:* ${args[0]}\n\nKetik ulang *${usedPrefix + command}* teks anda untuk mengubah teks lagi`,
      author, `☂️ Tiktok Search Disini ☂️`, listSections, m)
    if (args[1] === 'a') {
    ...
    if (args[1] === 'b') {
      let res = await fetch(`https://api.lolhuman.xyz/api/tiktok2?apikey=${lolkey}&url=${args[0]}`)
      let json = await res.json()
      let txt = `🚀 *Link:* ${await(await axios.get(`https://tinyurl.com/api-create.php?url=${args[0]}`)).data}`
      conn.sendFile(m.chat, json.result.link, '', txt, m)
    }
    if (args[1] === 'c') {
      let res = await fetch(`https://api.lolhuman.xyz/api/tiktok3?apikey=${lolkey}&url=${args[0]}`)
      let json = await res.json()
      let txt = `🚀 *Link:* ${await(await axios.get(`https://tinyurl.com/api-create.php?url=${args[0]}`)).data}`
      conn.sendFile(m.chat, json.result.link, '', txt, m)
    }
