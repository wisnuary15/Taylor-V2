...
const handler = async (m, {
  ...
  try {
    ...
    const {
      downloadLinks,
      title
    } = await new RetaTube().scrape(text);
    if (!downloadLinks.length) return m.reply("Video tidak ditemukan. Silakan coba URL lain.");
    const downloadLink = command === "retatubemp3" ? downloadLinks.find(link => link.url.endsWith(".mp3"))?.url : downloadLinks[0]?.url;
