...
const handler = async (m, {
  ...
  try {
    ...
    const searchResults = await appleMusic.ytsearch(appleSearchData.name, appleSearchData.artist, appleSearchData.albumname, appleSearchData.url);
    const videoData = await appleMusic.ytdl(searchResults.videoid);
    if (!videoData || !videoData.dlink) return m.reply("Tidak ada media ditemukan.");
    ...
    const infoReply = {
      contextInfo: {
        ...
        externalAdReply: {
          ...
          thumbnailUrl: appleSearchData.thumb,
          sourceUrl: ""
        }
      }
    };
