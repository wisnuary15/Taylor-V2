...
const getVideoData = async url => {
  try {
    ...
    const formattedVideoSrc = videoSrc && !videoSrc.startsWith("http") ? `https:${videoSrc}` : videoSrc;
    return {
...
const handler = async (m, {
  ...
  try {
    if (!args[0]) return await conn.reply(m.chat, `Use example: ${usedPrefix}${command} <URL>`, m);
    m.react(wait);
    const videoData = await getVideoData(args[0]);
    if (videoData.error) {
      return await conn.reply(m.chat, `Error fetching video data: ${videoData.error}`, m);
    }
    const responseText = `
