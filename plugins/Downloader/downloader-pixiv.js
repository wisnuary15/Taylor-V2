...
`
                conn.sendFile(m.chat, "https://pixiv.re/" + item.id + ".png" || item.meta_single_page.original_image_url || logo, "", cap, m)
            } catch (e) {
                m.reply(eror)
            }
        }
        ...
        if (feature === "vilipix") {
            ...
            try {
                ...
                let result = Object.entries(item.data.rows[0])
                    ?.map(([key, value]) => `	◦  *${key.charAt(0).toUpperCase() + key.slice(1).split('.').join(' ')}* : ${value}`)
                    .join('\n');
                conn.sendFile(m.chat, item.data.rows[0]?.original_url || item.data.rows[0]?.regular_url || logo, "", result, m)
            } catch (e) {
                m.reply(eror)
            }
        }
