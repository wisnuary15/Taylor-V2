...
const handler = async (m, {
  ...
  try {
    ...
    const {
      media,
      info
    } = await download(url, format, qualityToUse);
    ...
    const mimetype = format === "mp3" ? "audio/mpeg" : format === "m4a" ? "audio/m4a" : "audio/flac";
    if (format === "flac") {
...
handler.help = ["flydlmp3", "flydlm4a", "flydlflac"].map(v => `${v} <url> [quality]`);
