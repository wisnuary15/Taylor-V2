  ...
  try {
    ...
    if (lister.includes(feature)) {
      if (feature === "v1") {
        ...
        try {
          let lol = await fetch(`https://api.lolhuman.xyz/api/mediafire?apikey=${lolkey}&url=${inputs}`);
...
`;
          ...
          if (human.result.link) {
            conn.sendFile(m.chat, human.result.link, human.result.filename, "", m, null, {
...
`;
          ...
          await conn.relayMessage(m.chat, msg.message, {});
          if (bocil.url) {
            conn.sendFile(m.chat, bocil.url, bocil.filename, "", m, null, {
              mimetype: bocil.ext,
              asDocument: true
            });
          } else throw new Error("Error Gan");
...
`;
          ...
          await conn.relayMessage(m.chat, msg.message, {});
          if (scrap[0]?.link) {
            conn.sendFile(m.chat, scrap[0]?.link, scrap[0]?.nama, "", m, null, {
...
`;
          ...
          await conn.relayMessage(m.chat, msg.message, {});
          if (scrap.link) {
            conn.sendFile(m.chat, scrap.link, scrap.nama, "", m, null, {
              mimetype: scrap.mime,
              asDocument: true
            });
          } else throw new Error("Error Gan");
...
async function mediafireDl2(url) {
  try {
    ...
    const filesize = parseFloat(filesizeH) * (/GB/i.test(filesizeH) ? 1000000 : /MB/i.test(filesizeH) ? 1000 : /KB/i
      .test(filesizeH) ? 1 : /B/i.test(filesizeH) ? 0.1 : 0);
    return {
      link: Url,
      url2,
      nama: filename,
