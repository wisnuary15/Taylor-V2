...
const handler = async (m, {
  ...
  try {
    ...
    conn.sendFile(m.chat, img, '', txt, m, null, {
      contextInfo: {
        mimetype: 'audio/mp4',
        externalAdReply: {
...
async function twitterStalk(user) {
  try {
    ...
    const obj = {
      pp_user: $('div.relative.w-full.h-full.rounded-full.cursor-pointer.profile-image > img').text().trim() ??
        null,
      ...
      ...Object.fromEntries($('div.flex.justify-center > div.px-4').map((idx, el) => [
