...
class AppleMusic {
  async down(trackUrl) {
    try {
      ...
      const response1 = await fetch(`https://api.fabdl.com/apple-music/get?url=${encodeURIComponent(trackUrl)}`, {
        headers: headers
      });
      ...
      const response2 = await fetch(`https://api.fabdl.com/apple-music/mp3-convert-task/${gid}/${id}`, {
        headers: headers
      });
      ...
      while (isDownloading) {
        const response3 = await fetch(`https://api.fabdl.com/apple-music/mp3-convert-progress/${tid}`, {
          headers: headers
        });
  ...
  try {
    ...
    const infoReply = {
      contextInfo: {
        ...
        externalAdReply: {
          title: `A P P L E  M U S I C - A U D I O`,
