  ...
  try {
    ...
    if (data.length === 0) {
      ...
      for (let i = 0; i < data.length; i++) {
        conn.sendFile(m.chat, data[i], '', `Dari *(${i + 1}/${data.length})*`, m, false, {
          mentions: [m.sender]
        });
      }
    }
...
const decryptSnapSave = (data) => {
  const [h, u, n, t, e, r] = data.split("decodeURIComponent(escape(r))}(")[1].split("))")[0]?.split(",").map((v) => v
    .replace(/"/g, "").trim());
  const g = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ+/";
  const decode = (d, e, f) => {
    const h = g.slice(0, e);
    const i = g.slice(0, f);
    let j = d.split("").reverse().reduce((a, b, c) => (h.indexOf(b) !== -1) ? (a += h.indexOf(b) * Math.pow(e, c)) :
      a, 0);
    let k = "";
    while (j > 0) {
      k = i[j % f] + k;
...
const getImageLinks = async (url) => {
  ...
  try {
    ...
    const imageLinks = outs.join(',').split('<img').map(e => e.split('src="')[1]).filter(src => src && src.indexOf(
      'http') === 0).map(src => src.split('"')[0]);
    return imageLinks;
  } catch (error) {
    return {
      developer: "Wudysof",
