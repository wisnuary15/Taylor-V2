...
async function multiVideo(url) {
  try {
    ...
    const html = await (await fetch(proxyLink + "https://d0000d.com/f/" + idm[1])).text();
    const $ = cheerio.load(html);
    return [...new Set($("a[href]").get().map(el => el.attribs.href).filter(href => href.startsWith("https")))];
  } catch (e) {
    console.error(e);
    throw e;
  }
