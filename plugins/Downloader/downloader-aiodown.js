...
const handler = async (m, {
  ...
  try {
    ...
    if (!url) {
      return m.reply(`Masukkan URL video.\nContoh penggunaan:\n*${usedPrefix}${command}* <URL> [quality]`);
    }
    const formatMap = {
      aiodownmp3: "mp3",
      aiodownmp4: "mp4"
    ...
    const {
      mediaUrls
    } = await downloader.getDownloadLinks(url);
    if (!mediaUrls || mediaUrls.length === 0) throw new Error("Media tidak ditemukan atau gagal diunduh.");
    ...
    const qualityToUse = availableQualities.includes(inputQuality) ? inputQuality : availableQualities[0];
    const selectedDownloadLink = cleanQualities.find(media => media.quality === qualityToUse)?.url;
    if (!selectedDownloadLink) throw new Error("Gagal menemukan media dengan kualitas yang dipilih.");
