...
async function libDown(postUrl) {
  try {
    ...
    const {
      media
    } = await libDownload.igdl(postUrl);
    return media.map((v, i) => ({
      url: v,
      title: i
...
async function instagramVideos(postUrl) {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Gagal mengunduh video: ${response.status} ${response.statusText}`);
    }
    const {
      data
    } = await response.json();
    ...
    return result.filter((item, index, self) => index === self.findIndex(t => t.url === item.url));
  } catch (error) {
    console.error(error);
    return null;
  }
...
async function Shade(postUrl) {
  try {
    const response = await fetch(`https://nextbot.shade.cool/api/insta?url=${encodeURIComponent(postUrl)}`);
    const {
      data
    } = await response.json();
    const result = [{
