...
const handler = async (m, { conn: conn, args: args, usedPrefix: usedPrefix, command: command }) => {
  ...
  const { title: title, medias: medias } = await aiovideodl(args[0]);
  for (const { url: url, quality: quality, formattedSize: formattedSize } of medias) await conn.sendFile(m.chat, await (await fetch(url)).arrayBuffer(), "", `*AIOVIDEO DOWNLOADER*\n\n${title ? `*Title:* ${title}` : "NotFound"}\n*💽Format:* ${quality ? `${quality}` : "Unknown"}\n*📨Size:* ${formattedSize ? `${formattedSize}` : "countless"}\n`, m);
};
handler.help = ["aiovideo2"].map((v => v + " <url>")), handler.tags = ["downloader"],
