...
class VideoDownloader {
  async down(videoUrl) {
    try {
      const response = await fetch(`https://videodownloader.so/download?v=${encodeURIComponent(videoUrl)}&lang=en`, {
        method: "GET",
        headers: {
      ...
      return {
        ...
        videoData: $(".downloadsTable tbody tr").map((_, el) => {
