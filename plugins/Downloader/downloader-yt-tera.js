...
class YtTera {
  async down(videoUrl) {
    try {
      const response = await fetch("https://www.terabox.tech/api/yttera?id=" + videoUrl.split("/").pop(), {
        method: "GET"
      });
      ...
      try {
        const response = await fetch("https://wholly-api.skinnyrunner.com/get/website-data.php?get_html=https://www.terabox.tech/api/yttera?id=" + videoUrl.split("/").pop(), {
          method: "GET"
        });
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        const json = await response.json();
