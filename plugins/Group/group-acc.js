const handler = async (m, {
  ...
  switch (subCommand) {
    case "list":
      reply(`*Daftar Permintaan Bergabung:*\n\n${joinRequestList.length > 0 ? joinRequestList.map((request, i) => {
    ...
    case "approve":
      if ("all" === options) {
        for (const request of joinRequestList) await conn.groupRequestParticipantsUpdate(groupId, [request.jid], subCommand),
          console.log(`Meng-${subCommand} participant dengan JID: ${request.jid}`);
        reply(`*${"approve" === subCommand ? "Menyetujui" : "Menolak"} semua permintaan bergabung.*`);
      } else {
        ...
        if (participants.length > 0) {
          ...
          reply(`*${"approve" === subCommand ? "Menyetujui" : "Menolak"} Permintaan Bergabung:*\n\n${formattedResponse}`);
...
handler.help = ["acc *option*"], handler.tags = ["group"], handler.command = /^(acc)$/i,
  handler.group = !0, handler.admin = !0, handler.botAdmin = !0, handler.fail = null;
