...
○ ${usedPrefix + command} lock 5menit
    ...
    if (!actionDetails.check) {
      conn.reply(m.chat, `Grup ini sudah dalam kondisi yang tidak dapat diubah.`, m);
      return;
    }
    const timeInput = args[1];
    const timeMatch = timeInput.match(/^(\d+)([a-zA-Z]+)$/);
    if (!timeMatch || isNaN(timeMatch[1])) {
      conn.reply(m.chat, "Format waktu tidak valid. Gunakan format '1detik/menit/jam/hari'.", m);
      return;
    }
    ...
    if (!(timeUnit in timeUnits) || timeValue <= 0) {
      conn.reply(m.chat, "Waktu yang dimasukkan tidak valid. Gunakan angka positif untuk waktu.", m);
      return;
    }
    const timeInMilliseconds = timeValue * timeUnits[timeUnit];
    const actionMessage = actionDetails.message;
    const processingMessage = conn.reply(m.chat, `Sedang ${actionMessage} grup setelah ${timeInput}...`, m);
    setTimeout(async () => {
      await conn.groupSettingUpdate(m.chat, actionDetails.target);
      conn.reply(m.chat, `Grup telah ${actionMessage} setelah ${timeInput}`, processingMessage);
    }, timeInMilliseconds);
  } catch (error) {
    console.error(error);
