...
const handler = async (m, { conn: conn, text: text, participants: participants, groupMetadata: groupMetadata }) => {
  if (isNaN(text)) var number = text.split("@")[1];
  else if (!isNaN(text)) number = text;
  if (!text && !m.quoted) return await conn.reply(m.chat, "Berikan nomor, tag atau balas pesan target.", m);
  if (isNaN(number)) return await conn.reply(m.chat, "Nomor yang anda masukan tidak salah!", m);
  ...
  try {
    ...
    }) : (db.data.users[m.sender].pasangan = user, await conn.reply(m.chat, `Selamat anda resmi berpacaran dengan @${user.split("@")[0]}\n\nSemoga langgeng dan bahagia selalu @${user.split("@")[0]} 💓 @${me.split("@")[0]} 🥳🥳🥳`, m, {
      contextInfo: {
...
handler.help = ["terima *@tag*"], handler.tags = ["jadian"], handler.command = /^(terima)$/i,
  handler.group = !0, handler.limit = !1, handler.fail = null;
