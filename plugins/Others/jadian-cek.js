const handler = async (m, {
    ...
    if (db.data.users[user].pasangan === "") {
        ...
        conn.reply(m.chat, `${orang} sedang menunggu jawaban dari @${db.data.users[user].pasangan.split('@')[0]} karena sedang tidak diterima atau di tolak\n\nKetik *${usedPrefix}ikhlasin* untuk mengikhlaskan!`, m, {
            contextInfo: {
                mentionedJid: [db.data.users[user].pasangan]
            }
        })
    } else {
        conn.reply(m.chat, `${orang} sedang menjalani hubungan dengan @${db.data.users[user].pasangan.split('@')[0]} 💓💓💓`, m, {
            contextInfo: {
                mentionedJid: [db.data.users[user].pasangan]
            }
        })
