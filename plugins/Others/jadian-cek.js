const handler = async (m, {
  ...
  if (number.length > 15 || (number.length < 9 && number.length > 0)) return conn.reply(m.chat,
    `Maaf, Nomor yang anda masukan salah!`, m)
  if (!text && !m.quoted) {
    var user = m.sender
    var orang = "Kamu"
  } else if (text) {
  ...
  if (db.data.users[user].pasangan === "") {
    ...
    conn.reply(m.chat,
      `${orang} sedang menunggu jawaban dari @${db.data.users[user].pasangan.split('@')[0]} karena sedang tidak diterima atau di tolak\n\nKetik *${usedPrefix}ikhlasin* untuk mengikhlaskan!`,
      m, {
        contextInfo: {
    ...
    conn.reply(m.chat,
      `${orang} sedang menjalani hubungan dengan @${db.data.users[user].pasangan.split('@')[0]} 💓💓💓`, m, {
        contextInfo: {
          mentionedJid: [db.data.users[user].pasangan]
        }
      })
