...
const handler = async (m, { conn: conn, usedPrefix: usedPrefix, text: text, args: args, command: command }) => {
  ...
  if ("kanji" === args[0]) {
    for (let x of tes) {
      let res = await fetch("https://kanjiapi.dev/v1/" + args[0] + "/" + encodeURIComponent(x)),
        json = await res.json(),
        { kanji: kanji, grade: grade, stroke_count: stroke_count, meanings: meanings, kun_readings: kun_readings, on_readings: on_readings, name_readings: name_readings, jlpt: jlpt, unicode: unicode, heisig_en: heisig_en } = json;
  ...
  if ("reading" === args[0]) {
    for (let x of tes) {
      let res = await fetch("https://kanjiapi.dev/v1/" + args[0] + "/" + encodeURIComponent(x)),
        json = await res.json(),
        { reading: reading, main_kanji: main_kanji, name_kanji: name_kanji } = json;
