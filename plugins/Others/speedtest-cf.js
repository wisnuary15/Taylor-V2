...
const handler = async (m, {
  ...
  try {
    conn.reply(m.chat, 'Please wait, conducting speed test...', m);
    ...
    const downloadTests = [...(await measureDownload(101000, 10)), ...(await measureDownload(1001000, 8)), ...(
      await measureDownload(10001000, 6)), ...(await measureDownload(25001000, 4)), ...(await measureDownload(
      100001000, 1)), ];
    output += logDownloadSpeed(downloadTests);
    const uploadTests = [...(await measureUpload(11000, 10)), ...(await measureUpload(101000, 10)), ...(
