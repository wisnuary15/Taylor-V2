...
const handler = async (m, { conn: conn }) => {
  try {
    ...
    const [ping, serverLocationData, { ip: ip, loc: loc, colo: colo }] = await Promise.all([measureLatency(), fetchServerLocationData(), fetchCfCdnCgiTrace()]), city = serverLocationData[colo];
    let output = "```\n";
...
async function fetchServerLocationData() {
  return JSON.parse(await get("speed.cloudflare.com", "/locations")).reduce(((data, { iata: iata, city: city }) => {
    const data1 = data;
