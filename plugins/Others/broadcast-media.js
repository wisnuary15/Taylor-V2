...
const commandList = ["bcvn", "bcvid", "bcimg", "bcstick", "bctxt", "bctxt2"],
  ...
  handler = async (m, { conn: conn, command: command, args: args }) => {
    ...
    for (let id of groups)
      ...
      videoValue && (doc.video = videoValue, doc.mimetype = mimeVideo, doc.caption = teks,
      ...
      imageValue && (doc.image = imageValue, doc.mimetype = mimeImage, doc.caption = teks,
        await conn.sendMessage(id, doc, {
          quoted: null
        }));
      ...
      textValue && (doc.text = readMore, doc.contextInfo.externalAdReply.body = textValue,
        doc.contextInfo.externalAdReply.thumbnailUrl = flaaa.getRandom() + textValue, await conn.sendMessage(id, doc, {
          quoted: null
        }));
    } else if ("bctxt2" === command) {
      const textValue2 = m.quoted && "extendedTextMessage" === m.quoted?.mtype ? m.quoted?.text : teks;
      textValue2 && (doc.text = textValue2, doc.contextInfo.externalAdReply.body = author,
        await conn.sendMessage(id, doc, {
          quoted: null
        }));
    }
