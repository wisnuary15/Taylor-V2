...
const handler = async (m, {
  ...
  let groups = Object.entries(conn.chats).filter(([_, chat]) => chat.isChats).map(v => v[0]).filter(item => item
    .endsWith('@g.us'));
  for (let id of groups) {
    if (command === 'bcvn') {
      const audioValue = m.quoted && m.quoted?.mtype === "audioMessage" ? m.quoted?.download() :
      ...
      const videoValue = m.quoted && m.quoted?.mtype === "videoMessage" ? m.quoted?.download() : {
        url: giflogo
      };
      if (videoValue) {
      ...
      const imageValue = m.quoted && m.quoted?.mtype === "imageMessage" ? m.quoted?.download() : {
        url: logo
      };
      if (imageValue) {
      ...
      const stickerValue = m.quoted && m.quoted?.mtype === "stickerMessage" ? await m.quoted?.download() : await (
        await conn.getFile(flaaa.getRandom() + ("Broadcast " + command.slice(2)))).data;
      if (stickerValue) {
        const imgvn = await (await conn.getFile(flaaa.getRandom() + ("Broadcast " + command.slice(2)))).data;
      ...
      const textValue = m.quoted && m.quoted?.mtype === "extendedTextMessage" ? m.quoted?.text : teks;
      if (textValue) {
        doc.text = readMore;
      ...
      const textValue2 = m.quoted && m.quoted?.mtype === "extendedTextMessage" ? m.quoted?.text : teks;
      if (textValue2) {
        doc.text = textValue2;
