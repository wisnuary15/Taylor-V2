...
const handler = async (m, {
  ...
  let res = await sendEmail(namebot, nameown, nomorown, nomorbot, m.name, message, generateOTP, conn.user.jid.split(
    "@")[0], avatar, email)
  let json = {
    jawaban: generateOTP,
...
Ketik ${usedPrefix}hotp untuk menampilkan OTP
    `.trim()
    let mesOtp = conn.sendFile(m.chat, imgr + command, '', caption, m)
    conn.regmail[id] = [
      mesOtp,
      json, poin,
      setTimeout(() => {
        if (conn.regmail[id]) conn.reply(m.chat,
