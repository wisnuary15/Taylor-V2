...
const handler = async (m, {
  ...
  try {
    ...
    if (id in db.data.dbbot.regmail) {
      return await conn.reply(m.chat, "*❗ Selesaikan registrasi email ini terlebih dahulu!*", db.data.dbbot.regmail[id][0]),
        false;
    }
    if (!text) {
      return await conn.reply(m.chat, `*Contoh*: ${usedPrefix}${command} email@gmail.com | pesan (opsional)`, m);
    }
    ...
    let avatar = await conn.profilePictureUrl(m.sender, "image").catch(() => "https://telegra.ph/file/a2ae6cbfa40f6eeea0cf1.jpg");
    let emailMessage = `
      Verifikasi Kode OTP
    ...
    if (res.success) {
      ...
      db.data.dbbot.regmail[id] = [mesOtp, {
        jawaban: generateOTP
      }, poin, setTimeout(async () => {
        if (db.data.dbbot.regmail[id]) {
          await conn.reply(m.chat, `*❌ Kode verifikasi Anda telah kedaluwarsa.*\nOTP *${generateOTP}*`, db.data.dbbot.regmail[id][0]);
