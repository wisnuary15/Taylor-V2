const handler = async (m, { conn: conn, usedPrefix: usedPrefix, command: command, args: args }) => {
  const features = ["autodlTiktok", "autodlFacebook", "autodlInstagram", "autodlYoutube", "autolevelup", "antibot", "antiFoto", "antiVideo", "antiAudio", "antiCall", "antiDelete", "antiLink", "antiLinkFb", "antiLinkHttp", "antiLinkIg", "antiLinkTel", "antiLinkTik", "antiLinkWa", "antiLinkYt", "antiSatir", "antiSticker", "antiVirtex", "antiToxic", "antibule", "autoBio", "autoChat", "autoAi", "autoGpt", "autochatGpt", "autoJoin", "autoPresence", "autoReply", "autoSticker", "autoVn", "autoGempa", "viewStory", "bcjoin", "detect", "getmsg", "nsfw", "antiSpam", "simi", "alicia", "gptvoice", "characterai", "updateAnime", "updateAnimeNews", "viewonce", "welcome", "autoread", "gconly", "nyimak", "pconly", "self", "antirpg", "swonly", "lastAnime", "latestNews", "wabetainfo"].sort(),
    ...
    sections = features.map(((f, i) => {
      ...
      return {
        title: `${htki} ${i + 1} ${htka}`,
        highlight_label: "Switch " + status.trim(),
        rows: rows
      };
    })),
  ...
  activeFeatures.includes(featureName) ? db.data.chats[m.chat][featureName] = isEnable : ["autoChat"].includes(featureName) ? (conn.autochat = conn.autochat || {},
    conn.autochat.status = isEnable) : ["self", "pconly", "gconly", "swonly", "antirpg", "autoread", "jadibot", "restrict", "autorestart", "autorestart", "antibot"].includes(featureName) ? (db.data.settings[conn.user.jid][featureName] = isEnable,
    opts[featureName] = isEnable) : db.data.chats[m.chat][featureName] = isEnable, await conn.reply(m.chat, `Feature *${featureName.replace(/([A-Z])/g, " $1").replace(/^./, (str => str.toUpperCase()))}* mode *${isEnable ? "ON" : "OFF"}*`, m);
};
