const handler = async (m, {
    ...
    const sections = features?.map((f, i) => {
        const isActive = (["self", "pconly", "gconly", "swonly", "antirpg", "autoread", "jadibot", "restrict", "autorestart", "autorestart", "antibot"].includes(f)) ?
            activeFeatures.includes(f) ? !db.data.settings[conn.user.jid][f] : db.data.settings[conn.user.jid][f] :
            activeFeatures.includes(f) ? !db.data.chats[m.chat][f] : db.data.chats[m.chat][f];
