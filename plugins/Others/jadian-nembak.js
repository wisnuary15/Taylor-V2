...
const handler = async (m, {
  ...
  try {
    ...
    if (!users) return conn.reply(m.chat,
      `*_Target atau Nomor tidak ditemukan, mungkin sudah keluar atau bukan anggota grup ini.*_`, m)
    ...
    if (typeof db.data.users[user] === "undefined") return m.reply(
    ...
    if (db.data.users[m.sender].pasangan != "" && db.data.users[db.data.users[m.sender].pasangan].pasangan === m
      .sender && db.data.users[m.sender].pasangan != user) {
      conn.reply(m.chat,
        `Kamu sudah berpacaran dengan @${db.data.users[m.sender].pasangan.split('@')[0]}\n\nSilahkan putus dulu (ketik .putus untuk memutuskan hubungan) untuk menembak @${user.split('@')[0]}\n\nBtw yang setia dikit banget!`,
        m, {
          contextInfo: {
            mentionedJid: [user, db.data.users[m.sender].pasangan]
          }
        })
    } else if (db.data.users[user].pasangan != "") {
      if (pacar) {
        if (m.sender === pacar && db.data.users[m.sender].pasangan === user) return conn.reply(m.chat,
          `Anda sudah berpacaran dengan ${spac.split('@')[0]}`, m, {
            contextInfo: {
              mentionedJid: [spac]
            }
          })
