...
const handler = async (m, { conn: conn, args: args, usedPrefix: usedPrefix, text: text, command: command }) => {
  ...
  if (!canLevelUp(user.level, user.exp, multiplier)) {
    let { min: min, xp: xp, max: max } = xpRange(user.level, multiplier);
  ...
  if (beforeLevel !== user.level) {
    let ppuser, teks = `Selamat ${conn.getName(m.sender)} naik 🧬level\n.             ${user.role}`,
      ...
      { min: min, xp: xp, max: max } = xpRange(user.level, multiplier);
    await conn.profilePictureUrl(m.sender).catch((_ => "./src/avatar_contact.png")),
      user.exp, user.role, user.level, m.sender.substring(3, 7), Object.entries(db.data.users).sort(((a, b) => b[1].level - a[1].level)).map((v => v[0])).indexOf(m.sender);
    try {
