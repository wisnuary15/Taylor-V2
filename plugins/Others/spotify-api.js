  ...
  try {
    ...
    const response = await fetch(searchUrl, {
      headers: {
        ...
        Referer: "https://spotifyapi.caliphdev.com/#/default/get_api_search_tracks"
      }
    });
    ...
    const downloadRes = await fetch(downloadUrl, {
      headers: {
        accept: "application/json",
        "User-Agent": "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/128.0.0.0 Mobile Safari/537.36",
        Referer: "https://spotifyapi.caliphdev.com/#/default/get_api_download_track"
      }
    });
    if (!downloadRes.ok) throw new Error(`HTTP error! Status: ${downloadRes.status}`);
