...
const handler = async (m, {
  ...
  if (!support.convert && !support.magick && !support.gm) return handler.disabled =
    true // Disable if doesnt support
  ...
  if (command === 'nuliskiri') {
    for (let i = 0; i < teks.length; i++) {
      let bufs = [];
      const [_spawnprocess, ..._spawnargs] = [...(support.gm ? ['gm'] : support.magick ? ['magick'] : []),
        'convert',
        bukual, '-font',
        fontPath, '-size', '960x1280', '-pointsize', '22', '-interline-spacing', '2', '-annotate', '+140+153',
        hari, '-font',
        fontPath, '-size', '1024x784', '-pointsize', '18', '-interline-spacing', '1', '-annotate', '+806+102',
        tgl, '-font',
        fontPath, '-size', '1024x784', '-pointsize', '20', '-interline-spacing', '-7.5', '-annotate', '+344+142',
        teks[i], 'jpg:-'
      ]
      spawn(_spawnprocess, _spawnargs).on('error', e => m.reply(format(e))).on('close', async () => {
        await delay(1000)
        conn.sendFile(m.chat, Buffer.concat(bufs), '', `Bagian ${i + 1} dari ${teks.length}`, m)
  ...
  if (command === 'nuliskanan') {
    for (let i = 0; i < teks.length; i++) {
      ...
      const [_spawnprocess, ..._spawnargs] = [...(support.gm ? ['gm'] : support.magick ? ['magick'] : []),
        'convert',
        bukuar, '-font',
        fontPath, '-size', '960x1280', '-pointsize', '23', '-interline-spacing', '2', '-annotate', '+128+129',
        hari, '-font',
        fontPath, '-size', '1024x784', '-pointsize', '18', '-interline-spacing', '1', '-annotate', '+806+102',
        tgl, '-font',
        fontPath, '-size', '1024x784', '-pointsize', '20', '-interline-spacing', '-7.5', '-annotate', '+344+142',
  ...
  if (command === 'foliokanan') {
    for (let i = 0; i < teks.length; i++) {
      ...
      const [_spawnprocess, ..._spawnargs] = [...(support.gm ? ['gm'] : support.magick ? ['magick'] : []),
        'convert',
        folioal, '-font',
        fontPath, '-size', '1720x1280', '-pointsize', '23', '-interline-spacing', '4', '-annotate', '+48+185',
        hari, '-font',
        fontPath, '-size', '1024x784', '-pointsize', '18', '-interline-spacing', '1', '-annotate', '+806+102',
        tgl, '-font',
        fontPath, '-size', '1024x784', '-pointsize', '20', '-interline-spacing', '-7.5', '-annotate', '+344+142',
  ...
  if (command === 'foliokanan') {
    for (let i = 0; i < teks.length; i++) {
      ...
      const [_spawnprocess, ..._spawnargs] = [...(support.gm ? ['gm'] : support.magick ? ['magick'] : []),
        'convert',
        folioar, '-font',
        fontPath, '-size', '960x1280', '-pointsize', '23', '-interline-spacing', '3', '-annotate', '+89+190',
        hari, '-font',
        fontPath, '-size', '1024x784', '-pointsize', '18', '-interline-spacing', '1', '-annotate', '+806+102',
        tgl, '-font',
        fontPath, '-size', '1024x784', '-pointsize', '20', '-interline-spacing', '-7.5', '-annotate', '+344+142',
