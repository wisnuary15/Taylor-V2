...
`).join("\n")
    m.reply(list)
    m.reply("Untuk mengambil data, ketik *" + usedPrefix + command + "* url diatas")
  }
  if (command === "ehgmdata") {
...
`).join("\n")
    m.reply(list)
    m.reply("Untuk mengambil link torrent, ketik *" + usedPrefix + command + "* GID TOKEN")
  }
...
`).join("\n")
    m.reply(list)
    m.reply("Download uttorrent di playstore, dan input url diatas")
  }
}
