...
const handler = async (m, {
  ...
  if (text && /soundcloud\.com\/[^/]+\//.test(args[0])) {
    ...
    let msg = /^(?:-|--)doc$/i.test(args[1]) ? conn.sendFile(m.chat, data.thumbnail, '', txt, m) : await conn
      .sendButton(m.chat, txt, wait, data.thumbnail, [
        ['Audio (Document)', `${_p + cmd} ${args[0]} --doc`]
      ], m)
    let url = data.streams.find(v => /mp3/.test(v.extension) && v.has_audio).url
    let audio = await sc.req(url, {
...
export class SoundCloud {
  ...
  async search(query) {
    let res = await this.req(
      `https://api-mobi.soundcloud.com/search?q=${query}&client_id=iZIs9mchVcX5lhVRyQGGAYlNPVldzAoX&stage=`)
