  ...
  if (args[0] === 'search') {
    ...
    Object.values(sul).map((v, index) => {
      listSections.push(["Num. " + ++index, [
        ...
        ['Detail (v2) ' + v.track.title, usedPrefix + command + " detailv2 |" + v.track.hub.actions[0]
          ?.id, v.track.hub.type
        ],
        ['Music ' + v.track.title, usedPrefix + "get " + v.track.hub.actions[1].uri, v.track.hub.type]
      ]])
    })
    return conn.sendList(m.chat, htki + " 🗒️ List Effect " + htka, "⚡ Silakan pilih efek yang anda mau.", author,
      "[ Effect ]", listSections, m)
  }
  if (args[0] === 'event') {
