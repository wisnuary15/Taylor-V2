...
import got from "got";
const handler = async (m, { text: text, args: args, usedPrefix: usedPrefix, command: command }) => {
  let who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? conn.user.jid : m.sender,
    q = (await conn.profilePictureUrl(who).catch((_ => hwaifu.getRandom())),
      conn.getName(who), m.quoted ? m.quoted : m);
  if (!args[0]) {
