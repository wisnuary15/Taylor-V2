...
const fetchData = async (query, page) => {
...
}, handler = async (m, { conn: conn, text: text }) => {
  ...
  try {
    const { postInfos: postInfos } = await fetchData(query, pageNum);
    if (postInfos && postInfos.length > 0)
      if (partNum > 0 && partNum <= postInfos.length) {
        const { postID: postID, userID: userID, title: title, totalUpvotes: totalUpvotes, totalDownvotes: totalDownvotes, totalComments: totalComments, createTime: createTime, media: media, sensitive: sensitive, userUsername: userUsername } = postInfos[partNum - 1], message = `\n📌 *Post ID:* ${postID}\n👤 *User ID:* ${userID}\n📜 *Title:* ${title}\n👍 *Total Upvotes:* ${totalUpvotes}\n👎 *Total Downvotes:* ${totalDownvotes}\n💬 *Total Comments:* ${totalComments}\n⏰ *Create Time:* ${new Date(createTime).toLocaleString()}\n🖼️ *Media:* ${media}\n🚫 *Sensitive:* ${sensitive ? "Yes" : "No"}\n🧑‍💼 *User Username:* ${userUsername}\n\n📚 Contoh penggunaan: *lahelu query|page|part*`;
