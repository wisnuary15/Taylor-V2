...
const handler = async (m, { conn: conn, text: text, usedPrefix: usedPrefix, command: cmd }) => {
  let num = m.quoted?.sender || m.mentionedJid?.[0] || text;
  if (!num) throw `Ex: ${usedPrefix + cmd} @tag / 628xxx`;
  if (num = num.replace(/\D/g, "") + "@s.whatsapp.net", !(await conn.onWhatsApp(num))[0]?.exists) throw "User not exists";
...
handler.help = ["WhatsappStalk"], handler.tags = ["tools"], handler.command = /^(wa|whatsapp)stalk$/i;
