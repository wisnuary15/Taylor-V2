...
const handler = async (m, {
  ...
  try {
    ...
    if (!/video|audio/.test(mime)) return m.reply(`Reply video/audio yang ingin dikonversi dengan caption *${usedPrefix + command}*`);
    let media = await q.download();
    if (!media) return m.reply("Tidak bisa mengunduh media");
    let is8k = args[0] === "8k",
      audio;
    audio = await (is8k ? toAudio8k : toAudio)(media, "mp3").catch(async () => await (is8k ? toAudio : toAudio8k)(media, "mp3"));
