...
async function jadwalsholat(kota) {
  ...
  const precisionPrediction = prediction.filter(pred => pred.score >= .85).sort((a, b) => b.score - a.score)[0];
  if (!precisionPrediction) {
    throw new Error(`Did you mean ${prediction.map(item => item.city).join(", ")}?\n\nList of cities: ${cities.map(city => `- ${city}`).join("\n")}`);
  }
  const jadwal = listJadwalSholat.find(item => item.kota === precisionPrediction.city);
  const response = await fetch(`https://jadwalsholat.org/jadwal-sholat/monthly.php?id=${jadwal.value}`);
  if (!response.ok) {
  ...
  const schedules = $("tbody > tr[align=center]").filter(".table_highlight, .table_light, .table_dark").map(function() {
