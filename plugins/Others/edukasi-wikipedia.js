...
`
    conn.sendFile(m.chat, item.image, "", caption, m)
  } catch (e) {
    m.react(eror)
  }
}
...
async function Wikipedia(query) {
  ...
  $('table.infobox tr').each((index, element) => {
    ...
    const value = $(element).find('td.infobox-data').text().trim() || $(element).find('td.infobox-data a').text()
      .trim();
    if (label && value) {
  ...
  const data = {
    ...
    image: 'https:' + ($('#mw-content-text img').attr('src') ||
