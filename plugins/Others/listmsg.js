const handler = async (m, {
  ...
  let msg = (Object.entries(msgs).map(([nama, isi]) => {
    return {
      nama,
      ...isi
    }
  ...
  if (chat.getmsg === false) return conn.reply(m.chat, `kamu harus mengaktifkan getmsg dengan mengetik\n.on getmsg`,
    m)
  if (msg[0]) return conn.sendList(m.chat, htki + ' 📺 LIST MESSAGE 🔎 ' + htka,
    `⚡ Hai ${name} Berikut daftar Menu yg Ada di List msg...\nAkses langsung dengan mengetik namanya`, author,
    `☂️ ${command} Klik Disini ☂️`, listSections, m)
