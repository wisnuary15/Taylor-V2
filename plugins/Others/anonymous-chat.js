  ...
  try {
    ...
    switch (conn.anonymous = conn.anonymous || {}, input) {
      ...
      case "leave": {
        ...
        if (other && await conn.reply(other, "*Partner meninggalkan chat*", m), delete conn.anonymous[room.id],
          "leave" === input) break;
      }
      case "start": {
        ...
        if (room) await conn.reply(room.a, "*Partner ditemukan!*", m), room.b = m.sender, room.state = "CHATTING";
        else {
          let id = +new Date;
          conn.anonymous[id] = {
            id: id,
