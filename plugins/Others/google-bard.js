...
const handler = async (m, {
  ...
  try {
    if (mime && command === "bardimg") {
      ...
      for (const func of [RizzBardImg, joshweb_img, GeminiMilay]) {
        try {
      ...
      for (const func of [RizzBard, joshweb, widipeBard, GeminiMilay, GoogleBard]) {
        try {
          res = await func(text);
          if (res) break;
        } catch (e) {
...
async function GeminiMilay(prompt, image = null) {
  try {
    const blob = image ? Buffer.isBuffer(image) ? new Blob([image], {
    ...
    const response = image ? await fetch("https://api-milay-gemini.vercel.app/api/process", {
      method: "POST",
      body: form
    }).then(res => res.json()) : await fetch(`https://api-milay-gemini.vercel.app/api/query?prompt=${encodeURIComponent(prompt)}`).then(res => res.json());
