let handler = async (m, {
    ...
    try {
        ...
        if (id in conn.absen) {
            await conn.sendButton(m.chat, `_*Masih ada absen di chat ini!*_\n\n*${usedPrefix}hapusabsen* - untuk menghapus absen`,author, null, [["Hapus Absen", `${usedPrefix}hapusabsen`]], m)
        } else {
            conn.absen[id] = [
                await conn.sendButton(m.chat, `Berhasil memulai absen!\n\n*${usedPrefix}absen* - untuk absen\n*${usedPrefix}cekabsen* - untuk mengecek absen\n*${usedPrefix}hapusabsen* - untuk menghapus data absen`, author, null, [["Absen", `${usedPrefix}absen`], ["Cek Absen", `${usedPrefix}cekabsen`], ["Hapus Absen", `${usedPrefix}hapusabsen`]], m),
                [],
                text
            ]
        }
