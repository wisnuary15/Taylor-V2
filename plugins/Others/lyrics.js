...
handler.help = ["lirik"].map((v => v + " <judul>")), handler.tags = ["internet"],
...
async function getLyricsFreakLyrics(songTitle) {
  try {
    const { data: data } = await axios.get(`https://www.lyricsfreak.com/search.php?a=search&q=${songTitle}`), songLink = cheerio.load(data)(".song").attr("href");
