...
const handler = async (m, { conn: conn, text: text }) => {
  try {
    const { quote: quote, character: character, anime: anime } = text ? await quotesByName(text) : await quotesRandom();
...
async function quotesRandom() {
  try {
    const { quote: quote, character: character, anime: anime } = await (await fetch("https://animechan.xyz/api/random")).json();
    return {
      quote: quote,
      character: character,
      anime: anime
...
async function quotesByName(name) {
  try {
    const { quote: quote, character: character, anime: anime } = await (await fetch(`https://animechan.xyz/api/random/character?name=${name}`)).json();
    return {
      quote: quote,
      character: character,
      anime: anime
