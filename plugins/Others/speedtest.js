...
const exec = promisify(execCallback),
  handler = async (m, { conn: conn }) => {
    try {
      const { stdout: stdout, stderr: stderr } = await exec("python3 speed.py --share --json"), { download: download, upload: upload, ping: ping, server: server, client: client, timestamp: timestamp, bytes_sent: bytes_sent, bytes_received: bytes_received, share: share } = JSON.parse(stdout.match(/\{(.|\n)*\}/)[0]), message = `\n*🚀 Download Speed*: ${formatSpeed(download)} Mbps\n*📤 Upload Speed*: ${formatSpeed(upload)} Mbps\n*⏱️ Ping*: ${ping} ms\n\n*Server Details*:\n  *Name*: ${server.name}\n  *Country*: ${server.country}\n  *Sponsor*: ${server.sponsor}\n\n*Client Details*:\n  *IP*: ${client.ip}\n  *ISP*: ${client.isp}\n  *Country*: ${client.country}\n\n*📅 Timestamp*: ${formatTimestamp(timestamp)}\n*💾 Bytes Sent*: ${formatBytes(bytes_sent)}\n*💽 Bytes Received*: ${formatBytes(bytes_received)}\n*🔗 Share Link*: ${share}\n`;
