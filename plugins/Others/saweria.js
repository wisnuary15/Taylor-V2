...
const handler = async (m, { conn: conn, isOwner: isOwner, usedPrefix: usedPrefix, command: command, args: args }) => {
  ...
  if ("payment" === args[0] || "unban" === args[0] || "unblock" === args[0]) {
    ...
    m.reply(`Status Pembayaran: ✅ ${json.msg}`).then((() => {
      ...
      "PREMIUM" === gateway.package ? (data.limit += 5e3, data.expired += data.premium ? 2592e6 : Date.now() + 2592e6,
          data.premium = !0) : "UNBAN" === gateway.package ? (data.banned = !1, data.banTemp = 0,
          data.banTimes = 0) : "UNBLOCK" === gateway.package ? conn.updateBlockStatus(gateway.jid, "unblock") : "DEPOSITO" === gateway.package && (data.balance += gateway.amount),
