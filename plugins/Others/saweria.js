...
const handler = async (m, {
  ...
  if (args[0] === 'payment' || args[0] === 'unban' || args[0] === 'unblock') {
    ...
    if (pending || process) {
      return m.reply(
        `🚩 Selesaikan terlebih dahulu proses sebelumnya atau kirim *${usedPrefix}saweria n* untuk membatalkan.`);
    }
    ...
    teks +=
      `Kirim *${usedPrefix}saweria y* untuk melanjutkan proses pembayaran atau kirim *${usedPrefix}saweria n* untuk membatalkan.`;
    m.reply(teks).then(() => {
      conn.gateway.push({
        state: 'PENDING',
        jid: m.sender,
    ...
    teks +=
      `- Setelah pembayaran, tunggu sebentar lalu kirim *${usedPrefix}saweria check* untuk cek status pembayaran.\n`;
