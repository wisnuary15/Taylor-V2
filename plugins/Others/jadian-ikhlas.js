const handler = async (m, { conn: conn }) => "" === db.data.users[m.sender].pasangan ? await conn.reply(m.chat, "Kamu sedang tidak menembak siapapun!", m) : db.data.users[db.data.users[m.sender].pasangan].pasangan === m.sender ? await conn.reply(m.chat, `Kamu telah berpacaran dengan @${db.data.users[m.sender].pasangan.split("@")[0]}`, m, {
  contextInfo: {
