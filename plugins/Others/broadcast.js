...
let handler = async (m, { conn, text, args, usedPrefix, command }) => {
    ...
    for (let id of chats) {
        ...
        if (args[0] == 'polling') {
            ...
            const pollMessage = { name: cap, values: a, multiselect: false, selectableCount: 1 }
            await conn.sendMessage(id, { poll: pollMessage })
        } else if (args[0] == 'sharebot') {
            let contactArray = !imgr.endsWith('@g.us') ? { protocolMessage: { type: 11 } } : [
...
function checkDuplicate(arr) { return new Set(arr).size !== arr.length }
