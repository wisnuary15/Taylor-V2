  ...
  for (let id of chats) {
    ...
    if (args[0] === 'polling') {
      ...
      let contactArray = !imgr.endsWith('@g.us') ? {
        ...
        [conn.user.jid.split("@")[0], conn.getName(conn.user.jid), "🔥 Bot WhatsApp 🐣",
          "📵 Dont spam/call me 😢", "Nothing", "🇮🇩 Indonesia", "🚀 https://s.id/Cerdasin62/",
          "🤖 Hanya bot biasa yang kadang suka eror ☺"
        ]
      ]
      await conn.sendContactArray(id, contactArray, m)
    } else {
      conn.sendFile(id, imgr + 'BROADCAST', '', htki + ' *BROADCAST* ' + htka + '\n\n*Pesan:*\n' + text, m)
    }
  }
