...
const handler = async (m, {
  ...
  let parentw = conn
  if (conn.user.jid !== conn.user.jid) return parentw.reply(m.chat,
    'Perintah ini hanya dapat digunakan di bot utama! wa.me/' + conn.user.jid.split('@')[0], m)
  ...
  exec(comb.toString("utf-8"), async (err, stdout, stderr) => {
    ...
    async function jddt() {
      ...
      args[0] ? fs.writeFileSync("./jadibot/" + uniqid + "/creds.json", JSON.stringify(JSON.parse(Buffer
        .from(args[0], "base64").toString("utf-8")), null, '\t')) : ""
      const {
        state,
        saveState,
        saveCreds
      ...
      async function connectionUpdate(update) {
        ...
        const code = lastDisconnect?.error?.output?.statusCode || lastDisconnect?.error?.output?.payload
          ?.statusCode
        console.log(code)
        ...
        if (connection === 'open') {
          ...
          await parentw.sendMessage(m.chat, {
            text: args[0] ?
              "Berhasil terhubung ✅" : "*Berhasil terhubung✅* Dalam beberapa detik, kami akan mengirimkan ID yang harus Anda gunakan untuk terhubung kembali... Kirimkan ini ke bot secara pribadi ketika bot terputus."
          }, {
            quoted: m
          })
          await sleep(5000)
          if (!args[0]) parentw.sendMessage(m.chat, {
            text: usedPrefix + command + " " + Buffer.from(fs.readFileSync("./jadibot/" + uniqid +
              "/creds.json"), "utf-8").toString("base64")
          }, {
            quoted: m
          })
        }
