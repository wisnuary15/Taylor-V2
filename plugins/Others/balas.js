...
const handler = async (m, { conn, text, usedPrefix: _p }) => {
    ...
    if (!number) return conn.reply(m.chat, 'Silakan masukkan nomor yang akan dikirim', m);
    if (!pesan) return conn.reply(m.chat, 'Silakan masukkan pesannya', m);
    if (pesan.length > 500) return conn.reply(m.chat, 'Teks terlalu panjang!', m);
    const korban = `${number}@s.whatsapp.net`;
    const spam1 = `*「 SUKSES 」*\n\nDari: wa.me/${number}\nPesan: ${pesan}\n\n${wm}`;
    conn.reply(korban, spam1, 0, {
        contextInfo: {
