...
const search = async (query, limitValue = 50) => {
  try {
    const response = await fetch(`${MusicApiJamendoBaseUrl}/v3.0/tracks/?client_id=f5db3eb4&format=json&limit=${limitValue}&order=downloads_total&imagesize=200&namesearch=${encodeURIComponent(query)}`);
    if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
    const data = await response.json();
    if (data.results?.length) {
      return data.results.filter(v => v.audiodownload_allowed && v.audio).map(obj => ({
        title: obj.name,
...
const spotifyDownload = async query => {
  try {
    ...
    const searchUrl = isSpotifyLink ? `https://manaxu-seven.vercel.app/api/downloader/spotify?url=${query}` : `https://manaxu-seven.vercel.app/api/internet/spotify?query=${encodeURIComponent(query)}`;
    const res = await fetch(searchUrl);
    const data = await res.json();
    if (!data.status || (isSpotifyLink ? !data.result : data.result.length === 0)) {
      throw new Error("Tidak ditemukan hasil untuk query tersebut.");
    }
    return isSpotifyLink ? data.result : (await fetch(`https://manaxu-seven.vercel.app/api/downloader/spotify?url=${data.result[0]?.link}`)).json().result;
  } catch (e) {
    return {
...
🎶 *Title:* ${title}
🎤 *Artist:* ${artis}
⏱️ *Duration:* ${Math.floor(durasi / 6e4)}:${Math.floor(durasi % 6e4 / 1e3).toString().padStart(2, "0")}
📥 *Download:* ${download}
    `;
