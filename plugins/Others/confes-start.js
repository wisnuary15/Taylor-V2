const handler = async (m, {
  ...
  if (Object.values(conn.confess).find(mf => !0 === mf.status)) return !0;
  try {
    let id = +new Date(),
    ...
    await conn.reply(data.jid, txt, null).then(() => (m.reply("Berhasil mengirim pesan confess."), conn.confess[id] = {
      id: id,
      dari: m.sender,
      nama: name,
      penerima: data.jid,
