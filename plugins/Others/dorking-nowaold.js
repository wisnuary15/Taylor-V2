const handler = async (m, {
  ...
  let txt = '\t*• 2009 - 2016 •*\n' + data.filter(v => v.exists && !/Date/.test(v.setAt) && /09|1[0-6]$/.test(v
      .setAt)).map(v => `No : @${splitM(v.jid)}\nBio : ${v.status || ''}\nDate : ${v.setAt}`).join('\n\n') +
    '\n\n\t*• 2017 - NOW •*\n' + data.filter(v => v.exists && /1[7-9]|2[0-2]$/.test(v.setAt)).map(v =>
      `@${splitM(v.jid)}`).join('\n') + '\n\n\t*• INVALID DATE •*\n' + data.filter(v => v.exists && /Date/.test(v
      .setAt)).map(v => `@${splitM(v.jid)}`).join('\n')
  m.reply(txt, '', {
