...
const handler = async (m, {
  ...
  let Endspoint = ["artistEvents", "artistSearch", "artistTopFans", "artistTopTracks", "compareUserTaste",
    "compareUserToArtist", "eventAttendes", "eventInfo", "eventShout", "libraryAlbum", "libraryArtist",
    "libraryTracks", "metroArtistChart", "metroEvents", "metroHypeTrackChart", "metroTrackChart", "metros",
    "trackSearch", "trackSimilar", "trackTopFan", "userRecentTracks", "userRecommendedArtists", "userTopArtist",
    "userTopTracks", "userWeeklyArtistChart", "userWeeklyCharts", "userWeeklyTrackChart"
  ]
...
function compareUserTaste(user1, user2) {
  return apiUrl + "?method=tasteometer.compare&type1=user&type2=user&value1=" + user1 + "&value2=" + user2 +
    "&api_key=" + apiKey + "&format=json"
}

function compareUserToArtist(artist, user) {
  return apiUrl + "?method=tasteometer.compare&type1=artists&type2=user&value1=" + artist + "&value2=" + user +
    "&api_key=" + apiKey + "&format=json"
}
...
function metroHypeTrackChart(metro, country) {
  return apiUrl + "?method=geo.getmetrohypetrackchart&country=" + country + "&metro=" + metro + "&api_key=" + apiKey +
    "&format=json"
}
...
function metroArtistChart(metro, country) {
  return apiUrl + "?method=geo.getmetroartistchart&country=" + country + "&metro=" + metro + "&api_key=" + apiKey +
    "&format=json"
}

function metroTrackChart(metro, country) {
  return apiUrl + "?method=geo.getmetrotrackchart&country=" + country + "&metro=" + metro + "&api_key=" + apiKey +
    "&format=json"
}
