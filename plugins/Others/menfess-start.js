import fs from "fs";
const handler = async (m, { conn: conn, number: number, text: text, usedPrefix: usedPrefix, command: command }) => {
  m.sender.replace(/@.+/, ""), m.sender;
  if (conn.menfess = conn.menfess ? conn.menfess : {}, !text) throw `*Cara penggunaan :*\n\n${usedPrefix + command} nomor|nama pengirim|pesan\n\n*Note:* nama pengirim boleh nama samaran atau anonymous.\n\n*Contoh:* ${usedPrefix + command} ${m.sender.split("@")[0]}|Anonymous|Hai.`;
  ...
  if (Object.values(conn.menfess).find((mf => !0 === mf.status))) return !0;
  let id = +new Date,
