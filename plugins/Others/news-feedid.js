...
const handler = async (m, {
  ...
  if (command === 'antaranews') {
    let ar = ['terbaru', 'politik', 'hukum', 'ekonomi', 'metro', 'bola', 'olahraga', 'humaniora', 'lifestyle',
...
`.trim()
    ...
    conn.sendFile(m.chat, news.thumbnail, 'antaranews.png',
      `${news.title}\n\nDeskripsi :\n${news.description}\n\nSelengkapnya :\n${news.link}`, m)
  }
  if (command === 'cnbc') {
    let ar = ['terbaru', 'investment', 'news', 'market', 'enterpreneur', 'syariah', 'tech', 'lifestyle', 'opini',
      'profil'
    ]
    let er = `
...
`.trim()
    ...
    let news = Array[Math.floor(Math.random() * Array.length)]
    if (!news.thumbnail) throw `ERROR\nGagal Mengambil Berita`
    conn.sendFile(m.chat, news.thumbnail, 'cnbc.png',
      `${news.title}\n\nDeskripsi :\n${news.description}\n\nSelengkapnya :\n${news.link}`, m)
  }
  if (command === 'cnn') {
...
`.trim()
    ...
    let news = Array[Math.floor(Math.random() * Array.length)]
    if (!news.thumbnail) throw `ERROR\nGagal Mengambil Berita`
    conn.sendFile(m.chat, news.thumbnail, 'cnn.png',
      `${news.title}\n\nDeskripsi :\n${news.description}\n\nSelengkapnya :\n${news.link}`, m)
  }
  if (command === 'jpnn') {
    ...
    let news = Array[Math.floor(Math.random() * Array.length)]
    if (!news.thumbnail) throw `ERROR\nGagal Mengambil Berita`
    conn.sendFile(m.chat, news.thumbnail, 'JPNN.png',
      `${news.title}\n\nDeskripsi :\n${news.description}\n\nSelengkapnya :\n${news.link}`, m)
  }
  if (command === 'kumparan') {
    ...
    let news = Array[Math.floor(Math.random() * Array.length)]
    if (!news.thumbnail) throw `ERROR\nGagal Mengambil Berita`
    conn.sendFile(m.chat, news.thumbnail, 'kumparan.png',
      `${news.title}\n\nDeskripsi :\n${news.description}\n\nSelengkapnya :\n${news.link}`, m)
  }
  if (command === 'merdeka') {
    let ar = ['terbaru', 'jakarta', 'dunia', 'gaya', 'olahraga', 'teknologi', 'otomotif', 'khas', 'sehat', 'jabar',
      'jatim', 'jateng', 'sumut'
    ]
    let er = `
...
`.trim()
    ...
    let news = Array[Math.floor(Math.random() * Array.length)]
    if (!news.thumbnail) throw `ERROR\nGagal Mengambil Berita`
    conn.sendFile(m.chat, news.thumbnail, 'merdekadotcom.png',
      `${news.title}\n\nDeskripsi :\n${news.description}\n\nSelengkapnya :\n${news.link}`, m)
  }
  if (command === 'okezone') {
...
`.trim()
    ...
    let news = Array[Math.floor(Math.random() * Array.length)]
    if (!news.thumbnail) throw `ERROR\nGagal Mengambil Berita`
    conn.sendFile(m.chat, news.thumbnail, 'okezonedotcom.png',
      `${news.title}\n\nDeskripsi :\n${news.description}\n\nSelengkapnya :\n${news.link}`, m)
  }
...
`.trim()
    ...
    let news = Array[Math.floor(Math.random() * Array.length)]
    if (!news.thumbnail) throw `ERROR\nGagal Mengambil Berita`
    conn.sendFile(m.chat, news.thumbnail, 'republikadotcom.png',
      `${news.title}\n\nDeskripsi :\n${news.description}\n\nSelengkapnya :\n${news.link}`, m)
  }
  if (command === 'sindo') {
    let ar = ['terbaru', 'nasional', 'metro', 'ekbis', 'international', 'daerah', 'sports', 'otomotif', 'tekno',
      'sains', 'edukasi', 'lifestyle', 'kalam'
    ]
    let er = `
...
`.trim()
    ...
    let news = Array[Math.floor(Math.random() * Array.length)]
    if (!news.thumbnail) throw `ERROR\nGagal Mengambil Berita`
    conn.sendFile(m.chat, news.thumbnail, 'sindonews.png',
      `${news.title}\n\nDeskripsi :\n${news.description}\n\nSelengkapnya :\n${news.link}`, m)
  }
  if (command === 'suara') {
...
`.trim()
    ...
    let news = Array[Math.floor(Math.random() * Array.length)]
    if (!news.thumbnail) throw `ERROR\nGagal Mengambil Berita`
    conn.sendFile(m.chat, news.thumbnail, 'suaradotcom.png',
      `${news.title}\n\nDeskripsi :\n${news.description}\n\nSelengkapnya :\n${news.link}`, m)
  }
  if (command === 'temponews') {
    let ar = ['nasional', 'bisnis', 'metro', 'dunia', 'bola', 'cantik', 'tekno', 'otomotif', 'seleb', 'gaya',
      'travel', 'difabel', 'creativelab', 'inforial', 'event'
    ]
    let er = `
...
`.trim()
    ...
    let news = Array[Math.floor(Math.random() * Array.length)]
    if (!news.thumbnail) throw `ERROR\nGagal Mengambil Berita`
    conn.sendFile(m.chat, news.thumbnail, 'tempo.png',
      `${news.title}\n\nDeskripsi :\n${news.description}\n\nSelengkapnya :\n${news.link}`, m)
  }
  if (command === 'tribunnews') {
    let ar = ['terbaru', 'bisnis', 'superskor', 'sport', 'seleb', 'lifestyle', 'travel', 'parapuan', 'otomotif',
      'techno', 'kesehatan'
    ]
    let er = `
...
`.trim()
    ...
    let news = Array[Math.floor(Math.random() * Array.length)]
    if (!news.thumbnail) throw `ERROR\nGagal Mengambil Berita`
    conn.sendFile(m.chat, news.thumbnail, 'tribun.png',
      `${news.title}\n\nDeskripsi :\n${news.description}\n\nSelengkapnya :\n${news.link}`, m)
  }
}
handler.tags = ['news']
handler.help = ['antaranews', 'cnbc', 'cnn', 'jpnn', 'kumparan', 'merdeka', 'okezone', 'republika', 'sindo', 'suara',
  'temponews', 'tribunnews'
].map(v => v + ' <query>')
handler.command = ['antaranews', 'cnbc', 'cnn', 'jpnn', 'kumparan', 'merdeka', 'okezone', 'republika', 'sindo', 'suara',
  'temponews', 'tribunnews'
]
