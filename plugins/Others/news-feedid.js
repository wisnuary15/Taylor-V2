...
const handler = async (m, {
  ...
  if ("merdeka" === command) {
    ...
    if (!text) throw er;
    if (!ar.includes(text)) throw er;
    if (/terbaru/.test(text) && (merdekadotcom = await feedid.merdeka.terbaru()), /jakarta/.test(text) && (merdekadotcom = await feedid.merdeka.jakarta()), /dunia/.test(text) && (merdekadotcom = await feedid.merdeka.dunia()), /gaya/.test(text) && (merdekadotcom = await feedid.merdeka.gaya()), /olahraga/.test(text) && (merdekadotcom = await feedid.merdeka.olahraga()), /teknologi/.test(text) && (merdekadotcom = await feedid.merdeka.teknologi()), /otomotif/.test(text) && (merdekadotcom = await feedid.merdeka.otomotif()), /khas/.test(text) && (merdekadotcom = await feedid.merdeka.khas()), /sehat/.test(text) && (merdekadotcom = await feedid.merdeka.sehat()), /jabar/.test(text) && (merdekadotcom = await feedid.merdeka.jabar()), /jatim/.test(text) && (merdekadotcom = await feedid.merdeka.jatim()), /jateng/.test(text) && (merdekadotcom = await feedid.merdeka.jateng()), /sumut/.test(text) && (merdekadotcom = await feedid.merdeka.sumut()), !1 === merdekadotcom.success) throw merdekadotcom
      .message;
