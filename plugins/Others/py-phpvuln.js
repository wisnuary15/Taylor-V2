...
const handler = async (m, {
    ...
    let isPHP = /(application\/(octet-stream|php|x-php|x-httpd-php|x-httpd-php-source)|text(php|x-php))/.test(mime)
    if (!isPHP) throw `Mimetype ${mime} Tidak Di dukung`
    let media = await q?.download()
    let filename = md5(Math.random() * 999999999999) + '.php'
    ...
    try {
        ...
        if (stderr.trim()) m.reply(`File Mengandung Virus/Malware`)
        fs.unlinkSync(`./tmp/${filename}`)
    }
}
handler.help = ['phpvuln <media>', 'scanfilephp <media>']
handler.tags = ['php', 'python', 'sptools']
