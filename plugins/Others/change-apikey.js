...
const handler = async (m, {
  ...
  var Lolhum = ["327a6596e4c4baa20c756132", "bukanitucuy14315195", "85faf717d0545d14074659ad",
    "ed78c137a46873c5b8e5fe3b", "zulfikarbot", "SGWN"
  ]
  var Ame = [
  ...
  if (command) {
    switch (template) {
        ...
        if (!one) {
          Object.keys(OpenAi).map((v, index) => {
            listSections.push(["List. " + ++index, [
              [(OpenAi[v]).slice(0, 10) + "... ( Use This )", usedPrefix + command + " keyopenai |" +
                OpenAi[v], ""
              ]
            ]])
          })
      ...
      case "keyopenai":
        ...
        openaikey = one
        conn.sendFile(m.chat, logonya + (args[0]).slice(3), 'qr.png', openaiinfo, m)
        break
      case "lolhuman":
        if (!one) {
      ...
      case "keylolhuman":
        ...
        lolkey = one
        conn.sendFile(m.chat, logonya + (args[0]).slice(3), 'qr.png', lolhumaninfo, m)
        break
      case "ame":
        if (!one) {
      ...
      case "keyame":
        ...
        ameapikey = one
        conn.sendFile(m.chat, logonya + (args[0]).slice(3), 'qr.png', ameinfo, m)
        break
        //
    }
  }
