const handler = async (m, { conn: conn, text: text, usedPrefix: usedPrefix, command: command, args: args }) => {
  ...
  switch (command = command.toLowerCase(), conn.menfes = conn.menfes ? conn.menfes : {},
    command) {
    case "menfesleave": {
      ...
      if (other && await conn.reply(other, room.b + " *Meninggalkan chat*", null), delete conn.menfes[room.id],
        "menfesleave" === command) break;
    }
    case "menfesstart": {
      if (Object.values(conn.menfes).find((room => room.check(m.sender)))) return await conn.reply(m.chat, "*Kamu masih berada di dalam menfes chat, menunggu Balasan*", null);
      let room = Object.values(conn.menfes).find((room => "WAITING" === room.state && !room.check(m.sender)));
