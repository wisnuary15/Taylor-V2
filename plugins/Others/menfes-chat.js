...
`
  ...
  switch (command) {
    case 'menfesleave': {
      ...
      if (other) conn.reply(other, room.b + ' *Meninggalkan chat*', null)
      delete conn.menfes[room.id]
      if (command === 'menfesleave') break
    }
    case 'menfesstart': {
      if (Object.values(conn.menfes).find(room => room.check(m.sender))) return conn.reply(m.chat,
        '*Kamu masih berada di dalam menfes chat, menunggu Balasan*', null)
      let room = Object.values(conn.menfes).find(room => room.state === 'WAITING' && !room.check(m.sender))
      if (room) {
        room.b = m.sender
        room.state = 'CHATTING'
        conn.reply(room.a, '*Menfes Chat Tersambung!*\nDengan: ' + m.sender, null)
        conn.reply(m.sender, '*Menfes Chat Tersambung!*\nDengan: ' + room.a, null)
      } else {
        // Batas
        if (!m.quoted) {
          conn.reply(jid, tujuan + '\n' + cap, null)
        } else {
          conn.reply(jid, tujuan + '\n' + cap, null)
          let media = q ? await m.getQuotedObj() : false || m
          await conn.copyNForward(jid, media, false).catch(_ => _)
        }
        conn.reply(m.chat, suks, m, {
        ...
        conn.reply(m.chat, '*Menunggu Balasan...*', null)
      }
      break
    }
  }
