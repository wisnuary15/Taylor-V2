...
const handler = async (m, {
  ...
  try {
    ...
    let caption = results.map((v, index) => `*\`Hasil ${index + 1}\`*\n\n- *Judul:* ${v.title || "Tidak terdeteksi"}\n- *Link:* ${v.link || "Tidak terdeteksi"}\n- *Cuplikan:* ${v.snippet || "Tidak terdeteksi"}`).join("\n________________________\n");
    await conn.sendFile(m.chat, google_img, "", `*\`🔎 Hasil Pencarian\`*\n\n${caption}`, m);
  } catch (e) {
    try {
      let results = await googleit(text);
      let caption = results.articles.map((v, index) => `*\`Hasil ${index + 1}\`*\n\n- *Judul:* ${v.title || "Tidak terdeteksi"}\n- *Link:* ${v.url || "Tidak terdeteksi"}\n- *Deskripsi:* ${v.description || "Tidak terdeteksi"}`).join("\n________________________\n");
      if (!caption.length) throw `Query "${text}" Not Found`;
      await conn.sendFile(m.chat, google_img, "", `*\`🔎 Hasil Pencarian\`*\n\n${caption}`, m);
    } catch (e) {
      try {
        let API_KEY = "7d3eb92cb730ed676d5afbd6c902ac1f";
        let response = await fetch("http://api.serpstack.com/search?access_key=" + API_KEY + "&type=web&query=" + text);
        let data = await response.json();
        let caption = data.organic_results.map((v, index) => `*\`Hasil ${index + 1}\`*\n\n- *Judul:* ${v.title || "Tidak terdeteksi"}\n- *Link:* ${v.url || "Tidak terdeteksi"}\n- *Cuplikan:* ${v.snippet || "Tidak terdeteksi"}`).join("\n________________________\n");
        await conn.sendFile(m.chat, google_img, "", `*\`🔎 Hasil Pencarian\`*\n\n${caption}`, m);
      } catch (e) {
        m.react(eror);
      }
