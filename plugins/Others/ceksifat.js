  ...
  `;
    ...
    if (command == 'ceksifat') {
        await conn.reply(m.chat, cksft, m, {
            mentions: await conn.parseMention(cksft),
            fileLength: fsizedoc,
            seconds: fsizedoc,
            contextInfo: {
                mentionedJid: await conn.parseMention(cksft),
                externalAdReply: {
                    mediaUrl: sig,
                    mediaType: 2,
                    description: wm,
    ...
    if (command == 'masadepannya') {
        await conn.reply(m.chat, msdpn, m, {
            mentions: await conn.parseMention(msdpn),
            fileLength: fsizedoc,
            seconds: fsizedoc,
            contextInfo: {
                mentionedJid: await conn.parseMention(msdpn),
                externalAdReply: {
                    mediaUrl: sig,
                    mediaType: 2,
                    description: wm,
