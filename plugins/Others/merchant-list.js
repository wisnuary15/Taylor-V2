  ...
  switch (template) {
    case "list":
      if (!one && !two) {
        ...
        const listSections = Object.values(resu.data).sort((a, b) => a.index - b.index).map((v, index) =>
          `${index + 1}. ${v.type}\n- ${v.brand}`).join("\n\n");
      ...
      if (one && !two) {
        ...
        const listSections = Object.values(resul.data).sort((a, b) => a.index - b.index).filter((item) => brands
          .has(item.brand) ? false : (brands.add(item.brand), true)).map((v, index) =>
          `${index + 1}. ${v.type}\n- ${v.brand}`).join("\n\n");
    ...
    case "trx":
      ...
      if (TRX.result && data !== null) {
        let teks2 =
          `_Transaksi status *${data.status}*_\n\n*TRXID*: ${data.trxid}\n*orderID*: ${data.data}\n*itemID*: ${data.service}\n\n*_${TRX.message}_*`;
        m.reply(teks2);
      } else {
      ...
      if (stats.result) {
        ...
        let datas1 = data[0];
        let teks2 =
          `_Transaksi status *${datas1.status}*_\n\n*TRXID*: ${datas1.trxid}\n*orderID*: ${datas1.data}\n*itemID*: ${datas1.service}`;
        m.reply(teks2);
      }
      console.log(stats);
      break;
    case "status":
      ...
      if (stats2.result) {
        ...
        let datas = data[0];
        let teks3 =
          `_Transaksi status *${datas.status}*_\n\n*TRXID*: ${datas.trxid}\n*orderID*: ${datas.data}\n*itemID*: ${datas.service}`;
        m.reply(teks3);
      }
      break;
    case "profile":
