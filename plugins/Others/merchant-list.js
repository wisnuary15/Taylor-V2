import { profile, watch, prepaid, post, media } from "../../lib/tools/merchant-api.js";
const handler = async (m, { args: args, text: text, usedPrefix: usedPrefix, command: command }) => {
    ...
    case "list":
      ...
      if (one && !two) {
        ...
        const brands = new Set,
          listSections = Object.values(resul.data).sort(((a, b) => a.index - b.index)).filter((item => !brands.has(item.brand) && (brands.add(item.brand),
            !0))).map(((v, index) => `${index + 1}. ${v.type}\n- ${v.brand}`)).join("\n\n");
