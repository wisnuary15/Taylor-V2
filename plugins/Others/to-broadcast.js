...
const handler = async (m, { conn: conn, text: text, args: args, usedPrefix: usedPrefix, command: command }) => {
  ...
  if ("pc" === text.toLowerCase() && (chats = Object.keys(conn.chats).filter((v => v.endsWith("s.whatsapp.net")))),
    "gc" === text.toLowerCase() && (chats = Object.keys(conn.chats).filter((v => v.endsWith("g.us")))),
    "all" === text.toLowerCase() && (chats = Object.keys(conn.chats).filter((v => v))),
    !m.quoted) return m.reply("Reply pesan");
