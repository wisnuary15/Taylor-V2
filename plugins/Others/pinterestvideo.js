...
const handler = async (m, {
  ...
  if (!args[0])
  throw `*Perintah ini untuk mengunduh video dari pinterest dengan link*\n\ncontoh:\n${usedPrefix + command} https://id.pinterest.com/pin/27162403992537372/`
  if (!args[0]?.match(/https:\/\/.*pinterest.com\/pin|pin.it/gi))
  throw `*Link salah! Perintah ini untuk mengunduh video dari pinterest dengan link*\n\ncontoh:\n${usedPrefix + command} https://id.pinterest.com/pin/27162403992537372/`
  await spin(args[0]).then(async res => {
    let pin = JSON.stringify(res)
    let json = JSON.parse(pin)
    if (!json.status) throw `Tidak dapat diunduh`
    conn.sendFile(m.chat, json.data.url, '', `*Mythia Batford*`, m)
  })
}
