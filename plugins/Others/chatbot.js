const handler = async (m, {
  ...
  if (args[0] === "set") {
    if (dbchatbot[id] && dbchatbot[id].state !== "WAITING") {
      return await conn.reply(m.chat, "*Tidak bisa mengatur source karena sedang dalam sesi chatbot chat*", m);
    }
    if (args[1] && !isNaN(args[1])) {
      ...
      if (selectedSourceIndex >= 0 && selectedSourceIndex < sources.length) {
        ...
        return await conn.reply(m.chat, `*Source telah diatur sesuai dengan pilihanmu: ${selectedSource.name}*\nGunakan "/chatbot start" untuk memulai sesi chatbot chat`, m);
      } else {
  ...
  if (args[0] === "start") {
    if (dbchatbot[id] && dbchatbot[id].state === "WAITING") {
      if (!dbchatbot[id].user) {
        ...
        return await conn.reply(m.chat, "*Kamu sudah dalam sesi chatbot chat, menunggu partner*", m);
      }
    } else {
      return await conn.reply(m.chat, '*Atur source terlebih dahulu dengan "/chatbot set [urutan]*"', m);
    }
  }
  if (args[0] === "stop") {
    if (dbchatbot[id] && dbchatbot[id].state === "CHATTING") {
      ...
      return await conn.reply(m.chat, "*Kamu tidak sedang dalam sesi chatbot chat*", m);
    }
  }
