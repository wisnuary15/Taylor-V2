const handler = async (m, { conn: conn, args: args }) => {
  ...
  if ("start" === args[0]) return conn.chatbot[id] && "WAITING" === conn.chatbot[id].state ? "" === conn.chatbot[id].user ? (conn.chatbot[id].user = m.sender,
    conn.chatbot[id].state = "CHATTING", await conn.reply(m.chat, "*Percakapan dimulai!*", m)) : await conn.reply(m.chat, "*Kamu sudah dalam sesi chatbot chat, menunggu partner*", m) : await conn.reply(m.chat, '*Atur source terlebih dahulu dengan "/chatbot set [urutan]*"', m);
  if ("stop" === args[0]) return conn.chatbot[id] && "CHATTING" === conn.chatbot[id].state ? (delete conn.chatbot[id],
    await conn.reply(m.chat, "*Sesi chatbot chat dihentikan*", m)) : await conn.reply(m.chat, "*Kamu tidak sedang dalam sesi chatbot chat*", m);
  {
