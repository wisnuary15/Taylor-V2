...
const format = sizeFormatter({
  ...
  handler = async (m, { conn: conn, isRowner: isRowner }) => {
    try {
      const chats = Object.entries(conn.chats).filter((([id, data]) => id && data.isChats)),
        ...
        cpuModel = (osu.os.ip(),
          cpux.model()),
        [cpuPer, driveInfo, memInfo, netInfo, { ip: ip, country: cr, cc: cc }] = await Promise.all([cpux.usage().then((cpuPercentage => cpuPercentage)).catch((() => NotDetect)), drive.info().catch((() => ({
          totalGb: NotDetect,
          usedGb: NotDetect,
