...
const handler = async (m, { conn: conn, command: command, usedPrefix: usedPrefix, args: args }) => {
  const { sender: sender, chat: chat } = m;
  ...
  if ("create" === value) {
    ...
    if ("vote" === value) {
      ...
      return !0 === getPlayerById(chat, sender, parseInt(target), ww).db.isdead ? m.reply(`Player ${target} sudah mati.`) : ww[chat].player.length < parseInt(target) ? m.reply("Invalid") : !1 === getPlayerById(chat, sender, parseInt(target), ww) ? m.reply("Player tidak terdaftar!") : (vote(chat, parseInt(target), sender, ww),
        m.reply("✅ Vote"));
    }
    if ("exit" === value) {
