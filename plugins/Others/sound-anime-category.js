...
const AnimeSound = async (category, page) => {
  try {
    const url = page ? `https://audiojungle.net/search/${category}?page=${page}` : `https://audiojungle.net/search/${category}`;
    const response = await fetch(url);
    const html = await response.text();
    const $ = cheerio.load(html);
    return $("div.shared-item_cards-list-audio_card_component__root").map((i, elem) => {
