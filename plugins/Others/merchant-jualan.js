  ...
  if (command) {
    switch (template) {
        ...
        if (!one && !two) {
          Object.values(Panel).map((v, index) => {
            listSections.push(["List. " + ++index, [
              [v.name, usedPrefix + command + " buypanel |" + v.price + "|" + v.status + "|" + v.type, v
                .note
              ]
            ]])
          })
          return conn.sendList(m.chat, "READY PANEL RUN BOT WHATSAPP \nBY MAXXY BOTZ",
            "📮TOTAL RAM 80GB CPU 88 CORE", author, "[ Choose ]", listSections, m)
        }
        break
      case "buypanel":
        var panelinfo = `*Harga:* ${
        ...
        conn.sendFile(m.chat, kodeqr, 'qr.png', panelinfo, m)
        break
      case "insta":
        if (!one && !two) {
          Object.values(Insta).map((v, index) => {
            listSections.push(["List. " + ++index, [
              [v.name, usedPrefix + command + " buyinsta |" + v.price + "|" + v.status + "|" + v.type, v
                .note
              ]
            ]])
          })
      ...
      case "buyinsta":
        ...
        conn.sendFile(m.chat, kodeqr, 'qr.png', instainfo, m)
        break
        //
    }
  }
