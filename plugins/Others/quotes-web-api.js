...
async function fetchQuotes(page) {
  try {
    const response = await fetch('https://www.goodreads.com/quotes/tag/indonesia?page=' +
    page); // Ganti dengan URL halaman yang sesuai
    ...
    $('.quoteDetails').each((index, element) => {
      ...
      const tags = $('.quoteFooter a[href^="/quotes/tag/"]', element).map((index, tagElement) => $(tagElement)
        .text().trim()).get();
