...
const fetchQuranData = async surahNumber => {
  ...
  db.data.dbbot.quransData = db.data.dbbot.quransData ? db.data.dbbot.quransData : {};
...
handler.before = async (m, {
  ...
  if (db.data.dbbot.quransData = db.data.dbbot.quransData ? db.data.dbbot.quransData : {}, m.isBaileys || !(m.chat in db.data.dbbot.quransData)) return;
  ...
  if (isNaN(index) || index < 1 || index > list.length) m.reply("❌ Invalid ayah number. Please provide a valid ayah number from the list.");
    ...
    await conn.sendMessage(m.chat, {
      ...
      delete: key
    }), clearTimeout(db.data.dbbot.quransData[m.chat].timeout), delete db.data.dbbot.quransData[m.chat];
  }
