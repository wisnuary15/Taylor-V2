const handler = async (m, {
  ...
  switch (db.data.game.chessgame = db.data.game.chessgame || {}, text) {
    case "end":
      if (!db.data.game.chessgame[m.chat]) return await conn.ctaButton.setBody("Anda tidak sedang dalam sesi Chess. 🤔").addReply("Create now!", `${usedPrefix}${command} start`).run(m.chat, conn, m);
      delete db.data.game.chessgame[m.chat], m.reply("Berhasil keluar dari sesi Chess. 👋");
      break;
    case "start":
      if (db.data.game.chessgame[m.chat]) return await conn.ctaButton.setBody("Anda masih berada dalam sesi Chess. 🤖").addReply("Delete this!", `${usedPrefix}${command} end`).run(m.chat, conn, db.data.game.chessgame[m.chat].msg);
      try {
        ...
        let soal = await conn.ctaButton.setBody(txt).addReply("Accept!", `accept`).addReply("Cancel!", `cancel`).contextInfo({
