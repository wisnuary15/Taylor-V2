const handler = async (m, { conn: conn, usedPrefix: usedPrefix, args: args, command: command }) => {
    ...
    if (turn === turnNow && time === timeNow) {
      if (conn.war[m.chat][turn].hp -= 2500, await conn.reply(m.chat, `*@${conn.war[m.chat][turn].user.split("@")[0]} sedang AFK (Denda -2500 HP)*\n\n.war player = statistik pemain\n.attack @tag = serang lawan`, null, {
        ...
        let playerTotal = 0,
          playerKalah = 0;
        if (turn < 5) {
          for (let i = 0; i < 5; i++) "" != conn.war[m.chat][i].user && (playerTotal += 1,
            conn.war[m.chat][i].hp <= 0 && (playerKalah += 1));
          if (playerTotal > 0 && playerTotal === playerKalah) {
            var teamA = [],
              teamB = [],
              teamAB = [];
            for (let j = 0; j < 5; j++) "" != conn.war[m.chat][j].user && (db.data.users[conn.war[m.chat][j].user].money -= Number(conn.war2[m.chat].money),
              teamA.push(conn.war[m.chat][j].user), teamAB.push(conn.war[m.chat][j].user));
            for (let j = 5; j < 10; j++) "" != conn.war[m.chat][j].user && (db.data.users[conn.war[m.chat][j].user].money += Number(conn.war2[m.chat].money),
              teamB.push(conn.war[m.chat][j].user), teamAB.push(conn.war[m.chat][j].user));
          ...
          for (let i = 5; i < 10; i++) "" != conn.war[m.chat][i].user && (playerTotal += 1,
            conn.war[m.chat][i].hp <= 0 && (playerKalah += 1));
      ...
      let pergantian = !1;
      if (turn < 5)
        for (let i = 5; i < 10; i++) conn.war[m.chat][i].hp > 0 && "" != conn.war[m.chat][i].user && !1 === conn.war[m.chat][i].turn && (conn.war2[m.chat].turn = i,
          conn.war2[m.chat].time = 1, pergantian = !0);
      else
        for (let i = 0; i < 5; i++) conn.war[m.chat][i].hp > 0 && "" != conn.war[m.chat][i].user && !1 === conn.war[m.chat][i].turn && (conn.war2[m.chat].turn = i,
          conn.war2[m.chat].time = 1, pergantian = !0);
      if (!1 === pergantian)
        for (let l = 9; l >= 0; l--) "" != conn.war[m.chat][l].user && conn.war[m.chat][l].hp > 0 && (conn.war2[m.chat].turn = l,
          conn.war2[m.chat].time = 1), conn.war[m.chat][l].turn;
  ...
  if (conn.war = conn.war ? conn.war : {}, conn.war2 = conn.war2 ? conn.war2 : {},
    !(m.chat in conn.war)) return m.reply("*Tidak ada game di grup ini.*");
  ...
  if (conn.war2[m.chat].turn < 5) {
    for (let i = 5; i < 10; i++)
      if (conn.war[m.chat][i].user === args[0] && conn.war[m.chat][i].hp > 0) {
        ...
        for (let i = 0; i < 10; i++) conn.war[m.chat][i].user === target && (conn.war[m.chat][i].hp -= 500 * pointAttacker,
          conn.war[m.chat][conn.war2[m.chat].turn].turn = !0, await conn.reply(m.chat, `*@${attacker.split("@")[0]} menyerang @${target.split("@")[0]} sampai nyawanya berkurang ${500 * pointAttacker} (Sisa HP: ${conn.war[m.chat][i].hp})*\n\n*@${attacker.split("@")[0]} [${10 * pointAttacker}%] - [${10 * pointTarget}%] @${target.split("@")[0]}*\n*Level sangat mempengaruhi keberhasilan.*`, m, {
            contextInfo: {
    ...
    for (let i = 0; i < 5; i++)
      if (conn.war[m.chat][i].user === args[0] && conn.war[m.chat][i].hp > 0) {
        ...
        for (let i = 0; i < 10; i++) conn.war[m.chat][i].user === target && (conn.war[m.chat][i].hp -= 500 * pointAttacker,
          await conn.reply(m.chat, conn.war[m.chat][conn.war2[m.chat].turn].turn, m), conn.war[m.chat][conn.war2[m.chat].turn].turn = !0,
          await conn.reply(m.chat, conn.war[m.chat][conn.war2[m.chat].turn].turn, m), await conn.reply(m.chat, `*@${attacker.split("@")[0]} menyerang @${target.split("@")[0]} sampai nyawanya berkurang ${500 * pointAttacker} (Sisa HP: ${conn.war[m.chat][i].hp})*\n\n*@${attacker.split("@")[0]} [${10 * pointAttacker}%] - [${10 * pointTarget}%] @${target.split("@")[0]}*\n*Level sangat mempengaruhi keberhasilan.*`, m, {
            contextInfo: {
              mentionedJid: [attacker, target]
            }
          }), await sleep(2e3), conn.war[m.chat][i].hp <= 0 && await conn.reply(m.chat, `*@${target.split("@")[0]} sudah mati dalam pertarungan.*`, m, {
  ...
  if (conn.war2[m.chat].turn < 5) {
    let userAktif = 0,
      userMati = 0;
    for (let i = 5; i < 10; i++) "" != conn.war[m.chat][i].user && (userAktif += 1,
      conn.war[m.chat][i].hp <= 0 && (userMati += 1));
    ...
    for (let k = 5; k < 10; k++) conn.war[m.chat][k].hp > 0 && "" != conn.war[m.chat][k].user && !1 === conn.war[m.chat][k].turn && (conn.war2[m.chat].turn = k,
      conn.war2[m.chat].time = 1, turn2 = conn.war2[m.chat].turn);
    if (turn1 === turn2) {
      for (i = 0; i < 10; i++) conn.war[m.chat][i].turn = !1;
      for (i = 0; i < 5; i++) conn.war[m.chat][i].hp > 0 && "" != conn.war[m.chat][i].user && !1 === conn.war[m.chat][i].turn && (conn.war2[m.chat].turn = i,
    ...
    let turn1 = conn.war2[m.chat].turn,
      turn2 = conn.war2[m.chat].turn;
    for (let k = 0; k < 5; k++) conn.war[m.chat][k].hp > 0 && "" != conn.war[m.chat][k].user && !1 === conn.war[m.chat][k].turn && (conn.war2[m.chat].turn = k,
      conn.war2[m.chat].time = 1, turn2 = conn.war2[m.chat].turn);
    if (turn1 === turn2) {
  ...
  let totalUser = 0,
    totalTurn = 0;
  for (let i = 0; i < 10; i++) "" != conn.war[m.chat][i].user && (totalUser += 1),
    !0 === conn.war[m.chat][i].turn && (totalTurn += 1);
