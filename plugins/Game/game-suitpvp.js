let timeout = 6e4,
  poin = 500,
  poin_lose = -100;
const handler = async (m, { conn: conn, usedPrefix: usedPrefix }) => {
  let who;
  if (who = m.isGroup && m.mentionedJid[0] ? m.mentionedJid[0] : m.sender, conn.suit = conn.suit ? conn.suit : {},
    Object.values(conn.suit).find((room => room.id.startsWith("suit") && [room.p, room.p2].includes(m.sender)))) throw "Selesaikan suit sebelumnya terlebih dahulu.";
