const handler = async (m, {
    ...
    m.reply(`*Kamu* (level ${db.data.users[m.sender].level}) menantang *${await conn.getName(lawan)}* (level ${db.data.users[lawan].level}) dan sedang dalam pertarungan sengit.\n\nTunggu ${lamaPertarungan} menit lagi dan lihat siapa yg menang.`)
    ...
    if (pointPemain > pointLawan) {
        ...
        m.reply(`*${await conn.getName(m.sender)}* [${pointPemain * 10}] - [${pointLawan * 10}] *${await conn.getName(lawan)}*\n\n*Kamu* (level ${db.data.users[m.sender].level}) MENANG melawan *${await conn.getName(lawan)}* (level ${db.data.users[lawan].level}) karena kamu ${alasanMenang[getRandom(0,alasanMenang.length-1)]}\n\nHadiah Rp. ${hadiah.toLocaleString()}\n+1 Tiketcoin`)
    } else if (pointPemain < pointLawan) {
        let denda = (pointLawan - pointPemain) * 100000
        db.data.users[m.sender].money -= denda
        db.data.users[m.sender].tiketcoin += 1
        m.reply(`*${await conn.getName(m.sender)}* [${pointPemain * 10}] - [${pointLawan * 10}] *${await conn.getName(lawan)}*\n\n*Kamu* (level ${db.data.users[m.sender].level}) KALAH melawan *${await conn.getName(lawan)}* (level ${db.data.users[lawan].level}) karena kamu ${alasanKalah[getRandom(0,alasanKalah.length-1)]}\n\nUang kamu berkurang Rp. ${denda.toLocaleString()}\n+1 Tiketcoin`)
    } else {
