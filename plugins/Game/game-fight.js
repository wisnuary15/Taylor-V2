const handler = async (m, {
  ...
  db.data.game.fight = db.data.game.fight ? db.data.game.fight : {};
  if (void 0 !== db.data.game.fight[m.sender] && !0 === db.data.game.fight[m.sender]) return m.reply("*Tidak bisa melakukan pertarungan lagi karena anda sedang bertarung bro.*");
  ...
  m.reply(`*Kamu* (level ${db.data.users[m.sender].level}) menantang *${conn.getName(lawan)}* (level ${db.data.users[lawan].level}) dan sedang dalam pertarungan sengit.\n\nTunggu ${lamaPertarungan} menit lagi dan lihat siapa yg menang.`),
    db.data.game.fight[m.sender] = !0, await (time = 6e4 * lamaPertarungan, new Promise(res => setTimeout(res, time)));
  ...
  delete db.data.game.fight[m.sender];
};
