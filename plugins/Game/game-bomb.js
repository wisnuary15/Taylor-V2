const handler = async (m, {
  ...
  for (let i = 0; i < array.length; i += 3) teks += array.slice(i, i + 3).map(v => v.state ? v.emot : v.number)
    .join('') + '\n';
  teks +=
    `\nTimeout : [ *${((timeout / 1000) / 60)} menit* ]\nApabila mendapat kotak yang berisi bom maka point akan di kurangi.`;
  let msg = conn.reply(m.chat, teks, m);
  let {
    key
  } = msg
  let v;
  conn.bomb[id] = [
    ...
    setTimeout(() => {
      ...
      if (conn.bomb[id]) conn.reply(m.chat, `*Waktu habis!*, Bom berada di kotak nomor ${v.number}.`, conn.bomb[
        id][0]?.key);
      delete conn.bomb[id];
    }, timeout),
    key
