const generate = (x, y, bombs) => {
  ...
  handler = async (m, {
    ...
    db.data.game.minessweeper = db.data.game.minessweeper || {};
    const orgs = args[0],
      oX = args[1],
      oY = args[2],
      F = args[3];
    ...
    switch (orgs.toLowerCase()) {
      ...
      case "start":
        ...
        return void(db.data.game.minessweeper[m.chat] = {
          map: map,
          current: empty,
          key: key
        });
      ...
      case "stop":
      case "end":
        return db.data.game.minessweeper[m.chat] ? (delete db.data.game.minessweeper[m.chat], await conn.reply(m.chat, "🏳️ *You surrendered.*", m)) : await conn.reply(m.chat, "🚨 *No active game session.*", m);
      case "open":
      case "o":
      case "buka":
        if (!db.data.game.minessweeper[m.chat]) return await conn.reply(m.chat, "🚨 *No active game session.*", m);
        if (oX > 10 || !oX || !oY) return await conn.reply(m.chat, `🚨 *Invalid parameters. Example: ${usedPrefix + command} open 2 5*`, m);
        const g = db.data.game.minessweeper[m.chat];
        if ("f" === F) g.current[oY - 1][oX - 1] = "🚩";
          ...
          if (0 === openedCell) {
            ...
            if ("x" === openedCell) {
              delete db.data.game.minessweeper[m.chat], db.data.users[m.sender].exp -= 9e3;
              const {
                key: loseKey
        ...
        return void(db.data.game.minessweeper[m.chat].key = newKey);
    }
  };
