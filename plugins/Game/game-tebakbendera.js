...
const handler = async (m, {
  ...
  db.data.game.tebakbendera = db.data.game.tebakbendera ? db.data.game.tebakbendera : {};
  let json, id = m.chat;
  if (id in db.data.game.tebakbendera) return await conn.reply(m.chat, "Masih ada soal belum terjawab di chat ini", db.data.game.tebakbendera[id][0]), !1;
  try {
...
Balas pesan ini untuk menjawab!`;
  conn.logger.info(json.name), db.data.game.tebakbendera[id] = [await conn.reply(m.chat, caption, m, {
    contextInfo: {
    ...
    db.data.game.tebakbendera[id] && await conn.reply(m.chat, `*\`❌ TIMEOUT - ${command.toUpperCase()}\`*
Jawabannya adalah *${json.name}*`, db.data.game.tebakbendera[id][0]), delete db.data.game.tebakbendera[id];
  }, timeout)];
};
