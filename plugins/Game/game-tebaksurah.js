...
const handler = async (m, {
  ...
  db.data.game.tebaksurah = db.data.game.tebaksurah ? db.data.game.tebaksurah : {};
  let id = m.chat;
  if (id in db.data.game.tebaksurah) return await conn.reply(m.chat, "Masih ada soal belum terjawab di chat ini", db.data.game.tebaksurah[id][0]), !1;
...
Balas pesan ini untuk menjawab!`,
    ...
    db.data.game.tebaksurah[id] = [await conn.reply(m.chat, caption, m, {
      contextInfo: {
      ...
      db.data.game.tebaksurah[id] && await conn.reply(m.chat, `*\`❌ TIMEOUT - ${command.toUpperCase()}\`*
Jawabannya adalah ${captu}`, db.data.game.tebaksurah[id][0]), delete db.data.game.tebaksurah[id];
