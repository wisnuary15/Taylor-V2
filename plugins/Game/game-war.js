...
*${usedPrefix + command} start* = start game`)
  if (args[0] === "money") {
    ...
    if (m.sender === conn.war[m.chat][0]?.user) {
      ...
      return conn.reply(m.chat,
        `*Hanya @${conn.war[m.chat][0]?.user.split('@')[0]} sebagai pembuat room yang bisa mengganti modal awal perang*`,
        m, {
          contextInfo: {
  ...
  // JOIN
  if (args[0] === "join") {
    if (db.data.users[m.sender].money < 1000) return m.reply("*Uang kamu minimal Rp. 1000 untuk bermain game ini.*")
    // FIRST PLAYER
    if (!(m.chat in conn.war)) {
      ...
      if (args[1]) {
        if (args[1].toLowerCase() === "a") {
          if (conn.war2[m.chat].money === 0) return conn.reply(m.chat,
            `*Tolong @${conn.war[m.chat][0]?.user.split('@')[0]} tetapkan modal awal perang (minimal Rp. 1.000.000)*\n\n.war money 1000000`,
            m, {
              contextInfo: {
                mentionedJid: [conn.war[m.chat][0]?.user]
              }
          ...
          return m.reply('a')
          if (db.data.users[m.sender].money < conn.war2[m.chat].money) return m.reply(
            `*Uang kamu minimal Rp. ${conn.war2[m.chat].money.toLocaleString()} untuk bermain game ini.*`)
          for (let i = 1; i < 5; i++) {
            if (conn.war[m.chat][i].user === "") {
              let exp = db.data.users[m.sender].exp
              conn.war[m.chat][i] = {
          ...
          if (conn.war2[m.chat].money === 0) return conn.reply(m.chat,
            `*Tolong @${conn.war[m.chat][0]?.user.split('@')[0]} tetapkan modal awal perang (minimal Rp. 1000000)*\n\n.war money 1000000`,
            m, {
              contextInfo: {
                mentionedJid: [conn.war[m.chat][0]?.user]
              }
  ...
  if (args[0] === "player") {
    ...
    conn.reply(m.chat,
      `${conn.war2[m.chat].war ? '*Giliran : ' + '@' + conn.war[m.chat][conn.war2[m.chat].turn].user.split('@')[0] + '*\n*Taruhan : Rp. ' + Number(conn.war2[m.chat].money).toLocaleString() + '*\n\n' : '*Taruhan : Rp. ' + Number(conn.war2[m.chat].money).toLocaleString() + '*\n\n' }*TEAM A :*\n` +
      teamA.map((v, i) =>
        `${conn.war[m.chat][i].hp > 0 ? '❤️ ' : '☠️ ' }@${v.split('@')[0]} (Lv.${conn.war[m.chat][i].lvl} HP: ${conn.war[m.chat][i].hp})`
        ).join`\n` + "\n\n*TEAM B :*\n" + teamB.map((v, i) =>
        `${conn.war[m.chat][i+5].hp > 0 ? '❤️ ' : '☠️ ' }@${v.split('@')[0]} (Lv.${conn.war[m.chat][i+5].lvl} HP: ${conn.war[m.chat][i+5].hp})`
        ).join`\n`, m, {
        contextInfo: {
          mentionedJid: teamAB
        }
