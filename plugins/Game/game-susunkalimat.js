const sentences = {
  ...
  handler = async (m, {
    ...
    if (db.data.game.susunkalimat = db.data.game.susunkalimat || {}, db.data.game.susunkalimat[m.sender]) return m.reply("Kamu sedang bermain Susun Kalimat!");
    ...
    db.data.game.susunkalimat[m.sender] = {
      sender: m.sender,
      ...
      key: key,
      timeout: setTimeout(() => {
        db.data.game.susunkalimat[m.sender] && (conn.sendMessage(m.chat, {
          delete: key
        }), m.reply(`⌛ Waktu habis! Kamu gagal menyusun kalimat.\n*${originalSentence.toLowerCase()}*`), delete db.data.game.susunkalimat[m.sender]);
      }, 12e4)
    };
  };
handler.before = async (m, {
    ...
    if (db.data.game.susunkalimat = db.data.game.susunkalimat || {}, m.isBaileys || !(m.sender in db.data.game.susunkalimat) || !m.text) return;
    const {
      originalSentence,
      sender,
      shuffledSentence,
      key,
      timeout
    } = db.data.game.susunkalimat[m.sender], isAnswerCorrect = m.text.toLowerCase() === originalSentence.toLowerCase(), similarityIndex = jaccardSimilarity(m.text.toLowerCase(), originalSentence.toLowerCase());
    if (isAnswerCorrect) {
      const level = db.data.game.susunkalimat[m.sender].level;
      await conn.reply(m.chat, `✨ *Selamat*, @${m.sender.split("@")[0]}! Kamu berhasil menyusun kalimat dengan benar pada *level ${level.toUpperCase()}*!`, m, {
        ...
        delete: key
      }), clearTimeout(timeout), delete db.data.game.susunkalimat[m.sender];
