...
handler.before = async (m, { conn: conn }) => {
    ...
    const { originalSentence: originalSentence, sender: sender, shuffledSentence: shuffledSentence, key: key, timeout: timeout } = conn.susunKalimat[m.sender], isAnswerCorrect = m.text.toLowerCase() === originalSentence.toLowerCase(), similarityIndex = jaccardSimilarity(m.text.toLowerCase(), originalSentence.toLowerCase());
    if (isAnswerCorrect) {
