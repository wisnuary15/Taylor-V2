...
const handler = async (m, { conn: conn, text: text, command: command, usedPrefix: usedPrefix }) => {
  ...
  if (id in conn.quizz) return await conn.reply(m.chat, "Masih ada soal belum terjawab di chat ini", conn.quizz[id][0]),
    !1;
...
async function quizApi(difficulty) {
  ...
  return (await response.json()).map((({ question: question, answers: answers, correct_answers: correct_answers }) => ({
    soal: question,
