...
const handler = async (m, {
  ...
  if (id in db.data.game.quizz) return await conn.reply(m.chat, "Masih ada soal belum terjawab di chat ini", db.data.game.quizz[id][0]), !1;
...
Balas pesan ini untuk menjawab!`;
  db.data.game.quizz[id] = [await conn.reply(m.chat, caption, m, {
    contextInfo: {
    ...
    db.data.game.quizz[id] && await conn.reply(m.chat, `*\`❌ TIMEOUT - ${command.toUpperCase()}\`*
Jawabannya adalah *${json[0]?.jawaban}*`, db.data.game.quizz[id][0]), delete db.data.game.quizz[id];
  }, timeout)];
};
