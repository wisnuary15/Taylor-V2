...
class Deck {
  ...
  createDeck() {
    const card = (suit, value) => {
      ...
      if (value.toUpperCase().includes("J") || value.toUpperCase().includes("Q") || value.toUpperCase().includes(
          "K")) value = "10";
...
`;
      db.data.users[conn.blackjack[m.chat].idPemain].money += payout;
    ...
    switch (aksi) {
      case 'end':
        if (conn.blackjack[m.chat].idPemain === m.sender) {
          delete conn.blackjack[m.chat];
          conn.reply(m.chat, '*Anda keluar dari sesi blackjack.* 👋', m);
        } else {
          conn.reply(m.chat, '*Tidak ada sesi blackjack yang sedang berlangsung atau Anda bukan pemainnya.*', m);
        }
        break;
      case 'start':
        if (conn.blackjack[m.chat]) {
          conn.reply(m.chat,
            `*Sesi blackjack sudah berlangsung.* Gunakan *${usedPrefix + command} end* untuk keluar dari sesi.`, m
            );
        } else {
          ...
          conn.reply(m.chat, pesanStart, m);
        }
        break;
      case 'hit':
        if (!conn.blackjack[m.chat] || conn.blackjack[m.chat].idPemain !== m.sender) {
          conn.reply(m.chat, '*Anda tidak sedang bermain blackjack atau bukan pemainnya.*', m);
          break;
        }
      ...
      case 'stand':
        ...
        conn.reply(m.chat, pesanStand, m);
        break;
      case 'double':
        if (!conn.blackjack[m.chat] || conn.blackjack[m.chat].idPemain !== m.sender) {
          conn.reply(m.chat, '*Anda tidak sedang bermain blackjack atau bukan pemainnya.*', m);
        ...
        conn.reply(m.chat, pesanDouble, m);
        break;
      default:
        conn.reply(m.chat,
