...
const handler = async (m, {
  ...
  db.data.game.blackjack = db.data.game.blackjack || {};
  let [aksi, argumen] = args;
  const gameSession = db.data.game.blackjack[m.chat];
  ...
  try {
    switch (aksi) {
      case "end":
        if (isActive) {
          delete db.data.game.blackjack[m.chat];
      ...
      case "start":
        if (isStarted) {
          ...
          db.data.game.blackjack[m.chat] = new Blackjack(1);
          db.data.game.blackjack[m.chat].idPemain = m.sender;
          let betAmount = argumen ? parseInt(argumen) : 1e3;
          db.data.game.blackjack[m.chat].placeBet(betAmount);
          db.data.game.blackjack[m.chat].start();
          const table = db.data.game.blackjack[m.chat];
      ...
      case "double":
        ...
        const updatedTable = db.data.game.blackjack[m.chat];
