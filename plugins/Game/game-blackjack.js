class Blackjack {
  ...
  hit() {
    if ("waiting" === this.state) {
      ...
      return playerSum === sumCards(this.dealer) ? (this.state = "draw", this.emitEndEvent()) : 21 === playerSum ? (this.state = "player_blackjack",
          this.emitEndEvent()) : playerSum > 21 && (this.state = "dealer_win", this.emitEndEvent()),
        this.table;
    }
  }
  stand() {
  ...
  doubleDown() {
    ...
    this.table.doubleDowned = !0, this.player.push(...this.cards.dealCard(1)), this.updateTable(),
      this.stand();
  }
  calculatePayout() {
    "player_blackjack" === this.state ? this.table.payout = 1.5 * this.table.bet : "player_win" === this.state ? this.table.payout = this.table.bet : "dealer_win" === this.state || "dealer_blackjack" === this.state ? this.table.payout = 0 : "draw" === this.state && (this.table.payout = this.table.bet),
      this.table.doubleDowned && "draw" !== this.state && (this.table.payout *= 2), this.table.payout = Math.round(this.table.payout);
  }
  canDoubleDown() {
...
const formatter = new Intl.NumberFormat("id-ID", {
  ...
  templateBlackjackMessage = (usedPrefix, command, conn, m, blackjack) => {
    const { table: table, state: state } = blackjack, { bet: bet, dealer: dealer, player: player, payout: payout } = table;
    let message = "";
    ...
    switch (dealer.cards.length > 1 ? hiddenDealerCards += ", ❓" : hiddenDealerCards += `, ${dealer.cards[0]?.name}`,
      state) {
      case "player_win":
      case "dealer_win":
      case "draw":
