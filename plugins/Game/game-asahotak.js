...
Balas pesan ini untuk menjawab!`;
  conn.asahotak[id] = [await conn.reply(m.chat, caption, m, {
    contextInfo: {
    ...
    conn.asahotak[id] && await conn.reply(m.chat, `*\`❌ TIMEOUT - ${command.toUpperCase()}\`*
Jawabannya adalah *${json.jawaban}*`, conn.asahotak[id][0]), delete conn.asahotak[id];
  }, timeout)];
};
