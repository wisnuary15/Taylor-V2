...
*Hint:* ${usedPrefix}hkab
*Hadiah:* ${poin} XP  
*Waktu:* ${(timeout / 1e3).toFixed(2)} detik

Balas pesan ini untuk menjawab!`;
  conn.tebakkabupaten[id] = [await conn.reply(m.chat, caption, m, {
    contextInfo: {
    ...
    conn.tebakkabupaten[id] && await conn.reply(m.chat, `*\`🎮 Timeout GAME - ${command.toUpperCase()}\`*
Jawabannya adalah *${json.title}*`, conn.tebakkabupaten[id][0]), delete conn.tebakkabupaten[id];
  }, timeout)];
};
