...
const handler = async (m, {
  ...
  db.data.game.tebakkabupaten = db.data.game.tebakkabupaten ? db.data.game.tebakkabupaten : {};
  let id = m.chat;
  if (id in db.data.game.tebakkabupaten) return await conn.reply(m.chat, "Masih ada soal belum terjawab di chat ini", db.data.game.tebakkabupaten[id][0]), !1;
...
Balas pesan ini untuk menjawab!`;
  db.data.game.tebakkabupaten[id] = [await conn.reply(m.chat, caption, m, {
    contextInfo: {
    ...
    db.data.game.tebakkabupaten[id] && await conn.reply(m.chat, `*\`❌ TIMEOUT - ${command.toUpperCase()}\`*
Jawabannya adalah *${json.title}*`, db.data.game.tebakkabupaten[id][0]), delete db.data.game.tebakkabupaten[id];
  }, timeout)];
};
