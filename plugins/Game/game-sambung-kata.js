...
const handler = async (m, {
  ...
  if (id in db.data.game.sambungkata) {
    return await conn.reply(m.chat, "Masih ada soal belum terjawab di chat ini", db.data.game.sambungkata[id][0]), !1;
  }
...
Balas pesan ini untuk menjawab!`;
  db.data.game.sambungkata[id] = [await conn.reply(m.chat, caption, m, {
    contextInfo: {
  ...
  }), kata.toLowerCase(), poin, setTimeout(async () => {
    if (db.data.game.sambungkata[id]) {
      await conn.reply(m.chat, `*\`❌ TIMEOUT - ${command.toUpperCase()}\`*\nJawabannya adalah *${kata}*`, db.data.game.sambungkata[id][0]);
