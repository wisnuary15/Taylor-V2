...
let timeout = 12e4,
  poin = 4999;
const handler = async (m, { conn: conn, command: command, usedPrefix: usedPrefix }) => {
  let imgr = flaaa.getRandom();
  conn.lengkapikalimat = conn.lengkapikalimat ? conn.lengkapikalimat : {};
  let id = m.chat;
  if (id in conn.lengkapikalimat) return await conn.reply(m.chat, "Masih ada soal belum terjawab di chat ini", conn.lengkapikalimat[id][0]),
    !1;
