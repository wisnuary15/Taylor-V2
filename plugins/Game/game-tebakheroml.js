...
const handler = async (m, {
  ...
  db.data.game.tebakheroml = db.data.game.tebakheroml ? db.data.game.tebakheroml : {};
  let id = m.chat;
  let imgr = flaaa.getRandom();
  if (id in db.data.game.tebakheroml) return await conn.reply(m.chat, "Masih ada soal belum terjawab di chat ini", db.data.game.tebakheroml[id][0]), !1;
...
Balas pesan ini untuk menjawab!`;
  db.data.game.tebakheroml[id] = [await conn.sendFile(m.chat, imgr + command, "", caption, m), json, poin, setTimeout(async () => {
    db.data.game.tebakheroml[id] && await conn.reply(m.chat, `*\`❌ TIMEOUT - ${command.toUpperCase()}\`*
Jawabannya adalah *${json.title}*`, db.data.game.tebakheroml[id][0]), delete db.data.game.tebakheroml[id];
    ...
    quoted: db.data.game.tebakheroml[id][0]
  });
};
