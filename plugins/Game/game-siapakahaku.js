...
const handler = async (m, {
  ...
  db.data.game.siapakahaku = db.data.game.siapakahaku ? db.data.game.siapakahaku : {};
  let id = m.chat;
  if (id in db.data.game.siapakahaku) return await conn.reply(m.chat, "Masih ada soal belum terjawab di chat ini", db.data.game.siapakahaku[id][0]), !1;
...
Balas pesan ini untuk menjawab!`;
  db.data.game.siapakahaku[id] = [await conn.reply(m.chat, caption, m, {
    contextInfo: {
    ...
    db.data.game.siapakahaku[id] && await conn.reply(m.chat, `*\`❌ TIMEOUT - ${command.toUpperCase()}\`*
Jawabannya adalah *${json.jawaban}*`, db.data.game.siapakahaku[id][0]), delete db.data.game.siapakahaku[id];
  }, timeout)];
};
