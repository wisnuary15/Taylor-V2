...
const handler = async (m, {
  ...
  db.data.game.caklontong = db.data.game.caklontong ? db.data.game.caklontong : {};
  let id = m.chat;
  if (id in db.data.game.caklontong) return await conn.reply(m.chat, "Masih ada soal belum terjawab di chat ini", db.data.game.caklontong[id][0]), !1;
...
Balas pesan ini untuk menjawab!`;
  db.data.game.caklontong[id] = [await conn.reply(m.chat, caption, m, {
    contextInfo: {
    ...
    db.data.game.caklontong[id] && await conn.reply(m.chat, `*\`❌ TIMEOUT - ${command.toUpperCase()}\`*
Jawabannya adalah *${json.jawaban}*`, db.data.game.caklontong[id][0]), delete db.data.game.caklontong[id];
  }, timeout)];
};
