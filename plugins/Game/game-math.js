...
*Hint:* ${usedPrefix}calc ${math.str}
*Hadiah:* ${math.bonus} XP  
*Waktu:* ${(math.time / 1e3).toFixed(2)} detik

Balas pesan ini untuk menjawab!`;
  conn.math[id] = [await conn.reply(m.chat, caption, m, {
    contextInfo: {
    ...
    conn.math[id] && await conn.reply(m.chat, `*\`🎮 Timeout GAME - ${command.toUpperCase()}\`*
Jawabannya adalah *${math.result}*`, conn.math[id][0]), delete conn.math[id];
  }, math.time)];
};
