...
const handler = async (m, {
  ...
  let id = m.chat;
  if (id in db.data.game.math) return await conn.reply(m.chat, "Masih ada soal belum terjawab di chat ini", db.data.game.math[id][0]);
...
Balas pesan ini untuk menjawab!`;
  db.data.game.math[id] = [await conn.reply(m.chat, caption, m, {
    contextInfo: {
  ...
  }), math, 4, setTimeout(async () => {
    db.data.game.math[id] && await conn.reply(m.chat, `*\`🎮 Timeout GAME - ${command.toUpperCase()}\`*
Jawabannya adalah *${math.result}*`, db.data.game.math[id][0]), delete db.data.game.math[id];
  }, math.time)];
};
