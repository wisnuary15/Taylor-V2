...
const handler = async (m, {
  ...
  db.data.game.hangman = db.data.game.hangman || {};
  const [action, inputs] = args;
  try {
    switch (action) {
      case "end":
        if (db.data.game.hangman[m.chat]?.sessionId === m.sender) {
          delete db.data.game.hangman[m.chat];
        ...
        break;
      case "start":
        if (db.data.game.hangman[m.chat]) {
          ...
          db.data.game.hangman[m.chat] = new HangmanGame(m.sender);
          const gameSession = db.data.game.hangman[m.chat];
      ...
      case "guess":
        if (db.data.game.hangman[m.chat]) {
          ...
          if (result === "over" || result === "win") delete db.data.game.hangman[m.chat];
        } else {
