...
class HangmanGame {
  ...
  getRandomQuest = async () => {
    try {
      const { jawaban: jawaban, soal: soal } = await tebakkata();
      return {
        clue: soal,
  ...
  makeGuess = letter => this.isAlphabet(letter) ? (letter = letter.toLowerCase(), this.guesses.includes(letter) ? "repeat" : (this.guesses.push(letter),
    this.quest.quest.includes(letter) || (this.currentStage = Math.min(this.quest.quest.length, this.currentStage + 1)),
    this.checkGameOver() ? "over" : this.checkGameWin() ? "win" : "continue")) : "invalid";
...
const handler = async (m, { conn: conn, usedPrefix: usedPrefix, command: command, args: args }) => {
  ...
  try {
    switch (action) {
      case "end":
        conn.hangman[m.chat] && conn.hangman[m.chat].sessionId === m.sender ? (delete conn.hangman[m.chat],
          m.reply("Berhasil keluar dari sesi Hangman. 👋")) : m.reply("Tidak ada sesi Hangman yang sedang berlangsung atau Anda bukan pemainnya.");
        break;
      case "start":
