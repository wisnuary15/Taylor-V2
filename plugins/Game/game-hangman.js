...
const handler = async (m, {
  ...
  try {
    switch (action) {
      ...
      case 'guess':
        if (conn.hangman[m.chat]) {
          if (!inputs || !/^[a-zA-Z]$/.test(inputs)) {
            m.reply(`Masukkan huruf yang ingin ditebak setelah *guess*. Contoh: *${usedPrefix + command} guess a*`);
            return;
          }
          const gameSession = conn.hangman[m.chat];
          ...
          m.reply(messages[result]);
          if (result === 'over' || result === 'win') {
            delete conn.hangman[m.chat];
          }
        } else {
          m.reply('Tidak ada sesi Hangman yang sedang berlangsung. Gunakan *start* untuk memulai sesi.');
        }
        break;
      case 'hint':
        if (conn.hangman[m.chat]) {
      ...
      default:
        m.reply(
        `Aksi tidak valid. Gunakan ${usedPrefix + command} *help* untuk melihat cara menggunakan perintah.`);
    }
  } catch (error) {
    console.error('Error in hangman handler:', error);
    m.reply('Terjadi kesalahan dalam menangani permainan Hangman. Mohon coba lagi.');
  }
};
