...
const handler = async (m, {
  ...
  let id = m.chat;
  if (id in db.data.game.tebakkata) return await conn.reply(m.chat, "Masih ada soal belum terjawab di chat ini", db.data.game.tebakkata[id][0]), !1;
...
Balas pesan ini untuk menjawab!`;
  db.data.game.tebakkata[id] = [await conn.reply(m.chat, caption, m, {
    contextInfo: {
    ...
    db.data.game.tebakkata[id] && await conn.reply(m.chat, `*\`❌ TIMEOUT - ${command.toUpperCase()}\`*
Jawabannya adalah *${json.jawaban}*`, db.data.game.tebakkata[id][0]), delete db.data.game.tebakkata[id];
  }, timeout)];
};
