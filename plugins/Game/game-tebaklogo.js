...
const handler = async (m, {
  ...
  let id = m.chat;
  if (id in db.data.game.tebaklogo) return await conn.reply(m.chat, "Masih ada soal belum terjawab di chat ini", db.data.game.tebaklogo[id][0]), !1;
...
Balas pesan ini untuk menjawab!`;
  db.data.game.tebaklogo[id] = [await conn.reply(m.chat, caption, m, {
    contextInfo: {
    ...
    db.data.game.tebaklogo[id] && await conn.reply(m.chat, `*\`❌ TIMEOUT - ${command.toUpperCase()}\`*
Jawabannya adalah *${json.hasil.data.jawaban}*`, db.data.game.tebaklogo[id][0]),
