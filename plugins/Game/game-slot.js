const handler = async (m, { args: args, usedPrefix: usedPrefix, command: command }) => {
  ...
  return a === b && b === c ? (end = `JACKPOT! 🥳 *+${taruhan + taruhan} XP*`, users.exp += taruhan) : a === b || a === c || b === c ? end = "*TRY AGAIN!*" : (end = `LOSE 😥 *-${taruhan} XP*`,
    users.exp -= taruhan), users.lastslot = 1 * new Date, await conn.reply(m.chat, `\n*[ 🎰 | SLOTS ]*\n\n${end}\n\n${x[0]} ${y[0]} ${z[0]}\n${x[1]} ${y[1]} ${z[1]}\n${x[2]} ${y[2]} ${z[2]}`.trim(), m);
};
