    ...
    const isButton = {
      start: async () => {
        ...
        if (!args[1] || !difficult.includes(args[1])) {
          return new Button().setBody("Select The Difficulty!").addSelection("Click Here!").makeSections("Difficulty").makeRow("", "Easy", "Select Easy", ".buckshot start easy").makeRow("", "Normal", "Select Normal", ".buckshot start normal").makeRow("", "Hard", "Select Hard", ".buckshot start hard").run(m.chat, conn, m);
        }
      ...
      tembak: async () => {
        ...
        async function shot(target, siapa) {
          ...
          if (buckshot.round > Object.keys(ammo).length) {
            ...
            db.data.users[m.sender].money += 25 * 1000;
...
class Button {
  ...
  makeRow(header = "", title = "", description = "", id = "") {
    ...
    const buttonParams = JSON.parse(this._button[this._currentSelectionIndex].buttonParamsJson);
    ...
    this._button[this._currentSelectionIndex].buttonParamsJson = JSON.stringify(buttonParams);
    return this;
  }
  makeSections(title = "") {
  ...
  addSelection(title) {
    ...
    this._currentSelectionIndex = this._button.length - 1;
    this._currentSectionIndex = -1;
    return this;
  }
