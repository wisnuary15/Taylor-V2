...
const handler = async (m, {
  ...
  try {
    const cmd = args[0];
    db.data.game.buckshot = db.data.game.buckshot || {};
    db.data.game.buckshot[m.chat] = db.data.game.buckshot[m.chat] || {};
    let buckshot = db.data.game.buckshot[m.chat];
    const isButton = {
      start: async () => {
        ...
        if (db.data.game.buckshot.hasOwnProperty(m.chat)) delete db.data.game.buckshot[m.chat];
        buckshot = {
          on: true,
          mode: args[1],
          turn: true,
      ...
      tembak: async () => {
        ...
        async function shot(target, siapa) {
          ...
          if (buckshot.round > Object.keys(ammo).length) {
            delete db.data.game.buckshot[m.chat];
          ...
          if (!isi && !target && siapa === "me") {
            db.data.game.buckshot[m.chat] = buckshot;
...
async function reload(m, conn, buckshot) {
  ...
  db.data.game.buckshot[m.chat].ammo = ammo[buckshot.round][buckshot.subround].sort(() => Math.random() - .5);
  await delay(500);
  await showAmmo(m, conn, 3, buckshot);
  await delay(buckshot.delay);
  return buckshot;
