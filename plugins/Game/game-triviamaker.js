...
const handler = async (m, {
  ...
  try {
    ...
    if (!questions || questions.length === 0) {
      return m.reply(`Tidak dapat menghasilkan soal untuk topik "${inputText}"`);
    }
    if (!db.data.game.triviamaker[m.sender]) db.data.game.triviamaker[m.sender] = {};
    db.data.game.triviamaker[m.sender].questions = questions;
    db.data.game.triviamaker[m.sender].currentQuestion = 0;
    const currentQuestion = questions[db.data.game.triviamaker[m.sender].currentQuestion];
    const options = Object.keys(currentQuestion).filter(key => key.startsWith("option")).map(key => currentQuestion[key]).filter(opt => opt);
    ...
    const progress = Math.round((db.data.game.triviamaker[m.sender].currentQuestion + 1) / questions.length * 100);
    const reward = 1e4 + db.data.game.triviamaker[m.sender].currentQuestion * 5e3;
...
handler.before = async (m, {
  ...
  if (!db.data.game || !db.data.game.triviamaker || m.isBaileys || !(m.sender in db.data.game.triviamaker)) return;
  const {
    key: {
      id: keyId
    },
  ...
  if (m.quoted?.id === keyId && m.text.trim()) {
    ...
    try {
      ...
      if (jawaban === "END") {
        ...
        const correctAnswers = db.data.game.triviamaker[m.sender].currentQuestion;
      ...
      const options = Object.keys(question).filter(key => key.startsWith("option")).map(key => question[key]).filter(opt => opt);
      const answerIndex = jawaban.charCodeAt(0) - 65;
      const chosenAnswer = options[answerIndex];
      const correctAnswer = question.correctAnswer;
      if (answerIndex === -1 || !chosenAnswer) {
        return m.reply(`Jawaban tidak valid. Silakan pilih huruf opsi yang benar.`);
      }
      ...
      const currentReward = 1e4 + db.data.game.triviamaker[m.sender].currentQuestion * 5e3;
      if (isCorrect) {
      ...
      if (db.data.game.triviamaker[m.sender].currentQuestion >= questions.length) {
        const totalQuestions = questions.length;
        const correctAnswers = db.data.game.triviamaker[m.sender].currentQuestion;
        ...
        const nextQuestion = questions[db.data.game.triviamaker[m.sender].currentQuestion];
        const newOptions = Object.keys(nextQuestion).filter(key => key.startsWith("option")).map(key => nextQuestion[key]).filter(opt => opt);
        ...
        const newProgress = Math.round((db.data.game.triviamaker[m.sender].currentQuestion + 1) / questions.length * 100);
        const newReward = 1e4 + db.data.game.triviamaker[m.sender].currentQuestion * 5e3;
