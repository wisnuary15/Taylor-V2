...
const handler = async (m, {
  ...
  if (id in db.data.game.question) return await conn.reply(m.chat, "Masih ada soal belum terjawab di chat ini", db.data.game.question[id][0]), !1;
...
Balas pesan ini untuk menjawab!`;
  db.data.game.question[id] = [await conn.reply(m.chat, caption, m, {
    contextInfo: {
    ...
    db.data.game.question[id] && await conn.reply(m.chat, `*\`❌ TIMEOUT - ${command.toUpperCase()}\`*
Jawabannya adalah *${json.results[0]?.correct_answer}*`, db.data.game.question[id][0]),
