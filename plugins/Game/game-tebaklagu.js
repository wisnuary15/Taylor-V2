...
const handler = async (m, {
  ...
  let id = m.chat;
  if (id in db.data.game.tebaklagu) return await conn.reply(m.chat, "Masih ada soal belum terjawab di chat ini", db.data.game.tebaklagu[id][0]), !1;
  try {
...
Balas pesan ini untuk menjawab!`;
    db.data.game.tebaklagu[id] = [await conn.reply(m.chat, caption, m, {
      contextInfo: {
      ...
      db.data.game.tebaklagu[id] && await conn.reply(m.chat, `*\`❌ TIMEOUT - ${command.toUpperCase()}\`*
Jawabannya adalah *${json.judul}*`, db.data.game.tebaklagu[id][0]), delete db.data.game.tebaklagu[id];
