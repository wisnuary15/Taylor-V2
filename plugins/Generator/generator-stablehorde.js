...
const handler = async (m, {
  ...
  if (isNaN(urutan) || urutan < 1 || urutan > input_data.length) {
    ...
    return m.reply(
      `Nomor yang Anda masukkan tidak valid. Harap pilih nomor antara 1 dan ${input_data.length}:\n${validInputList}\n*Example:*\n*${usedPrefix + command}* 2|999|men ( image )\n*${usedPrefix + command}* 2|999|hallo ( text )`
      );
  }
  let out = input_data[urutan - 1];
  if (!queer) return m.reply(
    `Harap masukkan query untuk menghasilkan teks atau gambar.\n*Example:*\n*${usedPrefix + command}* 2|999|men ( image )\n*${usedPrefix + command}* 2|999|hallo ( text )`
    );
  switch (out) {
    case "text":
      ...
      if (isNaN(tema) || tema < 1 || tema > textModels.length) {
        ...
        return m.reply(
          `Nomor yang Anda masukkan tidak valid. Harap pilih nomor antara 1 dan ${textModels.length}:\n${validTemaList}`
          );
      }
      ...
      try {
        const generatedText = await (stableHorde.generateText(dataInputText) || aiHorde.generateText(
        dataInputText));
    ...
    case "image":
      ...
      if (isNaN(tema) || tema < 1 || tema > sortedNames.length) {
        ...
        return m.reply(
          `Nomor yang Anda masukkan tidak valid. Harap pilih nomor antara 1 dan ${sortedNames.length}:\n${validTemaList}`
          );
      }
      ...
      try {
        const generatedImageURL = await (stableHorde.generateImage(dataInputImage) || aiHorde.generateImage(
          dataInputImage));
