      ...
      for (let k = 1; k <= ccghm; k++) {
        ...
        if (clcd && ccck) {
          ...
          if (datax.getElementById['ccexpdat'].value) {
            ...
            const mes = (datax.getElementById['emeses'].value === 'rnd') ? unilenS(rnd(1, 12), 2, '0', 0) : datax
              .getElementById['emeses'].value
            const year = (datax.getElementById['eyear'].value === 'rnd') ? (dnowi.getFullYear() + rnd(2, 6)) : datax
              .getElementById['eyear'].value
            expDate = `|${mes}|${year}`
          }
          const eccv = (datax.getElementById['eccv'].value === 'rnd' && datax.getElementById['ccvi'].value) ? ((cdif
            .startsWith('34') || cdif.startsWith('37')) ? (Math.floor(Math.random() * (9999 - 1000 + 1)) + 1000) : (
            Math.floor(Math.random() * (998 - 112 + 1)) + 112)) : datax.getElementById['eccv'].value
          out +=
            `${formatPrefix}${cdif}${expDate}${(datax.getElementById['ccvi'].value) ? ('|' + eccv) : ''}${(datax.getElementById['ccbank'].value) ? ('|' + cn) : ''}\n`
        } else {
          out = 'Error'
          break
        }
