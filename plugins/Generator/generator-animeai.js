...
const handler = async (m, { conn: conn, usedPrefix: usedPrefix, command: command }) => {
  try {
    ...
    if (/webp/g.test(mime) ? out = meme + await webp2png(img) + memee : /image|video|gif|viewOnce/g.test(mime) && (out = meme + await uploadFile(img) + memee),
      m.reply("wait"), !out) throw "Error: Tidak dapat memproses gambar";
