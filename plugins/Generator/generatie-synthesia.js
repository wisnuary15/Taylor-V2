    ...
    for (;;) {
      ...
      if ("complete" === videoResponse.status) {
        await conn.sendFile(m.chat, videoResponse.download, "", "Synthesia", m, !1, {
          mentions: [m.sender]
        }), await conn.sendReact(m.chat, "✅", m.key), delete conn.synthesia[m.chat];
        break;
      }
      if ("in_progress" !== videoResponse.status) {
        m.reply("Terjadi kesalahan saat mengunduh video. Silakan coba lagi nanti."), await conn.sendReact(m.chat, "❌", m.key);
        break;
      }
      await new Promise((resolve => setTimeout(resolve, 5e3)));
    }
  } catch (error) {
    console.error(error), m.reply("Terjadi kesalahan saat mengunduh video. Silakan coba lagi nanti."),
      await conn.sendReact(m.chat, "❌", m.key);
  }
};
