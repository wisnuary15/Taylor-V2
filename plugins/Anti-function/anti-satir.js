...
export async function before(m, {
  ...
  database.satirWords[m.chat] = database.satirWords[m.chat] || [...satirWords];
  ...
  if (input?.trim().split(/\s+/)?.[0]?.toLowerCase() === "addsatirword" && input?.trim().split(/\s+/)?.slice(1).join(" ")) {
    ...
    database.satirWords[m.chat].push(newSatirWord);
  ...
  if (input?.trim().split(/\s+/)?.[0]?.toLowerCase() === "delsatirword" && input?.trim().split(/\s+/)?.slice(1).join(" ")) {
    ...
    const index = _.indexOf(database.satirWords[m.chat], wordToDelete);
    if (index > -1) {
      _.pullAt(database.satirWords[m.chat], index);
  ...
  if (input?.trim().split(/\s+/)?.[0]?.toLowerCase() === "listsatirword") {
    if (database.satirWords[m.chat].length > 0) {
      const list = _.map(database.satirWords[m.chat], (word, i) => `${i + 1}. ${word}`).join("\n");
  ...
  const detectedWords = _.filter(database.satirWords[m.chat], word => _.includes(input, word));
  if (chat.antiSatir && detectedWords.length > 0) {
    let warningMessage = `🚨 *Kata Satir Terdeteksi!*\nKata yang terdeteksi: ${detectedWords.join(", ")}\n`;
    if (!isBotAdmin) {
