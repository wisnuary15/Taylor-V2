...
export async function before(m) {
  ...
  if (chats[m.chat].antiSpam && !m.isBaileys && !['protocolMessage', 'pollUpdateMessage', 'reactionMessage'].includes(
      m.mtype) && m.msg && m.message && m.key.remoteJid === m.chat && !users[m.sender].banned && !chats[m.chat]
    .isBanned) {
    this.spam = this.spam || {};
    ...
    if (timeDifference < COOLDOWN_DURATION) {
      ...
      if (this.spam[m.sender].count >= SPAM_THRESHOLD) {
        ...
        const message = m.mtype.replace(/message$/i, '').replace('audio', m.msg.ptt ? 'PTT' : 'audio').replace(/^./,
