...
export async function before(m, {
  ...
  if (chats[m.chat].antiSpam && !m.isBaileys && !["protocolMessage", "pollUpdateMessage", "reactionMessage"].includes(m.mtype) && m.msg && m.message && m.key.remoteJid === m.chat && !users[m.sender].banned && !chats[m.chat].isBanned) {
    db.data.database.spam = db.data.database.spam || {}, db.data.database.spam[m.sender] = db.data.database.spam[m.sender] || {
      count: 0,
      lastspam: 0
    };
    const now = performance.now();
    if (now - db.data.database.spam[m.sender].lastspam < 1e4) {
      if (db.data.database.spam[m.sender].count++, db.data.database.spam[m.sender].count >= 5) {
        if (!isOwner) users[m.sender].banned = !0, db.data.database.spam[m.sender].lastspam = now + 5e3,
          setTimeout(() => {
            users[m.sender].banned = !1, db.data.database.spam[m.sender].count = 0, m.reply("✅ *Cooldown selesai*\nAnda bisa mengirim pesan lagi.");
          }, 5e3);
        const remainingCooldown = Math.ceil((db.data.database.spam[m.sender].lastspam - now) / 1e3),
