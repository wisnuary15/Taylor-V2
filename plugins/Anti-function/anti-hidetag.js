export async function before(m, {
  ...
  try {
    ...
    if (chat && m.isGroup && chat.antiHidetag) {
      ...
      let participantsLength = this.chats[m.chat]?.participants?.length || store.chats[m.chat]?.participants?.length || (await this.groupMetadata(m.chat)).participants?.length;
