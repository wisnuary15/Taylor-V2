...
async function handler(m, { conn: conn, args: args, text: text, usedPrefix: usedPrefix, command: command }) {
  ...
  try {
    ...
    "number" != typeof user.lastmisi && (db.data.users[m.sender].lastmisi = 0), "number" != typeof user.exp && (db.data.users[m.sender].exp = 0),
      "number" != typeof user.crystal && (db.data.users[m.sender].crystal = 0);
    ...
    if (console.error(e), m.sender in conn.mission) {
      let { timeout: timeout } = conn.mission[m.sender];
...
handler.before = async m => {
  ...
  let { timeout: timeout, json: json } = conn.mission[m.sender];
  let user = db.data.users[m.sender],
  ...
  try {
    if (/^accept?$/i.test(txt)) {
      ...
      return user.lastmisi = 1 * new Date, clearTimeout(timeout), delete conn.mission[m.sender],
        !0;
    }
