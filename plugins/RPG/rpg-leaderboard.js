...
const leaderboards = ['level', 'exp', 'limit', 'money', 'iron', 'gold', 'diamond', 'emerald', 'trash', 'potion',
  'petFood', 'wood', 'rock', 'string', 'common', 'uncommon', 'mythic', 'legendary', 'pet'
];
...
${usedPrefix}${command} legendary`.trim();
  ...
  let userEntries = await Promise.all(sortedItem?.slice(0, 5).map(async (user, i) => {
    let isParticipant = participants.some(p => areJidsSameUser(user.jid, p.id));
    let name = isParticipant ? conn.getName(user.jid) : 'Unknown';
