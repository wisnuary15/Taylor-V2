const handler = async (m, { conn: conn, usedPrefix: usedPrefix, participants: participants }) => {
  ...
  var time;
  m.reply(`*Pet Kamu* (🦅phonix ${db.data.users[m.sender].phonix}) ⚔️menantang 🦅phonixnya *${conn.getName(lawan)}* (🦅phonix ${db.data.users[lawan].phonix}) lagi berkelahi.\n\nTunggu ${lamaPertarungan} menit lagi dan lihat siapa yg menang🎮.`),
...
handler.help = ["fightphonix"], handler.tags = ["game"], handler.command = /^(fightphonix)$/i,
