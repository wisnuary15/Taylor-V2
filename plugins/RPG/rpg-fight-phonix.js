const handler = async (m, {
    ...
    m.reply(`*Pet Kamu* (🦅phonix ${db.data.users[m.sender].phonix}) ⚔️menantang 🦅phonixnya *${await conn.getName(lawan)}* (🦅phonix ${db.data.users[lawan].phonix}) lagi berkelahi.\n\nTunggu ${lamaPertarungan} menit lagi dan lihat siapa yg menang🎮.`)
    ...
    let kesempatan = []
    for (i = 0; i < db.data.users[m.sender].phonix; i++) kesempatan.push(m.sender)
    ...
    if (pointPemain > pointLawan) {
        ...
        m.reply(`*${await conn.getName(m.sender)}* [${pointPemain * 10}] - [${pointLawan * 10}] *${await conn.getName(lawan)}*\n\n*Pet🦅Kamu* (phonix ${db.data.users[m.sender].phonix}) MENANG melawan 🦅phonixnya *${await conn.getName(lawan)}* (phonix ${db.data.users[lawan].phonix}) karena phonix🦅kamu ${alasanMenang[Acakin(0,alasanMenang.length-1)]}\n\nHadiah Rp. ${hadiah.toLocaleString()}\n+1 Tiketcoin`)
    } else if (pointPemain < pointLawan) {
        let denda = (pointLawan - pointPemain) * 10000
        db.data.users[m.sender].money -= denda
        db.data.users[m.sender].tiketcoin += 1
        m.reply(`*${await conn.getName(m.sender)}* [${pointPemain * 10}] - [${pointLawan * 10}] *${await conn.getName(lawan)}*\n\n*Pet🦅Kamu* (phonix ${db.data.users[m.sender].phonix}) KALAH melawan 🦅phonixnya *${await conn.getName(lawan)}* (phonix ${db.data.users[lawan].phonix}) karena pet kamu ${alasanKalah[Acakin(0,alasanKalah.length-1)]}\n\nUang kamu berkurang Rp. ${denda.toLocaleString()}\n+1 Tiketcoin`)
    } else {
