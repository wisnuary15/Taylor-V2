...
const handler = async (m, { conn: conn, args: args, usedPrefix: usedPrefix, DevMode: DevMode }) => {
  try {
    ...
    t > 3e5 ? A > K ? (await conn.sendFile(m.chat, kb, "b.jpg", "*Kamu Tertangkap!* Korupsi dana bansos 🕴️💰, Denda *3 Juta* rupiah 💵", m),
      u.money -= 3e6, u.lastbansos = 1 * new Date) : A < K ? (u.money += 3e6, await conn.sendFile(m.chat, mb, "b.jpg", "*Berhasil Korupsi!* Dana bansos 🕴️💰, Dapatkan *3 Juta* rupiah 💵", m),
      u.lastbansos = 1 * new Date) : (await conn.reply(m.chat, "*Maaf!* Kamu tidak berhasil melakukan korupsi bansos dan kamu tidak akan masuk penjara karena kamu *melarikan diri* 🏃", m),
      u.lastbansos = 1 * new Date) : await conn.reply(m.chat, `*Sudah Melakukan Korupsi!* 💰\nHarus menunggu selama agar bisa korupsi bansos kembali\n▸ 🕓 ${timers}`, m);
  } catch (e) {
