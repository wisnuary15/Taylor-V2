...
const handler = async (m, {
  ...
  const tfcrates = Object.keys(tfinventory.tfcrates).map(v => user[v] && `⮕ ${rpg.emoticon(v)} ${v}: ${user[v]}`)
    .filter(v => v).join('\n').trim()
...
`.trim()
  ...
  let count = Math.floor(isNumber(args[1]) ? Math.min(Math.max(parseInt(args[1]), 1), Number.MAX_SAFE_INTEGER) :
    1) * 1
  if (!(type in listCrate)) return conn.sendFile(m.chat, imgr + 'open', '', `*${htki} OPEN CRATES ${htka}*\n` +
    info, m)
