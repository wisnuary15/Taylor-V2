...
const handler = async (m, {
  ...
  if (currentTime - lastgiftTime < cooldownDuration) {
    ...
    return conn.reply(m.chat,
      `⏰ Maaf, kamu harus menunggu ${remainingTime} lagi sebelum menggunakan FreeGift lagi!`, m);
  }
  ...
  if (!validGiftcode.length) {
    ...
    return conn.reply(m.chat,
      `Maaf, kode FreeGift tidak valid atau sudah digunakan. Silahkan coba lagi setelah ${remainingTime}!`, m);
  }
  const maxExp = 10000,
    maxMoney = 10000;
