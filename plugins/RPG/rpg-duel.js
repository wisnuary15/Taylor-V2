const handler = async (m, { conn: conn, text: text, args: args, command: command }) => {
  ...
  let who = conn.duel[0],
    ...
    Aku = (args[1] && args[1].length > 0 ? Math.min(100, Math.max(parseInt(args[1]), 1)) : Math.min(1),
      conn.getName(m.sender), 1 * `${Math.floor(101 * Math.random())}`.trim()),
  ...
  try {
    ...
    if (/dya/.test(command)) {
      ...
      user.lastduel = 1 * new Date, Aku > Kamu ? (user.money -= 900, enemy.money += 900,
        delete conn.duel[m.sender], await conn.reply(m.chat, `@${who.split("@")[0]} Menang Gelud🤼\n*Hadiah:*\n900 Money buat beli gorengan`.trim(), m)) : Aku < Kamu ? (user.money += 450,
        enemy.money -= 450, delete conn.duel[m.sender], await conn.reply(m.chat, `@${who.split("@")[0]} Kalah Gelud🤼\n*Hadiah:*\n 450 money Mayan buat beli Limit`.trim(), m)) : (user.money += 250,
        enemy.money += 250, delete conn.duel[m.sender], await conn.reply(m.chat, `@${who.split("@")[0]}\n *Seri*\n masing masing 250 Money`.trim(), m));
    }
    if (/dno/.test(command)) {
