const xpperlimit = 1,
  handler = async (m, { conn: conn, command: command, args: args }) => {
    ...
    if (count = count ? /all/i.test(count) ? Math.floor(user.bank / 1) : parseInt(count) : args[0] ? parseInt(args[0]) : 1,
      count = Math.max(1, count), 0 === user.atm) return await conn.reply(m.chat, "Kamu belum memiliki ATM!", m);
