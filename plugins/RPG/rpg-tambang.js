...
async function handler(m, { conn: conn, text: text }) {
  ...
  if ("start" === text) {
    ...
    "number" != typeof user.exp && (db.data.users[m.sender].exp = 0), "number" != typeof user.crystal && (db.data.users[m.sender].crystal = 0),
      "number" != typeof user.lastGameTime && (db.data.users[m.sender].lastGameTime = 0);
    ...
    let { area: area, txt: txt, reward: reward } = tambangs[0], currentArea = 0, hasilTambang = 0, totalReward = 0;
...
handler.before = async m => {
  ...
  let { areas: areas, currentArea: currentArea, hasilTambang: hasilTambang, lastTambangTime: lastTambangTime, totalReward: totalReward } = conn.tambang[m.sender];
  let user = db.data.users[m.sender],
  ...
  if (currentArea < areas.length && areas[currentArea].txt === msg) {
    let { area: area, reward: reward } = areas[currentArea];
    if (user.exp += reward.exp, user.crystal += reward.crystal, hasilTambang++, totalReward += reward.exp + reward.crystal,
      currentArea++, conn.tambang[m.sender].currentArea = currentArea, conn.tambang[m.sender].hasilTambang = hasilTambang,
      conn.tambang[m.sender].totalReward = totalReward, conn.tambang[m.sender].lastTambangTime = Date.now(),
      currentArea >= areas.length) return m.reply(`🎉 Selamat! Anda telah menyelesaikan semua area pertambangan.\nTotal hasil tambang: ${hasilTambang}\nExp yang didapatkan: ${totalReward}\nTotal crystal yang didapatkan: ${totalReward}`),
      delete conn.tambang[m.sender], !1;
    {
