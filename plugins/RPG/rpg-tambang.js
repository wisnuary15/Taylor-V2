...
async function handler(m, {
  ...
  db.data.rpg.tambang = db.data.rpg.tambang || {};
  let user = db.data.users[m.sender];
  if (m.sender in db.data.rpg.tambang) return db.data.rpg.tambang[m.sender].currentArea >= db.data.rpg.tambang[m.sender].areas.length ? m.reply("🏆 Anda telah menyelesaikan semua area pertambangan.") : m.reply("⏳ Anda masih memiliki area pertambangan yang belum selesai. Silakan selesaikan terlebih dahulu.");
  if ("start" === text) {
...
handler.before = async m => {
  if (db.data.rpg.tambang = db.data.rpg.tambang || {}, !(m.sender in db.data.rpg.tambang)) return;
  ...
  if ("stop" === msg) return m.reply("❌ Pertambangan telah dihentikan. Ketik *'tambang start'* untuk memulai pertambangan kembali."),
    delete db.data.rpg.tambang[m.sender], !1;
  if (currentArea < areas.length && areas[currentArea].txt === msg) {
    let {
      area,
      reward
    } = areas[currentArea];
    if (user.exp += reward.exp, user.crystal += reward.crystal, hasilTambang++, totalReward += reward.exp + reward.crystal, currentArea++, db.data.rpg.tambang[m.sender].currentArea = currentArea, db.data.rpg.tambang[m.sender].hasilTambang = hasilTambang, db.data.rpg.tambang[m.sender].totalReward = totalReward, db.data.rpg.tambang[m.sender].lastTambangTime = Date.now(), currentArea >= areas.length) return m.reply(`🎉 Selamat! Anda telah menyelesaikan semua area pertambangan.\nTotal hasil tambang: ${hasilTambang}\nExp yang didapatkan: ${totalReward}\nTotal crystal yang didapatkan: ${totalReward}`),
      delete db.data.rpg.tambang[m.sender], !1;
    {
