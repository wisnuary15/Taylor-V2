let confirmation = {};
async function handler(m, { conn: conn, args: args, usedPrefix: usedPrefix, command: command }) {
  ...
  let hasil = formatRupiah(Number(count)),
    ...
    { key: key } = await conn.reply(m.chat, confirm, m, {
      mentions: [m.sender]
    });
...
handler.before = async m => {
    ...
    if (!m.text) return;
    let { timeout: timeout, sender: sender, message: message, count: count, hasil: hasil, key: key, pesan: pesan } = confirmation[m.sender];
    if (m.id === message.id) return;
    let user = db.data.users[m.sender],
      _user = db.data.users[sender];
    /(✔️|y(es)?)/g.test(m.text.toLowerCase()) && (m.sender !== sender ? (user.money -= 1 * count,
        _user.money += 1 * count, m.reply(`✨ Terima kasih!\n${m.name.split("\n")[0]} telah memberi sumbangan sebesar *${hasil}*`),
        pesan.sendMessage(m.chat, {
          delete: key
        }), clearTimeout(timeout), delete confirmation[sender]) : m.reply("⚠️ Tidak bisa meminta sumbangan ke diri anda sendiri!.")),
      /(✖️|n(o)?)/g.test(m.text.toLowerCase()) && (m.reply(`😔 ${m.name.split("\n")[0]} kamu berdosa banget kak...`),
        pesan.sendMessage(m.chat, {
          delete: key
