...
export async function before(m) {
  ...
  if (m.isGroup && m.chat in this.casino) {
    if (m.sender === this.casino[m.chat].player_2 && /^(acc(ept)?|terima|gas|oke?|tolak|gamau|nanti|ga(k.)?bisa|y)/i
      .test(m.text)) {
      ...
      db.data.users[this.casino[m.chat].player_1].money -= this.casino[m.chat].count * 1;
      db.data.users[this.casino[m.chat].player_2].money -= this.casino[m.chat].count * 1;
      let randomplayer_1 = `${Math.floor(Math.random() * 10)}`.trim() * 1;
      let randomplayer_2 = `${Math.floor(Math.random() * 10)}`.trim() * 1;
      let sya = conn.getName(this.casino[m.chat].player_1);
      let lwn = conn.getName(this.casino[m.chat].player_2);
      if (randomplayer_1 > randomplayer_2) {
        ...
        db.data.users[this.casino[m.chat].player_1].money += this.casino[m.chat].count * 2;
        delete this.casino[m.chat];
        ...
        db.data.users[this.casino[m.chat].player_2].money += this.casino[m.chat].count * 2;
        delete this.casino[m.chat];
      } else {
        let caption = `💰 *C A S I N O - D U E L* 💰\n\n${htjava} @${
          this.casino[m.chat].player_1.split("@")[0]
