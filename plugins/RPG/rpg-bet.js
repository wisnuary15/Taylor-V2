...
handler.before = async m => {
  ...
  const { timeout: timeout, sender: sender, message: message, count: count, key: key } = better[m.sender], user = db.data.users[m.sender];
  if (/(✔️|y(es)?)/gi.test(m.text.toLowerCase())) {
    const Bot = Math.ceil(91 * Math.random()),
      Kamu = Math.floor(71 * Math.random());
    let status = "Kalah";
    return Bot < Kamu ? (user.money += count, status = "Menang") : Bot > Kamu ? user.money -= count : (status = "Seri",
        user.money += Math.floor(count / 1.5)), m.reply(`\n| *PLAYERS* | *POINT* |\n*🤖 BOT:*      ${Bot}\n*👤 KAMU:*    ${Kamu}\n\nKamu *${status}*, kamu ${"Menang" === status ? `Mendapatkan *+${2 * count}*` : "Kalah" === status ? `Kehilangan *-${1 * count}*` : `Mendapatkan *+${Math.floor(count / 1.5)}*`} Money 💹\n        `.trim()),
      clearTimeout(timeout), delete better[m.sender], !0;
  }
  return /(✖️|n(o)?)/gi.test(m.text.toLowerCase()) ? (clearTimeout(timeout), m.reply("Rejected"),
    delete better[m.sender], !0) : void 0;
