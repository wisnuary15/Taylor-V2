...
const handler = async (m, {
  ...
  if (!args[0] || !m.mentionedJid[0]) return conn.reply(m.chat, usedPrefix + "casino <jumlah> @lawan\n " +
  ...
  if (!db.data.users[m.mentionedJid[0]]) return m.reply(
    "Lawan tidak terdaftar didatabase bot atau format kamu salah. " + usedPrefix + "casino <jumlah> @lawan");
  let count = args[0];
  count = count ? /all/i.test(count) ? Math.floor(db.data.users[m.sender].money / 1) : parseInt(count) : args[0] ?
    parseInt(args[0]) : 1;
  count = Math.max(1, count);
  if (db.data.users[m.sender].money >= count * 1 && db.data.users[m.mentionedJid[0]].money >= count * 1) {
    conn.casino[m.chat] = {
      player_1: m.sender,
...
Atau Ketik Terima/Tolak`;
    ...
    if (db.data.users[m.sender].money <= count) return m.reply(
      "Money kamu tidak mencukupi untuk Casino silahkan *.claim* terlebih dahulu!");
    if (db.data.users[m.mentionedJid[0]].money <= count) return m.reply(
