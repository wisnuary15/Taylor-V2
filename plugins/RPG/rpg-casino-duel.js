...
const handler = async (m, {
    ...
    if (!db.data.users[m.mentionedJid[0]])
        return m.reply(
    ...
    count = count ?
        /all/i.test(count) ?
        Math.floor(db.data.users[m.sender].money / 1) :
        parseInt(count) :
        args[0] ?
        parseInt(args[0]) :
        1;
    ...
    if (
        ...
        db.data.users[m.mentionedJid[0]].money >= count * 1
    ) {
        conn.casino[m.chat] = {
            player_1: m.sender,
...
Atau Ketik Terima/Tolak`;
        ...
        if (db.data.users[m.sender].money <= count)
            return m.reply(
                "Money kamu tidak mencukupi untuk Casino silahkan *.claim* terlebih dahulu!"
            );
        if (db.data.users[m.mentionedJid[0]].money <= count)
            return m.reply(
