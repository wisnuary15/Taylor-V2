const handler = async (m, {
  ...
  if (db.data.rpg.casinomulti = db.data.rpg.casinomulti ? db.data.rpg.casinomulti : {}, m.chat in db.data.rpg.casinomulti) return m.reply("Masih ada yang melakukan casino disini, tunggu sampai selesai!!");
  ...
  if (count = count ? /all/i.test(count) ? Math.floor(db.data.users[m.sender].money / 1) : parseInt(count) : args[0] ? parseInt(args[0]) : 1, count = Math.max(1, count), db.data.users[m.sender].money >= 1 * count && db.data.users[m.mentionedJid[0]].money >= 1 * count) {
    db.data.rpg.casinomulti[m.chat] = {
      player_1: m.sender,
