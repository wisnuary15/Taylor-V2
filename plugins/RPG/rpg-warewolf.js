...
const handler = async (m, {
  ...
  if (inputcmd === "start") {
    ...
    if (players.length < 2) return m.reply(
    ...
    if (players.length >= maxPlayers) return m.reply(
    ...
    if (players.some((player) => player.name === playerName)) return m.reply(
      "😅 Kamu sudah bergabung dalam permainan.");
