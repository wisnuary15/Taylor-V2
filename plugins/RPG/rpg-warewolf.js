const roles = ["warewolf", "manusia"],
  handler = async (m, { conn: conn, command: command, usedPrefix: usedPrefix, text: text }) => {
    ...
    const { players: players, started: started } = conn.warewolf[groupId];
    if ("start" === inputcmd) {
      ...
      playerIndex === players.findIndex((player => "warewolf" === player.role)) ? (m.reply(`🎉 Selamat! ${warewolf.name} adalah Warewolf! Permainan berakhir.`),
        delete conn.warewolf[groupId]) : m.reply("😔 Tebakanmu salah! Coba lagi.");
