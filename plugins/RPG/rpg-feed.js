const handler = async (m, { conn: conn, args: args, usedPrefix: usedPrefix, command: command }) => {
  let type = (args[0] || "").toLowerCase(),
    user = (gc, thumb, db.data.users[m.sender]),
  ...
  switch (type) {
    case "fox":
      ...
      if (new Date - db.data.users[m.sender].foxlastclaim > 6e5)
        if (db.data.users[m.sender].makananpet > 0) {
          if (db.data.users[m.sender].makananpet -= 1, db.data.users[m.sender].foxexp += 20,
            db.data.users[m.sender].foxlastclaim = 1 * new Date, await conn.reply(m.chat, `Feeding ${type} success`, m),
            setTimeout((async () => {
              await conn.reply(m.chat, "Waktunya memberi makan *fox*\nKarena sudah lapar..", m);
            }), 6e5), fox > 0) {
            let naiklvl = 1e3 * fox - 1;
            db.data.users[m.sender].foxexp > naiklvl && (db.data.users[m.sender].fox += 1, db.data.users[m.sender].foxexp -= 1e3 * fox,
              await conn.reply(m.chat, "*Congratulations your pet fox level up*", m));
          }
    ...
    case "rhinoceros":
      ...
      if (new Date - db.data.users[m.sender].rhinoceroslastclaim > 6e5)
        if (db.data.users[m.sender].makananpet > 0) {
          if (db.data.users[m.sender].makananpet -= 1, db.data.users[m.sender].rhinocerosexp += 15,
            db.data.users[m.sender].rhinoceroslastclaim = 1 * new Date, await conn.reply(m.chat, `Feeding ${type} success`, m),
            rhinoceros > 0) {
            let naiklvl = 1e3 * rhinoceros - 1;
            db.data.users[m.sender].rhinocerosexp > naiklvl && (db.data.users[m.sender].rhinoceros += 1,
              db.data.users[m.sender].rhinocerosexp -= 100 * rhinoceros, await conn.reply(m.chat, "*Congratulations your pet rhinoceros level up*", m));
          }
        } else m.reply("Makanan Pet Kamu Tidak Cukup");
    ...
    case "lion":
      ...
      if (new Date - db.data.users[m.sender].lionlastclaim > 6e5)
        if (db.data.users[m.sender].makananpet > 0) {
          if (db.data.users[m.sender].makananpet -= 1, db.data.users[m.sender].lionexp += 15,
            db.data.users[m.sender].lionlastclaim = 1 * new Date, await conn.reply(m.chat, `Feeding ${type} success`, m),
            lion > 0) {
            let naiklvl = 1e3 * lion - 1;
            db.data.users[m.sender].lionexp > naiklvl && (db.data.users[m.sender].lion += 1,
              db.data.users[m.sender].lionexp -= 100 * lion, await conn.reply(m.chat, "*Congratulations your pet lion level up*", m));
          }
        } else m.reply("Makanan Pet Kamu Tidak Cukup");
    ...
    case "horse":
      ...
      if (new Date - db.data.users[m.sender].horselastclaim > 6e5)
        if (db.data.users[m.sender].makananpet > 0) {
          if (db.data.users[m.sender].makananpet -= 1, db.data.users[m.sender].horseexp += 20,
            db.data.users[m.sender].horselastclaim = 1 * new Date, await conn.reply(m.chat, `Feeding ${type} success`, m),
            setTimeout((async () => {
              await conn.reply(m.chat, "Waktunya memberi makan *horse*\nKarena sudah lapar..", m);
            }), 6e5), horse > 0) {
            let naiklvl = 1e3 * horse - 1;
            db.data.users[m.sender].horseexp > naiklvl && (db.data.users[m.sender].horse += 1,
              db.data.users[m.sender].horseexp -= 1e3 * horse, await conn.reply(m.chat, "*Congratulations your pet horse level up*", m));
          }
        } else m.reply("Makanan pet kamu tidak cukup");
    ...
    case "wolf":
      ...
      if (new Date - db.data.users[m.sender].wolflastclaim > 6e5)
        if (db.data.users[m.sender].makananpet > 0) {
          if (db.data.users[m.sender].makananpet -= 1, db.data.users[m.sender].wolfexp += 15,
            db.data.users[m.sender].wolflastclaim = 1 * new Date, await conn.reply(m.chat, `Feeding ${type} success`, m),
            setTimeout((async () => {
              await conn.reply(m.chat, "Waktunya memberi makan *wolf*\nKarena sudah lapar..", m);
            }), 6e5), wolf > 0) {
            let naiklvl = 1e4 * wolf - 1;
            db.data.users[m.sender].wolfexp > naiklvl && (db.data.users[m.sender].wolf += 1,
              db.data.users[m.sender].wolfexp -= 1e4 * wolf, await conn.reply(m.chat, "*Congratulations your pet wolf level up*", m));
          }
        } else m.reply("Makanan pet kamu tidak cukup");
    ...
    case "dragon":
      ...
      if (new Date - db.data.users[m.sender].dragonlastclaim > 6e5)
        if (db.data.users[m.sender].makanandragon > 0) {
          if (db.data.users[m.sender].makanandragon -= 1, db.data.users[m.sender].dragonexp += 10,
            db.data.users[m.sender].dragonlastclaim = 1 * new Date, await conn.reply(m.chat, `Feeding ${type} success`, m),
            setTimeout((async () => {
              await conn.reply(m.chat, "Waktunya memberi makan *dragon*\nKarena sudah lapar..", m);
            }), 6e5), dragon > 0) {
            let naiklvl = 1e4 * dragon - 1;
            db.data.users[m.sender].dragonexp > naiklvl && (db.data.users[m.sender].dragon += 1,
              db.data.users[m.sender].dragonexp -= 1e4 * dragon, await conn.reply(m.chat, "*Congratulations your pet dragon level up*", m));
          }
        } else m.reply("Makanan pet kamu tidak cukup");
    ...
    case "kyubi":
      ...
      if (new Date - db.data.users[m.sender].kyubilastclaim > 6e5)
        if (db.data.users[m.sender].makanankyubi > 0) {
          if (db.data.users[m.sender].makanankyubi -= 1, db.data.users[m.sender].kyubiexp += 10,
            db.data.users[m.sender].kyubilastclaim = 1 * new Date, await conn.reply(m.chat, `Feeding ${type} success`, m),
            setTimeout((async () => {
              await conn.reply(m.chat, "Waktunya memberi makan *Kyubi*\nKarena sudah lapar..", m);
            }), 6e5), kyubi > 0) {
            let naiklvl = 1e4 * kyubi - 1;
            db.data.users[m.sender].kyubiexp > naiklvl && (db.data.users[m.sender].kyubi += 1,
              db.data.users[m.sender].kyubiexp -= 1e4 * kyubi, await conn.reply(m.chat, "*Congratulations your pet Kyubi level up*", m));
          }
        } else m.reply("Makanan pet kamu tidak cukup");
    ...
    case "centaur":
      ...
      if (new Date - db.data.users[m.sender].centaurlastclaim > 6e5)
        if (db.data.users[m.sender].makanancentaur > 0) {
          if (db.data.users[m.sender].makanancentaur -= 1, db.data.users[m.sender].centaurexp += 10,
            db.data.users[m.sender].centaurlastclaim = 1 * new Date, await conn.reply(m.chat, `Feeding ${type} success`, m),
            setTimeout((async () => {
              await conn.reply(m.chat, "Waktunya memberi makan *Centaur*\nKarena sudah lapar..", m);
            }), 6e5), centaur > 0) {
            let naiklvl = 1e4 * centaur - 1;
            db.data.users[m.sender].centaurexp > naiklvl && (db.data.users[m.sender].centaur += 1,
              db.data.users[m.sender].centaurexp -= 1e4 * centaur, await conn.reply(m.chat, "*Congratulations your pet Centaur level up*", m));
          }
        } else m.reply("Makanan pet kamu tidak cukup");
    ...
    case "phonix":
      ...
      if (new Date - db.data.users[m.sender].phonixlastclaim > 6e5)
        if (db.data.users[m.sender].makananphonix > 0) {
          if (db.data.users[m.sender].makananphonix -= 1, db.data.users[m.sender].phonixexp += 10,
            db.data.users[m.sender].phonixlastclaim = 1 * new Date, await conn.reply(m.chat, `Feeding ${type} success`, m),
            setTimeout((async () => {
              await conn.reply(m.chat, "Waktunya memberi makan *Phonix*\nKarena sudah lapar..", m);
            }), 6e5), phonix > 0) {
            let naiklvl = 1e4 * phonix - 1;
            db.data.users[m.sender].phonixexp > naiklvl && (db.data.users[m.sender].phonix += 1,
              db.data.users[m.sender].phonixexp -= 1e4 * phonix, await conn.reply(m.chat, "*Congratulations your pet Phonix level up*", m));
          }
        } else m.reply("Makanan pet kamu tidak cukup");
    ...
    case "griffin":
      ...
      if (new Date - db.data.users[m.sender].griffinlastclaim > 6e5)
        if (db.data.users[m.sender].makanangriffin > 0) {
          if (db.data.users[m.sender].makanangriffin -= 1, db.data.users[m.sender].griffinexp += 10,
            db.data.users[m.sender].griffinlastclaim = 1 * new Date, await conn.reply(m.chat, `Feeding ${type} success`, m),
            setTimeout((async () => {
              await conn.reply(m.chat, "Waktunya memberi makan *Griffin*\nKarena sudah lapar..", m);
            }), 6e5), griffin > 0) {
            let naiklvl = 1e4 * griffin - 1;
            db.data.users[m.sender].griffinexp > naiklvl && (db.data.users[m.sender].griffin += 1,
              db.data.users[m.sender].griffinexp -= 1e4 * griffin, await conn.reply(m.chat, "*Congratulations your pet Greffin level up*", m));
          }
        } else m.reply("Makanan pet kamu tidak cukup");
    ...
    case "cat":
      ...
      if (new Date - db.data.users[m.sender].catlastclaim > 6e5)
        if (db.data.users[m.sender].makananpet > 0) {
          if (db.data.users[m.sender].makananpet -= 1, db.data.users[m.sender].catexp += 20,
            db.data.users[m.sender].catlastclaim = 1 * new Date, await conn.reply(m.chat, `Feeding ${type} success`, m),
            setTimeout((async () => {
              await conn.reply(m.chat, "Waktunya memberi makan *cat*\nKarena sudah lapar..", m);
            }), 6e5), cat > 0) {
            let naiklvl = 1e3 * cat - 1;
            db.data.users[m.sender].catexp > naiklvl && (db.data.users[m.sender].cat += 1, db.data.users[m.sender].catexp -= 1e3 * cat,
              await conn.reply(m.chat, "*Congratulations your pet cat level up*", m));
          }
        } else m.reply("Makanan pet kamu tidak cukup");
    ...
    case "rubah":
      ...
      if (new Date - user.foxlastfeed > 6e5)
        if (user.petFood > 0) {
          if (user.petFood -= 1, user.foxexp += 20, user.foxlastfeed = 1 * new Date, await conn.reply(m.chat, `Berhasil memberi makan pet ${type} 🦊`, m),
            rubah > 0) {
    ...
    case "kucing":
      ...
      if (new Date - user.catlastfeed > 6e5)
        if (user.petFood > 0) {
          if (user.petFood -= 1, user.catngexp += 20, user.catlastfeed = 1 * new Date, await conn.reply(m.chat, `Berhasil memberi makan pet ${type} 🐱`, m),
            kucing > 0) {
    ...
    case "anjing":
      ...
      if (new Date - user.doglastfeed > 6e5)
        if (user.petFood > 0) {
          if (user.petFood -= 1, user.dogexp += 20, user.doglastfeed = 1 * new Date, await conn.reply(m.chat, `Berhasil memberi makan pet ${type} 🐶`, m),
            anjing > 0) {
    ...
    case "kuda":
      ...
      if (new Date - user.horselastfeed > 6e5)
        if (user.petFood > 0) {
          if (user.petFood -= 1, user.horseexp += 20, user.horselastfeed = 1 * new Date, await conn.reply(m.chat, `Berhasil memberi makan pet ${type} 🐴`, m),
            kuda > 0) {
