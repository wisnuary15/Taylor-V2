const handler = async (m, {
    ...
    m.reply(`*Pet Kamu* (🦚griffin ${db.data.users[m.sender].griffin}) ⚔️menantang 🦚griffinnya *${await conn.getName(lawan)}* (🦚griffin ${db.data.users[lawan].griffin}) lagi berkelahi.\n\nTunggu ${lamaPertarungan} menit lagi dan lihat siapa yg menang🎮.`)
    ...
    if (pointPemain > pointLawan) {
        ...
        m.reply(`*${await conn.getName(m.sender)}* [${pointPemain * 10}] - [${pointLawan * 10}] *${await conn.getName(lawan)}*\n\n*Pet🦚Kamu* (griffin ${db.data.users[m.sender].griffin}) MENANG melawan 🦚griffinnya *${await conn.getName(lawan)}* (griffin ${db.data.users[lawan].griffin}) karena griffin🦚kamu ${alasanMenang[Acakin(0,alasanMenang.length-1)]}\n\nHadiah Rp. ${hadiah.toLocaleString()}\n+1 Tiketcoin`)
    } else if (pointPemain < pointLawan) {
        let denda = (pointLawan - pointPemain) * 100000
        db.data.users[m.sender].money -= denda
        db.data.users[m.sender].tiketcoin += 1
        m.reply(`*${await conn.getName(m.sender)}* [${pointPemain * 10}] - [${pointLawan * 10}] *${await conn.getName(lawan)}*\n\n*Pet🦚Kamu* (griffin ${db.data.users[m.sender].griffin}) KALAH melawan 🦚griffinnya *${await conn.getName(lawan)}* (griffin ${db.data.users[lawan].griffin}) karena pet kamu ${alasanKalah[Acakin(0,alasanKalah.length-1)]}\n\nUang kamu berkurang Rp. ${denda.toLocaleString()}\n+1 Tiketcoin`)
    } else {
