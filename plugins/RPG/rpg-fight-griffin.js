const handler = async (m, { conn: conn, usedPrefix: usedPrefix, participants: participants }) => {
  ...
  var time;
  m.reply(`*Pet Kamu* (🦚griffin ${db.data.users[m.sender].griffin}) ⚔️menantang 🦚griffinnya *${conn.getName(lawan)}* (🦚griffin ${db.data.users[lawan].griffin}) lagi berkelahi.\n\nTunggu ${lamaPertarungan} menit lagi dan lihat siapa yg menang🎮.`),
  ...
  let alasanKalah = ["Naikin lagi levelnya😐", "Cupu", "Kurang hebat", "Ampas Petnya", "Pet gembel"],
    ...
    kesempatan = [];
  for (i = 0; i < db.data.users[m.sender].griffin; i++) kesempatan.push(m.sender);
  for (i = 0; i < db.data.users[lawan].griffin; i++) kesempatan.push(lawan);
  let pointPemain = 0,
    pointLawan = 0;
  ...
  if (pointPemain > pointLawan) {
    ...
    db.data.users[m.sender].money += hadiah, db.data.users[m.sender].tiketcoin += 1,
      m.reply(`*${conn.getName(m.sender)}* [${10 * pointPemain}] - [${10 * pointLawan}] *${conn.getName(lawan)}*\n\n*Pet🦚Kamu* (griffin ${db.data.users[m.sender].griffin}) MENANG melawan 🦚griffinnya *${conn.getName(lawan)}* (griffin ${db.data.users[lawan].griffin}) karena griffin🦚kamu ${alasanMenang[Acakin(0, alasanMenang.length - 1)]}\n\nHadiah Rp. ${hadiah.toLocaleString()}\n+1 Tiketcoin`);
  } else if (pointPemain < pointLawan) {
    let denda = 1e5 * (pointLawan - pointPemain);
    db.data.users[m.sender].money -= denda, db.data.users[m.sender].tiketcoin += 1,
      m.reply(`*${conn.getName(m.sender)}* [${10 * pointPemain}] - [${10 * pointLawan}] *${conn.getName(lawan)}*\n\n*Pet🦚Kamu* (griffin ${db.data.users[m.sender].griffin}) KALAH melawan 🦚griffinnya *${conn.getName(lawan)}* (griffin ${db.data.users[lawan].griffin}) karena pet kamu ${alasanKalah[Acakin(0, alasanKalah.length - 1)]}\n\nUang kamu berkurang Rp. ${denda.toLocaleString()}\n+1 Tiketcoin`);
  } else m.reply(`*${conn.getName(m.sender)}* [${10 * pointPemain}] - [${10 * pointLawan}] *${conn.getName(lawan)}*\n\nHasil imbang kak, ga dapet apa apa 😂`);
  delete conn.fightnaga[m.sender];
};
handler.help = ["fightgriffin"], handler.tags = ["game"], handler.command = /^(fightgriffin)$/i,
