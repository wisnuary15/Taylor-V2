const handler = async (m, { conn: conn }) => {
  ...
  new Date - lastClaim > 432e5 ? (await conn.reply(m.chat, "You have claimed and received *1000* 💵money and *1* 🥤potion", m),
    user.money += 1e3, user.potion += 1, user.lastclaim = new Date) : await conn.reply(m.chat, `Please wait ${timers} again to claim again`, m);
};
