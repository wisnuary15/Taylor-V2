const handler = async (m, {
    ...
    m.reply(`*Pet Kamu* (🐉naga ${db.data.users[m.sender].naga}) ⚔️menantang 🐉naganya *${await conn.getName(lawan)}* (🐉naga ${db.data.users[lawan].naga}) lagi berkelahi.\n\nTunggu ${lamaPertarungan} menit lagi dan lihat siapa yg menang🎮.`)
    ...
    if (pointPemain > pointLawan) {
        ...
        m.reply(`*${await conn.getName(m.sender)}* [${pointPemain * 10}] - [${pointLawan * 10}] *${await conn.getName(lawan)}*\n\n*Pet🐉Kamu* (naga ${db.data.users[m.sender].naga}) MENANG melawan 🐉naganya *${await conn.getName(lawan)}* (naga ${db.data.users[lawan].naga}) karena naga🐉kamu ${alasanMenang[Acakin(0,alasanMenang.length-1)]}\n\nHadiah Rp. ${hadiah.toLocaleString()}\n+1 Tiketcoin`)
    } else if (pointPemain < pointLawan) {
        let denda = (pointLawan - pointPemain) * 100000
        db.data.users[m.sender].money -= denda
        db.data.users[m.sender].tiketcoin += 1
        m.reply(`*${await conn.getName(m.sender)}* [${pointPemain * 10}] - [${pointLawan * 10}] *${await conn.getName(lawan)}*\n\n*Pet🐉Kamu* (naga ${db.data.users[m.sender].naga}) KALAH melawan 🐉naganya *${await conn.getName(lawan)}* (naga ${db.data.users[lawan].naga}) karena pet kamu ${alasanKalah[Acakin(0,alasanKalah.length-1)]}\n\nUang kamu berkurang Rp. ${denda.toLocaleString()}\n+1 Tiketcoin`)
    } else {
