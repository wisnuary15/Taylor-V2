...
async function handler(m, { conn: conn, text: text }) {
  ...
  if ("start" === text) {
    ...
    let { pertanyaan: pertanyaan, jawaban: jawaban, reward: reward } = quest, currentQuestion = 0, jawabanBenar = 0;
    conn.quest[m.sender] = {
...
handler.before = async m => {
  ...
  let { pertanyaan: pertanyaan, jawaban: jawaban, reward: reward, currentQuestion: currentQuestion, jawabanBenar: jawabanBenar } = conn.quest[m.sender];
  let user = db.data.users[m.sender],
  ...
  if (txt === jawaban[currentQuestion].toLowerCase()) {
    if (user.exp += reward.exp, user.crystal += reward.crystal, currentQuestion++, jawabanBenar++,
      conn.quest[m.sender].currentQuestion = currentQuestion, conn.quest[m.sender].jawabanBenar = jawabanBenar,
      currentQuestion >= pertanyaan.length) return m.reply(`🎉 Selamat! Anda telah menyelesaikan semua questgame.\nTotal jawaban benar: ${jawabanBenar}\nExp yang didapatkan: ${reward.exp}\nCrystal yang didapatkan: ${reward.crystal}`),
      delete conn.quest[m.sender], !1;
    {
      let caption = `📜 *SEBUAH QUEST TELAH DIBERIKAN PADA ANDA!*\n\n*🎯 PERTANYAAN:* ${pertanyaan[currentQuestion]}\n\nJawab pertanyaan ini untuk melanjutkan ke pertanyaan berikutnya.`;
