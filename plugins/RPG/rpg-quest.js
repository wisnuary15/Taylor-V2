...
async function handler(m, {
  ...
  db.data.rpg.quest = db.data.rpg.quest ? db.data.rpg.quest : {};
  let user = db.data.users[m.sender];
  if (m.sender in db.data.rpg.quest) return db.data.rpg.quest[m.sender].currentQuestion >= db.data.rpg.quest[m.sender].pertanyaan.length ? m.reply("🏆 Anda telah menyelesaikan semua questgame.") : m.reply("⏳ Anda masih memiliki questgame yang belum selesai. Silakan selesaikan terlebih dahulu.");
  if ("start" === text) {
...
handler.before = async m => {
  if (db.data.rpg.quest = db.data.rpg.quest ? db.data.rpg.quest : {}, !(m.sender in db.data.rpg.quest)) return;
  ...
  if ("stop" === txt) return m.reply("❌ Game quest telah dihentikan. Ketik *'questgame start'* untuk memulai game kembali."),
    delete db.data.rpg.quest[m.sender], !1;
  if ("hint" === txt) {
  ...
  if (txt === jawaban[currentQuestion].toLowerCase()) {
    if (user.exp += reward.exp, user.crystal += reward.crystal, currentQuestion++, jawabanBenar++, db.data.rpg.quest[m.sender].currentQuestion = currentQuestion, db.data.rpg.quest[m.sender].jawabanBenar = jawabanBenar, currentQuestion >= pertanyaan.length) return m.reply(`🎉 Selamat! Anda telah menyelesaikan semua questgame.\nTotal jawaban benar: ${jawabanBenar}\nExp yang didapatkan: ${reward.exp}\nCrystal yang didapatkan: ${reward.crystal}`),
      delete db.data.rpg.quest[m.sender], !1;
    {
      let caption = `📜 *SEBUAH QUEST TELAH DIBERIKAN PADA ANDA!*\n\n*🎯 PERTANYAAN:* ${pertanyaan[currentQuestion]}\n\nJawab pertanyaan ini untuk melanjutkan ke pertanyaan berikutnya.`;
