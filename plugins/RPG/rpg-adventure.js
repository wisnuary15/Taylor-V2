...
let handler = async (m, {
    ...
    if (user.health < 80) return conn.sendButton(m.chat,
        `${htki} LOW HEALTH ${htka}`, `ʏᴏᴜʀ ʜᴇᴀʟᴛʜ ɪs ʙᴇʟᴏᴡ 80﹗
ᴩʟᴇᴀsᴇ ʜᴇᴀʟ ❤ ғɪʀsᴛ ᴛᴏ ᴀᴅᴠᴇɴᴛᴜʀᴇ ᴀɢᴀɪɴ.`.trim(), imgr + 'lowhealth', [["HEAL", usedPrefix + "heal"]], m)
    if (new Date - user.lastadventure <= cooldown) return conn.sendButton(m.chat,
        `${htki} COOLDOWN ${htka}`, `ʏᴏᴜ'ᴠᴇ ᴀʟʀᴇᴀᴅʏ *ᴀᴅᴠᴇɴᴛᴜʀᴇ*, ᴩʟᴇᴀsᴇ ᴡᴀɪᴛ ᴛɪʟʟ ᴄᴏᴏʟᴅᴏᴡɴ ғɪɴɪsʜ.

⏱️ ${timers.toTimeString()}`.trim(), imgr + 'cooldown', [["RPG", usedPrefix + "menulist rpg"]], m)
...
${cmenua}`
    ...
    conn.sendButton(m.chat,
        `${htki} ADVENTURE ${htka}\n`,
        teks.trim(), info.urlGambarOg, [["Profile", usedPrefix + "profile"]], m)
