const handler = async (m, { conn: conn }) => {
  ...
  let gameState = `🤠 Koboy Mengejar Penjahat 🥷\n\nWilayah saya:\n${"・".repeat(playerPosition)}🤠${"・".repeat(5 - playerPosition)}\nWilayah penjahat:\n${"・".repeat(criminalPosition)}🥷${"・".repeat(5 - criminalPosition)}\nKetik *'kanan'* untuk bergerak ke kanan.\nKetik *'kiri'* untuk bergerak ke kiri.`,
    { key: key } = await conn.reply(m.chat, gameState, m);
  conn.koboy[m.chat] = {
...
handler.before = async (m, { conn: conn }) => {
    ...
    let gameData = conn.koboy[m.chat],
      { playerPosition: playerPosition, criminalPosition: criminalPosition, key: key, oldkey: oldkey, moveCount: moveCount, maxMoves: maxMoves, timeout: timeout, earnedExp: earnedExp, earnedMoney: earnedMoney, sender: sender } = gameData;
