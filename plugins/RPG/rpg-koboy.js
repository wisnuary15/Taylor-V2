...
handler.before = async (m, {
  ...
  if (!conn.koboy[m.chat] || conn.koboy[m.chat].roomId !== m.chat || !['kiri', 'kanan'].includes(m.text
    .toLowerCase())) return;
  ...
  if (m.quoted || m.quoted?.id === key) {
    ...
    if (playerPosition === criminalPosition) {
      ...
      delete conn.koboy[m.chat];
      return conn.reply(m.chat, `😔 Kamu kalah! @${sender.split('@')[0]} sudah mencapai batas maksimum gerakan.`,
      m, {
        mentions: [sender]
      });
    }
