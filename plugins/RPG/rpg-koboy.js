const handler = async (m, {
  ...
  if (db.data.rpg.koboy = db.data.rpg.koboy || {}, db.data.rpg.koboy[m.chat]) return m.reply("Kamu sedang bermain game Koboy!");
  ...
  db.data.rpg.koboy[m.chat] = {
    ...
    roomId: m.chat,
    timeout: setTimeout(() => {
      db.data.rpg.koboy && db.data.rpg.koboy[m.chat] && db.data.rpg.koboy[m.chat].roomId === m.chat && (conn.sendMessage(m.chat, {
        delete: key
...
handler.before = async (m, {
    ...
    if (!db.data.rpg.koboy[m.chat] || db.data.rpg.koboy[m.chat].roomId !== m.chat || !["kiri", "kanan"].includes(m.text.toLowerCase())) return;
    ...
    if (m.quoted || m.quoted?.id === key) {
      ...
      if (playerPosition === criminalPosition) {
        ...
        return user.money = (user.money || 0) + earnedMoneys, user.exp = (user.exp || 0) + earnedExps,
          delete db.data.rpg.koboy[m.chat], await conn.reply(m.chat, `🎉 Selamat! @${sender.split("@")[0]} berhasil mengejar penjahat! 🎉\n\n💰 Mendapatkan uang senilai *${formatRupiah(earnedMoneys)}*\n🔼 Dapatkan *${earnedExps}* EXP\n`, m, {
            mentions: [sender]
          });
      }
      if (moveCount === maxMoves) return conn.sendMessage(m.chat, {
        delete: oldkey
      }), delete db.data.rpg.koboy[m.chat], await conn.reply(m.chat, `😔 Kamu kalah! @${sender.split("@")[0]} sudah mencapai batas maksimum gerakan.`, m, {
        mentions: [sender]
      });
      let gameState = `🤠 Koboy Mengejar Penjahat 🥷\n\nWilayah saya:\n${"・".repeat(playerPosition)}🤠${"・".repeat(5 - playerPosition)}\nWilayah penjahat:\n${"・".repeat(criminalPosition)}🥷${"・".repeat(5 - criminalPosition)}\nKetik *'kanan'* untuk bergerak ke kanan.\nKetik *'kiri'* untuk bergerak ke kiri.`,
      ...
      db.data.rpg.koboy[m.chat] = Object.assign({}, db.data.rpg.koboy[m.chat], additionalData);
    }
