const handler = async (m, { conn: conn, command: command, args: args, usedPrefix: usedPrefix }) => {
  ...
  try {
    ...
    if (/use|pakai/i.test(command)) {
      ...
      "potion" === args[0] ? healt < 100 ? db.data.users[m.sender].potion >= count ? (db.data.users[m.sender].potion -= count,
        db.data.users[m.sender].healt += usepotion * count, await conn.reply(m.chat, msgsucces, m)) : await conn.reply(m.chat, msgkurang, m) : await conn.reply(m.chat, msgpenuh, m) : args.length > 2 && "potion" !== args[0] && await conn.reply(m.chat, "Hanya bisa menggunakan potion\nContoh penggunaan: *" + usedPrefix + "use potion 1*", m);
      ...
      let count = /[0-9]/g.test(args[0]) ? Math.max(args[0] || Math.ceil((100 - healt) / usepotion), 1) : Math.ceil((100 - healt) / usepotion),
        msgsucces = pickRandom(["success memakai", "Nice succes menggunakan", "berhasil meminum ", "primitif anda menggunakan", "anda memakai", "Anda menggunakan"]) + ` *${count}* ${rpg.emoticon("potion")}Potion`;
      healt < 100 ? db.data.users[m.sender].potion >= count ? (db.data.users[m.sender].potion -= count,
        db.data.users[m.sender].healt += usepotion * count, await conn.reply(m.chat, msgsucces, m)) : await conn.reply(m.chat, msgkurang, m) : await conn.reply(m.chat, msgpenuh, m);
    }
  } catch (e) {
