...
class SnakeAndLadderGame {
  ...
  getBoardBuffer = async () => {
    ...
    for (const player of this.players) {
      const { x: x, y: y } = this.calculatePlayerPosition(player);
  ...
  startGame = async (m, player1Name, player2Name) => {
    ...
    const { key: key } = m.reply(await this.getBoardBuffer());
    this.keyId = key;
  };
  ...
  playTurn = async (m, player) => {
    ...
    const { key: key } = m.reply(boardBuffer);
    this.keyId = key;
  };
  addPlayer = player => this.players.length < 2 && !this.players.includes(player) && "" !== player && (this.players.push(player),
    !0);
  resetSession = () => {
...
const handler = async (m, { args: args, usedPrefix: usedPrefix, command: command }) => {
  ...
  switch (args[0]) {
    ...
    case "start":
      ...
      game.players.length >= 2 ? (await game.startGame(m, game.players[0], game.players[1]),
        sessions[sessionId].state = !0, m.reply("🛑 *Permainan sudah dimulai.*")) : m.reply("👥 *Tidak cukup pemain untuk memulai permainan.*\nDiperlukan minimal 2 pemain.");
      break;
    case "roll":
