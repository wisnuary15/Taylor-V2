...
class SnakeAndLadderGame {
  ...
  playTurn = async (m, player) => {
    if (!this.players.length) return m.reply(
      '🛑 *Tidak ada permainan aktif.*\nGunakan "!snake start" untuk memulai permainan baru.');
    if (player !== this.players[this.currentPlayerIndex]) return m.reply(
    ...
    for (const otherPlayer of this.players) {
      if (otherPlayer !== player && this.currentPositions[otherPlayer] === newPosition) {
        const message =
          `😱 *Oh tidak!* ${this.formatPlayerName(player)} *menginjak posisi* ${this.formatPlayerName(otherPlayer)}\n*Kembali ke awal cell.*`;
    ...
    if (newPosition <= this.boardSize) {
      ...
      if (this.currentPositions[player] === this.boardSize) {
        m.reply(`🎉 ${this.formatPlayerName(player)} menang!\n*Selamat!*`, null, {
          mentions: [player]
        });
      ...
      m.reply('🔄 Melebihi batas kotak, posisi direset dan giliran beralih ke pemain berikutnya.', null, {
        mentions: this.players
      });
  ...
  addPlayer = (player) => this.players.length < 2 && !this.players.includes(player) && player !== '' && (this.players
    .push(player), true);
  resetSession = () => {
    this.players = [];
    this.currentPositions = {};
...
const handler = async (m, {
  ...
  switch (args[0]) {
    case 'join':
      ...
      if (joinSuccess) {
        m.reply(`👋 ${game.formatPlayerName(playerName)} *bergabung ke dalam permainan.*`, null, {
          mentions: [playerName]
        });
      } else {
