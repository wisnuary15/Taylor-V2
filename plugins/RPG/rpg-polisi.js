...
const handler = async (m, {
  ...
  if (command === "polisi") {
    ...
    if (subCommand === "cari") {
      ...
      })).sort((a, b) => b.Pencuri_Tertangkap - a.Pencuri_Tertangkap).slice(0, 5); // Show top 5 players
      let leaderboardMsg = "*🏆 Leaderboard 🏆*\n\n";
      for (let i = 0; i < leaderboard.length; i++) {
        leaderboardMsg +=
          `${i + 1}. @${leaderboard[i].id.split("@")[0]} - ${leaderboard[i].Pencuri_Tertangkap} Pencuri Tertangkap\n`;
      }
      ...
      if (player.State !== STATES.SEARCHING) {
        return conn.reply(m.chat, "*🔍 Anda harus mencari pencuri terlebih dahulu dengan perintah 'polisi cari'.*",
          m);
      }
      ...
      if (!thiefActions[level].includes(polisiAction)) {
        return conn.reply(m.chat,
          `*🚓 Pilihan tindakan Anda (${polisiAction.toUpperCase()}) tidak sesuai dengan hasil yang dicari.*`, m);
      }
