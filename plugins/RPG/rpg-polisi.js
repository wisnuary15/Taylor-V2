  ...
  if ("polisi" === command) {
    ...
    if ("cari" === subCommand) {
      ...
      if (thiefActions[level].includes(player.ThiefAction)) {
        ...
        player.Pencuri_Tertangkap++, player.Uang += reward, user.money += reward,
          player.Uang < 5e3 && (player.Uang = 5e3), await conn.reply(m.chat, `*🚓 Anda berhasil melawan dan menangkap pencuri level ${level}!* Anda mendapatkan imbalan Rp${reward.toLocaleString()}. Total Uang Anda: Rp${player.Uang.toLocaleString()}.`, m);
