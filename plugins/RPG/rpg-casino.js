...
const handler = async (m, {
  ...
  if (db.data.rpg.casinosingle = db.data.rpg.casinosingle ? db.data.rpg.casinosingle : {}, m.chat in db.data.rpg.casinosingle) return m.reply("Masih ada yang melakukan casino disini, tunggu sampai selesai!!");
  if (db.data.rpg.casinosingle[m.chat] = !0, args.length < 1) return await conn.reply(m.chat, usedPrefix + command + " <jumlah>\n" + usedPrefix + command + " 1000", m);
  try {
    ...
    delete db.data.rpg.casinosingle[m.chat];
  }
};
