...
const handler = async (m, { conn: conn, args: args, usedPrefix: usedPrefix }) => {
  ...
  try {
    let randomaku = `${Math.floor(101 * Math.random())}`.trim(),
      ...
      Kamu = 1 * randomkamu,
      count = args[0];
    if (count = count ? /all/i.test(count) ? Math.floor(db.data.users[m.sender].exp / buatall) : parseInt(count) : args[0] ? parseInt(args[0]) : 1,
      count = Math.max(1, count), db.data.users[m.sender].exp >= 1 * count)
      if (db.data.users[m.sender].exp -= 1 * count,
        Aku > Kamu) {
