...
const checkUser = (id, adminList) => {
  ...
  handler = async (m, { conn: conn, args: args, usedPrefix: usedPrefix, command: command, groupMetadata: groupMetadata, participants: participants }) => {
    try {
      const adminList = groupMetadata.participants || participants,
        who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? conn.user.jid : m.sender,
        { exp: exp, limit: limit, level: level, role: role, money: money, lastclaim: lastclaim, lastweekly: lastweekly, registered: registered, regTime: regTime, age: age, banned: banned, pasangan: pasangan } = db.data.users[who] || {},
        { min: min, xp: xp, max: max } = xpRange(level, multiplier),
        name = m.name.split("\n")[0],
      ...
      const math = max - xp,
        ...
        profileBuffer = (potongString(contohStringPanjang),
          BannerBot(m.name.split("\n")[0]));
      try {
