    ...
    if (/fishing|mancing/i.test(command)) switch (type) {
      case "emosi":
        ...
        if (new Date - pengguna.lastfishing > 24e4) {
          ...
          setTimeout((async () => {
              await conn.reply(m.chat, mcng, null), psepick > 0 && (pengguna.psepick += 1 * psepick,
                await conn.reply(m.chat, `You Get 🎁chest weapons epic ${psepick} item`, m)), psenjata > 0 && (pengguna.psenjata += 1 * psenjata,
                await conn.reply(m.chat, `You Get 🎁chest weapons ${psenjata} item`, m));
