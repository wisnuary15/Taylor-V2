const handler = async (m, {
  ...
  if (args.length < 3) {
    ...
    let who = m.mentionedJid ? m.mentionedJid[0] : (args[2].replace(/[@ .+-]/g, '').replace(' ', '') +
      '@s.whatsapp.net')
    if (!m.mentionedJid || !args[2]) throw 'Tag salah satu, atau ketik Nomernya!!'
    let users = db.data.users
    switch (type) {
      case 'money':
        if (db.data.users[m.sender].money >= count * 1) {
          try {
            db.data.users[m.sender].money -= count * 1
            ...
            console.log(e)
            if (DevMode) {
              for (let jid of owner.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').filter(v => v != conn
                  .user.jid)) {
        ...
        break
      case 'tabungan':
        if (db.data.users[m.sender].atm >= count * 1) {
          try {
            db.data.users[m.sender].atm -= count * 1
        ...
        } else conn.reply(m.chat, `Limit kamu tidak mencukupi untuk mentransfer Uang dari Bank sebesar ${count}`
          .trim(), m)
        break
      case 'limit':
