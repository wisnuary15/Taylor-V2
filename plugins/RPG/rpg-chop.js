const handler = async (m, { conn: conn, usedPrefix: usedPrefix, DevMode: DevMode }) => {
  try {
    ...
    let { armor: armor, rubah: rubah, kuda: kuda, kucing: kucing } = user, health = Math.floor(101 * Math.random()), __health = health > 60 ? health - (kucing > 0 ? [0, 5, 10, 15, 21, 30][kucing] : 0) - (armor > 0 ? [0, 5, 10, 15, 21, 30][armor] : 0) : health, kayu = (0 === kucing && 0 === armor && Math.floor(11 * Math.random()),
      Math.floor(2 * Math.random()), Math.floor(3 * Math.random()), Math.floor(2 * Math.random()),
      pickRandom([1, 0, 0, 1]), pickRandom([1, 0, 0, 0]), Math.floor(300 * Math.random()),
      Math.floor(150 * Math.random())), batu = Math.floor(100 * Math.random()), iron = Math.floor(100 * Math.random()), exp = Math.floor(500 * Math.random()), uang = Math.floor(500 * Math.random());
