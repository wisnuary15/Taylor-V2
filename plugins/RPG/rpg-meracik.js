...
const handler = async (m, { conn: conn, args: args, usedPrefix: usedPrefix, command: command, text: text }) => {
          ...
          if (db.data.users[m.sender].string > 4)
            if (db.data.users[m.sender].batu > 9) {
              ...
              if (setTimeout((async () => {
                ...
                db.data.users[m.sender].anakpancingan > naiklvl && (db.data.users[m.sender].pancingan += 1,
                  db.data.users[m.sender].anakpancingan -= 1e4 * pancingan, await conn.reply(m.chat, "*Selamat pancingan kamu naik level*", m));
              }
