...
const handler = async (m, {
  ...
  switch (type) {
    case 'ramuan':
      ...
      if (apelu === 0 || angguru === 0 || manggau === 0 || pisangu === 0 || jeruku === 0) return m.reply(
...
`.trim()
    ...
    case 'potion':
      ...
      if (apele === 0 || anggure === 0 || manggae === 0 || pisange === 0 || jeruke === 0) return m.reply(
        '*Pastikan kamu memiliki semua buah buahan*\n*Seperti Apel, Mangga, Jeruk, Pisang, Anggur*')
      if (new Date - db.data.users[m.sender].lastpotionclaim > 600000) {
        if (db.data.users[m.sender].mangga > 499) {
          if (db.data.users[m.sender].apel > 499) {
...
`.trim()
    ...
    case 'string':
      ...
      if (apelg === 0 || anggurg === 0 || manggag === 0 || pisangg === 0 || jerukg === 0) return m.reply(
        '*Pastikan kamu memiliki semua buah buahan*\n*Seperti Apel, Mangga, Jeruk, Pisang, Anggur*')
...
`.trim()
    ...
    case 'sword':
      ...
      if (iron === 0 || kayu === 0 || string === 0) return m.reply(
...
`.trim()
    ...
    case 'weapon':
      ...
      if (ironw === 0 || kayuw === 0 || stringw === 0 || swordw === 0) return m.reply(
...
`.trim()
    ...
    case 'pancingan':
      ...
      if (kayuh === 0 || stringh === 0 || batuh === 0) return m.reply(
...
`.trim()
    ...
    default:
      return conn.reply(m.chat,
        `${usedPrefix + command} [ramuan | potion | string | iron | sword | weapon | pancingan]\nContoh penggunaan: *${usedPrefix + command} ramuan*`,
        m)
  }
}
