...
const handler = async (m, {
  ...
  try {
    ...
    const stek = new Sticker(img, {
      pack: packname,
      author: author,
      type: StickerTypes.FULL
    });
    const out = (/webp/g.test(mime)) ? await webp2mp4(img) : (/image/g.test(mime)) ? await uploadImage(img) : (
      /video/g.test(mime)) ? await uploadFile(img) : (/audio/g.test(mime)) ? await ffmpeg(media, [
      '-filter_complex', 'color', '-pix_fmt', 'yuv420p', '-crf', '51', '-c:a', 'copy', '-shortest'
    ], 'mp3', 'mp4') : (typeof out !== 'string') ? await uploadImage(img) : (/gif/g.test(mime)) ? stek : null;
    if (!out) throw new Error('Failed to convert sticker/audio to video.');
    out && conn.sendFile(m.chat, out, 'tovid.mp4', '✅ sticker a video', m);
  } catch (error) {
