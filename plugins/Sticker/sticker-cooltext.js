...
const handler = async (m, {
  ...
  if (!query || !style) {
    return m.reply(`Usage: ${usedPrefix}cooltext [text] | [style/id] | [index/URL]`);
  }
  if (!input) {
  ...
  if (Number(input)) {
    ...
    if (index < 1 || index > searchResults.length) {
      return m.reply(`Invalid index. Please use a value between 1 and ${searchResults.length}`);
    }
...
async function getImageUrl(textStyleId, text) {
  try {
    ...
    throw new Error(`Failed to get image URL: ${e.message}`);
  }
}
async function extractLogoIDFromLink(link) {
  try {
    ...
    throw new Error(`Failed to extract LogoID from the link: ${error.message}`);
  }
}
async function searchCoolText(query) {
  try {
    ...
    throw new Error(`Failed to perform the search: ${error.message}`);
  }
}