...
const handler = async (m, {
  ...
  let urls = /webp/g.test(mime) ? await QuotlyImg(await webp2png(img), name, pp, text, theme, reply) : /image/g
    .test(mime) ? await QuotlyImg(await uploadImage(img), name, pp, text, theme, reply) : await QuotlyImg(
      await uploadFile(img), name, pp, text, theme, reply);
