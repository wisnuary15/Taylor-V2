...
const handler = async (m, {
  ...
  try {
    ...
    if (/stickerMessage/g.test(mime)) {
      ...
      try {
        stiker = await addExif(await sticker(img, false, packnames || packname, authors || m.name), packnames ||
          packname, authors || m.name)
      } catch (e) {
        try {
      ...
      try {
        ...
        try {
          stiker = await video2webp(img) || await video2webp30(img) || await video2webp45(img) ||
      ...
      try {
        stiker = await addExif(await sticker(false, args[0], packnames || packname, authors || m.name), packnames ||
          packname, authors || m.name)
      } catch (e) {
        try {
      ...
      if (firstEmoji) {
        ...
        try {
          stiker = await addExif(await sticker(false, emj, packnames || packname, authors || m.name), packnames ||
            packname, authors || m.name)
        } catch (e) {
          try {
        ...
        if (foundLinks && foundLinks.length > 0) {
          for (let i = 0; i < foundLinks.length; i++) {
            ...
            if (emj) {
              try {
                stiker = await addExif(await sticker(false, emj, packnames || packname, authors || m.name),
                  packnames || packname, authors || m.name)
              } catch (e) {
                try {
                  stiker = await _createSticker(false, emj, packnames || packname, authors || m.name, 30)
                } catch (e) {
...
function getUrlByName(data, query) {
  ...
  return matchedObject ? matchedObject.image : data.length > 0 ? data[Math.floor(Math.random() * data.length)].image :
    null;
}
  ...
  try {
    ...
    const Format = {
      ...
      config: {
        ...
        chromiumArgs: ['--no-sandbox', '--disable-setuid-sandbox', '--aggressive-cache-discard', '--disable-cache',
          '--disable-application-cache', '--disable-offline-load-stale-cache', '--disk-cache-size=0'
        ],
