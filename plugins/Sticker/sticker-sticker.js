...
const handler = async (m, {
  ...
  try {
    ...
    const processSticker = async (_img, packnames, authors) => {
      try {
        return await addExif(await sticker(_img, false, packnames || packname, authors || m.name), packnames || packname, authors || m.name);
      } catch (e) {
        try {
          return await _createSticker(_img, false, packnames || packname, authors || m.name, 30);
        } catch (e) {
          console.log(e);
          stiker = null;
        }
    ...
    switch (true) {
      case /stickerMessage|imageMessage|documentMessage|interactiveMessage|viewOnceMessageV2/g.test(mime):
        img = await q?.download();
        stiker = await processSticker(img, packnames, authors);
        break;
      case /videoMessage/g.test(mime):
        if ((q.msg || q).seconds > 11) return m.reply("⏱️ Maksimal durasi video 10 detik!");
        img = await q?.download();
        try {
      ...
      case /extendedTextMessage/g.test(mime):
        ...
        const text = q.text;
        const isValidUrl = /https?:\/\/[^\s]+?\.(jpg|jpeg|png|gif|bmp|webp|mp4|mkv|avi|mov|wmv|flv)(\?[^\s]*)?/gi;
        ...
        if (firstEmoji) {
          ...
          if (foundLinks = text.match(/^(https?):\/\/[^\s/$.?#].[^\s]*$/i), !(foundLinks && foundLinks.length > 0)) {
            const thumbnailUrl = (await m.getQuotedObj())?.message[q.mtype]?.contextInfo?.externalAdReply?.thumbnailUrl || (await m.getQuotedObj())?.message[q.mtype]?.contextInfo?.externalAdReply?.thumbnail;
            if (thumbnailUrl) {
              try {
                stiker = await addExif(await sticker(false, thumbnailUrl, packnames || packname, authors || m.name), packnames || packname, authors || m.name);
              } catch (e) {
                try {
                  stiker = await _createSticker(false, thumbnailUrl, packnames || packname, authors || m.name, 30);
                } catch (e) {
                  console.log(e);
                  stiker = null;
                }
              ...
              try {
                stiker = await addExif(await sticker(thumbnail, false, packnames || packname, authors || m.name), packnames || packname, authors || m.name);
              } catch (e) {
                try {
                  stiker = await _createSticker(thumbnail, false, packnames || packname, authors || m.name, 30);
                } catch (e) {
                  console.log(e);
                  stiker = null;
                }
        ...
        break;
      default:
        return m.reply(`🔍 Tidak ada emoji atau link ditemukan!\nAtau\nBalas gambar/video/stiker/teks menggunakan perintah ${usedPrefix + command}`);
    }
  } catch (e) {
    console.log(e);
    stiker = null;
