...
const handler = async (m, {
  ...
  const input = args.length >= 1 ? args.slice(0).join(" ") : (m.quoted && m.quoted?.text || m.quoted?.caption || m
  ...
  if (isNaN(page)) {
    return conn.reply(m.chat, '❌ Nomor halaman tidak valid. Silakan masukkan nomor halaman antara 1 dan 67.', m);
  }
  ...
  if (!selectedLogo) {
    return conn.reply(m.chat, '❌ Hasil tidak ditemukan. Pastikan nomor halaman dan indeks yang dimasukkan benar.',
      m);
  }
  if (selectedLogo) {
...
async function getLogosByPage(query, page) {
  try {
    if (page < 1 || page > 70) {
      throw new Error('Nomor halaman tidak valid. Silakan masukkan nomor halaman antara 1 dan 70.');
    }
    const baseUrl = page === 1 ? `https://api.flamingtext.com/All-Logos/?text=${query}` :
