  ...
  try {
    ...
    if (count.toLowerCase() === 'all') {
      for (let i = 0; i < stickers.length; i++) {
        conn.sendFile(m.sender, stickers[i].url, '', `*Stiker Telegram (${i + 1}/${stickers.length})*`, m);
      ...
      if (isNaN(stickerNumber) || stickerNumber < 0 || stickerNumber >= stickers.length) {
        return m.reply(
          '*❗ Nomor stiker tidak valid.*\nHarap berikan nomor stiker yang valid atau gunakan "random" untuk mengirim stiker secara acak atau "all" untuk mendapatkan semua stiker.'
          );
      }
      return conn.sendFile(m.chat, stickers[stickerNumber].url, '',
        `*Stiker Telegram (${stickerNumber + 1}/${stickers.length})*`, m);
    }
  } catch (error) {
    return m.reply('*❗ Terjadi kesalahan saat mengambil stiker.*\nSilakan coba lagi nanti.');
  }
};
handler.help = ['stickertele [query]', 'stickertelegram [query]|[angka]', 'telesticker [query]|all',
