...
const fetchStickerSet = async query => {
  const response = await fetch(`https://cors.caliph.my.id/https://api.telegram.org/bot891038791:AAHWB1dQd-vi0IbH2NjKYUk-hqQ8rQuzPD4/getStickerSet?name=${encodeURIComponent(query)}`);
  if (!response.ok) return [];
  const data = await response.json();
  return data.result?.stickers || [];
};
const fetchStickerFile = async file_id => {
  const response = await fetch(`https://cors.caliph.my.id/https://api.telegram.org/bot891038791:AAHWB1dQd-vi0IbH2NjKYUk-hqQ8rQuzPD4/getFile?file_id=${file_id}`);
  if (!response.ok) return null;
  const data = await response.json();
  const filePath = data.result?.file_path;
  return filePath ? `https://cors.caliph.my.id/https://api.telegram.org/file/bot891038791:AAHWB1dQd-vi0IbH2NjKYUk-hqQ8rQuzPD4/${filePath}` : null;
};
