...
const {
  ...
  raterian,
  attp
} = await (await import('../../lib/maker/text2picture.js'));
const handler = async (m, {
  ...
  try {
    ...
    const commandMap = {
      ...
      ttp2: async () => {
        const apiUrl =
        `${apiEndpoint}/ttp2?apikey=${apiKey}&text=${encodeURIComponent(text.substring(0, 151))}`;
        const attpRes = await fetch(apiUrl);
        return attpRes.arrayBuffer();
      },
      ttp3: async () => {
        const apiUrl =
        `${apiEndpoint}/ttp3?apikey=${apiKey}&text=${encodeURIComponent(text.substring(0, 151))}`;
        const attpRes = await fetch(apiUrl);
        return attpRes.arrayBuffer();
      },
      ttp4: async () => {
        const apiUrl =
        `${apiEndpoint}/ttp4?apikey=${apiKey}&text=${encodeURIComponent(text.substring(0, 151))}`;
        const attpRes = await fetch(apiUrl);
        return attpRes.arrayBuffer();
      },
      ttp5: async () => {
        const apiUrl =
        `${apiEndpoint}/ttp5?apikey=${apiKey}&text=${encodeURIComponent(text.substring(0, 151))}`;
        const attpRes = await fetch(apiUrl);
        return attpRes.arrayBuffer();
      },
      ttp6: async () => {
        const apiUrl =
        `${apiEndpoint}/ttp6?apikey=${apiKey}&text=${encodeURIComponent(text.substring(0, 151))}`;
        const attpRes = await fetch(apiUrl);
        return attpRes.arrayBuffer();
      },
      ttp7: async () => {
        ...
        if (Array.isArray(response) && response.length > 0 && response[0]?.url) {
          return await fetch('https://wsrv.nl/?url=' + response[0]?.url + '&output=webp').then(res => res
            .arrayBuffer());
        } else {
    ...
    conn.sendFile(m.chat, data, 'atet.webp', '', m);
  } catch (error) {
    console.error(error);
    m.react(eror);
  }
