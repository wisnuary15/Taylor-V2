...
const handler = async (m, {
  ...
  try {
    ...
    conn.sendFile(m.chat, buff, "", data[0]?.title, m)
  } catch (e) {
    m.react(eror)
  }
}
...
async function rokuSearch(q) {
  try {
    ...
    return jsonData["manga-cards"].map((html) => {
      ...
      const timestampText = captionElement.contents().filter((i, el) => el.nodeType === 3).text().trim().replace(
        /\s+/g, ' ');
      const mangaLink = $('a').attr('href') || '';
      const allImageUrls = new Array(numImages).fill().map((_, i) =>
        `${mangaLink.replace("https://rokuhentai.com/", "https://rokuhentai.com/_images/pages/")}${i === 0 ? '0' : i}.jpg`
        );
      return {
        title,
        numImages,
