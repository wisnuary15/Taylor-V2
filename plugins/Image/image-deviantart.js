...
`
    ...
    if (feature === "info") {
      ...
      m.react(wait)
      try {
        if (!inputs.startsWith('https://www.deviantart.com/') && !inputs.startsWith(
            'https://backend.deviantart.com/')) return m.reply('Link salah');
...
🔖 *Tags:* ${item.tags}
`
        conn.sendFile(m.chat, item.url || logo, "", cap, m)
      } catch (e) {
        m.react(eror)
      }
    }
...
async function infoDeviantart(url) {
  const modifiedURL = url.includes('https://backend.deviantart.com/oembed?url=') ? url :
