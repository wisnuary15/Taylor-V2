...
async function wallpaper(query) {
  const response = await fetch(`https://www.shutterstock.com/search/${encodeURIComponent(query)}`, {
    headers: {
  ...
  const results = [...new Set([...$.html().matchAll(/https?:\/\/(image|www)\.shutterstock\.com\/([^"]+)/gim)].map(v => v[0]).filter(v => /.*\.(jpe?g|png)$/gi.test(v)))];
  return results;
}