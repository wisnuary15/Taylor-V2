    ...
    try {
      ...
      try {
        ...
        try {
          ...
          try {
            let response = await fetch("https://api.lolhuman.xyz/api/pinterest2?apikey=" + lolkey + "&query=" +
              text);
...
async function bochilPinterestImages(query) {
  const result = (await pinterest(query)).filter((v) => v !== undefined).map((v) => v.replace("236x", "originals"))
    .filter((url) => url.includes("/originals/"));
  return [...new Set(result)];
}
async function hxzPinterestImages(query) {
  const result = (await hxz.pinterest(query)).filter((v) => v !== undefined).map((v) => v.replace("736x",
    "originals")).filter((url) => url.includes("/originals/"));
  return [...new Set(result)];
}
async function searchPinterest(querry) {
  ...
  await axios.request(`https://id.pinterest.com/search/pins/?rs=typed&q=` + querry, {
    ...
    const output = hasil.filter((v) => v !== undefined).map((v) => v.replace("236x", "originals")).filter((url) =>
