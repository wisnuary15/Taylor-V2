  ...
  switch (command) {
    case "a2zapk":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Keterangan lebih lanjut dapat ditemukan di A2ZAPK.").addSelection("Klik di sini").makeSections("A2ZAPK", "Pilih aplikasi");
        res.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `🔗 *Url:* ${item.url}`, `${usedPrefix}a2zapkapp ${item.url}`);
        });
    ...
    case "a2zapkapp":
      try {
        ...
        await conn.sendFile(m.chat, url, appName || "Aplikasi", null, m, false, {
          quoted: m,
