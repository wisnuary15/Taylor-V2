  ...
  switch (command) {
    case "apkmaza":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di APKMaza.").addSelection("Klik di sini").makeSections("APKMaza", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `📝 *Description:* ${item.description}`, `${usedPrefix}apkmazaapp ${item.link}`);
        });
    ...
    case "apkmazaapp":
      try {
        ...
        if (!appInfo.length) return m.reply(`Link "${text}" tidak ditemukan :/`);
        const caption = `📦 *Detail Aplikasi* 📦\n\n` + appInfo.map(section => `📂 *${section.title}*\n` + `🔗 *Download Link:* ${section.downloadLink}\n` + `📏 *File Size:* ${section.fileSize}`).join("\n\n") + `\n\n🖼️ *Image:* ${appInfo[0]?.imageSrc || "N/A"}`;
        await conn.sendFile(m.chat, appInfo[0]?.imageSrc || "", "", caption, m);
        await conn.sendFile(m.chat, appInfo[0]?.downloadLink || "", appInfo[0]?.title || "Aplikasi", null, m, false, {
          quoted: m,
...
async function searchApkmaza(query) {
  try {
    const response = await fetch(`https://apkmaza.co/?s=${encodeURIComponent(query)}`),
