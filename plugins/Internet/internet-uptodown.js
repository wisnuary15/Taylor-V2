...
${wait}`
        conn.sendFile(m.chat, resl.screenshotSrc, "", cap, m)
        conn.sendFile(m.chat, resl.url, resl.title, null, m, true, {
          quoted: m,
...
async function getInfo(url) {
  ...
  const requirements = $('h2:contains("Persyaratan") + ul li').map((index, element) => ($(element).text().trim()))
    .get();
