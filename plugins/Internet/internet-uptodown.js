    ...
    switch (command) {
      case "uptodown":
        try {
          ...
          const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di Uptodown.").addSelection("Klik di sini").makeSections("Uptodown", "Pilih aplikasi");
          results.forEach((item, index) => {
            buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📖 *Title:* ${item.title}`, `📝 *Description:* ${item.description}`, `${usedPrefix}uptodownapp ${item.link}`);
          });
      ...
      case "uptodownapp":
        try {
          ...
          await conn.sendFile(m.chat, appInfo.screenshotSrc || "", "", caption, m);
          await conn.sendFile(m.chat, appInfo.url || "", appInfo.title || "Aplikasi", null, m, false, {
            quoted: m,
...
async function getApp(url) {
  ...
  try {
    ...
    return {
      ...
      isFree: button.find("p:last-of-type").text().trim().toLowerCase() === "gratis",
