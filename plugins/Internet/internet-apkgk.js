  ...
  switch (command) {
    case "apkgk":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di APKGK.").addSelection("Klik di sini").makeSections("APKGK", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `📅 *Date:* ${item.date}`, `${usedPrefix}apkgkapp ${item.href}`);
        });
    ...
    case "apkgkapp":
      try {
        ...
        await conn.sendFile(m.chat, appInfo.info.ogImageUrl || "", "", caption, m);
        await conn.sendFile(m.chat, appInfo.info.apkLink || "", appInfo.title || "Aplikasi", null, m, false, {
          quoted: m,
