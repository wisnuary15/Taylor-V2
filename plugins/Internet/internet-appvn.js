  ...
  switch (command) {
    case "appvn":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di AppVN.").addSelection("Klik di sini").makeSections("AppVN", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `📦 *Version:* ${item.version}`, `${usedPrefix}appvnapp ${item.url}`);
        });
    ...
    case "appvnapp":
      try {
        ...
        await conn.sendFile(m.chat, appInfo.downloadLink || "", appInfo.descriptionShort || "Aplikasi", null, m, false, {
          quoted: m,
...
async function searchAppVn(query) {
  try {
    const link = "https://appvn.com",
      response = await fetch(link + "/android/search?keyword=" + encodeURIComponent(query)),
