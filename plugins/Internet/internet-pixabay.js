...
const handler = async (m, {
  ...
  let tems = sp[1]
  if (command === 'piximg') {
    let res = await fetch('https://pixabay.com/api/?key=30089426-4575ed7bbbc8bfffe9a0b8eb4&q=' + encodeURIComponent(
      text))
    ...
    return conn.sendList(m.chat, htki + ' 📺 Pixabay Search 🔎 ' + htka,
      `⚡ Total ${sul.total_count} Code, Silakan pilih Pixabay Search di tombol di bawah...\n*Teks yang anda kirim:* ${text}\n\nKetik ulang *${usedPrefix + command}* teks anda untuk mengubah teks lagi`,
      author, `☂️ Pixabay Search Disini ☂️`, listSections, m)
  }
  if (command === 'pixvid') {
...
`
    conn.sendFile(m.chat, sul.hits[no].previewURL ? sul.hits[no].webformatURL : sul.hits[no].largeImageURL, '',
      desimg, m)
  }
  if (command === 'pixgetvid') {
    let res = await fetch('https://pixabay.com/api/videos?key=30089426-4575ed7bbbc8bfffe9a0b8eb4&q=' +
      encodeURIComponent(tems))
    let sul = await res.json()
    let desvid = `\n\n
*userImageURL:* ${sul.hits[no].userImageURL}
*comments:* ${sul.hits[no].comments}
...
*likes:* ${sul.hits[no].likes}
`
    conn.sendFile(m.chat, sul.hits[no].videos.large.url ? sul.hits[no].videos.medium.url : sul.hits[no].videos.small
      .url, '', desvid, m)
  }
}
