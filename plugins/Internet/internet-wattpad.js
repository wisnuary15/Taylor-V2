...
async function searchWattpad(q) {
  ...
  const results = $('section#section-results-stories article#results-stories ul.list-group li.list-group-item').map((
    index, element) => ({
...
async function listRead(url) {
  try {
    ...
    const html = await response.text();
    const $ = cheerio.load(html);
    const tableOfContents = $('ul.table-of-contents li[class=""]').map((index, element) => ({
