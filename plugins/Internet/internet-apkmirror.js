...
`
        conn.sendFile(m.chat, item.gambar, "", cap, m)
        conn.sendFile(m.chat, item.linkdl, item.title || 'Tidak diketahui', null, m, true, {
          quoted: m,
...
async function getApkmirror(url) {
  try {
    ...
    if (link.includes('#downloads')) {
      ...
      const linkdl =
        `https://www.apkmirror.com/wp-content/themes/APKMirror/download.php?id=${id}&key=${key}&forcebaseapk=${forcebaseapk}`;
      return {
        title: $('meta[property="og:title"]').attr('content'),
        gambar: $('meta[property="og:image"]').attr('content'),
        link: link,
