  ...
  switch (command) {
    case "apkmirror":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di APKMirror.").addSelection("Klik di sini").makeSections("APKMirror", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `📦 *Version:* ${item.version}`, `${usedPrefix}apkmirrorapp ${item.link}`);
        });
    ...
    case "apkmirrorapp":
      try {
        ...
        await conn.sendFile(m.chat, appInfo.linkdl || "", appInfo.title || "Aplikasi", null, m, false, {
          quoted: m,
