  ...
  switch (command) {
    case "apkcafe":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Keterangan lebih lanjut dapat ditemukan di APKCAFE.").addSelection("Klik di sini").makeSections("APKCAFE", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Name:* ${item.name}`, `🖼️ *Image:* ${item.imgUrl}`, `${usedPrefix}apkcafeapp ${item.href}`);
        });
    ...
    case "apkcafeapp":
      try {
        ...
        if (!appInfo || !appInfo.version.length) return m.reply(`Link "${text}" tidak ditemukan :/`);
        const caption = appInfo.version.map(info => `📦 *Detail Aplikasi* 📦\n\n` + `📝 *Title:* ${info.title}\n` + `🖼️ *Image:* ${info.image}\n` + `🔗 *URL:* ${info.url}\n` + `⬇️ *Download Link:* ${info.downloadLink}\n` + `📏 *File Size:* ${info.fileSize}\n` + `📱 *Device Info:* ${info.deviceInfo}\n` + `📱 *Android Version:* ${info.androidVersion}\n` + `🔗 *Download:* ${info.download.downloadText}\n` + `⬇️ *Direct Link:* ${info.download.directLink}\n`).join("\n\n");
        await conn.sendFile(m.chat, info.image || "", "", caption, m);
        await conn.sendFile(m.chat, info.download.directLink || "", info.title || "Aplikasi", null, m, false, {
          quoted: m,
    ...
    const promises = $("li.dwn_up").map(async (index, element) => {
      ...
      if (downloadLink.includes("https://apk.cafe/download?file_id")) {
        const fileSize = $(element).find("a.dwn1 span").text().trim(),
          ...
          androidVersion = $(element).find("div.additional_file_info .f_ifo:last-child").text().trim(),
