...
`
        ...
        conn.sendFile(m.chat, item.download.directLink || logo, item.title || 'Tidak diketahui', null, m, true, {
          quoted: m,
...
async function getInfo(url) {
  ...
  const promises = $('li.dwn_up').map(async (index, element) => {
    ...
    if (downloadLink.includes('https://apk.cafe/download?file_id')) {
      ...
      const androidVersion = $(element).find('div.additional_file_info').find('.f_ifo:last-child').text()
    .trim();
