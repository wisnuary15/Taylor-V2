  ...
  try {
    switch (command) {
      case "cerpenmu":
        ...
        if (!categoriesData.length) return m.reply("Tidak ada kategori cerpen yang ditemukan.");
        if (!categoryIndex) {
          const categoriesButtons = conn.ctaButton.setBody("Pilih kategori cerpen di bawah ini.").setFooter("Cerpen diambil dari Cerpenmu.com").addSelection("Klik di sini").makeSections("Kategori Cerpen", "Pilih kategori");
          categoriesData.forEach((category, index) => {
            categoriesButtons.makeRow(`📚 *[ KATEGORI ${index + 1} ]*`, `📖 *Judul:* ${category.name}`, `📑 *Total Cerpen:* ${category.count}`, `${usedPrefix}cerpenmu ${index + 1}`);
          });
          ...
          if (!articlesData.articles.length) return m.reply("Tidak ada artikel yang ditemukan dalam kategori ini.");
          if (!articleIndex) {
            const articlesButtons = conn.ctaButton.setBody(`Pilih artikel dalam kategori *${category.name}* di bawah ini.`).setFooter("Artikel diambil dari Cerpenmu.com").addSelection("Klik di sini").makeSections("Artikel Cerpen", "Pilih artikel");
            articlesData.articles.forEach((article, index) => {
              articlesButtons.makeRow(`📖 *[ ARTIKEL ${index + 1} ]*`, `📖 *Judul:* ${article.title}`, `✍️ *Penulis:* ${article.author}`, `${usedPrefix}cerpenmu ${categoryIndex} ${index + 1}`);
            });
            ...
            await conn.reply(m.chat, caption, m, {
              contextInfo: {
                ...
                externalAdReply: {
                  ...
                  thumbnailUrl: "https://cdn-icons-png.flaticon.com/128/12692/12692170.png"
                }
              }
            });
            m.react(sukses);
