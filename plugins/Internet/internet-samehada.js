    ...
    switch (command) {
      case "samehada":
        try {
          ...
          const buttons = conn.ctaButton.setBody("Pilih hasil pencarian di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di Samehadaku.").addSelection("Klik di sini").makeSections("Samehadaku", "Pilih hasil");
          results.forEach((item, index) => {
            buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `📜 *Description:* ${item.description}`, `${usedPrefix}samehadaeps ${item.link}`);
          });
      ...
      case "samehadaeps":
        try {
          ...
          `;
          const buttons = conn.ctaButton.setBody("Daftar episode tersedia di bawah ini.").setFooter("Pilih episode untuk melihat detail atau mengunduh.").addSelection("Klik di sini").makeSections("Samehadaku", "Pilih episode");
          detail.episode.forEach((episode, index) => {
            buttons.makeRow(`🎥 *[ EPISODE ${index + 1} ]*`, `📅 *Title:* ${episode.title}`, `🗓 *Released:* ${episode.date}`, `${usedPrefix}samehadadl ${episode.link}`);
          });
          buttons.run(m.chat, conn, m);
        } catch (detailError) {
      ...
      case "samehadadl":
        try {
          ...
          if (!episodeLink) return m.reply("Masukkan link episode yang ingin diunduh.");
          const downloadData = await samehadaku.download(episodeLink);
          if (!downloadData || !downloadData.downloads.length) return m.reply("Gagal mendapatkan link unduhan.");
          m.reply(`Unduhan episode dapat diakses di: ${downloadData.downloads.map(d => d.link).join(", ")}`);
          m.react(sukses);
        } catch (detailError) {
          console.error("Error in fetchSamehadaDetail:", detailError);
          m.react(eror);
