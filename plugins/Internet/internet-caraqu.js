  ...
  try {
    switch (command.toLowerCase()) {
      case "caraqu":
        ...
        const searchButtons = conn.ctaButton.setBody("Pilih kategori cerpen di bawah ini.").setFooter("Cerpen diambil dari Caraqu.com").addSelection("Klik di sini").makeSections("Kategori Cerpen", "Pilih kategori");
        searchResults.forEach((result, index) => {
          searchButtons.makeRow(`📖 *[ ARTIKEL ${index + 1} ]*`, `📖 *Judul:* ${result.title}`, `📅 *Excerpt:* ${result.excerpt}`, `${usedPrefix}caraquread ${query}|${index + 1}`);
        });
        searchButtons.run(m.chat, conn, m);
        break;
      case "caraquread":
        if (isNaN(searchIndex) || searchIndex <= 0 || !query) {
          m.reply("Nomor artikel tidak valid. Mohon masukkan nomor yang benar.");
          return;
        }
