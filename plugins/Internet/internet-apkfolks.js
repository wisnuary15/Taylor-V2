  ...
  switch (command) {
    case "apkfolks":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di APKFolks.").addSelection("Klik di sini").makeSections("APKFolks", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Headline:* ${item.headline}`, `✍️ *Author:* ${item.author}`, `${usedPrefix}apkfolksapp ${item.headlineLink}`);
        });
    ...
    case "apkfolksapp":
      try {
        ...
        const caption = `📦 *Detail Aplikasi* 📦\n\n` + `📰 *Title:* ${appInfo.title}\n` + `🔗 *App Link:* ${appInfo.link}\n` + `🖼️ *Image:* ${appInfo.image}\n` + `📄 *Description:* ${appInfo.description}\n` + `📊 *Version Info:*\n${Object.entries(appInfo.versionInfo).map(([ key, value ]) => `  *${key}* ${value}`).join("\n")}\n` + `🔗 *Download Links:*\n${Object.entries(appInfo.download).map(([ key, value ]) => `  *${key}* ${value}`).join("\n")}`;
        await conn.sendFile(m.chat, appInfo.image || "", "", caption, m);
        await conn.sendFile(m.chat, Object.entries(appInfo.download).map(([key, value]) => value)[0] || "", appInfo.title || "Aplikasi", null, m, false, {
          quoted: m,
