...
`
        ...
        let ini = Object.entries(item.download).map(([key, value]) => {
          return key + value.trim()
        }).join('\n')
        if (!Object.keys(item.download).length === 0 && !item.download.constructor === Object) {
          conn.sendFile(m.chat, item.image || logo, "", cap, m)
          conn.sendFile(m.chat, dlnow || logo, item.title || 'Tidak diketahui', null, m, true, {
            quoted: m,
            mimetype: "application/vnd.android.package-archive"
          })
          m.reply("Download link: " + ini)
        } else {
          m.reply("Download link: " + item.link)
        }
      } catch (e) {
        m.react(eror)
      }
