    ...
    switch (command) {
      case "rexdl":
        try {
          ...
          const buttons = conn.ctaButton.setBody("Pilih artikel di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di RexDL.").addSelection("Klik di sini").makeSections("RexDL", "Pilih artikel");
          results.forEach((item, index) => {
            buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📖 *Title:* ${item.title}`, `📝 *Excerpt:* ${item.excerpt}`, `${usedPrefix}rexdlapp ${item.titleUrl}`);
          });
      ...
      case "rexdlapp":
        try {
          ...
          if (!appInfo.info.imageData) return m.reply(`URL "${text}" tidak ditemukan :/`);
          ...
          await conn.sendFile(m.chat, appInfo.info.imageData || "", "", caption, m);
          await conn.sendFile(m.chat, appInfo.download.apkUrls[0] || "", appInfo.info.headingTitle || "Aplikasi", null, m, false, {
            quoted: m,
