...
${wait}`
        conn.sendFile(m.chat, resl.info.imageData, "", cap, m)
        conn.sendFile(m.chat, resl.download.apkUrls[resl.download.apkUrls.length - 1], resl.info.headingTitle, null,
          m, true, {
            quoted: m,
...
async function getRexdl(url) {
  ...
  const apkUrls = dlboxdown.find('a').map((index, element) => $down(element).attr('href')).get().filter(url => url
    .endsWith('.apk'));
