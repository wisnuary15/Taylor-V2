  ...
  switch (command) {
    case "apkgod":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di APKGod.").addSelection("Klik di sini").makeSections("APKGod", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `📅 *Version:* ${item.version}`, `${usedPrefix}apkgodapp ${item.link}`);
        });
