    ...
    switch (command) {
      case "playmods":
        try {
          ...
          const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di PlayMods.").addSelection("Klik di sini").makeSections("PlayMods", "Pilih aplikasi");
          results.forEach((item, index) => {
            buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `📚 *Detail:* ${item.detail}`, `${usedPrefix}playmodsapp ${item.link}`);
          });
      ...
      case "playmodsapp":
        try {
          ...
          await conn.sendFile(m.chat, appInfo.link || "", appInfo.name || "Aplikasi", null, m, false, {
            quoted: m,
