...
`
        conn.sendFile(m.chat, item.image || flaaa.getRandom() + command, "", teks, m)
      } catch (e) {
        m.react(eror)
      }
    }
...
async function searchGitagram(query) {
  ...
  const results = [];
  $('.main .section .container .columns .column .panel .table tbody tr').each((index, element) => results.push({
    index: index + 1,
