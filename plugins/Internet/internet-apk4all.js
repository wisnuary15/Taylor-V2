  ...
  switch (command) {
    case "apk4all":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Keterangan lebih lanjut dapat ditemukan di APK4ALL.").addSelection("Klik di sini").makeSections("APK4ALL", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `⭐ *Rating:* ${item.rating}`, `${usedPrefix}apk4allapp ${item.titleUrl}`);
        });
    ...
    case "apk4allapp":
      try {
        ...
        await conn.sendFile(m.chat, linkFull || "", dlTitle || "Unduhan", null, m, false, {
          quoted: m,
