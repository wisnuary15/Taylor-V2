  ...
  try {
    switch (command) {
      case "filehippo":
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di FileHippo.").addSelection("Klik di sini").makeSections("FileHippo", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `🛠️ *Developer:* ${item.developer}`, `${usedPrefix}filehippoapp ${item.link}`);
        });
      ...
      case "filehippoapp":
        ...
        await conn.sendFile(m.chat, info.download_url || "", info.name || "Aplikasi", null, m, false, {
          quoted: m,
          mimetype: info.mimetype || "application/vnd.android.package-archive"
        });
        m.react(sukses);
        break;
      default:
