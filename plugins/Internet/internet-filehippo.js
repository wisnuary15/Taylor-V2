...
`
        ...
        conn.sendFile(m.chat, res[0]?.icon_url || logo, "", teks, m)
        conn.sendFile(m.chat, res[0]?.download_url || logo, res[0]?.name || 'Tidak diketahui', null, m, true, {
          quoted: m,
...
async function searchHippo(app_name) {
  ...
  try {
    ...
    const programs = await Promise.all($('.list-programs__item').map(async (index, element) => {
...
async function getHippo(app_name) {
  ...
  const hippo_information = $(
    'body > div.page > div:nth-child(2) > div > div > div > section.mb-l > article > p:nth-child(3)').text();
  const hippo_version = $(
