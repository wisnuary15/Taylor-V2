...
📆 *Edition:* ${item.edition}`
    ...
    if (feature === "app") {
      ...
      try {
        ...
        conn.sendFile(m.chat, resl.downloadLinkURL, resl.downloadLink, null, m, true, {
          quoted: m,
...
async function getApkhome(url) {
  try {
    ...
    const data = {
      ...
      supportedAndroidVersions: gtBlockElement.filter(':contains("Versi Android yang didukung")').next('br').text()
        .trim(),
