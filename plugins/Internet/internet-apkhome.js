  ...
  switch (command) {
    case "apkhome":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di APKHome.").addSelection("Klik di sini").makeSections("APKHome", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `📅 *Edition:* ${item.edition}`, `${usedPrefix}apkhomeapp ${item.href}`);
        });
    ...
    case "apkhomeapp":
      try {
        ...
        await conn.sendFile(m.chat, appInfo.downloadLinkURL || "", appInfo.title || "Aplikasi", null, m, false, {
          quoted: m,
