  ...
  switch (command) {
    case "dlandroid":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di DLAndroid.").addSelection("Klik di sini").makeSections("DLAndroid", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `📝 *Description:* ${item.description}`, `${usedPrefix}dlandroidapp ${item.downloadLink}`);
        });
