  ...
  switch (command) {
    case "apkfab":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di APKFAB.").addSelection("Klik di sini").makeSections("APKFAB", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `⭐ *Rating:* ${item.rating}`, `${usedPrefix}apkfabapp ${item.link}`);
        });
    ...
    case "apkfabapp":
      try {
        ...
        await conn.sendFile(m.chat, appInfo.downloadURL || "", appInfo.title || "Aplikasi", null, m, false, {
          quoted: m,
