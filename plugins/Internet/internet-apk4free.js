  ...
  switch (command) {
    case "apk4free":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Keterangan lebih lanjut dapat ditemukan di APK4FREE.").addSelection("Klik di sini").makeSections("APK4FREE", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `🏷️ *Tag:* ${item.tag.join(", ")}`, `${usedPrefix}apk4freeapp ${item.url}`);
        });
    ...
    case "apk4freeapp":
      try {
        ...
        if (downloads.length) caption += `\n🔗 *Downloads:* ${downloads.join("\n")}\n`;
        await conn.sendFile(m.chat, images[0] || "", "", caption, m);
        await conn.sendFile(m.chat, downloads[0] || "", texts[0] || "Aplikasi", null, m, false, {
          quoted: m,
