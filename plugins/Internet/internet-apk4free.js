...
async function getApk4free(url) {
  try {
    ...
    return data.reduce(((result, item) => {
      const { type: type, content: content, src: src, link: link } = item;
      return "text" === type ? (result.text || (result.text = []), result.text.push(content)) : "image" === type ? (result.image || (result.image = []),
        result.image.push(src)) : "download" === type && (result.download || (result.download = []),
        result.download.push(link)), result;
    }), {});
  } catch (error) {
    console.log(error);
  }
