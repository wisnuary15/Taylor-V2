    ...
    if (feature === "app") {
      ...
      try {
        ...
        let cap = "*About:*\n" + resl.text.slice(4, 16).join("\n") + "\n" + "*Link:*\n" + resl.download.map((v,
          index) => (index + 1) + ". " + v).join("\n");
        conn.sendFile(m.chat, resl.image[0], "", cap, m)
      } catch (e) {
        m.react(eror)
      }
    }
...
async function searchApk4free(query) {
  ...
  $('article').each((index, element) => {
    ...
    articles.push({
      ...
      thumbnail: $article.find('.featured-image .thumb.hover-effect span.fullimage').css('background-image')
        .replace(/url\((.*)\)/, '$1'),
      category: $article.find('.tags a[href^="https://apk4free.org/category/"]').map((index, tagElement) => $(
        tagElement).text()).get(),
      tag: $article.find('.tags a[href^="https://apk4free.org/tag/"]').map((index, tagElement) => $(
        tagElement).text()).get(),
      description: $article.find('.post-excerpt p').text(),
      author: {
        name: $article.find('footer.author-meta a .author-name').text(),
        image: $article.find('footer.author-meta a .author-image').css('background-image').replace(
          /url\('(.*)'\)/, '$1'),
