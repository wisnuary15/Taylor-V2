...
*thumbnailUrl:* ${v.thumbnailUrl}
   `.trim()
    ...
    conn.sendFile(m.chat, outs[0]?.thumbnailUrl, "", teks, m)
  }
}
...
async function getData(query) {
  ...
  try {
    ...
    const language = $('.islandora-metadata .mods_language_languageTerm_text_authority_iso639-2b_ms td').text()
  .trim();
    const country = $('.islandora-metadata .mods_originInfo_place_placeTerm_text_authority_marccountry_ms td').text()
      .trim();
