...
async function search3asq(q) {
  try {
    const { data: data } = await axios.get(`https://3asq.org/?s=${q}&post_type=wp-manga`), $ = cheerio.load(data);
...
async function getAllChapters(url) {
  try {
    const { data: data } = await axios.get(url), $ = cheerio.load(data);
...
async function getChapterPdf(url) {
  try {
    const { data: data } = await axios.get(url), $ = cheerio.load(data), buffers = [], pdfDoc = new PDFDocument, pdfStream = new PassThrough;
