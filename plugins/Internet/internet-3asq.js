  ...
  if (lister.includes(feature)) {
    ...
    if (feature === "pdf") {
      ...
      try {
        ...
        const pdfTitle =
          `${mangaTitle.replace(/-/g, ' ').replace(/\b\w/g, (char) => char.toUpperCase())} : ${chapterNumber}`;
        conn.sendFile(m.chat, data, pdfTitle, "DONE", m, null, {
          mimetype: dpdf,
          contextInfo: {
...
async function search3asq(q) {
  try {
    const {
      data
    } = await axios.get(`https://3asq.org/?s=${q}&post_type=wp-manga`);
