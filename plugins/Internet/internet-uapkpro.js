    ...
    switch (command) {
      case "uapkpro":
        try {
          ...
          const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di UAPKPro.").addSelection("Klik di sini").makeSections("UAPKPro", "Pilih aplikasi");
          results.forEach((item, index) => {
            buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📖 *Title:* ${item.title}`, `⭐ *Rating:* ${item.rating}`, `${usedPrefix}uapkproapp ${item.url}`);
          });
