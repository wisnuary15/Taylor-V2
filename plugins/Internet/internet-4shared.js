  ...
  switch (command) {
    case "4shared":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih item di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di 4shared.").addSelection("Klik di sini").makeSections("4Shared", "Pilih item");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📜 *Title:* ${item.title}`, `📦 *Size:* ${item.size}`, `${usedPrefix}4sharedapp ${item.link}`);
        });
    ...
    case "4sharedapp":
      try {
        ...
        await conn.sendFile(m.chat, appInfo.url, appInfo.text || "Aplikasi", null, m, false, {
          quoted: m,
