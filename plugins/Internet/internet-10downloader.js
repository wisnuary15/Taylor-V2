...
`
        ...
        conn.sendFile(m.chat, proxyurl + res.video[0]?.link || logo, "", teks + vidlink, m)
      } catch (e) {
        m.react(eror)
      }
    }
...
view: ${res.view}`
        ...
        conn.sendFile(m.chat, logo, "", teks, m)
      } catch (e) {
        m.react(eror)
      }
    }
...
async function getMusic(url) {
  try {
    var check = await fetch("https://x2convert.com/ajax2/getFile.ashx?linkinfo=" + url +
      "&lang=id&option=100&country=ID");
