    ...
    if (feature === "down") {
      ...
      try {
        ...
        let cap = '🔍 *[ RESULT ]*\n\n📰 *Title:* ' + item.title + '\n👤 *Author:* ' + item.author +
          ...
          '\n🖼️ *Image:* ' + item.image + '\n📝 *Note:* ' + item.note.replace(/(Note of the author)(.*)/,
          '$1\n$2') + '\n⬇️ *Download:* ' + item.download
        let details = await getFileDetails(item.download)
        conn.sendFile(m.chat, item.image, "", cap, m)
        conn.sendFile(m.chat, item.download, item.title + details.fileFormat, null, m, true, {
          quoted: m,
...
async function downloadDafont(link) {
  ...
  const getImage = () => 'https://www.dafont.com' + $('.preview').css('background-image').replace(
    /^url\(["']?|['"]?\)$/g, '');
