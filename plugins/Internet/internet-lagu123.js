...
async function searchLagu123(query) {
  ...
  $('.clearfix.search-content').each((index, element) => {
    const item = {
      ...
      playUrl: "https://youtube.com/watch?v=" + $(element).find('[onclick^="playAudio"]').attr('onclick').match(
        /'([^']+)'/)[1],
      audioUrl: "https://ytmp3.mobi/button-api/#" + $(element).find('[onclick^="playAudio"]').attr('onclick')
        .match(/'([^']+)'/)[1] + "|mp3",
      videoUrl: "https://ytmp3.mobi/button-api/#" + $(element).find('[onclick^="playAudio"]').attr('onclick')
        .match(/'([^']+)'/)[1] + "|mp4|e74c3c|ffffff",
