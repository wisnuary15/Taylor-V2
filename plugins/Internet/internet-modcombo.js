  ...
  switch (command) {
    case "modcombo":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di ModCombo.").addSelection("Klik di sini").makeSections("ModCombo", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `🔗 *URL:* ${item.href}`, `${usedPrefix}modcomboapp ${item.href}`);
        });
