...
const handler = async (m, {
  ...
  if (command === 'food') {
    ...
    try {
      let f = await fetch(
        `https://api.spoonacular.com/food/menuItems/search?apiKey=da321822ad4040d1addcb3237db2aeba&query=${text}`)
      let xx = await f.json()
...
`.trim()
  ...
  if (command === 'ingredients') {
    ...
    try {
      let f = await fetch(
        `https://api.spoonacular.com/recipes/findByIngredients?apiKey=da321822ad4040d1addcb3237db2aeba&ingredients=${text}`
        )
      let xx = await f.json()
...
`.trim()
  ...
  if (command === 'recipes') {
    ...
    try {
      let f = await fetch(
        `https://api.spoonacular.com/recipes/${text}/ingredientWidget.json?apiKey=da321822ad4040d1addcb3237db2aeba`
        )
      let xx = await f.json()
...
`.trim()
      ...
      conn.sendFile(m.chat, `https://spoonacular.com/cdn/ingredients_100x100/${xx.ingredients[0]?.image}`,
        'food.jpg', str, m)
    } catch {
      m.react(eror)
    }
