  ...
  switch (command) {
    case "apkbolt":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Keterangan lebih lanjut dapat ditemukan di APKBOLT.").addSelection("Klik di sini").makeSections("APKBOLT", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `🖼️ *Image:* ${item.imageURL}`, `${usedPrefix}apkboltapp ${item.link}`);
        });
    ...
    case "apkboltapp":
      try {
        ...
        if (appInfo.downloadFile) {
          await conn.sendFile(m.chat, appInfo.downloadFile || "", appInfo.name || "Aplikasi", null, m, false, {
            quoted: m,
