...
const handler = async (m, {
  ...
  const inputArray = ["kolom", "lainnya/analisis", "lainnya/diplomatic-corner", "lainnya/editorial",
    "lainnya/infografis", "lainnya/investigasi", "lainnya/liyan-pojok", "lainnya/oase", "lainnya/obituary",
    "lainnya/opini", "lainnya/religi", "lainnya/serba-serbi", "lainnya/wawancara", "lifestyle", "news", "tech"
  ];
  let [query] = text.split(" ")
  const result = inputArray.includes(query) ? query : inputArray.filter(item => item.startsWith('lainnya/') &&
    query === item.split('/').pop()).map(item => item)[0] || null;
  if (!result) return m.reply("*Example:*\n.dailynews lifestyle\n\n*Pilih type yg ada*\n" + inputArray.map((v,
    index) => "  ○ " + v.split('/').pop()).join("\n"))
  m.react(wait)
  try {
