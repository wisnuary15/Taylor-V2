  ...
  switch (command) {
    case "modsapk":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di GetModsAPK.").addSelection("Klik di sini").makeSections("GetModsAPK", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `📝 *Version:* ${item.version}`, `${usedPrefix}modsapkapp ${item.url}`);
        });
    ...
    case "modsapkapp":
      try {
        ...
        await conn.sendFile(m.chat, appInfo.link || "", appInfo.ogTitle || "Aplikasi", null, m, false, {
          quoted: m,
