...
`
        conn.sendFile(m.chat, item.image || logo, "", cap, m)
      } catch (e) {
        m.react(eror)
      }
    }
...
async function searchStackover(q) {
  ...
  try {
    ...
    return $('.s-post-summary').map((index, element) => ({
      ...
      vote: parseInt($(element).find(
        '.s-post-summary--stats-item__emphasized .s-post-summary--stats-item-number').text()),
      answer: parseInt($(element).find('.s-post-summary--stats-item:eq(1) .s-post-summary--stats-item-number')
        .text()),
      views: parseInt($(element).find('.s-post-summary--stats-item:eq(2) .s-post-summary--stats-item-number')
        .text()),
      ...
      tags: $(element).find('.s-post-summary--meta-tags a.js-tagname').toArray().map(tagElement => $(tagElement)
        .text()),
      userInfo: {
