  ...
  switch (command) {
    case "apkpures":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di APKPure.").addSelection("Klik di sini").makeSections("APKPures", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Name:* ${item.name}`, `🔧 *Version:* ${item.version}`, `${usedPrefix}apkpureapp ${item.link}`);
        });
    ...
    case "apkpureapp":
      try {
        ...
        await conn.sendFile(m.chat, appInfo.link || "", appInfo.title || "Aplikasi", null, m, false, {
          quoted: m,
