...
💬 *Combined Data:*
${item.combinedData}`
        conn.sendFile(m.chat, item.ogImage, item.ogTitle, teks, m)
      } catch (e) {
        m.react(eror)
      }
    }
...
async function searchTNW(q) {
  try {
    const response = await fetch('https://www.technewsworld.com/search-results?keyword=' + q +
      '&orderby=post_date&order=desc') // Ganti dengan URL halaman yang sesuai
...
async function detailTNW(url) {
  try {
    ...
    const combinedData = $('div.story-content p').map((index, element) => {
