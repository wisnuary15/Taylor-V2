    ...
    switch (command) {
      case "revdl":
        try {
          ...
          const buttons = conn.ctaButton.setBody("Pilih hasil pencarian di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di RevDL.").addSelection("Klik di sini").makeSections("RevDL", "Pilih hasil");
          results.forEach((item, index) => {
            buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `📜 *Excerpt:* ${item.excerpt}`, `${usedPrefix}revdlapp ${item.titleUrl}`);
          });
