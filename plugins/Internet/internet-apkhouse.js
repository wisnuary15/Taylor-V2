  ...
  switch (command) {
    case "apkhouse":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di APKHouse.").addSelection("Klik di sini").makeSections("APKHouse", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `📦 *Version:* ${item.version}`, `${usedPrefix}apkhouseapp ${item.href}`);
        });
    ...
    case "apkhouseapp":
      try {
        ...
        await conn.sendFile(m.chat, appInfo.map(link => link.link)[0] || "", appInfo.map(link => link.text)[0] || "Aplikasi", null, m, false, {
          quoted: m,
