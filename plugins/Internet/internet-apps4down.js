  ...
  switch (command) {
    case "apps4":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di Apps4Download.").addSelection("Klik di sini").makeSections("Apps4Download", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `📈 *Version:* ${item.version}`, `${usedPrefix}apps4app ${item.href}`);
        });
