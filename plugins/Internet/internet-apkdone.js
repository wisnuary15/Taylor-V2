...
🗂️ *Category:* ${item.category}`
    ...
    if (feature === "app") {
      ...
      try {
        ...
        conn.sendFile(m.chat, resl.ogImageUrl, "", cap, m)
        conn.sendFile(m.chat, resl.links[0], inputs.split('/')[3], null, m, true, {
          quoted: m,
...
async function getApkdone(url) {
  ...
  return {
    links: $('script[type="text/javascript"]').map((index, element) => $(element).html()).get().filter(scriptText =>
      scriptText.includes('hole.apkdone.download')).map(scriptText => scriptText.match(
      /https?:\/\/hole\.apkdone\.download\/[^\s]+/g)).filter(matches => matches !== null).flat().map(link => link
      .replace(/"$/, '')),
