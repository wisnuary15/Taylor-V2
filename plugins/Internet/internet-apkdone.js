  ...
  switch (command) {
    case "apkdone":
      try {
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Keterangan lebih lanjut dapat ditemukan di APKDONE.").addSelection("Klik di sini").makeSections("APKDONE", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Name:* ${item.appName}`, `📦 *Version:* ${item.version}`, `${usedPrefix}apkdoneapp ${item.href}`);
        });
    ...
    case "apkdoneapp":
      try {
        ...
        if (!appInfo || !appInfo.links.length) return m.reply(`Link "${text}" tidak ditemukan :/`);
        const caption = `📦 *Detail Aplikasi* 📦\n\n` + `🖼️ *OG Image:* ${appInfo.ogImageUrl}\n` + `⬇️ *Download Links:*\n${appInfo.links.join("\n")}`;
        await conn.sendFile(m.chat, appInfo.ogImageUrl || "", "", caption, m);
        await conn.sendFile(m.chat, appInfo.links[0] || "", appInfo.links[0] || "Aplikasi", null, m, false, {
          quoted: m,
