  ...
  try {
    switch (command) {
      case "modded1":
        ...
        const buttons = conn.ctaButton.setBody("Pilih aplikasi di bawah ini.").setFooter("Detail lebih lanjut dapat ditemukan di Modded-1.").addSelection("Klik di sini").makeSections("Modded-1", "Pilih aplikasi");
        results.forEach((item, index) => {
          buttons.makeRow(`🔍 *[ RESULT ${index + 1} ]*`, `📰 *Title:* ${item.title}`, `📜 *Meta:* ${item.meta}`, `${usedPrefix}modded1app ${item.url}`);
        });
...
async function searchModded(query) {
  try {
    const response = await fetch(`https://modded-1.com/?s=${encodeURIComponent(query)}`),
...
async function getMod1(url) {
  try {
    const response = await fetch(url.endsWith("/download/0") ? url : `${url}/download/0`),
      html = await response.text(),
      $ = cheerio.load(html),
