...
`
    ...
    if (feature === "app") {
      ...
      try {
        ...
        conn.sendFile(m.chat, resl.url, resl.text, null, m, true, {
          quoted: m,
...
async function getMod1(query) {
  try {
    const response = await fetch(query.endsWith('/download/0') ? query : query +
    '/download/0'); // Ganti URL dengan URL sumber HTML yang sesuai
    const html = await response.text();
    const $ = cheerio.load(html);
    ...
    const data = {
      ...
      url,
      ogImageUrl
    }; // Buat objek dengan teks dan URL
    return data;
  } catch (error) {
    console.log('Terjadi kesalahan:', error);
    return {};
