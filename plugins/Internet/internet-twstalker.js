...
`
    ...
    if (feature === "video") {
      ...
      try {
        ...
        if (inputs_ > videoSources.length) return m.reply(
          `Pilih angka yg ada dari *${1}* sampai *${videoSources.length}* saja!`)
...
`
        conn.sendFile(m.chat, directLink || giflogo, item.title || 'Tidak diketahui', cap, m)
      } catch (e) {
        m.react(eror)
      }
    }
