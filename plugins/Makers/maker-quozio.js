...
async function createQuote(author, message) {
    try {
        ...
        const quote = await fetch(`${host}api/v1/quotes`, {
            ...
            body: JSON.stringify({ author, quote: message }),
        }).then(res => res.json());
        ...
        const imageUrl = await fetch(`${host}api/v1/quotes/${quoteId}/imageUrls?templateId=${template.templateId}`)
            .then(res => res.json())
            .then(data => data.medium);
