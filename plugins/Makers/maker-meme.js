...
const handler = async (m, {
  ...
  let url = command === 'meme1' || command === 'meme4' || command === 'meme5' ?
    `https://api.lolhuman.xyz/api/${command}?${new URLSearchParams({ ...params, text })}` : command === 'meme9' ?
    `https://api.lolhuman.xyz/api/creator/kannagen?${new URLSearchParams({ ...params, text })}` : command ===
    'meme10' ? `https://api.lolhuman.xyz/api/creator/ohno?${new URLSearchParams({ ...params, text })}` : command ===
    'meme11' ? `https://api.lolhuman.xyz/api/creator/changemymind?${new URLSearchParams({ ...params, text })}` :
