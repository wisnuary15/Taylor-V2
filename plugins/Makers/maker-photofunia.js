...
💬 *Comments:* ${result.desc}`;
        ...
        if (effect[0]?.inputs[0]?.input === 'image') {
          if (input) {
            if (effect[0]?.inputs.length === 2) {
              ...
              const output = await photofunImg2(result.link, fs.readFileSync(imagePath), fs.readFileSync(
                imagePath2));
              conn.sendFile(m.chat, output.url || result.thumb, '', message, m);
            ...
            m.reply("Masukkan " + effect[0]?.inputs.length + " link gambar di akhir dengan pemisah spasi.");
          }
          ...
          if (input) {
            ...
            m.reply("Masukkan " + effect[0]?.inputs.length + " teks Anda di akhir dengan pemisah simbol -.");
          }
        }
      } else if (!part) {
        ...
        conn.reply(m.chat,
          '❌ Nomor bagian tidak valid. Harap masukkan nomor bagian yang tepat.\n\n📚 Contoh penggunaan: *photofunia query|page|part*',
          m);
      }
    } else {
    ...
    conn.reply(m.chat,
      '❌ Terjadi kesalahan saat mengambil data. Pastikan format input benar.\n\n📚 Contoh penggunaan: *photofunia query|page|part*',
      m);
  }
};
