...
const handler = async (m, {
  ...
  try {
    ...
    if (isNaN(effectIndex) || effectIndex < 1 || effectIndex > effects.length) {
      const itemsList = effects.map((effect, index) => `${index + 1}. ${effect.judul}`).join("\n");
    ...
    let photoResult;
    if (buffer) {
      photoResult = await img(selectedEffect, [buffer], ...oter ? oter.split("-") : []);
    } else {
