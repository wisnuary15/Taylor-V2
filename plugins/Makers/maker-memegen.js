...
const getFonts = async () => {
    let res = await fetch('https://api.memegen.link/fonts', { headers: { 'accept': 'application/json' } });
...
let handler = async (m, { conn, usedPrefix, text, command }) => {
    ...
    try {
        ...
        try {
            let out = await wibusoft.tools.makeSticker(json.url, { author: packname, pack: m.name, keepScale: true });
            await m.reply(out);
        } catch (e) {
