...
const handler = async (m, { conn, usedPrefix, command, args }) => {
    ...
    if (command === "vace") {
        const res = [
            "adios", "batmanslap", "carreverse", "changemymind", "distractedbf", "dockofshame", "drip", "ejected", 
            "emergencymeeting", "firsttime", "grave", "heaven", "iamspeed", "icanmilkyou", "npc", "peeposign", 
            "rankcard", "stonks", "tableflip", "water", "wide", "wolverine", "womanyellingatcat"
        ];
        const listSections = res.map((v, index) => [
            "                [ RESULT " + (index + 1) + " ]",
    ...
    if (command === "vaceget") {
        try {
            ...
            await conn.sendFile(m.chat, res, 'vace.jpg', `Sudah Jadi`, m, false);
        } catch (error) {
