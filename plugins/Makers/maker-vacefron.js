...
const handler = async (m, {
  ...
  if (command === "vace") {
    const res = ["adios", "batmanslap", "carreverse", "changemymind", "distractedbf", "dockofshame", "drip",
      "ejected", "emergencymeeting", "firsttime", "grave", "heaven", "iamspeed", "icanmilkyou", "npc",
      "peeposign", "rankcard", "stonks", "tableflip", "water", "wide", "wolverine", "womanyellingatcat"
    ];
    const listSections = res.map((v, index) => ["                [ RESULT " + (index + 1) + " ]",
      [
        [v.toUpperCase(), `${usedPrefix}${command}get ${v}|${text}`, ""]
      ]
    ]);
    return conn.sendList(m.chat, "📺 Vace Maker 🔎",
  ...
  if (command === "vaceget") {
    try {
      ...
      conn.sendFile(m.chat, res, 'vace.jpg', `Sudah Jadi`, m, false);
    } catch (error) {
