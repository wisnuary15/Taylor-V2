...
export async function before(m) {
  try {
    if (!m.isBaileys && !m.messageStubType && !m.messageStubParameters?.[0]?.includes("No") && m.sender && m.isGroup) return true;
    ...
    if (!(m.messageStubType in messages)) return;
    const resultString = _.startCase(_.toLower(WAMessageStubType[m.messageStubType].replace(/_/g, " ")));
    await this.reply(m.chat, `${edtr} ${messages[m.messageStubType]}`, m, {
      contextInfo: {
