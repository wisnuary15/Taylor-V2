export async function before(m) {
  ...
  if (chat.autoReply && !m.isBaileys) {
    if (m.mtype === 'groupInviteMessage' || m.text.startsWith('https://chat') || m.text.startsWith(
      'Buka tautan ini')) {
      this.reply(m.chat, `✨ *Undang Bot ke Grup* ✨\n💎 7 Hari / Rp 5,000\n💎 30 Hari / Rp 15,000`, m, {
        mentions: [m.sender]
      });
      this.reply(m.sender + '@s.whatsapp.net',
    ...
    if (triggerWords.some(word => lowerText === word)) {
      const apsih = ["Kenapa", "Ada apa", "Naon meng", "Iya, bot disini", "Luwak white coffee passwordnya",
        "Hmmm, kenapa", "Apasih", "Okey bot sudah aktif", "2, 3 tutup botol", "Bot aktif"
      ];
