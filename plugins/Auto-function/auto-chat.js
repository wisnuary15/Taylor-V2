    ...
    try {
      ...
      if (text.startsWith('settype')) {
        ...
        if (setType === 1) {
          ...
          this.reply(m.chat, `Set settype to ${setType}.`, m);
        } else if (setType === 2) {
          delete this.autochat.modelId;
          this.reply(m.chat, `Set settype to ${setType}.`, m);
        } else {
          this.reply(m.chat, 'Invalid settype value.', m);
        }
        ...
        if (this.autochat.settype === 1 && selectedModelIndex < replicateModel.length) {
          ...
          this.reply(m.chat, `Set model to Replicate model ${selectedModelIndex + 1}.`, m);
          ...
          this.reply(m.chat, `Set model to Cohere model ${selectedModelIndex + 1}.`, m);
        } else {
          this.reply(m.chat, 'Invalid setmodel value or model type mismatch.', m);
        }
        ...
        if (this.autochat.settype === 1) {
          if (!this.autochat.modelId) {
            this.reply(m.chat, 'Please set the model ID for Replicate API using setmodel.', m);
            return;
          }
          ...
          if (!this.autochat.modelName) {
            this.reply(m.chat, 'Please set the model name for Cohere API using setmodel.', m);
            return;
          }
          generatedText = await textGenerationApi.generateTextFromCohere(this.autochat.modelName, messages);
        } else {
          this.reply(m.chat, 'Please set the type using settype.', m);
          return;
        }
        this.reply(m.chat, generatedText, m);
      } else if (text.startsWith('reset')) {
        delete this.autochat;
        this.reply(m.chat, `*Autochat reset*`, m);
      } else if (text.startsWith('off')) {
        this.autochat.status = false;
        this.reply(m.chat, `*Autochat OFF*`, m);
      }
    } catch {
...
const replicateModel = ['replicate/llama7b-v2-chat:058333670f2a6e88cf1b29b8183405b17bb997767282f790b82137df8c090c1f',
