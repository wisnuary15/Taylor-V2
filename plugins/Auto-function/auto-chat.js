import { TextGenerationAPI } from "../../lib/ai/auto-chat.js";
...
export async function before(m) {
  ...
  if (text) try {
    ...
    if (text.startsWith("settype")) {
      ...
      this.autochat.settype = setType, 1 === setType ? (delete this.autochat.modelName,
        await this.reply(m.chat, `Set settype to ${setType}.`, m)) : 2 === setType ? (delete this.autochat.modelId,
        await this.reply(m.chat, `Set settype to ${setType}.`, m)) : await this.reply(m.chat, "Invalid settype value.", m);
      ...
      1 === this.autochat.settype && selectedModelIndex < replicateModel.length ? (this.autochat.modelId = replicateModel[selectedModelIndex],
        await this.reply(m.chat, `Set model to Replicate model ${selectedModelIndex + 1}.`, m)) : 2 === this.autochat.settype && selectedModelIndex < cohereModel.length ? (this.autochat.modelName = cohereModel[selectedModelIndex],
        await this.reply(m.chat, `Set model to Cohere model ${selectedModelIndex + 1}.`, m)) : await this.reply(m.chat, "Invalid setmodel value or model type mismatch.", m);
    ...
    } else text.startsWith("reset") ? (delete this.autochat, await this.reply(m.chat, "*Autochat reset*", m)) : text.startsWith("off") && (this.autochat.status = !1,
