  ...
  try {
    ...
    if (this.autoGempa = this.autoGempa || {}, 0 === Object.keys(this.autoGempa).length) {
      const response = await fetch(link + "autogempa.json"),
        { gempa: gempa } = (await response.json()).Infogempa;
      this.autoGempa.data = gempa, this.autoGempa.state = !0;
    }
    setInterval((async () => {
      try {
        const response = await fetch(link + "autogempa.json"),
          { gempa: newEarthquakeData } = (await response.json()).Infogempa;
        if (JSON.stringify(newEarthquakeData) !== JSON.stringify(this.autoGempa.data) && (this.autoGempa.state = !1,
            !this.autoGempa.state)) {
