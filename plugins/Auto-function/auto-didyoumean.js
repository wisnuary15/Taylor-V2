...
export async function before(m, {
  ...
  try {
    ...
    const help = Object.values(plugins).flatMap(v => v.help ?? []).map(entry => entry.trim().split(' ')[0]
    ...
    const shouldSendMessage = (!filteredMatches.some(({
      target
    }) => target === noPrefix) && !m?.isCommand && !help.includes(noPrefix) && !['=>', '>', '$'].some(char => (m
      ?.text || m?.caption).startsWith(char)));
