...
export async function before(m, { match: match }) {
  try {
    ...
    const usedPrefix = match?.[0]?.[0] || "",
      ...
      help = (words.slice(1).join(" ").toLowerCase(),
        Object.values(plugins).flatMap((v => v.help ?? [])).map((entry => entry.trim().split(" ")[0]?.toLowerCase())).filter(Boolean)),
    ...
    if (!!(filteredMatches.some((({ target: target }) => target === noPrefix)) || m?.isCommand || help.includes(noPrefix) || ["=>", ">", "$"].some((char => (m?.text || m?.caption).startsWith(char))))) return;
