...
export async function before(m) {
  ...
  if (!lister.includes(versions.toLowerCase())) return m.reply("*Example:*\nlink v2\n\n*Pilih type yg ada*\n" + lister
    .map((v, index) => "  ○ " + v.toUpperCase()).join("\n"));
  try {
    ...
    if (versions === "v1") {
      ...
      this.sendFile(m.chat, video?.videos[0] || video?.videos[1] || video?.videos[2] || giflogo, "", caption, m);
    }
    if (versions === "v2") {
    ...
    if (versions === "v5") {
      ...
      let caption = `${spaces}*[ T I K T O K ]*\n` + `🔗 ID: ${video?.video_id || ''}\n` +
        ...
        `🎵 Music: ${video?.music?.title} - ${video?.music?.author || ''}\n` +
        `🖼️ Thumbnail URL: ${(await short.tinyurl(video?.thumbnail)) || ''}\n` + `${spaces}*[ Snaptikpro ]*`;
      this.sendFile(m.chat, video?.videos || video?.videos[0] || video?.videos[1] || giflogo, "", caption, m);
    }
    if (versions === "v6") {
    ...
    if (versions === "v7") {
      ...
      let caption = `${spaces}*[ T I K T O K ]*\n` + `🔗 ID: ${video?.video_id || ''}\n` +
        ...
        `🎵 Music: ${video?.music?.title} - ${video?.music?.author || ''}\n` +
        `🖼️ Thumbnail URL: ${(await short.tinyurl(video?.thumbnail)) || ''}\n` + `${spaces}*[ Tikcdn ]*`;
      this.sendFile(m.chat, video?.videos[0] || video?.videos[1] || giflogo, "", caption, m);
    }
    if (versions === "v8") {
      ...
      let caption = `${spaces}*[ T I K T O K ]*\n` + `🔗 ID: ${video?.video_id || ''}\n` +
        ...
        `🎵 Music: ${video?.music?.title} - ${video?.music?.author || ''}\n` +
        `🖼️ Thumbnail URL: ${(await short.tinyurl(video?.thumbnail)) || ''}\n` + `${spaces}*[ Tikmate ]*`;
      this.sendFile(m.chat, video?.videos || video?.videos[0] || giflogo, "", caption, m);
    }
    if (versions === "v9") {
...
\n${spaces}*[ ${versions.toUpperCase()} ]*`
      this.sendFile(m.chat, obj?.download?.nowm || obj?.download?.wm || obj?.download?.nowm || giflogo, "", ScrapCap,
        m)
    }
    if (versions === "v13") {
      let god = await axios.get("https://godownloader.com/api/tiktok-no-watermark-free?url=" + links +
        "&key=godownloader.com")
      let GoCap = `${spaces}*[ T I K T O K ]*

*Desc:* ${god?.data?.desc}
\n${spaces}*[ ${versions.toUpperCase()} ]*`
      this.sendFile(m.chat, god?.data?.video_no_watermark, "", GoCap, m)
    }
    if (versions === "v14") {
      let spinner = ora({
        text: 'Downloading...',
...
${getVideoInfo(video)}
\n${spaces}*[ ${versions.toUpperCase()} ]*`
        this.sendFile(m.chat, buffer || giflogo, "", PrevCap, m)
      } catch (e) {
...
\n${spaces}*[ ${versions.toUpperCase()} ]*`
      this.sendFile(m.chat, videoX?.download?.nowm || giflogo, "", XctCap, m)
    }
  } catch (e) {
    m.reply(e.toString());
  }
  ...
  async function API_URL(videoId) {
    const API = {
      ...
      constructApiQuery: async (videoId, appVersion, manifestAppVersion) => {
        ...
        const queryParams = Object.keys(parameters).map((key, index) =>
          `${index > 0 ? '&' : '?'}${key}=${parameters[key]}`).join('');
