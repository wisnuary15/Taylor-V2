...
export async function before(m) {
  ...
  try {
    if (chat.deepenglish) {
      if (db.data.dbai.autodeepenglish[m.chat] || {
          first: !0
        }) {
        if (!(m.quoted && m.quoted?.fromMe && m.quoted?.isBaileys && m.text)) return !0;
        if (db.data.dbai.autodeepenglish = db.data.dbai.autodeepenglish || {}, !db.data.dbai.autodeepenglish[m.chat] || m.quoted?.id != db.data.dbai.autodeepenglish[m.chat].msg.key.id) return;
        ...
        if (result) {
          ...
          db.data.dbai.autodeepenglish[m.chat].msg = soal, (db.data.dbai.autodeepenglish[m.chat].first = !1) && await this.sendMessage(m.chat, {
