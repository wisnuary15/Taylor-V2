...
export async function before(m) {
  ...
  if (this.talkai = this.talkai || {
      type: "chat"
    }, m.isBaileys || !m.text || !chat.autoAi) return !1;
  const text = m.text;
  try {
    if (chat.autoAi) {
      if (chat.autoAi && "talkaitype" === text?.split(" ")[0]) {
        const requestedType = text?.split(" ")[1];
        return "chat" === requestedType || "image" === requestedType ? (this.talkai.type = requestedType, void await this.reply(m.chat, "Tipe berhasil di set: " + requestedType, m)) : void await this.reply(m.chat, "Tipe yang tersedia: chat, image", m);
      }
      if ("talkaitype" !== text?.split(" ")[0]) {
        const openAIResponse = await TalkAI(this.talkai.type, text);
        if (openAIResponse)
          if ("image" === this.talkai.type) {
            const result = openAIResponse;
            await this.reply(m.chat, result.data[0]?.url, m);
          } else if ("chat" === this.talkai.type) {
