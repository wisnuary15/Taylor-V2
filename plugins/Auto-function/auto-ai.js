  ...
  try {
    if (chat.autoAi) {
      if (chat.autoAi && "talkaitype" === text?.split(" ")[0]) {
        ...
        return "chat" === requestedType || "image" === requestedType ? (this.talkai.type = requestedType,
          void await this.reply(m.chat, "Tipe berhasil di set: " + requestedType, m)) : void await this.reply(m.chat, "Tipe yang tersedia: chat, image", m);
      }
