...
export async function before(m) {
  try {
    ...
    if (!m.isGroup || !m.msg || !m.message || m.key.remoteJid !== m.chat || db.data.users[m.sender].banned || db.data
      .chats[m.chat].isBanned) return false;
    const gemini = new Gemini('__Secure-1PSID',
    ...
    if (m.mtype === 'protocolMessage' || m.mtype === 'pollUpdateMessage' || m.mtype === 'reactionMessage' || m
      .mtype === 'stickerMessage') return;
