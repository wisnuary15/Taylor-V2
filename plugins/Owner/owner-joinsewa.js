const handler = async (m, {
  ...
  let time = db.data.users[m.sender].lastjoin + 86400000
  ...
  try {
    ...
    if (e.length) conn.reply(res, `Ada @${anubot} Owner-ku Di Sini, Aku Mau Keluar Aja Dah, Takut Kena Marah.
...
Thanks dah invite Gua *${m.name}*`, fkonn, {
      ...
      conn.reply(res, `Tapi Boong 🤭`, 0)
      conn.reply(nomorown + '@s.whatsapp.net',
        `*INVITING!*\n\n@${m.sender.split('@')[0]} telah mengundang ${conn.user.name} ke grup\n\n${conn.getName(res)}\n\n${res}\n\nPesan : ${args[0]}\n\nBot akan keluar otomatis setelah *${msToDate(db.data.chats[res].expired - now)}*`,
        null, {
          mentions: [m.sender]
        })
    })
    if (!e.length) conn.reply(nomorown + '@s.whatsapp.net',
      `*INVITING!*\n\n@${m.sender.split('@')[0]} telah mengundang ${conn.user.name} ke grup\n\n${conn.getName(res)}\n\n${res}\n\nPesan : ${args[0]}\n\nBot akan keluar otomatis setelah *${msToDate(db.data.chats[res].expired - now)}*`,
      null, {
        mentions: [m.sender]
      })
