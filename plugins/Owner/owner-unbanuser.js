const handler = async (m, {
  ...
  try {
    ...
    if (!who) {
      return m.reply("⚠️ *Error:* Mohon tag atau sebutkan seseorang yang ingin di-unban.");
    }
    ...
    if (!txt) {
      return m.reply("⚠️ *Error:* Mohon sebutkan alasan atau keterangan untuk unban.");
    }
    user.banned = false;
    user.BannedReason = "";
    ...
    m.reply(`*Kamu telah di-unban oleh OWNER atau MODERATOR!!*\n\n*HUBUNGI*\n${ownerContacts}\n\n${modContacts}`, who);
  } catch (error) {
