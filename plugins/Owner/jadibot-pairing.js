import { useMultiFileAuthState, fetchLatestBaileysVersion, makeCacheableSignalKeyStore, PHONENUMBER_MCC, DisconnectReason } from "@whiskeysockets/baileys";
...
const handler = async (m, { conn: conn, text: text, usedPrefix: usedPrefix, command: command }) => {
  ...
  const __dirname = await __dirname(process.argv[1]),
    { state: state, saveCreds: saveCreds } = await useMultiFileAuthState("jadibot/" + m.sender.split("@")[0]),
    { version: version } = await fetchLatestBaileysVersion(),
    config = {
      printQRInTerminal: !1,
      mobile: !1,
      version: version,
  ...
  async function reloadHandler(restartConn) {
    ...
    if (restartConn) {
      ...
      return connection = {
          ...connection,
          ...makeWASocket(config)
        }, isInit || (connection.ev.off("messages.upsert", connection.handler), connection.ev.off("group-participants.update", connection.participantsUpdate),
          connection.ev.off("message.delete", connection.onDelete), connection.ev.off("connection.update", connection.connectionUpdate),
          connection.ev.off("creds.update", connection.credsUpdate)), connection.handler = handler.handler.bind(connection),
  ...
  connection = makeWASocket(config), connection.ev.on("connection.update", (async update => {
    const { connection: connection, lastDisconnect: lastDisconnect } = update;
