const handler = async (m, {
  ...
  if (command === "listpremium") {
    ...
    let len = args[0] && args[0].length > 0 ? Math.min(100, Math.max(parseInt(args[0]), 10)) : Math.min(10, sortedP
      .length);
    let myName = conn.getName(m.sender);
    let names = await Promise.all(sortedP.slice(0, len).map(async ({
      jid,
      registered
    }) => {
      return registered ? db.data.users[jid].name : conn.getName(jid);
    }));
...
function clockString(ms) {
  ...
  let [ye, mo, d, h, m, s] = [31104000000, 2592000000, 86400000, 3600000, 60000, 1000].map(u => Math.floor(ms / u) % (
    u === 1000 ? 60 : (u === 2592000000 ? 12 : (u === 31104000000 ? 10 : 30))));
  return ['\n┊', ye, ' *Tahun 🗓️*\n', '┊', mo, ' *Bulan 🌙*\n', '┊', d, ' *Hari ☀️*\n', '┊', h, ' *Jam 🕐*\n', '┊', m,
    ' *Menit ⏰*\n', '┊', s, ' *Detik ⏱️*'
  ].join('');
}
