const handler = async (m, { conn: conn, command: command, args: args }) => {
  if ("listpremium" === command) {
    ...
    let premiumTime = db.data.users[m.sender]?.premiumTime || 0,
      ...
      names = await Promise.all(sortedP.slice(0, len).map((async ({ jid: jid, registered: registered }) => registered ? db.data.users[jid].name : conn.getName(jid)))),
      myPrem = `┌✦ *⚕️My Premium Time⚕️*\n┊• *👤Nama:* ${myName}\n┊• *PremiumTime:* ${prem ? waktu : "Expired 🚫"}\n╚┈┈┈┈┈┈┈┈┈✧\n`,
      allPrem = "–––『 𝗔𝗟𝗟 𝗣𝗥𝗘𝗠𝗜𝗨𝗠 』–––\n\n" + sortedP.slice(0, len).map((({ jid: jid, premiumTime: premiumTime }, i) => `┌✦ ( ${i + 1} )\n┊👤 Nama: ${names[i]}\n┊☎ Nomor: ${jid.split("@")[0]}\n┊ *Link:* wa.me/${jid.split("@")[0]}\n┊ *Waktu Premium:* ${premiumTime > (new Date).getTime() ? clockString(premiumTime - (new Date).getTime()) : "Expired 🚫"}\n╚┈┈┈┈┈┈┈┈┈✧`)).join("\n"),
      capt = myPrem + "\n" + allPrem;
    m.reply(capt, null, {
