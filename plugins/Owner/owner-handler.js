const handler = async (m, { conn: conn, args: args }) => {
  try {
    ...
    let parsedValue;
    if (void 0 !== plugin[paramName] && await conn.reply(m.chat, `Mengganti nilai ${paramName} yang sudah ada di ${key.split("/").pop().slice(0, -3)}`, m),
      /^(true|false|null|undefined)$/i.test(paramValue)) parsedValue = eval(paramValue);
    else try {
