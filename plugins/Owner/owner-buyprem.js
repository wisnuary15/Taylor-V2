...
const handler = async (m, {
  ...
  try {
    ...
    const rows = ["money", "diamond"].flatMap(currency => hargaPremium[currency].map(({
      harga,
      hari
    }, idx) => ({
    ...
    m.react(wait);
    if (!/^[1-9]\d*$/.test(list) || !validCurrency.includes(currency.toLowerCase()) || !hargaPremium[currency.toLowerCase()]?.[listNum - 1]) {
...
handler.before = async (m, {
  ...
  try {
    db.data.dbbot.buyprem = db.data.dbbot.buyprem || {};
    if (!db.data.dbbot.buyprem || m.isBaileys || !(m.chat in db.data.dbbot.buyprem)) return;
    ...
    const {
      harga,
      hari
    } = hargaPremium[currency][list - 1];
    if (m.quoted?.id === key.id && input) {
      m.react(wait);
      if (yesRegex.test(input)) {
        if (currency === "money") {
          if (user.money < harga) return await conn.reply(m.chat, `🚫 *Anda membutuhkan setidaknya Rp${harga.toLocaleString()} untuk membeli paket ini.* 🚫`, m);
          user.money -= harga;
        } else {
          if (user.diamond < harga) return await conn.reply(m.chat, `🚫 *Anda membutuhkan setidaknya ${harga} diamond untuk membeli paket ini.* 🚫`, m);
          user.diamond -= harga;
        }
        const currentPremiumEnd = moment(user.premiumTime || undefined);
        const newPremiumEnd = currentPremiumEnd.clone().add(hari, "days");
