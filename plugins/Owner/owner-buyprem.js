...
handler.before = async (m, { conn: conn }) => {
  ...
  const { list: list, key: key, hargaPremium: hargaPremium, timeout: timeout } = conn.buyprem[m.chat], harga = hargaPremium[list];
  if (m.quoted && m.quoted?.id === key.id && m.text)
    ...
    } else "N" === input && (await conn.reply(m.chat, "✅ *Anda telah membatalkan peningkatan premium.* ✅", m),
      conn.sendMessage(m.chat, {
        delete: key
      }), clearTimeout(timeout), delete conn.buyprem[m.chat]);
