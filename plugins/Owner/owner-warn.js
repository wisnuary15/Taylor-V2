const handler = async (m, {
  ...
  if (args.length > 0) {
    ...
    let ban = m.mentionedJid[0] || m.quoted?.sender || conn.parseMention(args[0]) || (args[0]?.replace(/[@.+-]/g,
      '').replace(' ', '') + '@s.whatsapp.net') || ''
    let warn = db.data.users[ban].warn
    if (warn < 5) {
      db.data.users[ban].warn += 1
      m.reply(`*berhasil Warn!*`)
      conn.reply(ban, '*Kamu di warn oleh moderator, dan sekarang kamu punya ' + (warn + 1) +
        ' warn. Ingat Jika kamu mendapat warn 5 kali kamu akan otomatis ke banned*', null)
    } else if (warn === 5) {
