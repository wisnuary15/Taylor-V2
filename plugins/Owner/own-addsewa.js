...
let handler = async (m, {
  ...
  try {
    ...
    if (!m.isGroup) {
      ...
      if (!db.data.chats[who]) db.data.chats[who] = {
        expired: now,
        isBanned: false
      };
      ...
      await m.reply(`Berhasil Menetapkan Hari Kadaluarsa Untuk Grup Ini Selama ${teks[0]} Hari.\n\nHitung Mundur : ${clockString(db.data.chats[who].expired - now)}`);
