      ...
      if (querys[0] && querys[1] && !isNaN(querys[0]) && !isNaN(querys[1])) {
        ...
        if (categoryIndex >= 1 && categoryIndex <= uniqueCategories.length) {
          ...
          if (pluginIndex >= 1 && pluginIndex <= pluginNames.length) {
            ...
            try {
              const { stdout: stdout, stderr: stderr } = await execPromise(commandToExecute);
...
handler.help = ["df"].map((v => v + " *index*")), handler.tags = ["owner"],
  handler.command = /^(df)$/i, handler.rowner = !0;
