const handler = async (m, { conn: conn, text: text }) => {
  ...
  for (let jid in users) users[jid].limit < 0 && (users[jid].limit = 0, total += 1),
    users[jid].money < 0 && (users[jid].money = 0, total += 1), users[jid].healt < 0 && (users[jid].healt = 0,
      total += 1), users[jid].healt > 100 && (users[jid].healt = 100, total += 1), users[jid].stamina < 0 && (users[jid].stamina = 0,
      total += 1), users[jid].stamina > 100 && (users[jid].stamina = 100, total += 1),
