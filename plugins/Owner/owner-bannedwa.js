...
const handler = async (m, { conn: conn, text: text }) => {
  ...
  try {
    const { data: data, headers: headers } = await axios.get("https://www.whatsapp.com/contact/noclient/"), email = (await axios.get("https://www.1secmail.com/api/v1/?action=genRandomMailbox&count=1")).data[0], $form = cheerio.load(data)("form"), url = new URL($form.attr("action"), "https://www.whatsapp.com").href, form = new URLSearchParams({
      ...
      __rev: "1006630858",
      __comment_req: "0"
    }), { data: resData } = await axios.post(url, form, {
      headers: {
