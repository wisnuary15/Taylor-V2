...
export async function before(m) {
  if (m.isBaileys || !db.data.chats[m.chat].simi || !m.text) return false;
  const text = m.text.replace(/[^\x00-\x7F]/g, '').trim();
  if (!text) return false;
  const urls = [`https://api.simsimi.net/v2/?text=${encodeURIComponent(text)}&lc=id`,
    ...
    try {
      ...
      if (res.success || res.cnt) {
        ...
        if (text.trim().toUpperCase() === 'SIMI STOP') {
          ...
          this.reply(m.chat, `*Simi stop success*`, m);
        }
        return true;
      }
    } catch {
  ...
  this.reply(m.chat, `*Simi error*`, m);
  return true;
}
