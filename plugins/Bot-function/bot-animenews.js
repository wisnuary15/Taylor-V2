...
import { home, detailAnime, epsAnime } from "../../lib/download/otakudesu.js";
export async function before(m) {
  ...
  chat.latestNews || (chat.latestNews = []), chat && chat.updateAnimeNews && setInterval((async () => {
    ...
    let { id: id } = (await home()).home.on_going[0], { thumb: cover, episode_list: episode_list } = await detailAnime(id), { title: title, id: url } = episode_list[0];
    ...
    let animeDetail = await epsAnime(url),
      ...
      quoted = await this.sendButtonMessages(m.chat, [
