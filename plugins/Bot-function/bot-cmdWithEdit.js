...
export async function all(m, chatUpdate) {
  try {
    ...
    if (editedMsg.imageMessage || editedMsg.videoMessage || editedMsg.documentMessage || (editedMsg.editedMessage && (
        editedMsg.editedMessage.imageMessage || editedMsg.editedMessage.videoMessage || editedMsg.editedMessage
        .documentMessage))) {
      return;
    }
    let hash = {
      text: editedMsg.message?.protocolMessage?.editedMessage?.extendedTextMessage?.text || editedMsg
        .extendedTextMessage?.text || null,
