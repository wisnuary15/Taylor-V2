...
export async function all(m, chatUpdate) {
  ...
  try {
    ...
    if (transcription) await this.appendTextMessage(m, m.prefix ? `${m.command?.[0]?.[0]}${transcription}` : transcription, chatUpdate);
  } catch (error) {
    console.error(error);
    try {
      ...
      const response = await fetch("https://app.giz.ai/api/data/users/inferenceServer.infer", {
        method: "POST",
        headers: {
