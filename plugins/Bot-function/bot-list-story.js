const handler = async (m, { conn: conn }) => {
  ...
  const formattedMessages = list.map(((obj, index) => {
    const { type: type, sender: sender, caption: caption } = obj, messageType = convertMessageType(type), senderUsername = sender.split("@")[0];
    let text = `*${index + 1}.* ${messageType.toUpperCase()} - @${senderUsername}`;
    return caption && (text += `\n${caption}`), text + "\n";
  })).join("\n");
  let { key: key } = await conn.reply(m.chat, `🔧 Daftar Story:\n\n${formattedMessages}\n\nBalas pesan ini dengan nomor cerita yang ingin ditampilkan.`, m, {
    mentions: [m.sender]
  });
...
handler.before = async (m, { conn: conn }) => {
    ...
    const { list: list, key: key } = conn.storyData[m.chat];
