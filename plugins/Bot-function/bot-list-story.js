const handler = async (m, {
    ...
    if (list.length === 0) {
        return conn.reply(m.chat, `Tidak ada cerita yang tersedia saat ini. Silakan tambahkan cerita dengan mengirim gambar, video, atau pesan suara.`, m, {
            mentions: [m.sender]
        });
    }
...
handler.before = async (m, {
    ...
    if (isNaN(index) || index < 1 || index > list.length) {
        conn.reply(m.chat, "⚠️ Masukkan nomor video yang valid.", m);
    } else {
        ...
        if (selectedObj.type === 'imageMessage' || selectedObj.type === 'videoMessage') {
            ...
            conn.sendFile(m.chat, selectedObj.buffer, '', caption, selectedObj.quoted, false, {
                mentions: [m.sender]
            });
        } else if (selectedObj.type === 'audioMessage') {
            conn.sendFile(m.chat, selectedObj.buffer, '', '', selectedObj.quoted);
        } else if (selectedObj.type === 'extendedTextMessage') {
            const message = selectedObj.message ? selectedObj.message : '';
            conn.reply(m.chat, message, selectedObj.quoted, {
                mentions: [m.sender]
            });
        }
