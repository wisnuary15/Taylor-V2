...
const more = String.fromCharCode(8206),
  ...
  handler = async (m, { conn: conn, __dirname: __dirname }) => {
    try {
      ...
      const chats = Object.entries(conn.chats).filter((([id, data]) => id && data.isChats)),
        ...
        cpus = (process.memoryUsage(),
          _cpus().map((cpu => (cpu.total = Object.values(cpu.times).reduce(((last, type) => last + type), 0),
            cpu))));
      cpus.reduce(((last, cpu, _, { length: length }) => ({
