...
export async function before(m) {
  ...
  if (chat && chat.updateAnime) {
    setInterval(async () => {
      ...
      if (/Movie/.test(detailAnime.episode)) return this.reply(m.chat,
        'Bot cannot send video files because they are too large...', quoted);
      let res = await downloadAnime(detailAnime.download[0]?.link || detailAnime.download[1].link || detailAnime
        .download[2].link || detailAnime.download[3].link).catch(() => null);
