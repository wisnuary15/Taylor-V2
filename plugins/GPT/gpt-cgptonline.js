...
async function uuidv4() {
  return ([1e7] + -1e3 + -4e3 + -8e3 + -1e11).replace(/[018]/g, c => (c ^ crypto.getRandomValues(new Uint8Array(1))[
...
async function chatGPT(msg) {
  ...
  const inputString = await (await fetch(
    `https://try.cgptonline.tech/index.php?chat_history_id=${result.id}&id=${userId}`)).text();
