...
const handler = async (m, {
  ...
  db.data.dbai.gptgod = db.data.dbai.gptgod || {};
  ...
  if ("gptgodset" === command) {
    ...
    if (selectedCharacter) return db.data.dbai.gptgod = {
      name: selectedCharacter,
      profile: characterCategories[selectedCategory][selectedCharacter]
    }, m.reply(`Karakter diatur menjadi: *${db.data.dbai.gptgod.name}*`);
    {
  ...
  if (!db.data.dbai.gptgod.name && !db.data.dbai.gptgod.profile) return m.reply(`Atur karakter sebelum menggunakan.\nGunakan command *${usedPrefix}gptgodset* untuk mengatur karakter.\nKarakter yang tersedia:\n${categoryNames.map((v, i) => `*${i + 1}.* ${v}`).join("\n")}`);
  if ("gptgod" === command) {
    const text = args.length >= 1 ? args.slice(0).join(" ") : m.quoted && m.quoted?.text || m.quoted?.caption || m.quoted?.description || null;
    if (!text) return m.reply(`Masukkan teks atau reply pesan dengan teks yang ingin diolah.\nContoh penggunaan:\n*${usedPrefix}${command} Hai, apa kabar?*`);
    m.react(wait);
    try {
      const output = await chatAI(text, db.data.dbai.gptgod.profile);
      output ? m.reply(`*${db.data.dbai.gptgod.name}*\n\n${output}`) : m.reply("Tidak ada output yang dihasilkan.");
    } catch (error) {
