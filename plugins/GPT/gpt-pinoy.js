...
handler.before = async (m, {
  ...
  if (!session || m.isBaileys || m.quoted?.id !== session.key.id || !m.text.trim()) return;
  m.react(wait);
  try {
...
async function PinoyGpt(prompt) {
  try {
    const response1 = await fetch("https://www.pinoygpt.com/wp-json/mwai/v1/start_session", {
      method: "POST",
      headers: {
    ...
    const response2 = await fetch("https://www.pinoygpt.com/wp-json/mwai-ui/v1/chats/submit", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
    ...
    const endIndex = lines.findIndex(line => line.indexOf('"type":"end"') !== -1);
    return JSON.parse(endIndex !== -1 ? JSON.parse(lines[endIndex].slice(6)).data : null).reply;
  } catch (error) {
