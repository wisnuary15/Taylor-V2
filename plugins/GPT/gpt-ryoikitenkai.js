...
async function ryoikitenkai(prompt) {
  try {
    const response = await fetch(`https://smfahim.onrender.com/chatfun?question=${encodeURIComponent(prompt)}`);
...
async function ryoikitenkai2(prompt) {
  try {
    const response = await fetch(`https://api.fumifumi.xyz/api/gpt4?query=${encodeURIComponent(prompt)}`);
