...
handler.help = ["chataigpt"], handler.tags = ["gpt"], handler.command = /^(chataigpt)$/i;
...
async function gptAudio(audioBuffer) {
  try {
    ...
    const response = await fetch("https://chataigpt.org/wp-admin/admin-ajax.php", {
      method: "POST",
      body: data
    });
...
async function getInfo() {
  try {
    const html = await (await fetch("https://chataigpt.org")).text(),
      $ = cheerio.load(html);
