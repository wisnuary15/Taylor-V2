...
const handler = async (m, { conn: conn, args: args, usedPrefix: usedPrefix, command: command }) => {
  ...
  try {
    if (m.quoted?.text) {
      ...
      if (!m.quoted?.mimetype.includes("audio")) return m.reply("Reply Teks/Audio untuk menggunakan gpt ini");
      {
...
handler.help = ["chatgptde"], handler.tags = ["gpt"], handler.command = /^(chatgptde)$/i;
...
async function getInfo() {
  try {
    const html = await (await fetch("https://chatopenai.de")).text(),
      $ = cheerio.load(html);
